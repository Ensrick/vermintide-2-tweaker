...
| 12 | Package/global residency is not renderer closure (#749) | A resident package/resource does not prove a spawned unit resolved real material handles or that one exact Gui registered the material. Prove the consumer-local closure immediately before the native bind/draw; see BUG_CLASSES 71 and `_lib_resource_residency.lua` V2. |
