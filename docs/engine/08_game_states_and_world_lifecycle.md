...
| FIXED (gut_dev v0.2.217-dev, shipped 2026-07-12) | gui_tweaker_dev | `gui_tweaker_dev/scripts/mods/gui_tweaker_dev/_gut_freecam.lua` | Every world lookup routes through the has_world-gated `_live_world` helper; `on_game_state_changed` force-exits free flight while the world is still live (`("exit","StateIngame")` fires before teardown), with vanilla `_clear_free_flight` as the dead-world release. rt: `freecam_invariants` INVARIANT 6 |
