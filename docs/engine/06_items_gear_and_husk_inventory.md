...
| `SimpleHuskInventoryExtension._wield_slot` | every husk weapon swap | `_cos_husk_wield_runtime.lua` owns the one string-form hook (VMF delays until class load). It sets a stack-style context around vanilla for downstream `get_item_units`, restores it on success or error, and preserves eight return values. | ALWAYS delegate to vanilla - an early bail leaves `wielded_slot` nil and vanilla `wield` then nil-indexes at `simple_husk_inventory_extension.lua:540`. Post-vanilla glow and LA replay stays ordered and bounded. |
...
| `MenuWorldPreviewer._spawn_item_unit` | per-unit preview spawn | Full wrapper: WT pre-validates `attachment_node_linking` and applies transforms; Cosmetics' `_cos_spawn_boundary.lua` combines MH-before-vanilla with authored surface/glow/LA/score-paint work after vanilla. | No hand indicator - never use for per-hand ops (WEAPON_APPEARANCE_STANDARD 1). |
...
| cosmetics per-weapon scale ("thiccc") + grip offsets + glow | cosmetics | `_cos_equipment_assembly.lua` applies scale/offset after `GearUtils.create_equipment`; `_cos_preview_runtime.lua` applies scale from the resolved `spawn_data.unit_name` in inventory and illusion previews; `_cos_render.lua` delegates ordinary transform math to the shared WeaponAppearance instance (#420). Glow retains its separate apply/transport owners. | yes | yes | glow has peer broadcast/replay; scale/offset has no husk adapter and retains native transform | yes | yes |
...
### 5.5 [AUDITED 2026-07-11, wt v0.12.208-dev] career_name drop root-caused: external Material Hijack; wt compensations stay

Repo-wide audit of every `GearUtils.create_equipment` wrapper against the 12-param vanilla signature (`gear_utils.lua:7`): wt (`weapon_tweaker.lua:3712/3751`), cosmetics (`cosmetics_tweaker.lua:5212/5229`), cwv (`character_weapon_variants.lua:10615/10616`), ct (`chaos_wastes_tweaker.lua:5794/5811`), ct_dev (`chaos_wastes_tweaker_dev.lua:6628/6645`), frozen tweaker (`tweaker.lua:175/177/220` - the prior prime suspect, exonerated), and the active parity-gated wt_dev mirror ALL name and forward full arity. The dropper is **third-party standalone Material Hijack**: its wrapper names only 11 params, truncating the arg list at `override_item_units` (`misc-vermintide-mods/Material-Hijack/scripts/mods/Material-Hijack/Material-Hijack.lua:212/218`; the archived patched port inherited the defect, `material_hijack_patched_archive/.../material_hijack_patched.lua:332/337`). That matches the crash-dump signature exactly: career_name (arg 12) cut, `override_item_units` (arg 11) surviving - which is why wt's pre-resolve compensation works. cosmetics' EMBEDDED MH copy is clean (folded into its own full-arity hook, `_material_hijack_embedded.lua:396-397`). **wt's two compensations (weapon_tweaker.lua:3712-3749) are deliberately KEPT** - any peer running standalone MH re-introduces the drop; deleting them would resurrect the ghost-scythe class engine fatal (`backend_utils.lua:159-162` skip). Remaining option if this bites again: retire standalone MH from the user cohort in favor of the cosmetics embed.
...
### 5.7 Shared WeaponAppearance adoption (#420)

`tools/shared_lib/_lib_weapon_appearance.lua` is the canonical copied primitive.
CWV, both WT streams, WOC, and Cosmetics each construct an isolated mod-local
instance; Cosmetics installs its instance before `_cos_render.lua` loads and the
ordinary scale/additive-offset adapter contains no private setter math. Each mod
still owns item identity, hand, perspective, renderer, lifecycle, and any measured
durable-retention policy. The exact Cosmetics source regression is
`#420 Cosmetics adoption rejects private scale and position setters`; broader
surface/retained-state convergence remains under #660.
