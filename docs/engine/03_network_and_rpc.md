...
| P2 | cwv | `character_weapon_variants.lua:10424-10447` | Skin-null wire safety covers ONLY the `game_object_initialized` encode site; vanilla encodes `weapon_skin_id` on other `rpc_add_equipment` paths too (`interactions.lua:1244/1334`, `inventory_system.lua:235/335`). -> Audit whether a cwv skin can live in `slot_data.skin` when those sites fire (mid-mission pickup/give); if reachable, either extend the swap or stop storing the cwv skin key in `slot_data.skin` at all (side table keyed by slot, applied owner-side at render) so the wire exposure is removed by construction. | Eliminates residual class-31 surface instead of whack-a-mole per encode site. |
...
| P3 | cwv/WOC | cwv `character_weapon_variants.lua:9392-9396`; WOC `weapons_of_chaos.lua:256-260` | Direct item_names injectors do NOT mirror into `damage_sources` (MIL does, `_moreitemslibrary_embedded.lua:279-280`); safe today only because clones keep `entry.name = base_weapon` so damage attribution encodes a vanilla key (`network_lookup.lua:325` appends item_names into damage_sources at boot). -> Add a regression assert: every runtime-registered item either keeps a vanilla `name` or is mirrored into `damage_sources`. | Turns an implicit invariant into a checked one before a future variant breaks it. |
