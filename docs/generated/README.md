...
| `OPEN_ISSUE_AUDIT.generated.json` | Live open-issue lifecycle/type/severity doctrine audit plus ranked closed-issue ancestry. Every medium/high-confidence relation is retained; weak review-only matches fill the configured minimum. Every relationship carries its exact score inputs and prior closure evidence; similarity is review-only and never auto-reopens an issue. |
