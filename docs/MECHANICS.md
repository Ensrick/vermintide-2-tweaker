...
- The Athanor weapon list checks both `CareerSettings[career_name].item_slot_types_by_slot_name[selected_slot_name]` and the item's `can_wield` membership. [src: scripts/ui/views/hero_view/windows/hero_window_weave_forge_weapons.lua:97-119]
- `BackendInterfaceItemPlayfab.set_loadout_item` rejects missing and magic-rarity items, then writes the requested slot without checking that career slot's allowed item types; importers must validate slot compatibility before this setter. [src: scripts/managers/backend_playfab/backend_interface_item_playfab.lua:635-669]
- `BuffExtension` enforces each stacking cap through `_stacking_buffs[sub_buff_template.name]`; two sibling sub-buffs with the same `name` consume one shared `max_stacks` budget even when they modify different stats. [src: scripts/unit_extensions/default_player_unit/buffs/buff_extension.lua:184-197,274-275,330-345,520-567]
- A proc sub-buff with no `authority` passes `BuffExtension`'s authority filter on every process that reaches the event; `authority="server"` admits only `is_server`, while `authority="client"` admits only `is_local`. [src: scripts/unit_extensions/default_player_unit/buffs/buff_extension.lua:1297-1332]
