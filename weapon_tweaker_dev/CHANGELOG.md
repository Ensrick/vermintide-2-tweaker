...
- Carries the same six-write atomic guard/refusal contract, exact identity-safe
...
## 0.12.329-dev (2026-09-01) -- mirror complete Hotfix 6.11.2 route slice (#1436) [not-started]

- Mirrors public beta `0.12.328`: **Saltzpyre's Axe and Falchion** gains the
  **Game Version 6.11.1** state for Heavy Attacks 1 and 2's exact right-hand
  damage-profile routes; `Current` remains inert.
- Public and Dev carry byte-identical generated data for the three-route
  Hotfix 6.11.2 boundary, including the existing Sienna Dagger route.
- Both Axe & Falchion writes are guarded and atomic, use native profile
  identities, restore exact table identities, and add no transport or globals.
...
- Mirrors public beta 0.12.320: Kruber's Blunderbuss gains a bounded
  **Pre-Patch 3.1 (3.0.x source)** option that restores maximum ammunition from
  `16` to the exact earlier value of `12`; `Current` remains gameplay-inert.
- Carries the same adjacent/current source pins, current-only Versus exclusion,
  dual-host regeneration, exact restoration, and fail-closed completeness
  ledger as the public stream.
- Preserves the friends-only development overlay without adding profiles,
  hooks, RPCs, or presentation state. #1436 remains open for the remaining
  historical slices.
...
## 0.12.305-dev (2026-08-13) -- bound Deepwood package references across transitions (#282)
...
- Entry size: 3230 -> 2371 non-blank lines, the first time this stream's
...
## 0.12.298-dev (2026-08-09) -- damage-profile application floors hardened (#1158) [untested]
...
- Mirrors staged beta `0.12.295-beta`: native weapons use the game's ordinary
  loadout writer, while true cross-career overrides are isolated by career,
  saved loadout index, and slot.
- Selected and explicit row writes, every-row menu previews, add/delete row
  lifecycle, cache disable/clear behavior, and bot fallthrough are covered by
  the new offline policy suite and live runtime check. Native ownership uses an
  immutable game-derived catalog and cannot be tainted by another mod's live
  `can_wield` changes.
- Run a full game restart before verification, then use
