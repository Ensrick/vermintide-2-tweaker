...
_queue_enriched_popup = function(state_loading_self, body_text, diff)
    ...
    -- state's standard dismissal path runs on close. The left action key
    -- `gt_lobby_open_workshop` is custom -- our poller below drives it.
...
-- Result poller via gt's central update registry. StateLoading polls vanilla
-- actions itself; "gt_lobby_open_workshop" isn't a known action string so we
-- drive it. After the URL fires we cancel the popup so the state machine
-- moves on as if the user closed it.
...
mod:hook("StateLoading", "create_popup", function(func, self, error_key, header, action, right_button, ...)
    ...
    assert(self._popup_id == nil, "[gt:lobby] enriched popup intercept while popup already up")
