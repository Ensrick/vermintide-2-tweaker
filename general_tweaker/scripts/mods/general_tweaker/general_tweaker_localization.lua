-- Localization for gt. Ordered to MIRROR the widget tree in
-- general_tweaker_data.lua: one banner per top-level group, groups A->Z,
...
return {
    ...
    -- ============================================================
    gttools_group = { en = "Dev Tools" },

    gttools_bot_hud = { en = "Bot behavior HUD" },
    gttools_bot_hud_tooltip = { en = "Draws a column per bot showing its current behavior-tree action, the teammate it follows, distance to that teammate and to you, whether it just teleported, its teleport tally, and the last twenty actions it entered. On-screen, host-only, dev build only." },

    gttools_leash_lines = { en = "Bot leash lines (3D)" },
    gttools_leash_lines_tooltip = { en = "Runs a line in the world from each bot to the teammate it follows and another to you, so the follow leash is visible at a glance. Purely visual, host-only, dev build only." },
