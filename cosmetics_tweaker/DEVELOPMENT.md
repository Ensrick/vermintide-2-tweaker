...
| `_cos_modded_illusion_swap.lua` | Single #504 owner for the eight modded-realm illusion selection/crafting/completion hooks and their private fake-id/pending-request state. Issue #1465 makes every callback yield only to a validated schema-1 `_cim_illusion_swap_provider` published by either `cim` or `cim_dev`; absent, malformed, or throwing providers retain this standalone fallback. Installation remains idempotent and the malformed local-craft completion guard stays beside the request producer. |
