...
| `_cos_composite_icon_catalog.lua` / `_cos_composite_icons.lua` | #650 exact-instance layered icon catalog and pure descriptor/cache/cell-restoration policy. The entry's shared `ItemGridUI` adapter consumes `mod._cos_composite_icons` for inventory/equipment only and replaces the native icon at its source-backed nested cell path, `content[hotspot_*][item_icon_*]`. Layer order is native rarity/background, primary, offhand, optional exact-identity glow mask, native frame. Missing identity, mapping, or renderer-local texture fails closed with bounded reason telemetry; crafting and Hold-Tab remain pending adapters. |
