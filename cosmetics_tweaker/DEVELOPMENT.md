...
| `_mh_package_lifecycle.lua` | Pure #282 held/release-pending ledger for the embedded Material-Hijack skin packages. It owns the one-reference dedupe, preserves delayed releases until the engine queue confirms completion, and exposes the stable pre-manager-destroy postcondition. `_material_hijack_embedded.lua` supplies PackageManager and diagnostics; the entry owns the single post-`StateIngame.on_exit` release hook. |
