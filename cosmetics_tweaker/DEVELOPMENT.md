...
| `_cos_composite_icon_catalog.lua` / `_cos_composite_icons.lua` | #650 exact-instance layered icon catalog and descriptor/cache/cell-restoration policy. It also owns the weak renderer-publication map and bounded reason-telemetry budget so the near-ceiling entry chunk spends no additional top-level locals. The entry's shared `ItemGridUI` adapter consumes `mod._cos_composite_icons` for inventory/equipment only and replaces the native icon at its source-backed nested cell path, `content[hotspot_*][item_icon_*]`. Layer order is native rarity/background, primary, offhand, optional exact-identity glow mask, native frame. Missing identity, mapping, or renderer-local texture fails closed; crafting and Hold-Tab remain pending adapters. The Lua QA suite must `loadfile` the entry so the Lua 5.1 200-local compile ceiling fails before packaging. |
