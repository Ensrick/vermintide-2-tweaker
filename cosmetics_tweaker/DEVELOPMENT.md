...
| `_cos_diagnostics.lua` | Read-only dump/probe chat commands (`/flush_log`, `/dump_glows`, `/dump_skin_rarities`, `/dump_all_names`, `/check_vmf`, `/probe_hat`, `/probe_cosmetics`, `/cos_421_diag`, `/cos_485_diag`, `/cos_1000_diag`). The #421 command reports custom-key catalog symmetry, the four wire-surface registrations, the pure substitution/restore proofs, and whether the live repro actually has a custom skin equipped. `/cos_485_diag` prints the capped unsupported heroic-pose ledger and `/cos_1000_diag` the one-line Weave Season 5-10 frame census; both keep a comma-free callback body so their single receipt stays command-owned. Reads `mod._cos.flush_log`, `mod._cos_weapon_pose_evidence` and `mod._cos.weave_frames`; no exports. |
...
| `_cos_unlocks.lua` | Per-career cosmetic unlocks (`apply_cosmetic_unlocks` + `_CHARACTER_CAREERS`), Unlock-All portrait frames, vanilla-unobtainable cosmetic grants, the two `PlayFabMirrorAdventure` hooks, `/frames_status` + `/cosmetics_status`. Exports `mod._cos.apply_cosmetic_unlocks` and publishes the #1000 provider `mod._cos.weave_frames`. |
| `_cos_weave_frame_catalog.lua` | #1000 data-only Weave Season 5-10 frame provider (API version 1): 24 snapshot entries plus registration-gap, atlas and vanilla-localization censuses. Registers nothing and never touches `NetworkLookup`; `/cos_1000_diag` in `_cos_diagnostics.lua` prints its one-line census. See `WEAVE_FRAME_PROVIDER.md`. |
...
| `_cos_weapon_pose_policy.lua` / `_cos_weapon_poses.lua` | #485 pure authored-pose catalog plus the local modded-realm SocialWheelUI adapter. Replaces only the gathered pose rows; never grants backend ownership or mutates ItemMasterList. Missing-parent fallback is a capped unsupported-parent ledger (32 parents per module generation, read by `/cos_485_diag`) pending compatibility proof. The module owns its `issue485_authored_weapon_poses_local_only` check through `install_checks`, called from `_cos_runtime_checks.lua` because this module loads before the command owner. |
