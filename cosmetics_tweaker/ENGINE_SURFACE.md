...
| Class.method (kind) | Vanilla behavior | Why cosmetics hooks it | Trap / invariant |
|---|---|---|---|
| `PlayerProjectileUnitExtension` / `PlayerProjectileHuskExtension` `.hit_enemy` / `.hit_level_unit` / `.hit_non_level_unit` [safe] `:10440` | Resolves a fired projectile's impact [src: `player_projectile_unit_extension.lua`] | Spawn a decorative Moonfire-arrow impact puff for `we_deus_01*` arrows behind `cos_moonfire_cosmetic_puff` (`:10440`) | Defers to `wt`'s AOE revert if that already puffs (no double-up); FX package rides the equipped Moonfire Bow, so `create_particles` is safe on hit (`:10411` memo) |
