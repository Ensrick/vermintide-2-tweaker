...
| `HeroWindowItemCustomization._setup_illusions` [hook] `:3591` | Builds the illusion grid for the selected item [src: `hero_window_item_customization.lua:1518`] | Inject the offhand (shield) picker row-2, auto-select the equipped illusion, idempotently add #377 committed exact-item+skin glow badges, and automatically census #704's exact Sword+Mace row families | Console variant is a separate class; skin lookup chain is `item.skin` -> `get_skin(backend_id)` -> `default_skins` -> template default (DEVELOPMENT). Badge state is Apply-only, never dirty preview state. The #704 census reuses this hook after vanilla and lazy pool construction, reads the live `_cwv_dual_offhand_contract` rather than copying family declarations, caps signatures/field bytes/output, emits deduplicated `[cos:704]` console lines, and never mutates the grid. This is picker-provider evidence; browser mesh rendering remains #419 and cross-surface replay remains #660. |
