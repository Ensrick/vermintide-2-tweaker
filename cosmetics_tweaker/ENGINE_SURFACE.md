...
2026-07-15 against the decompile and the mod source.
...
> `_cos_render.lua`. The `create_equipment` hook remains in the entry; the
> `_spawn_item*` and `LootItemUnitPreviewer.spawn_units` hooks later moved to
...
> `create_equipment` remains in the entry while `_spawn_item*` and
> `LootItemUnitPreviewer.spawn_units` now live in `_cos_preview_runtime.lua`;
> their glow apply step calls
...
> persistent state holder whose 16 injected dependencies refresh before the
> reinstall guard. Engine backend/package tables and the shared `mod._cos`
> render/glow consumer map remain action-time lookups; the owner adds no network,
> husk, wield, or live `create_equipment` seam.
>
> **Phase 5 news-feed safety owner.** `_cos_news_feed_safety.lua` retains the one
...
>
> **Phase 5 item-presentation runtime owner.**
> `_cos_item_presentation_runtime.lua` owns the exact-instance presentation
> adapter in two historical phases: the one UIUtils hook is installed at the
> former resolver block, while the Hold-Tab peer-cache adapter becomes visible
> only after LA receivers install. The pure descriptor policy remains in
> `_cos_item_presentation.lua`; neither phase adds transport, persistence, or a
> frame/lifecycle owner.
...
|---|---|---|---|
| `GearUtils.create_equipment` [hook] `_cos_equipment_assembly.lua` | Builds the equipment record and spawns 1p/3p units for a slot [src: `gear_utils.lua:7`] | In-game render path for offhand mesh override + LA texture paint + glow re-key (`_cos_equipment_assembly.lua`); reads `result.skin` to gate has_skin (DEVELOPMENT "Render paths") | Multi-return collapse (`docs/VMF_RECIPES.md` §2); hot-reload unsafe (CLAUDE.md); single registration shared with the folded MH work (`_material_hijack_embedded.lua:405`) |
...
| `BackendUtils.get_item_units` [hook,tbl] `_cos_equipment_assembly.lua` | Resolves per-hand unit paths: skin entry first, else `item_data.right_hand_unit`; `backend_id = item_data.backend_id or backend_id` [src: `backend_utils.lua:144`,`:156`] | Force the picked offhand/illusion mesh when a skin is equipped and the package is resident (`_cos_equipment_assembly.lua`); the one mesh seam all four render paths ride. Husk branch swaps the REMOTE wearer's offhand mesh from the synced stores: LA armoury via `_la_equips_by_peer` (kind="unit"), and #416 VANILLA meshes via the parallel `mod._offhand_mesh_by_peer` (offhand_unit sync) | Fires for EVERY caller incl. all 4 previewers - cannot see its context. A nil-ID customization fallback is allowed only for the same normalized weapon family; exact hand selections must exist in that family's pool, and husks never use the local fallback. `_override_package_ready` must confirm both `<unit>`+`<unit>_3p` loaded first. |
...
|---|---|---|---|
| `UIUtils.get_ui_information_from_item` [hook,tbl] (owner: `_cos_item_presentation_runtime.lua`) | Receives the exact backend item and returns inventory icon, display name, description, and store icon; weapon icons resolve from `item.skin` [src: `scripts/helpers/ui_utils.lua:219-260`] | Resolve one exact-instance component card: direct LA Armoury identities and bridge clones both use authored `SKIN_LIST` icons; the exact item skin wins over a representative paint fallback; duals retain row-1/main-right ownership; shields follow their selected left-hand icon; titles compose primary plus component; flavor text belongs to the selected component (#376/#583/#641/#883/#923) | Preserve all four returns; missing exact LA mapping keeps the native icon and never enters sibling-prone generic unit recovery. Pending preview/selected row-one identity outranks stale equipped state. Never leave the primary description after an independent component resolves. Icon/text ownership never changes DLC/unlock policy or saved/network identity. Never mutate shared `WeaponSkins.skins` or `ItemMasterList` presentation fields globally (v0.9.9.0 rollback). `[cos:883]` records at most 32 distinct exact-item resolution outcomes per session and never writes to chat. |
| **Table contact:** `_offhand_options[item_type][hand]` merge (catalog owner: `_cos_offhand_catalog.lua`; LA merge consumer: entry) | Vanilla has no independent component pool; Cosmetics authors base/CWV rows and later imports Loremaster rows after that provider registers | Merge imported rows through `OFFHAND_NAMES.merge_unique` using `(component kind, component identity, hand)`. An authored row replaces/preserves a generic duplicate so name, description, and icon have one owner (#641). | Never deduplicate by array index, display text, or mesh alone: different components may share a receiver mesh. This changes selection metadata only; saved identity and RPC payload remain untouched. |
...
| `SimpleHuskInventoryExtension._wield_slot` [hook] `_cos_husk_wield_runtime.lua` | Husk-side wield: attaches 1p/3p units for a REMOTE player's view [src: `simple_husk_inventory_extension.lua:641`] | Bracket vanilla with the remote-wearer context consumed by equipment assembly, then bind/repaint glow and reconcile LA on the freshly spawned units | Chosen over `SimpleHuskInventoryExtension.wield` because `_tpe.lua:511` already reserves `wield` and VMF drops the second `hook_safe` (LA_SYNC §6.8); always restore the stack context and preserve all returns, including when vanilla raises; husk resolves BASE `item_data` (memory `reference_vt2_husk_resolves_base_item_data`) |
| `SimpleHuskInventoryExtension.init` [safe] `_cos_la_husk_identity_runtime.lua` | Constructs the remote-view inventory extension [src: `simple_husk_inventory_extension.lua:314`] | Seed remote wearer/career state before the first husk wield and enroll the bounded LA spawn replay | Husk class is a separate root from the owner - hooking one never covers the other (CLAUDE.md "Self-owned vs husk"); registration is exclusive and idempotent |
...
| Class.method (kind) | Vanilla behavior | Why cosmetics hooks it | Trap / invariant |
|---|---|---|---|
| `CosmeticUtils.update_cosmetic_slot` [hook,tbl] | Encodes cosmetic + skin ids and writes them into the player's `player_sync_data` GameSession object (`set_data`) for hat/skin/pose slots [src: `cosmetic_utils.lua:230-251`] | Substitute LA item/skin backend_ids to vanilla and route `ct_*` skins through `_cos_wire_safe_custom_skin` before the sync write (#421, crash fa479a72) | Fourth #421 skin wire surface and a GameSession channel, not an RPC; it must register `update_cosmetic_slot` in `_cos_skin_wire_surfaces`. A peer decodes on the inspect/playerlist read path [src: `cosmetic_utils.lua:168-178`]; `CosmeticUtils` is a plain table (`= CosmeticUtils or {}` [src: `cosmetic_utils.lua:3`]) - use table-form |
...
| `PlayerUnitAttachmentExtension.game_object_initialized` [hook] `_cos_attachment_spawn_sync.lua` | First attachment broadcast, encodes `attachment_id = NetworkLookup.item_names[slot.name]` [src: `player_unit_attachment_extension.lua:55`] | Substitute LA hat/armor clone item names to vanilla before the encode (`_cos_attachment_spawn_sync.lua`) | The LA-NAME axis; the skin axis rides `SimpleInventoryExtension` (a different class) |
...
| `AttachmentUtils.hot_join_sync` [hook,tbl] `_cos_attachment_spawn_sync.lua` | Replays attachments to a joiner via `rpc_create_attachment`, `attachment_id = NetworkLookup.item_names[slot_data.name]` [src: `attachment_utils.lua:81`,`:99-102`] | Substitute LA clone names and attempt targeted cosmetic/glow replay | Plain-table dispatcher; table-form hook. This callback can precede joiner ingame membership, so glow correctness comes from the joiner's acknowledged `cos_la_state_req` pull-on-ready; the host reuses existing `cos_glow_apply` replies, then the receiver performs bounded local material rehydrate (40 attempts/10s, no network retry) |

### Attachment / hat unit spawning + link guards (owner: `docs/engine/06`, §22)

| Class.method (kind) | Vanilla behavior | Why cosmetics hooks it | Trap / invariant |
|---|---|---|---|
| `AttachmentUtils.create_attachment` [hook,tbl] `_cos_spawn_boundary.lua` | Spawns `item_units.unit` for a hat/attachment slot via `UnitSpawner.spawn_local_unit` -> `World.spawn_unit`, then links it [src: `attachment_utils.lua:5`,`:16`] | Skip cleanly when the swapped headpiece package is not resident on a viewer, returning vanilla's empty `{unit=nil,...}` slot_data shape [src: `attachment_utils.lua:38-44`] (`_cos_spawn_boundary.lua`, #270) | `item_units.unit` == `item_data.unit` (skin block only overrides hand/ammo units, [src: `backend_utils.lua:153`]) - gate on `item_data.unit`; non-resident spawn is a `World.spawn_unit` C-assert that bypasses `pcall` (`c_api_world.cpp:67`) |
| `AttachmentUtils.link` [hook,tbl] `_cos_attachment_link_policy.lua` installed by `_cos_spawn_boundary.lua` | Links source->target by `node_linking` via `Unit.node` per link [src: `attachment_utils.lua:66-73`] | Pre-validate every source/target node with `Unit.has_node`, preserve every valid pair, and queue the unit for LA re-paint (#270 crash B / #950) | `Unit.node` on an absent node is an engine C-assert (`c_api_unit.cpp:74`) that bypasses `pcall` (CLAUDE.md Lua quirks); a nil target from a skipped spawn also faults here |
| `PlayerHuskAttachmentExtension.create_attachment` [hook] `_cos_attachment_spawn_sync.lua` | Husk-side hat spawn; `remove_attachment` first if the slot has old data [src: `player_husk_attachment_extension.lua:45`,`:94`] | Pre-patch `item_data.unit = la_unit_path` from `_la_equips_by_peer` BEFORE delegating, then paint, so the late vanilla `rpc_create_attachment` is idempotent with cosmetics' earlier `cos_la_apply` (`_cos_attachment_spawn_sync.lua`, LA_SYNC §6.7, v0.9.0.9) | `slot_hat`-only today; `_la_equips_by_peer` must be populated on CLIENTS, not just host (v0.9.0.7 mirror write) |
...
| `World.link_unit` [safe,tbl] `_cos_spawn_boundary.lua` | Engine C-API: links child unit/node to parent unit/node | Cover hats linked via the low-level `World` API (bypassing `AttachmentUtils`) - queue them for LA re-paint | Fires for EVERY link in the world; gate on `Unit.has_data(child,"unit_name")` + `LA_BRIDGE.registered` before acting |
...
| `SocialWheelUI._gather_weapon_poses_by_parent_item` / `._is_dirty` [hook] `_cos_weapon_poses.lua` | Vanilla gathers only the current parent's backend-unlocked poses, sorts by `pose_index`, then builds a local-only pose page (`social_wheel_ui.lua:974-1057,1075-1096`) | #485 substitutes the complete authored exact-parent catalog in the modded realm and invalidates the page when the option changes | Pose execution is `PingTypes.LOCAL_ONLY` (`social_wheel_ui.lua:1016-1034`). Never write the PlayFab mirror; no-catalog parents fail back to vanilla and are recorded once each under a 32-parent module-generation cap. `_is_dirty` itself calls the gather (`social_wheel_ui.lua:907-908`), so the gather path runs repeatedly and must stay deduplicated. |
...
| `BackendInterfaceItemPlayfab.set_loadout_item` [safe] + `BackendUtils.set_loadout_item` [hook,tbl] | Persists an equipped item to a career loadout [src: `backend_interface_item_playfab.lua`] | Capture equip events to drive LA persistence + cache; table-form for the LA-clone dispatch path; #925 publishes one bounded local presentation generation after a successful write | Hook the post-LA `BackendUtils` ref, not cold `_G` (memory `reference_cim_equip_capture_la_dispatch`). The #925 payload is scalar-only and LA clone ids normalize to their vanilla cosmetic key; it is invalidation, not another loadout store. |
...
| `_G.Localize` [hook] `_cos_illusions.lua` | Global loc-key -> string lookup; a missing key renders `<key>` [src: `foundation/scripts/managers/localization/localization_manager.lua:3-5`]; item cards read `WeaponSkins.skins[skin].description` [src: `scripts/helpers/ui_utils.lua:219-231`] | Supply display names for LA-clone + `ct_*` keys and exact registered custom-illusion descriptions via the private localizer (#913); route vanilla's two misspelled griffon-foot description keys (`wh_deus_skin_02_magic_02_desciption` at `item_master_list_versus_rewards.lua:1653` / `weapon_skins_versus_rewards.lua:1186`, `wh_deus_01_magic_desciption` at `weapon_skins_morris.lua:573,588`) to the first sibling key the native chain resolves (#1567; data-only `_cos_description_parity.lua`) | VMF `_localization.lua` is NOT auto-registered into global `Localize`; private description misses retain presentation/LA/native fallback (`docs/VMF_RECIPES.md` section 7); an alias whose siblings all miss keeps vanilla's `<key>` so the gap stays visible; `/cos_1567_diag` prints the bounded census |
...
The customization-window suite drives the two-row offhand picker and the contextual glow editor.

| Class.method (kind) | Vanilla behavior | Why cosmetics hooks it | Trap / invariant |
|---|---|---|---|
| `MenuWorldPreviewer.equip_item` + `HeroPreviewer.equip_item` [hook] (owner: `_cos_preview_runtime.lua`) | Equips an item into a preview slot; `item_name` is the WEAPON master key, `skin` is a separate arg [src: `world_hero_previewer.lua:649`] | Capture the `skin` arg into `_equip_skin_by_item` (weak-keyed) + `_cos_current_equip_backend_id` so `_spawn_item*` can gate has_skin and thread glow; the base hook also swaps/paints the score-lineup LA hat after TeamPreviewer stamps a verified wearer (#513) | Hook DERIVED and base independently - `class()` copies methods at load [src: `class.lua:51-57`]; the keep previewer is `MenuWorldPreviewer` (CLAUDE.md) |
| `HeroPreviewer.post_update` [safe] (owner: `_cos_preview_runtime.lua`) | Updates visibility before polling the hero spawn/package request; `MenuWorldPreviewer.post_update` delegates to this base implementation [src: `world_hero_previewer.lua:98-105`; `menu_world_previewer.lua:298-300`] | Replays the authored #629 Grail Knight outfit texture on the inventory/score hero body after the preview mesh is visible | Do not cache on the hidden spawn frame: `_set_character_visibility(true)` restores `skin_data.material_changes` on the next update [src: `world_hero_previewer.lua:204-254,367-379,543-585`]. Invalidate on hide, then repaint once after show. |
| `TeamPreviewer._spawn_hero` [hook] + `cb_hero_unit_spawned_skin_preview` [safe] (owner: `_cos_preview_runtime.lua`) | Requests each end-lineup hero from `hero_data`, then equips its preview items after spawn [src: `team_previewer.lua:99-126`]; `LevelEndView._get_hero_from_score` preserves profile/career but drops peer/local identity [src: `level_end_view_v2.lua:259-288`] | Resolve a human score row to the synced per-peer LA store before the preview spawns, then paint its hat/outfit (#513) | `ScoreboardHelper` assigns bots their owner's `peer_id` and separately records `is_player_controlled` [src: `scoreboard_helper.lua:352,360,393-398`]. Require exact profile+career + player-controlled + complete peer/local tuple; bot/untrusted rows fail closed in `_cos_score_identity.lua` |
| `HeroPreviewer._spawn_item` + `MenuWorldPreviewer._spawn_item` [hook] (owner: `_cos_preview_runtime.lua`) | Spawns a single preview unit for an item [src: `world_hero_previewer.lua:895` / `menu_world_previewer.lua:635`] | Force offhand mesh + LA paint on the inventory mannequin via the captured skin | String-keyed `_item_info_by_slot` vs numeric `_equipment_units` - bridge on `spawn_data[1].slot_index` (CLAUDE.md) |
| `HeroPreviewer._spawn_item_unit` + `MenuWorldPreviewer._spawn_item_unit` [hook] (owner: `_cos_spawn_boundary.lua`) | Spawns one unit, no hand indicator | One combined hook keeps MH `replace_textures`/`add_particles` before vanilla, then authored-hat surface, glow re-key, LA re-paint queue, and score-hat paint after vanilla (#612/#513) | No per-hand context - never use for per-hand ops (CLAUDE.md); the single owner makes final-writer order explicit and folds the removed MH `_spawn_item_unit` hook (`_material_hijack_embedded.lua:459`) |
| `LootItemUnitPreviewer.spawn_units` [hook] + `load_package` [hook] + `destroy` [safe] + `update` [hook] (owner: `_cos_preview_runtime.lua`; exact CIM policy: `_cos_cim_preview.lua`) | Illusion browser and CIM Athanor constructors share this previewer. Unit resolution and `load_package` run synchronously inside construction; the caller writes `self._spawned_units` only after `spawn_units` returns [src: `loot_item_unit_previewer.lua:246-305,538`; caller `:504`] | Ordinary browser: apply its existing offhand mesh/LA paint/scale behavior. Exact `cim_preview`: consume CIM's scoped identity during `BackendUtils.get_item_units`, retain only an already-loaded known parent package, pair the returned unit with `spawn_data[i].unit_name`, then run bounded generation-safe material reconcile (#481). | MUST be `[hook]` not `[safe]` - `_spawned_units` is nil at safe time. CIM never borrows generic customization identity, never force-loads an absent parent, and never takes the ordinary LA 2.0 scale path. Backendless/foreign/stale/path-mismatched/nonresident/timeout cases keep resident base presentation. Destroy releases only this previewer's parent refs; simultaneous previewers remain independent. |
| `HeroWindowItemCustomization._setup_illusions` [hook] (owner: `_cos_offhand_picker.lua`) | Builds the illusion grid for the selected item [src: `hero_window_item_customization.lua:1518`] | Inject the offhand (shield) picker row-2, auto-select the equipped illusion, bind #923 LA options to an immutable exact target item type, idempotently add #377 committed exact-item+skin glow badges, and automatically census #704's exact Sword+Mace row families | Console variant is a separate class; skin lookup chain is `item.skin` -> `get_skin(backend_id)` -> `default_skins` -> template default (DEVELOPMENT). LA provider icons are attached only to per-item runtime copies; expanded targets without an exact authored icon retain the native card. Badge state is Apply-only, never dirty preview state. The #704 census reuses this hook after vanilla and lazy pool construction, reads the live `_cwv_dual_offhand_contract` rather than copying family declarations, caps signatures/field bytes/output, emits deduplicated `[cos:704]` console lines, and never mutates the grid. This is picker-provider evidence; browser mesh rendering remains #419 and cross-surface replay remains #660. |
| `mod._cos.cwv_offhand_identity(backend_id, item_type, hand_field)` [read-only provider] (owner: `_cos_offhand_state_runtime.lua`) | No vanilla equivalent; CWV needs the independently committed offhand identity to render an exact dual pair remotely (#579) | Returns only the persisted `vanilla_key` after matching it against the exact current item-family/hand option pool and optional saved unit evidence | Apply-gated state only; dirty preview state and unit paths never cross the boundary. Missing state means follow-primary. A stale, mesh-only, or foreign-family record fails closed. |
| `ItemGridUI.init` / `add_item_to_slot_index` / `_populate_inventory_page` [safe] | Initializes the reusable hero inventory grid and fills each visible slot from an exact backend item [src: `item_grid_ui.lua:15`,`:468`,`:585`] | Add the authored #377 glow overlay once per runtime widget and refresh its committed exact-item+skin tint after vanilla population | Missing atlas/material fails closed; weak registries and the Apply callback bound refresh work. Do not decode persistence per frame or mutate the base inventory icon. |
| `HeroWindowItemCustomization` picker/craft/draw suite [hook]/[safe] (offhand setup/input/draw owner: `_cos_offhand_picker.lua`; remaining preview/glow adapters: entry) + `HeroWindowCosmeticsLoadout.on_exit`/`update`/`draw` [safe] | The customization window's craft-button, illusion-index, preview-widget, environment, input and draw callbacks. Vanilla `_state_draw_overview` draws `_info_widgets` independently from the base `info_window` frame [src: `hero_window_item_customization.lua:1060-1070`; definitions `info_window`:38-48, widget:1344-1349]. | Wire the offhand picker interaction + manual glow-editor lifecycle + apply-illusion into the modded flow. While #377 is open for the exact item+illusion, transactionally suppress native Information contents and bind the editor to the live Information node. Selection and wield never auto-open it. | Restore `_info_widgets` after the wrapped call, including on error. Keep the frame/model/illusion/controller paths vanilla-owned. `_cos_glow_panel_layout.lua` is the only panel/bounds policy, and its editor root must match vanilla's `scale = "fit"` transform; fixed screen-coordinate editor formulas are forbidden. One consolidated hook per `(Class, method)`; menu strings carry raw loc keys. |

#925 extends the existing `_apply_weapon_skin_craft_complete` safe hook rather
than registering another hook. Vanilla's final `_present_item` precedes
Cosmetics' exact offhand commit, so the adapter re-resolves the active backend
item through `UIUtils.get_ui_information_from_item` and updates only the five
fields owned by `_present_item`. See `docs/engine/09_ui_views_and_widgets.md`
§2.4a and the shared-library contract in `docs/CROSS_MOD_ARCHITECTURE.md`.

### Global UI stale-material containment (owner: `_cos_news_feed_safety.lua`)

| Class.method (kind) | Vanilla behavior | Why cosmetics hooks it | Trap / invariant |
|---|---|---|---|
| `NewsFeedUI.draw` [origin] `_cos_news_feed_safety.lua` | Begins one UI renderer pass, draws each active news widget in order, and ends the pass [src: `scripts/ui/hud_ui/news_feed_ui.lua:492-509`] | Keep a stale hot-reload widget or unavailable material from aborting the pass and crashing later UI updates | Preserve begin/draw/end ordering; collect failures during the pass, then purge descending and recycle only after `end_pass`; keep one hook and the historical five-purge diagnostic threshold |
...
| `PlayerManager.remove_player` [safe,tbl] `:8313` / `add_remote_player` [safe,tbl] `:8329` | Adds/removes a remote player record | Maintain `_la_equips_by_peer` / peer caches as players join and leave; peer-add also re-arms the existing owner's complete durable hat/outfit/offhand publication so the joiner's acknowledged state pull has records to replay (#629/#660) | Table-form against the manager instance; do not consume the local publish until exact career, bridge, loadout cache, both equipped weapon records, saved-offhand convergence, and every queued/emitted result are proven |
...
| Class.method (kind) | Vanilla behavior | Why cosmetics hooks it | Trap / invariant |
|---|---|---|---|
| `PlayerProjectileUnitExtension` / `PlayerProjectileHuskExtension` `.hit_enemy` / `.hit_level_unit` / `.hit_non_level_unit` [safe] `_cos_moonfire_puff_runtime.lua` | Resolves a fired projectile's impact [src: `player_projectile_unit_extension.lua`] | Spawn a decorative Moonfire-arrow impact puff for `we_deus_01*` arrows behind `cos_moonfire_cosmetic_puff` | Defers to `wt`'s AOE revert if that already puffs (no double-up); FX package rides the equipped Moonfire Bow, so `create_particles` is safe on hit |
...
Runtime owner: `_cos_offhand_catalog.lua`; the pure generation ledger remains
in `_cos_offhand_preload_lifecycle.lua`, and entry-owned lifecycle/update
callbacks only invoke the returned release/preload functions.
...
crash fa479a72). Cosmetics substitutes LA ids to their vanilla equivalent and routes
`ct_*` skins through the same `_cos_wire_safe_custom_skin` policy used by the three
RPC sender wrappers before the write. The four-surface registry is a required runtime
invariant, not merely a census of RPC hooks. The hook also carries the LA
...
gates on residency at both `AttachmentUtils.create_attachment`
(`_cos_spawn_boundary.lua`) and
...
`PlayerHuskAttachmentExtension.create_attachment` hook (`_cos_attachment_spawn_sync.lua`, LA_SYNC §6.7).
