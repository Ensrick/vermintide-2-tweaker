...
| `LootItemUnitPreviewer.spawn_units` [hook] + `load_package` [hook] + `destroy` [safe] + `update` [hook] (owner: `_cos_preview_runtime.lua`) | Illusion browser spawns display units, writing `self._spawned_units` in the CALLER after return [src: `loot_item_unit_previewer.lua:538`,`:504`] | Read the returned `units` to apply offhand mesh + LA paint in the cosmetic picker; force-load parent packages (LA_SYNC §6.4) | MUST be `[hook]` not `[safe]` - `_spawned_units` is nil at safe time. Pair returned units with exact queued `spawn_data[i].unit_name`; unreadable runtime metadata is not proof. Authored paint requires the declared 1P/3P mesh, and an exact row-2 component suppresses a second whole-skin painter (#481). |
