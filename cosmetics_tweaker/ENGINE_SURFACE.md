...
| `HeroWindowItemCustomization` picker/craft/draw suite [hook]/[safe] (offhand setup/input/draw owner: `_cos_offhand_picker.lua`; illusion ownership bridge: `_cos_modded_illusion_swap.lua`; remaining preview/glow adapters: entry) + `HeroWindowCosmeticsLoadout.on_exit`/`update`/`draw` [safe] | The customization window's craft-button, illusion-index, preview-widget, environment, input and draw callbacks. Vanilla `_state_draw_overview` draws `_info_widgets` independently from the base `info_window` frame [src: `hero_window_item_customization.lua:1060-1070`; definitions `info_window`:38-48, widget:1344-1349]. | Wire the offhand picker interaction + manual glow-editor lifecycle + apply-illusion into the modded flow. While #377 is open for the exact item+illusion, transactionally suppress native Information contents and bind the editor to the live Information node. Selection and wield never auto-open it. When either public `cim` or `cim_dev` exposes the validated schema-1 illusion-swap provider and returns exact `true`, Cosmetics yields only the shared Apply seams while retaining its glow/offhand observers (#1465). | Restore `_info_widgets` after the wrapped call, including on error. Keep the frame/model/illusion/controller paths vanilla-owned. A missing, malformed, foreign-schema, throwing, or false CIM provider retains the Cosmetics fallback; a mod id alone is not proof of ownership. `_cos_glow_panel_layout.lua` is the only panel/bounds policy, and its editor root must match vanilla's `scale = "fit"` transform; fixed screen-coordinate editor formulas are forbidden. One consolidated hook per `(Class, method)`; menu strings carry raw loc keys. |
