...
|---|---|---|---|
| `UIUtils.get_ui_information_from_item` [hook,tbl] | Receives the exact backend item and returns inventory icon, display name, description, and store icon; weapon icons resolve from `item.skin` [src: `scripts/helpers/ui_utils.lua:219-260`] | Resolve one exact-instance component card: duals retain row-1/main-right icon ownership, shields follow their selected left-hand icon, titles compose primary plus component, and flavor text belongs to the selected component (#376/#583/#641) | Preserve all four returns; missing item/metadata fails closed. Never leave the primary description after an independent component resolves. Icon/text ownership never changes DLC/unlock policy or saved/network identity. Never mutate shared `WeaponSkins.skins` or `ItemMasterList` presentation fields globally (v0.9.9.0 rollback) |
