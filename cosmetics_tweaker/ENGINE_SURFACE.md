...
|---|---|---|---|
| `UIUtils.get_ui_information_from_item` [hook,tbl] | Receives the exact backend item and returns inventory icon, display name, description, and store icon; weapon icons resolve from `item.skin` [src: `scripts/helpers/ui_utils.lua:219-260`] | Replace only the inventory-icon return for an exact persisted item: duals retain row-1/main-right ownership, shields follow their selected left-hand icon, and LA shields use `SKIN_LIST[armoury_key].icons[vanilla_skin]` (#376/#583) | Preserve all four returns; missing item/metadata fails closed. Icon ownership never changes DLC/unlock policy. Never mutate `WeaponSkins.skins[*].inventory_icon` or `ItemMasterList[*].inventory_icon` globally (v0.9.9.0 rollback) |
