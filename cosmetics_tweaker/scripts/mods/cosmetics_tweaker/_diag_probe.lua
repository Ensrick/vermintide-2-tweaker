...
-- tag:     channel tag, e.g. "174:loadout" or "cos:sync" (no brackets).
...
-- Walks up the stack from `start` for `depth` frames so a chokepoint probe can
-- name the code path that reached it (VMF's own hook-wrapper frame shows up as
-- hooks.lua and is expected noise; the game-source frame beside it is the
-- answer). pcall-guarded — debug.getinfo on a C frame returns nil, we stop.
