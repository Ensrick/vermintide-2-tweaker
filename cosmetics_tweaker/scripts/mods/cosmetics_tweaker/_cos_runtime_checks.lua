...
-- #485: the executed catalog/decision/evidence-cap check registers from the
-- module that owns the guarded code (_cos_weapon_poses.lua, section 2.2a rule 6).
if WEAPON_POSES and type(WEAPON_POSES.install_checks) == "function" then
    WEAPON_POSES.install_checks(_rt_register)
else
    _rt_register("issue485_authored_weapon_poses_local_only", function()
        return "weapon-pose module check owner missing"
    end)
end
