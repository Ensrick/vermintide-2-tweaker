...
| Detection | `/cos_regression_test` passes `glow_picker_apply_transaction_574` and `glow_picker_render_fanout_574`; one successful click logs one `[glow_picker:apply] committed` line. Bounded `[cos:574] sync ...`, `state-pull reply`, `rehydrate armed|complete|expired`, and `repaint path=husk_wield ...` lines prove the owner, preview, and peer paths. Coop verification confirms both clients repaint the wearer on initial/hot join without a weapon swap, after Apply, preview rebuilds, and lobby leave/re-entry. The join retry is local paint only, capped at 40 attempts/10 seconds, with zero new RPC channels. |
