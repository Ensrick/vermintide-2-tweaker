...
| Readiness | The exact 3P unit must already be resident. A unit-kind variant also requires its known parent material package to be globally loaded before a per-previewer reference is taken; an absent parent is never force-loaded. |
...
---
## Shared ordinary weapon-transform owner (#420)

| Field | Value |
|---|---|
| Owner | The entry loads its bundled `_lib_weapon_appearance.lua` once, constructs one Cosmetics-local instance, and installs that exact instance on `mod._cos` before `_cos_render.lua` loads. No sibling mod or shared runtime provider is required. |
| Delegation | `_cos_render.lua` keeps unit-path, career, hand, renderer, and lifecycle decisions. Its ordinary one-shot scale and additive grip-offset channels pass plain descriptors to the shared instance; private scale/position setter math is forbidden in this adapter. LA unit scaling, TPE attachments, and durable animation-tick retention remain separate owners outside #420. |
| Failure boundary | Malformed descriptors or rejected engine writes fail closed through `WeaponAppearance`; a narrowly proven unit-class compatibility adapter may preserve shared math ownership but may not restore private composition. |
| Detection | Offline `test_shared_weapon_appearance.lua` proves loader order, both delegated channels, and adversarial rejection of restored private setters. `test_cos_runtime_modules.lua` pins the injected instance and malformed-contract failure. `/cos_regression_test` passes `issue420_shared_weapon_appearance_owner`. |
...
| Vanilla-sender owner | The two VANILLA encode paths that would otherwise carry an LA backend_id to a peer - `CosmeticUtils.update_cosmetic_slot` and `LoadoutUtils.sync_loadout_slot` - are `_cos_la_loadout_safety.lua` (#1159), together with `mod.loadout_cache` and the four backend hooks that maintain it. The substitution is UNCONDITIONAL: no settings read, and no vanilla equivalent means the send is skipped, never passed through. Offline `test_cos_la_loadout_safety.lua` pins exclusive ownership of all six hooks, the entry-side absence of each, the absence of any `mod:get` on the wire path, the shadow-not-mutate contract on `sync_loadout_slot`, and the bot-loadout guard; it also carries the control/treatment pair for the late-bound `_send_la_apply` getter, because this owner installs ABOVE the entry's sender assignment and a by-value capture would freeze nil while every substitution test stayed green. `_net_safe_hook_status` is NOT its state - it stays entry-owned and entry-brokered to `_cos_attachment_spawn_sync`, pinned by a comment-excluding absence row in `qa/rt_textual_invariants.psd1`. |
