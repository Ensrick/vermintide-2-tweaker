...
- [ ] **(weapon_tweaker) Heavy rapier-pistol shots, ammo-limited** — alternate option in `weapon_tweaker` settings for Saltzpyre's fencing sword (Rapier + Pistol, `wh_1h_rapier_pistol` / fencing template). When toggled, the pistol shot fired via the weapon special action does much higher damage and reaches much longer range, but the weapon has a small ammo pool (5? 10?) and EACH special action consumes 1 ammo instead of being free-fire. Implementation idea: patch the fencing template's `action_two` (or whichever sub-action triggers the pistol) to consume `ammo_data` like a ranged weapon, plus damage/range tuning. Toggle in `weapon_tweaker_data.lua`. NOT a new CWV item — modifies the existing vanilla weapon for any career that wields it.
