...
## Godmode outgoing armor boundary (#1008, 0.2.258-dev)

| Surface | Ownership and invariant |
|---|---|
| `DamageUtils.calculate_damage` | Vanilla resolves hit-zone armor here, returning zero when the exact attack-armor modifier rejects the hit and an explicit second value when hard invulnerability rejects it. With Godmode and **9999 Damage Per Strike** active for the attacking human, GT promotes only category 2/6 armor zeros whose authored attack power is positive and exact armor modifier is zero. The hook preserves invulnerability, no-damage/zero-power profiles, non-enemies, enemy players, unarmored zeros, and Godmode-off behavior; it mutates no shared profile or armor table. [src: `scripts/helpers/damage_utils.lua:119-427,449-571`; `scripts/helpers/action_utils.lua:35-79,133-163,202-218,237-275`] |
...
### Surface 1a - Bot aid / revive / teleport priority (owner docs: `docs/engine/02`, `/07`; `_gt_bot_aid_owner.lua`, `_gt_bot_fixes.lua`, `_gt_improved_bot_combat.lua`)
...
| `StateInGameRunning._award_end_of_level_rewards` [hook] `_gt_level_control.lua` | Awards end-of-level rewards, reading the reservation-derived profile; vanilla calls it only when the cached boot realm is trusted [src: `state_ingame_running.lua:690-726,766-788`] | First reconcile the cached realm through the shared launch-parameter authority (#1509): a modded run restores `_booted_eac_untrusted` and skips the PlayFab request exactly as vanilla intended. Official runs retain the existing `pcall` profile guard | A mutable `script_data["eac-untrusted"]` value is not sufficient because native customization brackets temporarily clear it. The immutable launch parameter survives that window. The restored cache also satisfies vanilla's native untrusted `rewards_ready` branch [src: `state_ingame_running.lua:870-885`] |
...
| `DamageUtils.calculate_damage` [hook] `general_tweaker_dev.lua` (#1008) | Resolves hit-zone armor before `apply_buffs_to_damage`; ordinary/super-armor rejection is already zero at #549's final seam [src: `scripts/helpers/damage_utils.lua:449-571`] | Promote only an opted-in Godmode human's armor-category 2/6 zero whose exact attack modifier is zero and authored attack power is positive | GT's ONLY hook on this pair. Hard invulnerability, authored no-damage/zero-power profiles, allies, self hits, enemy players, unarmored zeros, and Godmode-off calls fail open to vanilla. |
...
| `ConflictDirector.spawn_queued_unit` [hook] / `update_spawn_queue` [hook] / `spawn_unit_immediate` [hook] `general_tweaker_dev.lua` (#242) | `spawn_queued_unit` enqueues and returns a spawn-queue id consumers treat as a real handle (nil-keyed table write in `bt_chaos_sorcerer_summoning_action.lua:406-408`; `remove_queued_unit` ferrors on unknown ids, `conflict_director.lua:1832`); `update_spawn_queue` is the single per-frame drain (`conflict_director.lua:1654/1835`); `spawn_unit_immediate`'s only vanilla callers are the training-dummy PICKUPS (`pickups.lua:147/179`) | Disable Enemy Spawns / Freeze AI: `spawn_queued_unit` never refuses (ids must stay real); the block parks the queue at `update_spawn_queue`; `spawn_unit_immediate` refuses only non-pickup (AI enemy) spawns | `spawn_queued_unit` is CONSOLIDATED - also calls `mod._gt_solo_on_spawn_queued` (assassin/packmaster warning) and a necro pet-skeleton probe, merged because VMF drops a 2nd hook. Deferral trade-off: entries queued while blocked drain (1/frame, vanilla rate) after toggle-off |
...
| `DamageUtils.debug_heal` [native call, no hook] `_gt_dev_heal.lua` (#1143) | Host calls `DamageUtils.heal_network(..., "debug")`; client sends vanilla `rpc_request_heal`, whose server receiver calls the same heal owner. The `debug` heal type is one of the native wound-removal types. [src: `scripts/helpers/damage_utils.lua:2488-2615`; `scripts/entity_system/systems/damage/health_system.lua:547-579`; `scripts/unit_extensions/generic/generic_status_extension.lua:2250-2252`] | `/heal` requests one max-health debug heal for the living local hero, host or client | No direct health/wound write and no custom RPC. Health/status reads and dead/disabled states fail closed. A two-second observer compares permanent health to the observed post-heal maximum and requires a known `wounded=false`; unavailable state is logged as `unknown`, and timeout/error never masquerades as success. Native modifiers, immunity, or shared-health behavior may legitimately prevent confirmation. Lifecycle changes cancel pending observation. |
