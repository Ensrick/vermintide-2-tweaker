...
MOD_VERSION `0.2.222-dev`, friends-only Workshop 3733367409), the ACTIVE working
...
| `gt_godmode_state` [rpc] `general_tweaker_dev.lua:703` | - | Each peer broadcasts its godmode state keyed by peer_id so the authoritative host can suppress a client's damage; stored with a heartbeat clock | Schema-validated; `_GT_GODMODE_TIMEOUT = 9.0` (~3 missed 3 s heartbeats); self-healing rebroadcast via `mod.update` because a cold client->host VMF send can drop during mission-load bot churn (re-handshakes `ping_vmf_users` first) |
