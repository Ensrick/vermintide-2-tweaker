...
MOD_VERSION `0.2.198-dev`, friends-only Workshop 3733367409), the ACTIVE working
...
| `AIBotGroupSystem._update_mule_pickups` [safe] `_gt_bot_fixes.lua:1666` / `_update_health_pickups` [safe] `_gt_bot_fixes.lua:1751` | Assign carryable pickups to bots; mule only when `num_players==0`, health reserves one item per empty-slot human before leftovers go to bots [src: `ai_bot_group_system.lua:1891-2048` / `:2050-2361`] | FIX 10 greedy pickup: re-run assignment minus the `num_players==0` gate (mule) / hand leftovers to empty-handed bots inside the follow-range gate (health) | Rebuilds a local `claimed` set because vanilla's `ASSIGNED_MULE_PICKUPS_TEMP` file-local is unreadable [src: `:1889`]; leaves `force_use_health_pickup` untouched [src: `:2355-2358`] - greedy changes who CARRIES, not who gets healed |
...
| Class.method (kind) | Vanilla behavior | Why gt hooks it | Trap / invariant |
|---|---|---|---|
| `GenericStatusExtension.add_fatigue_points` [hook,tbl] `_gt_hacks.lua:318` | Adds fatigue (stamina cost) on blocks/dodges/pushes | Infinite stamina: short-circuit when active | Always-on wrapper flagged by `_gt_stamina_active` so it never re-registers (VMF errors on duplicate registration) |
