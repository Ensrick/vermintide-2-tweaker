...
MOD_VERSION `0.2.216-dev`, friends-only Workshop 3733367409), the ACTIVE working
...
| `PlayerBotBase._select_ally_by_utility` [hook] `_gt_bot_fixes.lua:814` | The bot's ally-aid picker; line 903 EXPLICITLY excludes any ally for whom `status:is_ready_for_assisted_respawn()` is true [src: `scripts/unit_extensions/human/player_bot_unit/player_bot_base.lua:843-1008`] | Rescue awaiting-respawn allies by relabelling them "knocked_down" (FIX 3), force revive/rescue priority (FIX 3b), and the #492 suppress-pick | Calls the original FIRST so it composes with other bot mods that also hook this pair; the root fix reads `side:player_units()` (unfiltered roster [src: `scripts/managers/side.lua:222`]) not the aid-filtered `PLAYER_AND_BOT_UNITS` [src: `scripts/managers/side_manager.lua:338-339`] |
