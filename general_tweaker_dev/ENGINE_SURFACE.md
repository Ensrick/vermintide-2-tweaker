...
MOD_VERSION `0.2.227-dev`, friends-only Workshop 3733367409), the ACTIVE working
...
| `BTBotTeleportToAllyAction.run` [hook] `_gt_bot_fixes.lua:1505` | The teleport action itself: flips `has_teleported` and snaps the bot to its follow target | FIX 7 confirmation + #139 decision probe + the lab's pre/post position boxing and tether-yank block | CONVERTED from `hook_safe` to a full `mod:hook` so the lab can read the bot position BEFORE and AFTER the snap (`hook_safe` sees only AFTER) |
| `PlayerBotBase._select_ally_by_utility` [hook] `_gt_bot_fixes.lua:797` | The bot's ally-aid picker; line 903 EXPLICITLY excludes any ally for whom `status:is_ready_for_assisted_respawn()` is true [src: `scripts/unit_extensions/human/player_bot_unit/player_bot_base.lua:843-1008`] | Rescue awaiting-respawn allies by relabelling them "knocked_down" (FIX 3), bound that injected pick by #300's unlimited / normal-follow / custom-range policy, force revive/rescue priority (FIX 3b), and the #492 suppress-pick | Calls the original FIRST so it composes with other bot mods that also hook this pair; the root fix reads `side:player_units()` (unfiltered roster [src: `scripts/managers/side.lua:222`]) not the aid-filtered `PLAYER_AND_BOT_UNITS` [src: `scripts/managers/side_manager.lua:338-339`]. The normal fallback follows vanilla's 40 m teleport gate (`FOLLOW_TELEPORT_DISTANCE_SQ = 1600`) [src: `scripts/unit_extensions/human/player_bot_unit/bt_bot_conditions.lua:14,451-480`] |
...
| `AIBotGroupSystem._update_mule_pickups` [safe] `_gt_bot_pickups.lua:121` / `_update_health_pickups` [safe] `_gt_bot_pickups.lua:210` | Assign carryable pickups to bots; mule only when `num_players==0`, health reserves one item per empty-slot human before leftovers go to bots [src: `ai_bot_group_system.lua:1891-2048` / `:2050-2361`] | FIX 10 greedy pickup: re-run assignment minus the `num_players==0` gate (mule) / hand leftovers to empty-handed bots inside the follow-range gate (health); #364 unconditionally releases and skips `bardin_survival_ale`, whose pickup definition is a `slot_level_event` career drop [src: `scripts/settings/equipment/pickups.lua:741-759`] | Rebuilds a local `claimed` set because vanilla's `ASSIGNED_MULE_PICKUPS_TEMP` file-local is unreadable [src: `:1889`]; the host-owned postpass may see vanilla's assignment first, so it clears reserved ale claims before the greedy toggle gate; leaves `force_use_health_pickup` untouched [src: `:2355-2358`] - greedy changes who CARRIES, not who gets healed |
...
| `BTBotMeleeAction._choose_attack` [hook] `_gt_improved_bot_combat.lua:87` / `PlayerBotBase._enemy_path_allowed` [hook] `:232` / `PlayerBotBase._in_line_of_fire` [hook] `:268` | Bot attack selection / chase-path allow / take-cover-from-ranged test | Smarter attack choice, cap chase at ~7 m (`CHASE_MAX_DIST_SQ=50`), only take cover from close in-lane shooters | All gated on `gt_improved_bot_combat`, fall back to vanilla when off; `_in_line_of_fire` maintains `taking_cover_from` and returns `(in_lof, changed)` |
...
| `PassiveAbilityNecromancerCharges._on_talents_changed` [safe] `_gt_bots_keep.lua:332` | Sets `_pets_forbidden_in_level = pets_forbidden_in_hub and is_in_inn_level`; `spawn_pet` then early-returns [src: `scripts/unit_extensions/default_player_unit/passive_abilities/passive_ability_necromancer_charges.lua:108-110`] | Necromancer keep skeletons (v0.2.147): clear the gate for BOT necromancers only so the raise-dead loop can spawn in the keep | `hook_safe` CHAINS so it coexists with other necro mods; scoped tight to bot owners; EXPERIMENTAL fallback documented (suppress the raise-dead loop instead) |
...
| Class.method (kind) | Vanilla behavior | Why gt hooks it | Trap / invariant |
|---|---|---|---|
| `DamageUtils.add_damage_network` [hook] `general_tweaker_dev.lua:792` / `add_damage_network_player` [hook] `:800` | Apply already-final damage on the authoritative machine (DoTs/bombs/explosions; player-weapon path) | Godmode HP block: return `0` when `_gt_godmode_active(attacked_unit)` | Static fns, no `self`; `_gt_godmode_active` answers HUMANS ONLY - bots are owned by the host peer, so a bare peer check made every host bot invincible (regression fixed v0.2.91) |
| `DamageUtils.apply_buffs_to_damage` [hook] `general_tweaker_dev.lua` (#549) | Populates victim units and applies target/attacker mitigation before both damage funnels perform their authoritative health writes [src: `scripts/helpers/damage_utils.lua:2134-2450`; consumers `:1783-1831`, `:1916-1987`] | After vanilla side effects, return 9999 for a positive enemy hit whose human attacker has Godmode + the child toggle | gt's ONLY hook on this pair; client child state is an optional trailing field on the existing heartbeat and expires with it; zero/immune, friendly, self, bot, and disabled paths preserve vanilla's return |
...
| `IngameHud.update` [safe] `_gt_melee_warning.lua:369` | Per-frame HUD composite | DRAW the warning: a four-edge red `UIRenderer.draw_rect` flash on a `hud_scale_fit` scenegraph (the proven HUD-composite path, mirrored from gut) | Acquires the renderer off `self._ingame_ui_context.ui_renderer`; the sound cue side probes `has_world` first and `pcall`s `WwiseWorld.trigger_event(ww,"hud_ping")` (C-faults on a non-resident event) |
