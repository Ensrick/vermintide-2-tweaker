...
| `DamageUtils.debug_heal` [native call, no hook] `_gt_dev_heal.lua` (#1143) | Host calls `DamageUtils.heal_network(..., "debug")`; client sends vanilla `rpc_request_heal`, whose server receiver calls the same heal owner. The `debug` heal type is one of the native wound-removal types. [src: `scripts/helpers/damage_utils.lua:2488-2615`; `scripts/entity_system/systems/damage/health_system.lua:547-579`; `scripts/unit_extensions/generic/generic_status_extension.lua:2250-2252`] | `/heal` requests one max-health debug heal for the living local hero, host or client | No direct health/wound write and no custom RPC. Dead/disabled heroes fail closed. A bounded update observer prints `[gt:1143] ... result=ok` only after full permanent health and cleared wound are both visible; timeout/error never masquerade as success. |
