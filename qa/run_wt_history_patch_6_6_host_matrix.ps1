# Blocking dual-host provenance gate for issue #1436's Patch 6.6 Deepwood
# Staff slice. Both PowerShell hosts independently validate the pinned ledger,
...
$checkPath = Join-Path $PSScriptRoot 'check_wt_history_patch_6_6_reproducibility.ps1'
...
foreach ($requiredPath in @($checkPath, $freshnessPath, $sourceCheckoutPath)) {
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
        Write-Host "[wt-history-patch-6-6-host-matrix] ERROR -- missing $requiredPath" -ForegroundColor Red
        exit 2
    }
}
...
foreach ($hostEntry in $hosts) {
    ...
    if (-not $Quiet) {
        Write-Host ("=== {0}: Patch 6.6 history reproduction ===" -f $hostEntry.Name) -ForegroundColor Cyan
    }
...
if ($failed.Count -gt 0) {
    Write-Host ("[wt-history-patch-6-6-host-matrix] FAILED -- {0}" -f ($failed -join ', ')) -ForegroundColor Red
    exit 2
}

if (-not $Quiet) {
    Write-Host '[wt-history-patch-6-6-host-matrix] OK -- Patch 6.6 provenance reproduces under both PowerShell hosts.' -ForegroundColor Green
}
exit 0
