...
| 19f | GitHub issue STATUS-LABEL or live-card deployment-contract drift — an open issue lacks exactly one of `not-started` / `diagnostics-armed` / `verify-fix`, uses retired open `Fixed` / `verify-fix-coop`, or leaks an invalid issue into the live-test queue. | `PROJECT_STANDARDS.md § 11` "Labels" | The advisory `check_issue_status_labels.ps1` retains the CHANGELOG-reference nudge. The blocking `tools/github/check-lifecycle-cardinality.ps1` pages open labels and complete ready-issue comments through GraphQL, resolves the latest release manifest, and uses `tools/verify/live_test_contract.ps1` to inspect each deployed build's exact source commit. It rejects cardinality drift, blocked/coop/tooling leakage, stale co-op routing, malformed or incorrectly pinned cards, stale/unbound versions and load tags, cross-build or invented backticked commands, contradictory Workshop/manifest IDs, and requested diagnostic log evidence without a same-build bounded `printf` receipt path. CI checks out blob-filtered full history so historical deployed commits remain inspectable; the dedicated workflow uses non-cone sparse `qa`, `tools`, and `*/scripts/mods/` patterns to hydrate shared Lua blobs without bundle trees or per-command promisor fetches, the contract batch-prefetches drift-missing deployed blobs in bounded promisor fetches (partial-clone self-test fixture proves recovery with lazy fetch forbidden), the guard emits per-phase timing evidence against its five-minute ceiling, and visible legacy manifest fallbacks require exact version equality. `check_ci_hardening.ps1` plants removal of that active-source hydration boundary. `tools/ship/ship.ps1` consumes the shared content parser before tracker mutation, while `tools/ship/refresh-cards.ps1` discovers a delegated banner across the exact active source root. Offline dual-host fixtures cover valid solo/co-op, lifecycle/pin failures, exact historical source, multiline command/receipt registration, planted version mismatch, cross-build command/receipt rejection, separate-line Workshop binding, receipt ordering, and invented command/evidence rejection. The lightweight workflow runs on tracker events, manual dispatch, and daily schedule because GitHub has no comment-pin workflow activity. | AUTO (advisory local + blocking CI) |
