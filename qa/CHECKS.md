...
| 19f | GitHub issue STATUS-LABEL drift — a fix/probe shipped in a mod's latest CHANGELOG entry but the referenced OPEN issue carries no worked status, or an open issue carries zero/multiple lifecycle labels (`not-started` / `verify-fix` / `diagnostics-armed` / `Fixed`; exactly one per PROJECT_STANDARDS §11 and #498). | `PROJECT_STANDARDS.md § 11` "Labels" | `check_issue_status_labels.ps1` — reads each active mod's TOP `## <version>` entry, harvests `#N` refs, and via `gh issue view N` warns on any OPEN shipped-work issue with no worked status; a whole-open-issue pass warns unless lifecycle cardinality is exactly one, including documentation/tooling issues (they use `not-started` while open, never human verify-* states). `coop-required` is an orthogonal tester-routing qualifier and does not increase lifecycle cardinality. SKIPS localization-sweep top entries (tags-only #301 passes reference issues as tag CONTEXT, not shipped work). Self-exits 0 when `gh` is offline/unauthenticated. WARNING-only, non-blocking (Advisory). Self-test: `-SelfTest` (pure parser, zero/one/multiple lifecycle, qualifier, and tooling/documentation fixtures). RETIRED-LABEL SWEEP (2026-07-20): pass 3 also reports any open issue still carrying the retired `verify-fix-coop` and exits 1 on that alone; the blocking `tools/github/check-lifecycle-cardinality.ps1` hard-fails on it with a `dead_labels` field naming the retired label, since a retired label satisfies no lifecycle and would otherwise read as zero-lifecycle drift. Reconcile to `verify-fix` + `coop-required`. PREVENTION at the source: `tools/ship/ship.ps1` step 6 (issue #326 part 1) honors explicit `[coop-required]` CHANGELOG intent (and still parses legacy `[verify-fix-coop]` as `verify-fix` + `coop-required`, preserving the 2+-tester signal rather than dropping it) and atomically removes competing lifecycle labels while leaving the orthogonal qualifier alone. | AUTO (script, in run_all) |
