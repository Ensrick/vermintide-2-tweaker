...
| `check_cwv_old_musket_asset_contract` | `Standard` | Issue #1155 deterministic source-asset ratchet. It pins the original Old Musket source SHA/topology/component signature, source/output axis bases, semantic trigger-to-native-handgun anchor, normalized output bounds, exact generated hashes, binary FBX identifiers, and byte identity between the checked-in 1P/3P outputs. It runs in Quick/full QA without Blender or the external source DAE. |
| `check_cwv_old_musket_compiled_contract` | `Standard` | Issue #1155 post-compiler ratchet. A dependency-free VT2 v189 parser opens the exact CWV root bundle and requires one 1P unit, one 3P unit, and one authored material resource; one `rifle` renderer per view with exactly 16,483 triangles; signed bounds within 0.005 m of the immutable asset contract; dominant +Y orientation; exact 1P/3P geometry parity; and `rifle_mat` bound to the authored material. Missing or stale bundles fail clearly. Ordinary Quick/full QA runs it after the source-asset gate. `ship.ps1 -BuildOnly` skips it during pre-build QA, then runs it explicitly against the newly compiled CWV root before reporting success. |
