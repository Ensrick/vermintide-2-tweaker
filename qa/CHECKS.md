...
Docs/tests-only and bundle-only reconciliation diffs pass. The existing
`VT2-Promotion` trailer exempts only stable metadata-only promotion diffs;
runtime source is never exempted. Fixture coverage lives in
