...
| `check_decomposition_contracts.ps1` | ✅ OK (2026-08-10) | Issue #504 machine-readable census: 3 complete and 7 partial phases on `b320920d`; all ten entry ceilings and 93 extracted-owner wires pass. `weapons_of_chaos` was inventoried at 2455 lines (issue #1159) because a file under the 2500 hard limit has no `file_sizes.json` row and nothing else would stop it crossing. Blocking in Quick/full QA; self-test covers growth, owner loss, and phase deletion. Structural evidence only, not a claim that the seven partial phases are complete or in-game verified. |
