...
| 52g | A custom `.unit` compiles into an unrooted sibling bundle, so source tests and upload-file counts pass but asynchronous UI loading fatals with `Resource '#ID[...]' was not found`. | CWV Greataxe package failure, 2026-07-14 | `check_custom_unit_bundle_reachability.ps1` Murmur-hashes every authored custom unit and inspects compiled bundle contents with the VT2 unpacker. The corresponding `.unit` resource must be resident in a bundle named by an explicit source `.mod` package root; forwarding-only sibling bundles fail. The check runs in Quick/full QA when the local unpacker and compression dictionary are available. | AUTO (script, local tool-backed) |
...
| `run_selftests.ps1` | ✅ OK (2026-07-13) | Unit-test entry point: auto-discovers every `qa/check_*.ps1` with a `[switch]$SelfTest` param plus `tools/ship/ship.ps1` (34 cases as of 2026-07-13: explicit coop/diagnostic intent, exactly-one lifecycle transitions, issue-#489 native-probe guard, and publish-release `-Mods` interface/param-stomp contract, rows 19f/60/60a), runs each `-SelfTest`, replays output on failure. All fixture-based/offline. Exit 2 on any regression — Standard policy in `run_all` (full pass, not `-Quick`) so a broken check BLOCKS; also a blocking CI step in `qa.yml`. Failure path verified with a planted failing check. |
