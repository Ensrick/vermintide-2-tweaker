...
| `check_logging` | `Standard` | Logging-hygiene surface (issues #429/#427/#169, rows 58a-e). Existing echo/per-frame debt remains an exit-1 warning, while scanner failures, any new warning-backed diagnostic helper, or executable resurrection of the retired per-mod debug key exit 2. Two monotonic floors: the #427 floor tolerates only the three public stable-stream helper lines already corrected in their dev twins, and the #169 floor tolerates only General Tweaker stable's exact `_dbg_on` read (`_gt_debug_probes.lua`), whose dev twin already carries the VMF predicate; authorized promotion may remove any of them monotonically. |
...
| `check_logging.ps1` | Historical baseline: 56 findings (2026-07-08); current #427 warn-chat debt: 3 exact public stable-stream sites; current #169 retired-key debt: 1 exact GT stable site (2026-09-12) | Rows 58a-e (issues #429/#427/#169). The original warn-chat backlog fell from 16 to CT/GT/VDL stable only; all dev twins and single-stream owners are console-only. The retired-key backlog is down to GT stable's `_dbg_on` read, pinned until its promotion. Ordinary census debt exits 1, while any new warn-chat fingerprint, any retired-key site outside the pinned floor, or scanner failure exits 2. Self-test covers seven scanner fixtures, both monotonic floors, and a live scan of the pinned stable file under both PowerShell hosts. |
