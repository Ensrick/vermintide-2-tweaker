...
| `check_logging` | `Standard` | Logging-hygiene surface (issues #429/#427/#169/#727, rows 58a-f). Existing unclassified echo/per-frame debt remains an exit-1 warning, while scanner failures, any new warning-backed diagnostic helper, or executable resurrection of the retired per-mod debug key exit 2. Two monotonic debt floors remain: the #427 floor tolerates only the three public stable-stream helper lines already corrected in their dev twins, and the #169 floor tolerates only General Tweaker stable's exact `_dbg_on` read (`_gt_debug_probes.lua`), whose dev twin already carries the VMF predicate. The separate #727 reviewed-intent registry binds each legitimate chat reply/edge-latched log to exact file/category/text/count; source or count drift reappears in the advisory census. |
