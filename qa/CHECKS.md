...
| `check_logging` | `Standard` | Logging-hygiene surface (issues #429/#427/#169, rows 58a-e). Existing echo/per-frame debt remains an exit-1 warning, while scanner failures, any new warning-backed diagnostic helper, or executable resurrection of the retired per-mod debug key exit 2. The #427 floor tolerates only the three public stable-stream helper lines already corrected in their dev twins; authorized promotion may remove them monotonically. |
...
| 58e | Active mod Lua executes `get`/`set` for the retired `enable_debug_logging` key or declares it as a widget. Comments, quoted historical prose, and changelogs must remain legal. | #169 + `PROJECT_STANDARDS.md` §3.6 | `check_logging.ps1` category **retired-debug-key** combines raw literal matching with comment/string-blanked executable shapes. Any live site exits 2. A planted fixture proves executable read/widget rejection and comment/string allowance under PowerShell 7 and Windows PowerShell 5.1. | AUTO (blocking error path in the Standard `run_all` check) |
...
| `check_logging.ps1` | Historical baseline: 56 findings (2026-07-08); current #427 warn-chat debt: 3 exact public stable-stream sites | Rows 58a-e (issues #429/#427/#169). The original warn-chat backlog fell from 16 to CT/GT/VDL stable only; all dev twins and single-stream owners are console-only. Ordinary census debt exits 1, while any new warn-chat fingerprint, executable retired debug key, or scanner failure exits 2. Self-test covers seven scanner fixtures plus the monotonic migration floor under both PowerShell hosts. |
