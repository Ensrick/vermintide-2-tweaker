...
| 60i | The outer ship wrapper verifies a worktree-local claim, but nested VMBLauncher reads the machine-global claim directory and reports the same upload as unclaimed; separate worktrees can also allocate independently. | issue #724; VMBLauncher `ShipClaimGate.DefaultClaimsDir()` | `claim.ps1` defaults acquire/verify/release to `%APPDATA%\VMBLauncher\ship_claims`, the exact directory consumed by VMBLauncher. Its self-test reproduces the former split, proves nested visibility and owner release, and races two processes from distinct fixture worktrees against one authority. `-ClaimsDir` remains an isolated test/diagnostic override. | AUTO (script, in run_selftests; PS7 + PS5 direct fixture) |
