...
| `check_custom_unit_gate_reporting` | `Standard` | Issue #1562 reporting ratchet. Executes the entire custom-unit checker and the exact production BuildOnly post-build block with private inventory/files and an unpacker command double. Missing unpacker/dictionary remains explicit SKIP even with Quiet; reachable/unreachable list responses, malformed-tool errors, invalid hashes, and no authored units retain distinct PASS/FAIL/error/NOT APPLICABLE outcomes. BuildOnly shows the child verdict, never calls SKIP verification, and cannot print completion or run the next compiled gate after failure. Runs in Quick/full QA without real dictionary, extraction, launcher, build, or network; these fixtures prove reporting/dispatch, not genuine compiled-resource integrity. |
...
| `check_logging` | `Standard` | Logging-hygiene surface (issues #429/#427/#169/#727/#1637, rows 58a-g). Existing unclassified echo/per-frame debt remains an exit-1 warning, while scanner failures, any new warning-backed diagnostic helper, executable resurrection of the retired per-mod debug key, or any runtime write to the global/environment `printf` in an active mod tree (#1637, no floor, no escape) exit 2. Two monotonic debt floors remain: the #427 floor tolerates only the three public stable-stream helper lines already corrected in their dev twins, and the #169 floor tolerates only General Tweaker stable's exact `_dbg_on` read (`_gt_debug_probes.lua`), whose dev twin already carries the VMF predicate. The separate #727 reviewed-intent registry binds each legitimate chat reply/edge-latched log to exact file/category/text/count; source or count drift reappears in the advisory census. |
...
| 19h | Promotion leak at SHIP time (issue #327) — a stable/public split-mod ship carrying (a) forbidden lifecycle/status metadata in localization, (b) a pre-release MOD_VERSION suffix nobody named, or (c) a canonical MOD_VERSION whose exact full value does not match/increase over the stable CHANGELOG. | `docs/PROMOTION_PROCESS.md` "Invariants" + `LOCALIZATION_STANDARD.md § 13` | `check_promotion_authorization.ps1` and `check_promotion.ps1` share `promotion_version_reader.ps1`: its Lua-aware scanner excludes line/long comments and short/long strings, accepts exactly one top-level canonical local assignment, and fails closed on duplicate/ambiguous/reassigned versions. `check_promotion.ps1` is the stable-ship defense-in-depth gate and compares the full version including suffix; the repository-wide row 19e gate already forbids lifecycle metadata in every stream. `VT2_SUFFIX_OK=1` retains the user-named public prerelease override only after the exact per-directory authorization path grants it. Dual-host self-tests cover lexical decoys, reassignment, exact suffix matching, and missing-vs-present-empty capability records. | AUTO (ship.ps1, stable targets) |
...
| 29b | A new hand-written bidirectional `NetworkLookup` append owner bypasses the shared rollback-safe registration transaction, or an existing manual owner grows while issue #428 migration is incomplete. | Issue #428; `tools/shared_lib/_lib_network_lookup.lua` | `check_network_lookup_append_budget.ps1` scans only canonical active-mod Lua from `tools/mod-inventory.psd1`, recognizes aliased and direct-table rawset/direct-index append forms, and freezes each remaining path/axis count. Removal/shrinkage is accepted as migration progress; a new owner or increased count blocks. Its offline self-test covers both write syntaxes, direct and aliased table references, `NetworkLookup and` guards, scope boundaries, growth/new-owner rejection, and reduction acceptance. | AUTO (blocking script, full `run_all`) |
...
| 52b | Accidental edit to a split-mod STABLE dir, or a promotion PR self-authorizes through branch-owned metadata/code (dev/stable split; stable is write-by-promotion-only). | CLAUDE.md NON-NEG #3 + `PROMOTION_PROCESS.md` | `check_dev_only_edits.ps1` flags stable-dir changes. A base-owned `pull_request_target` status checks out only protected-branch code; `check_promotion_authorization.ps1` permits only a same-repository PR with the live `stable-promotion-approved` label and an admin/maintain-authored comment bound to every changed stable dir, the shared Lua-reader-derived canonical MOD_VERSION, and exact head SHA. `qa-gate` exports exact `directory@version` records plus a proven-present explicit suffixed subset (which may be empty), reconciles the checked tree immediately before each directory gate, and sets or clears `VT2_SUFFIX_OK` independently for that exact pair. Missing, duplicate, mismatched, ambient, or cross-directory permission fails closed; missing is never coerced into present-empty. Branch protection requires both statuses. The gate resolves the exact PR diff: authorization-bound Markdown maintenance other than `CHANGELOG.md` skips release-only suffix/version checks, while every runtime, cfg, localization, changelog, bundle, or other stable-file change retains them. A push, label removal, fork, untrusted actor, stale version, edited-after-grant comment, or PR-authored gate rewrite fails closed. Local promotion retains the explicit `VT2_PROMOTION=1` maintainer path. The checks carry offline PS7/PS5.1 adversarial self-tests. Issues #429/#676/#1025. | AUTO (base-owned authorization + protected PR QA + run_all + pre-commit) |
| 52b1 | Active guidance hardcodes a dated, user-specific, or intentionally empty in-repository VMBLauncher executable path, bypassing the canonical resolver (#1025). | `docs/PORTABLE_SETUP.md` + PROJECT_STANDARDS §6.6 | `check_publication_doctrine.ps1` rejects every new or changed hardcoded launcher path. Six exact receipt/promotion-bound legacy rows remain frozen by path, trimmed source text, and maximum occurrence count: deletion is clean, while copying or editing one returns it to the blocking census. Root guidance contains no exception and points only to the resolver doctrine. | AUTO (PS7/PS5 self-test + active-doc scan in Quick/full QA) |
...
| 52j1 | A public-release closure borrows Dev-only verification, stale/edited evidence, an unrelated public target, or an incomplete authority snapshot (#1527). | `tools/verify/PUBLIC_RELEASE_CLOSURE.md` | `check_public_release_closure_policy.ps1` exercises isolated deterministic attestation validation with strict card selection, trusted author/association, raw body digests, exact public artifact and closure-generation chronology. PS7/PS5-compatible; historical evidence and unavailable infrastructure do not grant reopening. | AUTO (Quick/full offline fixtures only; live close-event adapter NOT implemented) |
| 52j2 | Closure collection fabricates completeness from truncated pages, unstable comments/pins, an obsolete closure event or unauthenticated metadata (#1527). | `tools/verify/PUBLIC_RELEASE_CLOSURE.md` | `check_public_release_closure_collector.ps1` executes the real bounded two-pass one-issue collector through a GraphQL request stub, then the unchanged strict policy. It covers exact generation/scope, pagination and byte bounds, author/Boolean fidelity, observed races and unavailable retained authority. | AUTO (PS7/PS5 Quick/full offline; all results MayMutate=false, no workflow or issue-action adapter) |
| 52j3 | A trusted closure transport buffers unbounded responses, resets deadlines between pages, follows redirects with credentials, coerces timestamp/body bytes, or leaks tokens/errors (#1527). | `tools/verify/PUBLIC_RELEASE_CLOSURE.md` | `check_public_release_closure_transport.ps1` invokes the actual fixed-HTTPS streaming owner and unchanged collector/policy through a private in-memory HTTP handler. It covers shared header/stream/body cancellation, byte/request budgets, exact null cursors, strict UTF-8/JSON, raw timestamps, authentication/redirect failures and deterministic cleanup/redaction. The public API exposes no URL/query/handler callback. | AUTO (PS7/PS5 Quick/full offline; no live requests or mutation, event adapter still absent) |
| 52j4 | A newly closed public issue awaiting its generation-bound attestation is treated as a proven bad closure, an elapsed grace window is not distinguished from a still-open one, or an unsigned retained snapshot is mistaken for authenticated mutation authority (#1527). | `tools/verify/PUBLIC_RELEASE_CLOSURE.md` | `check_public_release_closure_evidence.ps1` exercises the deterministic full-snapshot evidence candidate and observe-only action planner. It covers body/authority tamper, property-order stability, invalid Unicode, duplicate/new closure generations, explicit bounded grace policy at either side of the exact deadline, Pending-vs-Rejected classification, and proves that neither a valid digest nor an expired deadline grants mutation. No operational grace duration is inferred: a future trusted owner must supply the reviewed value explicitly. | AUTO (PS7/PS5 Quick/full offline; authenticated durable storage and live action adapter still absent) |
| 52g | A custom `.unit` compiles into an unrooted sibling bundle, or a preview loads the unit path as a package but the same-ID `.package`/material closure was never compiled, so source tests and upload-file counts pass while asynchronous UI loading fatals with `Resource '#ID[...]' was not found`. | CWV Greataxe package failure (2026-07-14); Encarmine Helmet `BD55DCA31255AAEC.package` failure (#612, 2026-07-15) | `check_custom_unit_bundle_reachability.ps1` Murmur-hashes every authored custom unit and inspects compiled bundle contents with the VT2 unpacker. Every `.unit` must be resident in a bundle named by an explicit source `.mod` package root. A sibling `.package` additionally promises that PackageManager can load the unit path directly: the gate requires its same-ID package forwarder in the explicit root, its standalone bundle, its unit, every material bound by the unit, and every texture referenced by those materials. The Encarmine contract also pins both supplied diffuse SHA-256 values and the four unchanged vanilla normal/packed source hashes. The check runs in Quick/full QA when the local unpacker and compression dictionary are available. Missing dependencies remain optional exit-zero SKIP, always visible even with Quiet; zero authored units is NOT APPLICABLE, not resource proof. BuildOnly invokes the owner without Quiet and qualifies its final summary rather than advertising SKIP as verification (#1562). This reporting repair introduces no required-release policy and changes no resource assertion or dependency discovery. | AUTO (script, local tool-backed) |
...
| 60e | GitHub's release-by-tag route can return HTTP 503 while the releases-list, release-asset, and upload routes remain healthy. Treating every failed tag probe as "absent" either aborts a filtered ship or attempts a duplicate release; budgeting asset GETs like metadata can also cancel a valid carry-forward download at 30 seconds. The tag, list, and `/releases/latest` routes can also embed a STALE asset array (2026-09-23: ghost `manifest.json` id 583157616 returned 404 on download and delete while `GET /releases/{id}/assets` served the live id), failing every ship on the manifest download. | issue #651; `BUG_CLASSES.md` class 55 | `check_github_release_fallback.ps1 -SelfTest` covers canonical success, 404, 503 exact-list fallback, bounded pagination/exhaustion, ambiguity, exact asset selection, asset-ID download, size-scaled GET budgets, invalid-size rejection, immutable byte-count validation, unchanged upload scaling, release-ID clobber with manifest last, and the asset-inventory refresh from `GET /releases/{id}/assets` (ghost embedded id replaced by the live id, 100-row pagination, embedded fallback with one warning when the assets route fails, latest-list entry refreshed). `check_workshop_upload_result_release.ps1` and the `check_local_deployment_receipt.ps1 -SelfTest` worker reproduce the ghost-id tag route end to end and require the live id. Auto-discovered by `run_selftests.ps1`; `ship.ps1 -SelfTest` pins delegation/no duplicate tag probe. | AUTO (script, in run_selftests) |
...
| 60m | Post-ship card reconciliation advances version and ManifestID surfaces but strands source/root/ZIP fields explicitly labeled as the current or exact live artifact (#1102; observed on #932 and #798), while a broad hash rewrite would corrupt historical feature evidence (#774). | issue #1102 | Latest-release authority records carry the exact source commit, root bundle/SHA-256, and ZIP filename/SHA-256. One pure typed-heading reviewer is shared by `refresh-cards.ps1` and the lifecycle strict guard: current/live headings are checked and atomically repaired, structural ambiguity fails closed, and exact `Feature provenance` plus untyped evidence stay byte-identical. Exact #932/#798/#774 fixtures and hostile partial, extra-hash, wrong-name/right-digest, ambiguity, idempotence, and report-only/strict cases run under PowerShell 7 and Windows PowerShell 5.1. | AUTO (post-ship refresher + strict/report-only guard + dual-host self-tests) |
...
| 61l | Receipt bytes are provable but canonical ship, GitHub release, hosted receipt, and VMBLauncher staging disagree on authority, so removing tracked bundles either blocks every release or tempts a mutable-path/upload bypass (#1426). | issue #1426 + #558 phases A-C | Receipt publication requires `receipt-authority-publication-v1`. Tracked publishing preserves pre-build Git-blob selection. Receipt publishing clean-builds and normalizes before capture, binds the committed schema-3 receipt/source/output/policy/builder proof into the hosted receipt, and makes VMBLauncher independently reconstruct that proof before exact locked-staging comparison. Receipt bootstrap, remote deploy, and updater/recovery consumption remain disabled; local deployment has the separate #1429 gate below. No active row migrates in this phase. Capability, receipt, authority, snapshot, staging-set, tamper, and ordering fixtures fail closed under both PowerShell hosts and launcher xUnit. | AUTO (blocking full QA + launcher tests + dual-host fixtures) |
| 61l-local | Canonical ship treats receipt authority as tracked deployment, omits sidecars, or creates an unauthenticated deployment receipt before the existing publication phase. | issue #1429 | `check_local_deployment_receipt.ps1` executes the real ship deployment statement under isolated external-boundary stubs, preserving tracked argv and verifying local-only receipt handoff, refusal-before-launch, and finally cleanup. Its bounded child fixture uses a real temporary Git/schema-3 snapshot, private claim and native owning lease, real shared GitHub API helpers with only HTTP stubbed, and an independent-thread release-mutex probe. Existing published-release identity, exact purpose/asset/builder, remote/bootstrap/missing-target, draft/string/missing/duplicate metadata, original-error, claim/source ownership, delete/upload/readback failures, and byte-exact target census adversaries fail closed. All inventory rows stay tracked; default 0.6.1 and pending builder claims remain unchanged. No real launcher, Steam, ACL, or release mutation occurs. | AUTO (Quick source/installed-boundary checks; blocking full dual-host fixtures) |
...
The machine-transaction prequeued-owner fixture retains its real 5000ms lease
wait and hard-death path. Parent observation shares a separate 20-second terminal
ceiling across both workers. A failure reports bounded owner/contender output,
exit state, monotonic observation/trigger timings, and fixture marker/record
evidence before normal temporary-file cleanup. Pure tests cover aggregate ceiling
exhaustion, throwing/invalid observations, exact timeout/recovery-refusal messages,
read caps, and reader disposal. Unreadable/blank process exit codes are explicitly
reported as unavailable. The ready marker precedes `Ensure`/`Acquire`; its timestamp
is not claimed to start the production mutex wait. Missing evidence is not classified as a harmless
timing failure; these diagnostics do not relax the acquisition assertions.

The 2026-09-06 PS5.1 instrumented fixture failure correlated queued owner PID
16812 (`0x41AC`) with Application log .NET Runtime 1025 at 05:27:34 UTC,
Application Error 1000 at 05:27:35 UTC (exception `0x80131623`), and Windows
Error Reporting 1001 at 05:27:41 UTC, report
`b917d88d-dab5-4c25-b1c6-98aa65b3b81e`. The first event explicitly identifies
the fixture's `FailFast` call; parent observation saw owner exit 8273ms after
the trigger, while the prequeued contender retained its 5000ms acquisition
budget. This is fixture crash-report overhead, not evidence of a production
lease defect. [Microsoft documents that Windows `FailFast` creates the event
and dump before process termination](https://learn.microsoft.com/en-us/dotnet/api/system.environment.failfast?view=netframework-4.8.1#remarks).
The hard-death worker therefore calls parameterless
`Process.GetCurrentProcess().Kill()` on itself only, leaving descendant cleanup
to the actual production Job. [Microsoft distinguishes process termination
from descendant completion](https://learn.microsoft.com/en-us/dotnet/api/system.diagnostics.process.kill?view=netframework-4.8.1#remarks).
PID-bound markers prove every hard-death scenario skips worker `finally`, while
ordinary release, planted throw, and ordinary descendant-drain controls execute
it. Existing stale-record, repeated-refusal, prequeued wait, no-mutation timeout,
and actual child/grandchild death assertions remain required. Neither production
lease code nor WER/registry settings change.
...
| 58e | Active mod Lua executes `get`/`set` for the retired `enable_debug_logging` key or declares it as a widget. Comments, quoted historical prose, and changelogs must remain legal. | #169 + `PROJECT_STANDARDS.md` §3.6 | `check_logging.ps1` category **retired-debug-key** recognizes literal-key calls (colon/first-argument dot syntax with or without parentheses, and explicit-self dot syntax with an identifier/member-path receiver) and widgets (`setting_id =` or `["setting_id"] =`) after comment/string normalization, including long literals whose skipped leading line break Lua ignores. It covers the check's active-mod Lua scan scope (the shared exclusions such as legacy `tweaker\` still apply). Recognized sites exit 2 with no inline escape; this is not general expression evaluation or computed-key analysis. Exactly one site is pinned as promotion debt (`$legacyRetiredDebugKeyDebt`, keyed on path + exact text like the #427 floor, one tolerated occurrence per key, root prefix matched case-insensitively after short-name expansion): General Tweaker stable's `_dbg_on` read in `_gt_debug_probes.lua`, which rides the next user-authorized GT stable promotion. Any other live site, a duplicate, or a rewritten pinned line exits 2. The planted fixture proves read/write/widget rejection and comment/string/safe-identifier allowance, including independent exact-count checks against sentinel collisions, missed no-parentheses calls, leading-break literals, bracketed fields and line-number drift; the self-test proves the floor (exact debt accepted once in any root casing, removal accepted, duplicate/rewritten line and dev twin rejected) and scans the live stable file, requiring exactly one tolerated site while the pinned read is present and zero afterwards, under both PowerShell hosts. `test_gt_vmf_debug_gate.lua` drives the Dev production predicate, scans both load manifests (stable may execute the key only in its pinned probe owner), verifies exact raw-global restoration after missing-file, loader, runtime and normal exits, and trips when the stable read disappears without enabling the stable predicate locks. | AUTO (blocking error path in the Standard `run_all` check) |
| 58f | A standing, intentional player reply or edge-latched log is repeatedly reported as unclassified debt, obscuring actual chat pollution and hot-path logging. | #727 | `$approvedLoggingIntent` records only reviewed echo/per-frame sites by exact relative path, category, trimmed source line, and maximum occurrence count, with a human-readable rationale. Removal is clean; changed text, another path/category, or an occurrence above the reviewed bound returns to the advisory census. The self-test proves exact acceptance plus overflow/text-drift rejection. The routine CT bot-boon echo and three stable warn-chat helpers are deliberately not approved. | AUTO (script, exact bounded advisory classification) |
| 58g | Deployed mod Lua mutates the global/environment `printf` (`_G.printf =`, `_G["printf"] =`, `rawset(_G, "printf", ...)`, `mod:hook*(_G, "printf", ...)`, any `setfenv`, ambient `getfenv()`/`getfenv(0)`, `getfenv(...).printf =`). The deployed-source authority (`tools/verify/live_test_source_authority.ps1`, `Test-VtLuaMutatesGlobalPrintf`) rejects the whole deployed record of any mod that does, because raw `printf` evidence for every mod becomes untrustworthy; gut_dev 0.2.353-dev shipped such a swap inside an in-game regression proof, nothing offline caught it, and every lifecycle guard, card refresh and ship label step failed repo-wide until 0.2.354-dev. | #1637 | `check_logging.ps1` category **printf-mutation**: the same comment/string-normalized whole-source scan as 58e with the `"printf"` literal kept as a sentinel, mirroring the authority's shapes plus the VMF hook spelling. Reads (`rawget`), `pcall(printf, ...)` calls, comparisons, table fields (`{ printf = printf }`) and `getfenv(<function>)` stay legal. Exit 2, no inline escape, no floor; the offline Lua harness under `qa/lua` is outside the scan scope and is the only place a printf swap may live. Planted fixture `logging_printf_mutation_bad.lua` (17 shapes) plus independent per-shape asserts; the self-test also proves the harness stays out of scope while its swap shape is detected when scanned directly, and that the deployed gut_dev recovery module scans clean. Source-text lock: `rt_textual_invariants.psd1` absence needles on the recovery module and `_gut_bot_pose.lua`. | AUTO (blocking error path in the Standard `run_all` check) |
...
`check_wt_history_patch_2_0_6_reproducibility.ps1` pins the exact three-file
evidence census and two-artifact ledger for the bounded Handgun slice. It
reproduces the three adjacent 2.0.5-to-2.0.6 penetration leaves through both
independent evaluators, rehydrates their exact current guards, pins the current
source's gameplay-identical Kruber/Bardin clone relationship, and regenerates
the byte-identical six-operation public/dev catalogs. It explicitly excludes
current-only Versus clones, key-order churn, and the adjacent `weapons.lua`
damage-over-time network fix. The blocking
`run_wt_history_patch_2_0_6_host_matrix.ps1` repeats the patch-specific gate
...
`check_wt_history_patch_2_0_9_1_reproducibility.ps1` pins the exact three-file
evidence census and two-artifact ledger for Kruber's Halberd. It requires the
complete 20-leaf push-follow-up chain boundary, rehydrates all 20 current
guards including three exact nested-table rows, independently reproduces both
evaluator lanes, and regenerates byte-identical public/dev catalogs. One stale,
missing, or foreign leaf refuses the complete family transaction before any
write; Current is inert and restore preserves original table identities. The
blocking `run_wt_history_patch_2_0_9_1_host_matrix.ps1` repeats the exact proof
...
share the same visible-skip/required-failure contract as the other gates,
...
against the current 6.12.1 content anchor, and requires byte-exact primary evidence,
...
for the Patch 6.11.0 Longbow, shared Hammer/Mace, Swiftbow and Grudge-Raker
ammunition slices. It selects two exact automatic-zoom leaves, six block/dodge
leaves and three maximum-ammunition leaves from the adjacent boundary, then
rehydrates all eleven current guards through the primary and independent
evaluators, and regenerates byte-identical eleven-operation public/Dev catalogs. Source-less
...
| `check_wt_history_reproducibility.ps1` | ✅ OK (2026-09-03) | Issues #1529/#1436 full-QA gate for the pinned Patch 5.2 evidence ledger and byte-exact generated catalog; exact regeneration runs only from a checkout whose complete pinned commit/path/blob ledger passes the read-only selector. It derives all 36 source paths referenced across the thirteen family catalogs/specifications and proves byte-identical Git blobs between the 6.12.0 and 6.12.1 anchors. |
...
| `run_wt_history_patch_2_0_9_1_host_matrix.ps1` | ✅ OK (2026-08-31) | Issue #1436 blocking dual-host gate for the exact Patch 2.0.9.1 Halberd 20-leaf push-follow-up boundary, nested current guards, atomic refusal/restoration, independent-oracle agreement, and byte-exact public/dev catalogs. |
...
| `run_wt_history_patch_6_11_0_host_matrix.ps1` | ✅ OK (2026-09-12) | Issue #1436 blocking dual-host gate for Patch 6.11.0 Longbow automatic zoom, shared Hammer/Mace block/dodge, Swiftbow and normal/Versus Grudge-Raker ammunition-only history: five source files, ten evidence artifacts, eleven guarded leaves, independent-oracle agreement and byte-exact public/Dev catalogs. |
...
| `run_bundle_output_set_host_matrix.ps1` | ✅ OK (2026-09-06) | Issues #1400/#1412/#1422/#1426/#1429/#1430 dual-host gate: runs the shared output-set, exact tracked/receipt authority, handle-locked normalization, schema-3 receipt, immutable snapshot, receipt-publication, separate receipt-local deployment, and durable recovery-record adversarial suites under PowerShell 7 and Windows PowerShell 5.1; full `run_all.ps1` treats either host's failure as blocking. |
...
| `check_dofile_package_coverage.ps1` | ✅ OK (2026-08-31) | Blocking Quick gate hardened after the WOC #595 regression: a comment-aware Lua tokenizer covers multiline colon/dot/`pcall` literal targets under PS7 and PS5.1, requires source plus the active package `lua` section, and preserves exact/wildcard semantics. Prevents source-only helpers from becoming Workshop `Resource not found` startup failures. |
...
| `check_logging.ps1` | Historical baseline: 56 findings (2026-07-08); 31 exact #727 intent rows classified; current actionable census: 4 findings (one routine CT echo plus three #427 public stable helpers); current #169 retired-key debt: 1 exact GT stable site (2026-09-20) | Rows 58a-f (issues #429/#427/#169/#727). The original warn-chat backlog fell from 16 to CT/GT/VDL stable only; all dev twins and single-stream owners are console-only. Reviewed player replies and edge-latched WT logs are exact path/category/text/count entries, not broad suppressions. The retired-key backlog is down to GT stable's `_dbg_on` read, pinned until its promotion. Ordinary census debt exits 1, while any new warn-chat fingerprint, any retired-key site outside the pinned floor, or scanner failure exits 2. Self-test covers seven scanner fixtures, the #727 exact-count/text-drift contract, both monotonic debt floors, and a live scan of the pinned stable file under both PowerShell hosts. |
