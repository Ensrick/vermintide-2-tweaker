...
| 52j4 | A newly closed public issue awaiting its generation-bound attestation is treated as a proven bad closure, or an unsigned retained snapshot is mistaken for authenticated mutation authority (#1527). | `tools/verify/PUBLIC_RELEASE_CLOSURE.md` | `check_public_release_closure_evidence.ps1` exercises the deterministic full-snapshot evidence candidate and observe-only action planner. It covers body/authority tamper, property-order stability, invalid Unicode, duplicate/new closure generations, Pending-vs-Rejected classification, and proves that a valid digest remains unauthenticated and never grants mutation. | AUTO (PS7/PS5 Quick/full offline; authenticated durable storage and live action adapter still absent) |
