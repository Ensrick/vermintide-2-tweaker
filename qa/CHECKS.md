...
| 60f | A clean-worktree ship resolves VMBLauncher from configured `ProjectRoot`, but a later release phase hardcodes the invoking worktree's ignored launcher path and fails after Workshop mutation. | issue #683; `BUG_CLASSES.md` class 53 | `check_vmb_launcher_path.ps1 -SelfTest` exercises the production shared resolver with a clean worktree and external configured executable, immutable path/source/approval-anchor handoff across mutable global-root drift, real version metadata, unapproved/missing explicit paths, source mismatch, no candidate, invalid env override, and static ship-to-publisher wiring. The same fixture passes under Windows PowerShell 5.1 and PowerShell 7 and is auto-discovered by `run_selftests.ps1`. | AUTO (script, in run_selftests) |
