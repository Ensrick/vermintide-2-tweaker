...
|---|---|---|---|
| `check_file_sizes` | `qa/baselines/file_sizes.json` | 8 canonical entries currently remain over the 2500-line hard limit after Career balance decomposition; hidden repository-internal worktree checkouts are excluded | a NEW hand-written Lua file crossing the hard limit, or a baselined file GROWING beyond its frozen line count. Generator-owned pure-data tables are recognized only by an `AUTO-GENERATED ... DO NOT HAND-EDIT` banner plus a top-level `return {`, matching PROJECT_STANDARDS §2.1's data exemption. Target-tier (1500–2500) overages stay plain warnings. |
...
| 52 | File exceeds 2500-line hard limit | PROJECT_STANDARDS §2.1 | `check_file_sizes.ps1` line-count, **ratcheted** against `qa/baselines/file_sizes.json` (issue #429): the 8 remaining canonical oversized files are frozen; repository-internal worktree checkouts are excluded; fails only on a NEW file over the limit or a baselined file growing past its frozen count. Target-tier (1500–2500) overages are plain warnings. | AUTO (script + `-SelfTest`) |
...
| `check_file_sizes.ps1` | ⚠ baselined (issue #429) | The baseline retains the 8 remaining canonical oversized modules so regrowth still fails; Career balance was removed after its bounded decomposition. Nested repository-internal worktree copies are excluded. A NEW file crossing the limit or a baselined file growing still blocks. Target-tier overages remain plain warnings. |
