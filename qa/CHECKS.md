...
| `check_native_resource_contracts` | `Standard` | Full-tree #749 renderer-resource census. Exact file/kind/count rows cover texture/material binds, Gui creation/lookups, UIRenderer creation hooks, shading-environment writes, and shared V2 residency proofs. Any new, removed, or moved boundary blocks until its policy and evidence are reviewed; global wrappers are explicitly distinguished from strict Tweaker-owned calls. Repository-relative exclusions make the census identical in the primary checkout and linked worktrees, including worktrees whose parent path contains `.claude` (#1101). |
...
|---|---|---|---|
| `check_file_sizes` | `qa/baselines/file_sizes.json` | 7 canonical entries currently remain over the 2500-line hard limit after the Career balance and cim_dev decompositions; hidden repository-internal worktree checkouts are excluded | a NEW hand-written Lua file crossing the hard limit, or a baselined file GROWING beyond its frozen line count. Generator-owned pure-data tables are recognized only by an `AUTO-GENERATED ... DO NOT HAND-EDIT` banner plus a top-level `return {`, matching PROJECT_STANDARDS §2.1's data exemption. Target-tier (1500–2500) overages stay plain warnings. |
...
| 19f | GitHub issue STATUS-LABEL drift — an open issue lacks exactly one of `not-started` / `diagnostics-armed` / `verify-fix`, uses retired open `Fixed` / `verify-fix-coop`, or leaks an invalid issue into the live-test queue. | `PROJECT_STANDARDS.md § 11` "Labels" | The advisory `check_issue_status_labels.ps1` retains the CHANGELOG-reference nudge. The blocking `tools/github/check-lifecycle-cardinality.ps1` pages open labels, then complete ready-issue comments through GraphQL, and rejects cardinality drift, blocked/coop/tooling leakage, stale co-op routing, malformed newest `## CURRENT LIVE TEST` cards, unpinned newest cards, and multiple pinned exact cards. `tools/ship/ship.ps1` consumes the shared content parser before tracker mutation, strips stale `coop-required` on non-ready/solo targets, and never auto-labels tooling. `tools/verify/generate_playtest.ps1` validates every candidate before writing either in-game queue. Offline self-tests cover valid solo/co-op, solo-first, blocked, retired, latest-card-wins, internal-key rejection, pin-state absence, unpinned newest cards, and duplicate pins. The lightweight workflow runs on tracker events, manual dispatch, and daily schedule because GitHub has no comment-pin workflow activity. | AUTO (advisory local + blocking CI) |
...
| 52 | File exceeds 2500-line hard limit | PROJECT_STANDARDS §2.1 | `check_file_sizes.ps1` line-count, **ratcheted** against `qa/baselines/file_sizes.json` (issue #429): the 7 remaining canonical oversized files are frozen; repository-internal worktree checkouts are excluded; fails only on a NEW file over the limit or a baselined file growing past its frozen count. Target-tier (1500–2500) overages are plain warnings. | AUTO (script + `-SelfTest`) |
...
| 61a | Protected master accepts runtime/version/config/newest-release changes without the owning mod's exact root bundle, or silently deletes a tracked sibling bundle during clean-build/merge resolution (#724, #1100; PRs #759/#765/#766/#767/#769/#1099). | issue #724/#1100 evidence + reconciliation PRs #771/#772/#773/#1099 | `check_release_bundle_atomicity.ps1` compares the staged/PR/push diff against per-mod `RootBundle` identities. Exact root A/M is required; shared/sidecar bundles do not count. Any tracked bundle deletion requires a newly added exact `VT2-Bundle-Retirement: <16-hex>.mod_bundle` trailer in the newest release; an active canonical root cannot be deleted. Docs/tests-only, historical changelog prose, and bundle-only additions/reconciliations pass. Stable metadata-only promotion requires the existing explicit promotion sanction. `-SelfTest` loads the direct fixture matrix. | AUTO (blocking Quick/full CI + pre-commit) |
...
| `check_file_sizes.ps1` | ⚠ baselined (issue #429) | The baseline retains the 7 remaining canonical oversized modules so regrowth still fails; Career balance and cim_dev were removed after their decompositions carried them back under the hard limit (their ratchets live in check_decomposition_contracts). Nested repository-internal worktree copies are excluded. A NEW file crossing the limit or a baselined file growing still blocks. Target-tier overages remain plain warnings. |
| `check_decomposition_contracts.ps1` | ✅ OK (2026-08-11) | Issue #504 machine-readable census: 3 complete and 7 partial phases; all ten entry ceilings and 107 extracted-owner wires pass in the current registry. `weapons_of_chaos` was inventoried at 2455 lines (issue #1159) because a file under the 2500 hard limit has no `file_sizes.json` row and nothing else would stop it crossing. Blocking in Quick/full QA; self-test covers growth, owner loss, and phase deletion. Structural evidence only, not a claim that the seven partial phases are complete or in-game verified. |
