...
| `check_loc_tags` | `Advisory` | Dev status-tag doctrine surface (issues #301/#335, `docs/LOCALIZATION_STANDARD.md` § 13). It uses the standard 0/1/2 convention (1 = findings, 2 = self-error) but is pinned Advisory so it NEVER blocks. It flags stable leaks, vocabulary/mutex drift, and tags on pure `type="group"` navigation containers; opt-in `-EvidenceAudit` inventories every non-group `[working]` candidate requiring confirmation evidence. |
...
| 19e | Dev status-tag drift (issues #301/#335) — a sanctioned tag leaked into a STABLE build, an unknown/invented tag, a mutex combo (`[crash]`/`[working]`/`[untested]` together), or any status tag on a pure `type="group"` navigation container. | `docs/LOCALIZATION_STANDARD.md § 13` "Dev status tags" | `check_loc_tags.ps1` parses single- + multi-line `en = "..."` forms and correlates loc keys to literal group widgets in each mod's `_data.lua`; checkbox master toggles remain behavioral options. `-EvidenceAudit` adds the #335 non-group `[working]` candidate inventory; `-SummaryOnly` emits counts. WARNING-only, non-blocking (Advisory). Self-test: `-SelfTest` covers group-vs-master classification and evidence inventory alongside vocabulary/mutex/stable rules. | AUTO (script, in run_all) |
| 19f | GitHub issue STATUS-LABEL drift — a fix/probe shipped in a mod's latest CHANGELOG entry but the referenced OPEN issue carries no worked status, or an open issue carries zero/multiple lifecycle labels (`not-started` / `verify-fix` / `verify-fix-coop` / `diagnostics-armed` / `Fixed`; exactly one per PROJECT_STANDARDS §11 and #498). | `PROJECT_STANDARDS.md § 11` "Labels" | `check_issue_status_labels.ps1` — reads each active mod's TOP `## <version>` entry, harvests `#N` refs, and via `gh issue view N` warns on any OPEN shipped-work issue with no worked status; a whole-open-issue pass warns unless lifecycle cardinality is exactly one, including documentation/tooling issues (they use `not-started` while open, never human verify-* states). `coop-required` is an orthogonal tester-routing qualifier and does not increase lifecycle cardinality. SKIPS localization-sweep top entries (tags-only #301 passes reference issues as tag CONTEXT, not shipped work). Self-exits 0 when `gh` is offline/unauthenticated. WARNING-only, non-blocking (Advisory). Self-test: `-SelfTest` (pure parser, zero/one/multiple lifecycle, qualifier, and tooling/documentation fixtures). PREVENTION at the source: `tools/ship/ship.ps1` step 6 (issue #326 part 1) honors explicit `[verify-fix-coop]` / `[coop-required]` CHANGELOG intent and atomically removes competing lifecycle labels. | AUTO (script, in run_all) |
...
| 19h | Promotion leak at SHIP time (issue #327) — a stable/public split-mod ship carrying (a) sanctioned dev status tags in its localization (the 13.5 strip step skipped — stable cim's 7 `[untested]`), (b) a pre-release MOD_VERSION suffix nobody named (ct went public as `0.7.130-beta`), or (c) a MOD_VERSION that doesn't match the top stable CHANGELOG entry / doesn't increase over the previous one. | `docs/PROMOTION_PROCESS.md` "Invariants" + `LOCALIZATION_STANDARD.md § 13.5` | `check_promotion.ps1` — invoked BLOCKING by `tools/ship/ship.ps1` for the five stable split dirs (NOT in run_all; it gates ships, not commits). Hard exit 1 on any finding; `VT2_SUFFIX_OK=1` overrides (b) when the USER named a suffixed public version (issue #328 ruling). Self-test: `-SelfTest` (9 fixture cases, green on pwsh 7 + PS 5.1). Unlike 19e (Advisory, warns for weeks), this is the red gate the 2026-07-04 process review asked for. | AUTO (ship.ps1, stable targets) |
| 19i | A recurring open issue is handled without checking the empirically-related closed history, or a fuzzy match is mistaken for proof and reopened automatically. | `docs/REGRESSION_CHECKLIST.md` + live GitHub issue history | `tools/github/audit-open-issues.ps1` compares every open issue with all closed issues. Ranking records direct references, exact subsystem labels, lifecycle/surface classes, exact code identifiers, corpus-rare terms, and actual closure/verification evidence. The report is read-only and explicitly forbids automatic reopening. `-SelfTest` fixtures prove direct ancestry ranks first, unrelated closure evidence cannot create a relation, transparency reasons survive, and every open issue retains three fallbacks. The self-test is auto-run by `qa/run_selftests.ps1`; live GitHub generation remains a deliberate networked maintenance action. | AUTO (offline self-test in full run_all; live report on demand) |
...
| 52h | An appearance concern gains a provider/adapter but omits one render surface, replay edge, or named offline test, recreating the issue #660 whack-a-mole class while source-local tests remain green. | `docs/WEAPON_APPEARANCE_STANDARD.md` §8a + issue #660 closed-history audit | `appearance_contracts.psd1` is the registry; `check_appearance_contracts.ps1` requires every registered concern to declare every closed-vocabulary surface and replay edge as `covered`, `deferred`, or `not-applicable`, requires reasons for non-covered cells, and requires each covered cell to map to an existing named test. Contract claims are fixed to `structural-only`; this is not runtime verification. Blocking in Quick/full `run_all`. `-SelfTest` proves success plus missing-surface, missing-edge, missing-tests, unmapped-covered-cell, and missing-test-name failures. | AUTO (script, Quick/full run_all) |
...
| `check_loc_tags.ps1` | ⚠ 141 warnings (2026-07-14) | 7 stable-cim leaks + 2 vocabulary findings + 132 tagged pure navigation groups. Opt-in #335 evidence audit inventories 679 non-group `[working]` candidates. Self-test passes; Advisory (never blocks). |
...
| `run_selftests.ps1` | ✅ OK (2026-07-16) | Unit-test entry point: auto-discovers every `qa/check_*.ps1` with a `[switch]$SelfTest` param plus `tools/ship/ship.ps1` (including explicit coop/diagnostic intent, exactly-one lifecycle transitions, native-probe handling, filtered-release contracts, and descriptor deploy equivalence; rows 19f/60/60a/60b/60c), runs each `-SelfTest`, and replays output on failure. All fixtures are offline. Exit 2 on any regression — Standard policy in `run_all` (full pass, not `-Quick`) so a broken check BLOCKS; also a blocking CI step in `qa.yml`. Failure path verified with a planted failing check. |
