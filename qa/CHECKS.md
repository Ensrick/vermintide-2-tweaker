...
| 19f | GitHub issue STATUS-LABEL drift — a fix/probe shipped in a mod's latest CHANGELOG entry but the referenced OPEN issue carries no worked status, or an open issue carries zero/multiple lifecycle labels (`not-started` / `verify-fix` / `verify-fix-coop` / `diagnostics-armed` / `Fixed`; exactly one per PROJECT_STANDARDS §11 and #498). | `PROJECT_STANDARDS.md § 11` "Labels" | `check_issue_status_labels.ps1` — reads each active mod's TOP `## <version>` entry, harvests `#N` refs, and via `gh issue view N` warns on any OPEN shipped-work issue with no worked status; a whole-open-issue pass warns unless lifecycle cardinality is exactly one. SKIPS localization-sweep top entries (tags-only #301 passes reference issues as tag CONTEXT, not shipped work) and documentation/tooling issues from the human-verification lifecycle pass per §11. Self-exits 0 when `gh` is offline/unauthenticated. WARNING-only, non-blocking (Advisory). Self-test: `-SelfTest` (pure parser and zero/one/multiple lifecycle fixtures). PREVENTION at the source: `tools/ship/ship.ps1` step 6 (issue #326 part 1) auto-adds the status label at dev-ship time. Coop-verify issues get the manual `verify-fix` → `verify-fix-coop` swap after ship. | AUTO (script, in run_all) |
