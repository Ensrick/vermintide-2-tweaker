...
| 19f | GitHub issue STATUS-LABEL drift — a fix/probe shipped in a mod's latest CHANGELOG entry but the referenced OPEN issue carries no lifecycle, or an open issue carries zero/multiple universal lifecycle labels (`verify-fix` / `verify-fix-coop` / `diagnostics-armed`; exactly one per PROJECT_STANDARDS §11), including a canonical lifecycle mixed with retired `not-started` / `Fixed`. | `PROJECT_STANDARDS.md § 11` "Labels" | `check_issue_status_labels.ps1` reads each active mod's TOP `## <version>` entry, harvests `#N` refs, and via `gh issue view N` warns on any OPEN shipped-work issue with no lifecycle; a whole-open-issue pass rejects zero/multiple canonical states and any mixed retired state for runtime and repository issues. The explicit `tooling` label changes verification routing, while `documentation` is content-only and `coop-required` is orthogonal. Localization-sweep top entries remain skipped because their refs are context. Self-exits 0 when `gh` is offline/unauthenticated. WARNING-only, non-blocking (Advisory). Self-test: `-SelfTest` (pure parser, retired-only, mixed-retired, zero/one/multiple, qualifier, and repository-only fixtures). PREVENTION: `tools/ship/ship.ps1` requires exactly one explicit CHANGELOG lifecycle marker plus a shared-policy-valid runnable method/expected result before mutation, blocks unevidenced verify-to-diagnostics downgrades, and atomically removes competing/retired lifecycle labels. `tools/verify/generate_playtest.ps1` writes runtime queues plus `VERIFY_REPOSITORY.md` for repository-only autonomous work. | AUTO (script, in run_all) |
