...
@{
  entries = @(
    ...
    # The #535 template publication and impact-hook loop moved out of both
    # entries into _wt_moonfire_aoe.lua. Issue #428 now owns NetworkLookup
    # insertion through the canonical shared helper: the positive rows require
    # that owner and its malformed-shape failure, while the negative rows keep
    # the removed private half-pair rawsets from returning. The entry-side rows
    # still prevent duplicate hook registration (VMF silently drops a second
    # hook on the same Class/method pair).
    ...
    @{ mod='wt'; file='weapon_tweaker/scripts/mods/weapon_tweaker/_wt_moonfire_aoe.lua'; needle='(?s)local _network_lookup = mod:dofile\("scripts/mods/weapon_tweaker/_lib_network_lookup"\).*?_network_lookup\.register_named\(.*?wt_moonfire_aoe_revert NetworkLookup registration rejected \(%s; %s\)'; literal=$false; polarity='present'; minCount=1; maxCount=1; issueRef='#428'; note='public WT loads the manifest-synced canonical helper, delegates the named registration to it, and fails the runtime check on a present malformed lookup shape.' }
    @{ mod='wt'; file='weapon_tweaker/scripts/mods/weapon_tweaker/_wt_moonfire_aoe.lua'; needle='rawset\(lookup,\s*(?:idx,\s*_MOONFIRE_AOE_NAME|_MOONFIRE_AOE_NAME,\s*idx)\)'; literal=$false; polarity='absent'; issueRef='#428'; note='the former private forward/reverse append must not bypass the shared dense symmetric validator.' }
    ...
    @{ mod='wt_dev'; file='weapon_tweaker_dev/scripts/mods/weapon_tweaker_dev/_wt_moonfire_aoe.lua'; needle='(?s)local _network_lookup = mod:dofile\("scripts/mods/weapon_tweaker_dev/_lib_network_lookup"\).*?_network_lookup\.register_named\(.*?wt_moonfire_aoe_revert NetworkLookup registration rejected \(%s; %s\)'; literal=$false; polarity='present'; minCount=1; maxCount=1; issueRef='#428'; note='Dev WT loads the manifest-synced canonical helper, delegates the named registration to it, and fails the runtime check on a present malformed lookup shape.' }
    @{ mod='wt_dev'; file='weapon_tweaker_dev/scripts/mods/weapon_tweaker_dev/_wt_moonfire_aoe.lua'; needle='rawset\(lookup,\s*(?:idx,\s*_MOONFIRE_AOE_NAME|_MOONFIRE_AOE_NAME,\s*idx)\)'; literal=$false; polarity='absent'; issueRef='#428'; note='the mirrored owner must not regain a private forward/reverse append.' }
    ...
    # Source: chaos_wastes_tweaker_dev/CHANGELOG.md 0.7.351-dev (#289 progressive modifier stacking).
    @{ mod='ct_dev'; file='chaos_wastes_tweaker_dev/scripts/mods/chaos_wastes_tweaker_dev/_ct_progressive_modifier_runtime.lua'; needle='mod:hook("GameModeDeus", "mutators"'; literal=$true; polarity='present'; minCount=1; maxCount=1; issueRef='#289'; note='the composed mission-list seam stays ONE full hook in the progressive modifier owner.' }
    @{ mod='ct_dev'; file='chaos_wastes_tweaker_dev/scripts/mods/chaos_wastes_tweaker_dev/_ct_modifier_stack_audit.lua'; needle='mod:hook'; literal=$true; polarity='absent'; issueRef='#289'; note='the audit is observation-only; a second (GameModeDeus, mutators) registration would be silently dropped by VMF.' }
    @{ mod='ct_dev'; file='chaos_wastes_tweaker_dev/scripts/mods/chaos_wastes_tweaker_dev/chaos_wastes_tweaker_dev.lua'; needle='"GameModeDeus", "mutators"'; literal=$true; polarity='absent'; issueRef='#289'; note='the entry must not re-add the pair; VMF keeps only one registration per (Class, method).' }
    # Source: chaos_wastes_tweaker_dev/CHANGELOG.md 0.7.352-dev (#253 Metal wind adapter).
    @{ mod='ct_dev'; file='chaos_wastes_tweaker_dev/scripts/mods/chaos_wastes_tweaker_dev/_ct_weave_metal_runtime.lua'; needle='mod:hook("MutatorHandler", "init"'; literal=$true; polarity='present'; minCount=1; maxCount=1; issueRef='#253'; note='the per-level Metal append stays ONE full pre-hook on the handler constructor, owned by the adapter.' }
    @{ mod='ct_dev'; file='chaos_wastes_tweaker_dev/scripts/mods/chaos_wastes_tweaker_dev/_ct_weave_metal_runtime.lua'; needle='mod:hook_safe("MutatorHandler", "destroy"'; literal=$true; polarity='present'; minCount=1; maxCount=1; issueRef='#253'; note='the teardown armor-restoration proof stays ONE safe hook on the handler destructor.' }
    @{ mod='ct_dev'; file='chaos_wastes_tweaker_dev/scripts/mods/chaos_wastes_tweaker_dev/_ct_weave_metal_runtime.lua'; needle='Managers.weave'; literal=$true; polarity='absent'; issueRef='#253'; note='the adapter supplies wind strength from its own context and never reads, replaces or stubs the global Weave manager.' }
    @{ mod='ct_dev'; file='chaos_wastes_tweaker_dev/scripts/mods/chaos_wastes_tweaker_dev/_ct_weave_metal_runtime.lua'; needle='pcall(printf, "[ct:253:diag] action=%s result=%s'; literal=$true; polarity='present'; minCount=1; maxCount=1; issueRef='#253'; note='the diagnostics-armed card cites this exact command-owned receipt route; it must stay a single literal emitter.' }
    @{ mod='ct_dev'; file='chaos_wastes_tweaker_dev/scripts/mods/chaos_wastes_tweaker_dev/_ct_weave_curse_audit.lua'; needle='mod:hook'; literal=$true; polarity='absent'; issueRef='#253'; note='the feasibility audit stays observation-only; the adapter owns every #253 hook.' }
    @{ mod='ct_dev'; file='chaos_wastes_tweaker_dev/scripts/mods/chaos_wastes_tweaker_dev/chaos_wastes_tweaker_dev.lua'; needle='"MutatorHandler", "init"'; literal=$true; polarity='absent'; issueRef='#253'; note='the entry must not re-add the pair; VMF keeps only one registration per (Class, method).' }
    @{ mod='ct_dev'; file='chaos_wastes_tweaker_dev/scripts/mods/chaos_wastes_tweaker_dev/chaos_wastes_tweaker_dev_data.lua'; needle='weave_metal'; literal=$true; polarity='absent'; issueRef='#253'; note='the Metal wind adapter stays hidden from the curse menu until the two-player verification matrix passes.' }
    ...
    @{ mod='gut_dev'; file='gui_tweaker_dev/scripts/mods/gui_tweaker_dev/_gut_spawn_weapon_recovery.lua'; needle='record(ledger, "miss", ok_miss, err_miss, d)'; literal=$true; polarity='present'; minCount=1; maxCount=1; issueRef='#1637'; note='the miss route records its pcall result in the module-local ledger the in-game check reads; the route is proven from the ledger, never by replacing the global printf.' }
    @{ mod='gut_dev'; file='gui_tweaker_dev/scripts/mods/gui_tweaker_dev/_gut_spawn_weapon_recovery.lua'; needle='rawset(_G, "printf"'; literal=$true; polarity='absent'; issueRef='#1637'; note='0.2.353-dev swapped the global printf for its in-game proof; the deployed-source authority (Test-VtLuaMutatesGlobalPrintf) rejects any deployed mod that mutates global printf, record-wide and fail-closed, and every CI lifecycle guard, card refresh and ship label step failed repo-wide.' }
    @{ mod='gut_dev'; file='gui_tweaker_dev/scripts/mods/gui_tweaker_dev/_gut_spawn_weapon_recovery.lua'; needle='(?m)^(?!\s*--).*\b(?:_G|_ENV)\s*\.\s*printf\s*=(?!=)'; literal=$false; polarity='absent'; issueRef='#1637'; note='no direct global printf assignment either (same authority rule); comment mention allowed.' }
    @{ mod='gut_dev'; file='gui_tweaker_dev/scripts/mods/gui_tweaker_dev/_gut_spawn_weapon_recovery.lua'; needle='_G["printf"]'; literal=$true; polarity='absent'; issueRef='#1637'; note='no bracketed global printf write either (same authority rule).' }
    @{ mod='gut_dev'; file='gui_tweaker_dev/scripts/mods/gui_tweaker_dev/_gut_bot_pose.lua'; needle='rawset(_G, "printf"'; literal=$true; polarity='absent'; issueRef='#1637'; note='the regression checks read the route ledger; they never swap the global printf.' }
    ...
    @{ mod='ct_dev'; file='chaos_wastes_tweaker_dev/scripts/mods/chaos_wastes_tweaker_dev/_ct_regression_resource_safety.lua'; needle='(?m)^(?!\s*--).*\bio\.open\s*\('; literal=$false; polarity='absent'; issueRef='#511'; note='ct resource-safety regression owner must stay marker-based; io is nil in the retail sandbox.' }
