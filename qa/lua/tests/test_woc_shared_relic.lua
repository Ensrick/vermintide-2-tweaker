return function(H, repo_root)
	...
	runtime_file:close()
	local owner_file = assert(io.open(repo_root
	...
	H.test("WOC #934 runtime uses same-mod authority and vanilla trophy ids", function()
		...
		H.truthy(contract_source:find(
			'identity_listener = notify_identity', 1, true),
			"TeamPreviewer must consume the authenticated lease snapshot")
		H.truthy(contract_source:find(
			'function runtime:identity_for_peer(peer_id)', 1, true),
			"preview consumers need a read-only semantic snapshot")
		H.truthy(contract_source:find(
