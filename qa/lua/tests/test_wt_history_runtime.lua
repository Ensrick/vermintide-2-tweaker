...
local function register(H, context)
    ...
    H.test("WT #1436 Hotfix 6.11.2 changes only Sienna Dagger H2 profile", function()
        with_profile_globals(function()
            ...
            local function roots_with_profile(damage_profile)
                ...
                local weapons = { one_handed_daggers_template_1 = template }
                return {
            ...
            for _, case in ipairs({
                ...
                local current_runtime = Runtime.install({
                    ...
                    mod = mod_fixture({ wt_history_sienna_dagger = "current" },
                        { value = true }),
                    policy = Policy,
                    roots = current_roots,
                })
                ...
                local history_runtime = Runtime.install({
                    catalog = case.catalog,
                    mod = mod_fixture({ wt_history_sienna_dagger = "6_11_1" },
                        { value = true }),
                    policy = Policy,
                    roots = history_roots,
                })
            ...
            H.equal(DamageProfileTemplates, profile_root,
                "Dagger history must not replace the global profile registry")
            H.equal(DamageProfileTemplates.native_profile, native_profile,
                "Dagger history must not replace a native profile")
            H.equal(NetworkLookup, lookup_root,
                "Dagger history must not replace NetworkLookup")
            H.equal(NetworkLookup.damage_profiles, lookup,
                "Dagger history must not replace the damage-profile lookup")
            H.deep_equal(DamageProfileTemplates, profile_before,
                "Dagger history must not add a private global profile")
            H.deep_equal(NetworkLookup, lookup_before,
                "Dagger history must not add a global lookup row")
        end)
    end)
