return function(H, repo_root)
    ...
    H.test("production proves static panels and separates mission secondary overview", function()
        local file = assert(io.open(cim_root .. "_cim_mission_forge_safety.lua", "rb"))
        local source = file:read("*a")
        file:close()
        H.truthy(source:find('mod:hook("HeroWindowWeaveForgeOverview", "_create_item_previewer"', 1, true))
        H.truthy(source:find("overview_preview_x", 1, true))
        H.truthy(source:find("_cim882_mirrored_viewport = invert_rendering == true", 1, true))
        H.truthy(source:find("content._cim882_mirrored_viewport == true", 1, true))
        H.truthy(source:find("policy.sanitize_widgets(entry.widgets", 1, true))
        H.truthy(source:find("icon_policy.renderer_has_texture(", 1, true))
        H.truthy(source:find("window._ui_renderer", 1, true))
        H.truthy(source:find("window._ui_top_renderer", 1, true))
        H.equal(source:find('local _CIM_ATLAS_ATHANOR_PREFIX = "athanor_skilltree_slot"', 1, true), nil)
    end)
end
