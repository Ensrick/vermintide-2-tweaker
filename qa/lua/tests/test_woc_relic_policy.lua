return function(H, repo_root)
	...
	H.test("WOC #637 provider definition is one immutable Cursed relic", function()
		local row = definition("woc_one", "woc_one_001")
		H.truthy(policy.is_definition(row))
		H.equal(row.rarity, "cursed")
		H.equal(row.skin_combination_table, nil)
		H.equal(row.mod_data.rarity, "cursed")
		H.equal(row.mod_data.CustomData.rarity, "cursed")
