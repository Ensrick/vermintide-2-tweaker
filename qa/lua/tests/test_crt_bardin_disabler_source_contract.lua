return function(H, repo_root)
    local production_probe = repo_root
    ...
    H.test("CRT #440 retired probe stays out of production", function()
        H.equal(exists(production_probe), false)

        local entry = read(repo_root
            .. "/career_tweaker/scripts/mods/career_tweaker/career_tweaker.lua")
        H.equal(entry:find("_crt_bardin_disabler_probe", 1, true), nil)
        H.equal(entry:find("_crt_bardin_disabler_tick", 1, true), nil)
    end)
    ...
    H.test_if(has_vanilla_source,
        "CRT #440 source contract distinguishes the three disabler boundaries", function()
        ...
        H.truthy(prepare:find(
            "Unit.world_position(blackboard.target_unit, 0) + Vector3(0, 0, 0.2)", 1, true))
