return function(H, repo_root)
    ...
    local function fixture(options)
        ...
        local careers = { es_mercenary = { item_slot_types_by_slot_name = {
        ...
        local checks = {}
        local mod = {
            _cim_rt_register = function(name, check) checks[name] = check end,
        ...
        return { importer=importer, build=build, equipped=equipped, previous=previous,
            masters=masters, careers=careers, checks=checks,
    ...
    H.test("Ranald importer preflights and atomically applies five slots plus six talents", function()
        ...
        H.equal(type(f.checks.issue1141_ranald_slot_admission), "function")
        H.equal(f.checks.issue1141_ranald_slot_admission(), nil)
    end)
    ...
    local function assert_refused_without_writes(f, expected)
        local ok, reason = f.importer.apply(f.build)
        H.equal(ok, false)
        H.truthy(reason:find(expected, 1, true), reason)
        H.equal(#f.created_order, 0)
        H.equal(next(f.registered), nil)
        H.equal(#f.removed, 0)
        H.equal(#f.finalized, 0)
        H.equal(f.dirties(), 0)
    ...
    H.test("Ranald importer rejects ranged primary and melee secondary before writes", function()
        for _, slot in ipairs({ "slot_melee", "slot_ranged" }) do
            local f = fixture()
            local other = slot == "slot_melee" and "slot_ranged" or "slot_melee"
            f.build.slots[slot] = f.build.slots[other]
            assert_refused_without_writes(f, "item slot mismatch: " .. slot)
        end
    end)

    H.test("Ranald importer rejects missing malformed and empty career slot policies", function()
        for _, value in ipairs({ false, "ranged", {} }) do
            local f = fixture()
            f.careers.es_mercenary.item_slot_types_by_slot_name.slot_ranged = value
            assert_refused_without_writes(f, "item slot mismatch: slot_ranged")
        end
        local f = fixture()
        f.careers.es_mercenary.item_slot_types_by_slot_name = nil
        assert_refused_without_writes(f, "item slot mismatch: slot_melee")
    end)

    H.test("Ranald importer refuses accessories whose existing snapshot has another slot kind", function()
        for _, entry in ipairs({
                { "necklace_base", "slot_necklace", "melee" },
                { "ring_base", "slot_ring", "necklace" },
                { "trinket_base", "slot_trinket_1", "ring" },
            }) do
            local f = fixture()
            f.masters[entry[1]].slot_type = entry[3]
            assert_refused_without_writes(f, "item slot mismatch: " .. entry[2])
        end
    end)

    H.test("Ranald importer applies career-approved dual melee without hardcoded exceptions", function()
        for _, entry in ipairs({
                { 6, "dr_slayer", 5, "dr_1h_axe" },
                { 16, "es_questingknight", 15, "es_1h_sword" },
                { 1, "es_mercenary", 15, "es_1h_sword" },
            }) do
            local f = fixture()
            f.careers[entry[2]] = { item_slot_types_by_slot_name = {
                slot_melee = { "melee" }, slot_ranged = { "melee", "ranged" },
            } }
            f.masters[entry[4]] = { slot_type = "melee", can_wield = {entry[2]},
                property_table_name = "melee", trait_table_name = "melee" }
            f.build.career_id, f.build.career_name = entry[1], entry[2]
            f.build.slots.slot_melee.weapon_id = entry[3]
            f.build.slots.slot_ranged = f.build.slots.slot_melee
            local ok, result = f.importer.apply(f.build)
            H.equal(ok, true, entry[2] .. ": " .. tostring(result))
            H.equal(#result.backend_ids, 5)
            H.equal(f.registered[result.by_slot.slot_ranged].item_key, entry[4])
        end
    end)

    H.test("Ranald importer slot allowance never bypasses can_wield", function()
        local f = fixture()
        f.masters.es_1h_sword.can_wield = { "es_questingknight" }
        assert_refused_without_writes(f, "career cannot wield es_1h_sword")
    end)
