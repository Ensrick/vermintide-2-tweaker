return function(H, repo_root)
	...
	H.test("WOC #613 score identity resolves only the exact human wearer", function()
		...
		for _, local_id in ipairs({ 0, 2 }) do
			peer, source = identity.resolve_score_peer(5, 4, {
				row = {
					peer_id = "peer-client", local_player_id = local_id,
					profile_index = 5, career_index = 4,
					is_player_controlled = true,
				},
			})
			H.equal(peer, nil,
				"a peer-only lease must reject a different local player")
			H.equal(source, "score_snapshot_untrusted")
		end
	end)

	H.test("WOC #613 live identity rejects bots and profile-only matches", function()
		local human = {
			peer_id = "peer-human", profile = 5, career = 4, local_id = 1,
		}
		local wrong_career = {
			peer_id = "peer-other", profile = 5, career = 1, local_id = 1,
		}
		local bot = {
			peer_id = "peer-human", profile = 2, career = 3,
			local_id = 1, bot_player = true,
		}
		local function profile_for(player)
			return player.profile, player.career, player.local_id
		end
		...
		peer, source = identity.resolve_live_peer(5, 4,
			{ human, {
				peer_id = "peer-rival", profile = 5, career = 4, local_id = 1,
			} },
			profile_for)
		H.equal(peer, nil)
		...
		peer, source = identity.resolve_live_peer(5, 4, {
			{ peer_id = "peer-human", profile = 5, career = 4, local_id = 2 },
		}, profile_for)
		H.equal(peer, nil,
			"local player 2 must not inherit local player 1's peer lease")
		H.equal(source, "live_miss")
		peer = identity.resolve_live_peer(5, 4, {
			{ peer_id = "peer-human", profile = 5, career = 4 },
		}, profile_for)
		H.equal(peer, nil, "missing local-player identity must fail closed")
		peer, source = identity.resolve_live_peer(5, 4, { human }, function()
			error("profile reader failed")
		end)
		H.equal(peer, nil, "a throwing profile reader must fail closed")
		H.equal(source, "live_miss")
	end)
