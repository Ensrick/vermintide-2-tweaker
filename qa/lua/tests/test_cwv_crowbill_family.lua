return function(H, repo_root)
    ...
    H.test("CWV Crowbill identities defaults fallback and approved models are exact", function()
        ...
        H.deep_equal(dawi_default.right_hand_scale, { 0.5, 0.5, 0.5 })
        H.deep_equal(dawi_default.right_hand_rotation, { -90, -90, -90 })
        H.equal(dawi_default.right_hand_scale_3p, nil)
    ...
    H.test("Dawi Crowbill Model 01 owns its reviewed all-surface transform", function()
        local target = row_for(family.MODELS, "cwv_dr_dawi_crowbill_skin")
        H.truthy(target)
        H.deep_equal(target.right_hand_scale, { 0.5, 0.5, 0.5 })
        H.deep_equal(target.right_hand_rotation, { -90, -90, -90 })
