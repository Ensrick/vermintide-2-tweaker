return function(H, repo_root)
	local policy = dofile(repo_root
		.. "/character_weapon_variants/scripts/mods/character_weapon_variants/_cwv_axe_balance.lua")
	...
	H.test("CWV #601 Greataxe light crit floors at ten percent and includes the CWV clone", function()
	...
	H.test("CWV #601 light crit preserves a stronger authored bonus", function()
	...
	H.test("CWV #601 Dual Axes light crit excludes heavy and utility actions", function()
	...
	H.test("CWV #601 Dual Axes cleave clones all direct profiles and restores exactly", function()
		local dual = axe_template()
		...
		local actions = dual.actions.action_one
		H.equal(actions.light_attack_left.damage_profile, "cwv_axe_cleave_axe_light")
		H.equal(actions.light_attack_bopp.damage_profile_left, "cwv_axe_cleave_axe_bopp")
		H.equal(actions.light_attack_bopp.damage_profile_right, "cwv_axe_cleave_axe_bopp")
		H.equal(actions.heavy_attack.damage_profile_left, "cwv_axe_cleave_axe_heavy")
		H.equal(power_levels[damage_profiles.cwv_axe_cleave_axe_light.cleave_distribution].attack, 2.2)
		H.equal(power_levels[damage_profiles.cwv_axe_cleave_axe_heavy.cleave_distribution].impact, 8.8)
		H.truthy(math.abs(damage_profiles.cwv_axe_cleave_axe_bopp.cleave_distribution.attack - 3.3) < 0.000001)
		H.equal(damage_profiles.cwv_axe_cleave_axe_bopp.marker, "bopp")
		H.equal(power_levels.cleave_light.attack, 2)
		H.equal(damage_profiles.axe_bopp.cleave_distribution.attack, 3)
		H.equal(recorded.cwv_axe_cleave_axe_heavy, "axe_heavy")
		H.equal(registered.cwv_axe_cleave_axe_light, true)
	...
	H.test("CWV #601 Dual Axes crit and cleave toggles remain independent", function()
		local dual = axe_template()
		...
		H.equal(dual.actions.action_one.light_attack_left.damage_profile, "cwv_axe_cleave_axe_light")
	...
	H.test("CWV #601 settings default on and compose with canonical lifecycle", function()
		...
		H.truthy(enabled:find("_om._apply_axe_balance(nil, false)", 1, true))
		H.truthy(disabled:find("_om._apply_axe_balance(nil, true)", 1, true))
		H.truthy(source:find("_om._apply_axe_balance(setting_id, false)", 1, true))
	end)
end
