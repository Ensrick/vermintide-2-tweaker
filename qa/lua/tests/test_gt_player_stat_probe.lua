return function(H, repo_root)
    ...
    H.test("GT #797 runtime remains local read-only and lifecycle cached", function()
        ...
        H.truthy(main:find('local MOD_VERSION = "0.2.259-dev"', 1, true))
    end)

    H.test("GT #797 documents engine surfaces and release identity", function()
        local engine = read(repo_root .. "/general_tweaker_dev/ENGINE_SURFACE.md")
        local changelog = read(repo_root .. "/general_tweaker_dev/CHANGELOG.md")
        local commands = read(repo_root .. "/docs/COMMANDS.md")
        H.truthy(engine:find("BuffExtension.apply_buffs_to_value", 1, true))
        H.truthy(engine:find("WeaponUnitExtension.current_action_settings.lookup_data", 1, true))
        H.truthy(engine:find("1.5 / max * 100", 1, true))
        H.truthy(engine:find("activation factor", 1, true))
        H.truthy(changelog:find("v0.2.255-dev", 1, true))
        H.truthy(changelog:find("Source-backed live player stat HUD", 1, true))
        H.truthy(changelog:find("1.5 / authoritative_max_fatigue_points * 100", 1, true))
        H.truthy(commands:find("gt_stat_hud_page", 1, true))
        H.truthy(commands:find("gt_stat_hud_metrics", 1, true))
    end)
end
