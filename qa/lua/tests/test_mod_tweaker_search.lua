return function(H, repo_root)
    ...
    H.test("Mod Tweaker search binds the native inventory magnifier with text clearance", function()
        ...
        file:close()
        H.truthy(source:find('ICON_TEXTURE, ICON_SIZE, ICON_X, ICON_Y = "search_filters_icon", 95, -28, -3', 1, true))
        H.truthy(source:find('native_icon_y = -4', 1, true))
        ...
        H.truthy(view_source:find('sc.text = _search_placeholder(self)', 1, true))
        H.truthy(view_source:find('return "Search " .. tostring(label)', 1, true))
    end)
end
