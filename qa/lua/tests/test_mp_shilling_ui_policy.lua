return function(H, repo_root)
    ...
    main_file:close()
    local loc_file = assert(io.open(repo_root .. "/modded_progression/scripts/mods/modded_progression/modded_progression_localization.lua", "rb"))
    local loc_source = loc_file:read("*a")
    loc_file:close()
    ...
    H.test("MP shilling presentation remains byte-for-byte vanilla", function()
        ...
        H.equal(Policy.wallet_text, nil)
        H.equal(main_source:find('mod:hook(_G, "Localize"', 1, true), nil)
        H.equal(main_source:find('mod:hook(BackendUtils, "get_fake_currency_item"', 1, true), nil)
        H.equal(main_source:find('mod:hook("StoreWindowItemPreview", "_set_price"', 1, true), nil)
        H.equal(main_source:find("mp_local_wallet_prefix", 1, true), nil)
        H.equal(main_source:find("mp_local_purchase_button", 1, true), nil)
        H.equal(loc_source:find("Local Silver Shillings", 1, true), nil)
        H.equal(loc_source:find("Buy with Local Shillings", 1, true), nil)
        H.equal(loc_source:find("[Local]", 1, true), nil)
    end)
