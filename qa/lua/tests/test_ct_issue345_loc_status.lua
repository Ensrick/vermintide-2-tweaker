return function(H, repo_root)
    ...
    H.test("CT player-facing rows omit issue lifecycle state", function()
        local expected = {
            inject_adventure_maps = "Inject Adventure Missions into CW Map Pool",
            progressive_difficulty = "Progressive Difficulty",
            finale_dominant_god = "Finale God",
            respawn_on_chest_complete = "Revive Team on Chest Completion",
            disable_boon_ct_meta_ammo = "Disable Boon: (Mod Boon) Quiver Cascade",
            start_boon_ct_meta_ammo = "Starting Boon: (Mod Boon) Quiver Cascade",
            enable_boon_vauls_anvil = "Rework: Vaul's Anvil as Boon (Unique)",
            start_boon_ct_boon_vauls_anvil = "Starting Boon: (Mod Boon) Vaul's Anvil",
        }
    ...
    H.test("CT obsolete runtime status synchronization is removed", function()
        ...
        main_file:close()
        H.equal(main:find('issue345_ct_localization_status_sync', 1, true), nil)
    end)
end
