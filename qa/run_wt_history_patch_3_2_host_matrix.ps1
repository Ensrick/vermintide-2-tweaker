# Blocking dual-host provenance gate for issue #1436's Patch 3.2 Kerillian
# One-Handed Axe slice. Both PowerShell hosts independently validate the pinned
# ledger, reproduce adjacent/current source evidence when the source checkout
# exists, and regenerate the byte-exact catalog outside the repository.

[CmdletBinding()]
param([switch]$Quiet)

$ErrorActionPreference = 'Stop'
$checkPath = Join-Path $PSScriptRoot 'check_wt_history_patch_3_2_reproducibility.ps1'
...
foreach ($requiredPath in @($checkPath, $freshnessPath, $sourceCheckoutPath)) {
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
        Write-Host "[wt-history-patch-3-2-host-matrix] ERROR -- missing $requiredPath" -ForegroundColor Red
        exit 2
    }
}
...
foreach ($hostEntry in $hosts) {
    ...
    if (-not $Quiet) {
        Write-Host ("=== {0}: Patch 3.2 history reproduction ===" -f $hostEntry.Name) -ForegroundColor Cyan
    }
...
if ($failed.Count -gt 0) {
    Write-Host ("[wt-history-patch-3-2-host-matrix] FAILED -- {0}" -f ($failed -join ', ')) -ForegroundColor Red
    exit 2
}

if (-not $Quiet) {
    Write-Host '[wt-history-patch-3-2-host-matrix] OK -- Patch 3.2 provenance reproduces under both PowerShell hosts.' -ForegroundColor Green
}
exit 0
