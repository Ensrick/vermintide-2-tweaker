...
$checkPath = Join-Path $PSScriptRoot 'check_wt_history_patch_3_1_reproducibility.ps1'
...
foreach ($requiredPath in @($checkPath, $freshnessPath, $sourceCheckoutPath)) {
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
        Write-Host "[wt-history-patch-3-1-host-matrix] ERROR -- missing $requiredPath" -ForegroundColor Red
        exit 2
    }
}
...
foreach ($hostEntry in $hosts) {
    ...
    if (-not $Quiet) {
        Write-Host ("=== {0}: Patch 3.1 history reproduction ===" -f $hostEntry.Name) -ForegroundColor Cyan
    }
...
if ($failed.Count -gt 0) {
    Write-Host ("[wt-history-patch-3-1-host-matrix] FAILED -- {0}" -f ($failed -join ', ')) -ForegroundColor Red
    exit 2
}

if (-not $Quiet) {
    Write-Host '[wt-history-patch-3-1-host-matrix] OK -- Patch 3.1 provenance reproduces under both PowerShell hosts.' -ForegroundColor Green
}
exit 0
