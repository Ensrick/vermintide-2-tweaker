            ...
            Concerns = @(
                @{
                    ...
                    ReplayEdges = @{
                        ...
                        peer_ready = @{ Disposition = 'covered'; Evidence = 'the authenticated host snapshot triggers one generation-bound TeamPreviewer re-equip whose spawned unit consumes the transform adapter' }
                    ...
                    Tests = @(
                        ...
                        @{
                            ...
                            ReplayEdges = @('peer_ready', 'preview_open', 'preview_reopen', 'lobby_score_create')
                        }
                    )
                }
            )
            ...
            Concerns = @(
                @{
                    ...
                    Surfaces = @{
                        ...
                        lobby = @{ Disposition = 'covered'; Evidence = 'TeamPreviewer resolves an exact non-bot live profile and career before asynchronous hero spawn, then consumes only the accepted host lease snapshot' }
                        score_team = @{ Disposition = 'covered'; Evidence = 'TeamPreviewer requires an exact player-controlled score row before consuming the accepted host lease snapshot' }
                    ...
                    Tests = @(
                        ...
                        @{
                            ...
                            Names = @(
                                'WOC #613 host snapshot reaches cache rewield and TeamPreviewer once'
                            )
                            Surfaces = @('husk')
