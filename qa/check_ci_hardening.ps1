    ...
    steps:
      ...
      - name: Validate tracker lifecycle and pinned test cards
        ...
        run: |
          ...
          ./tools/github/check-lifecycle-cardinality.ps1 -Repository '${{ github.repository }}'
'@
    # #1321: Linux pwsh can preserve this CRLF-authored script's literal line
    # endings. Normalize the mutation source, then prove both encodings below.
    $goodIssueLifecycleWorkflow = $goodIssueLifecycleWorkflow -replace '\r\n?', "`n"
    $goodIssueLifecycleVariants = @(
        @{ Name = 'LF'; Text = $goodIssueLifecycleWorkflow; Newline = "`n" },
        @{ Name = 'CRLF'; Text = ($goodIssueLifecycleWorkflow -replace "`n", "`r`n"); Newline = "`r`n" }
    )
    foreach ($variant in $goodIssueLifecycleVariants) {
        $goodIssueLifecycle = @(Test-IssueLifecycleCheckoutContract $variant.Text)
        if ($goodIssueLifecycle.Count -ne 0) {
            throw ("valid $($variant.Name) issue lifecycle checkout was rejected: " + ($goodIssueLifecycle -join '; '))
        }
    }
...
'@
    ...
    foreach ($variant in $goodIssueLifecycleVariants) {
        $badSkippedWorkflow = $variant.Text -replace '(?m)^(      - name: Checkout default-branch policy)\r?$', ('$1' + $variant.Newline + '        if: false')
        $badSkippedWorkflow = $badSkippedWorkflow -replace '(?m)^(      - name: Validate tracker lifecycle and pinned test cards)\r?$', ('$1' + $variant.Newline + '        continue-on-error: true')
        ...
        if (-not ($badSkipped -match 'checkout step cannot be skipped') -or
            ...
            throw "issue lifecycle $($variant.Name) skipped/error-ignored step spoof was not detected"
        }
    }
    ...
    $badDuplicateTimeoutWorkflow = $goodIssueLifecycleWorkflow -replace '(?m)^(    timeout-minutes: 20)$', "`$1`n    timeout-minutes: 30"
    ...
    $badJobContinueWorkflow = $goodIssueLifecycleWorkflow -replace '(?m)^(    timeout-minutes: 20)$', "`$1`n    continue-on-error: true"
