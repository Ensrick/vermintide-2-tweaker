...
function Test-CiContract {
    param([string]$Workflow, [string]$TrustedWorkflow, [string]$ProtectionTool)
    $errors = @()
...
'@
    ...
    $goodTool += '; stable-promotion-authorization'
    $good = @(Test-CiContract $goodWorkflow $goodTrustedWorkflow $goodTool)
    ...
    $bad = @(Test-CiContract $badWorkflow $badTrustedWorkflow $badTool)
...
if (-not (Test-Path $workflowPath) -or -not (Test-Path $trustedWorkflowPath) -or -not (Test-Path $protectionPath)) {
    Write-Host "[check_ci_hardening] ERROR - QA/trusted workflow or protection tool missing." -ForegroundColor Red
    exit 2
}
...
$errors = @(Test-CiContract $workflow $trustedWorkflow $protection)
