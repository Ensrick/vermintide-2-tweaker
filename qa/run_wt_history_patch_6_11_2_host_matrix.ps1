...
$checkPath = Join-Path $PSScriptRoot `
    'check_wt_history_patch_6_11_2_reproducibility.ps1'

if (-not (Test-Path -LiteralPath $checkPath -PathType Leaf)) {
    Write-Host "[wt-history-patch-6-11-2-host-matrix] ERROR -- missing $checkPath" -ForegroundColor Red
    exit 2
}
...
foreach ($hostEntry in $hosts) {
    ...
    if (-not $Quiet) {
        Write-Host ("=== {0}: Hotfix 6.11.2 history reproduction ===" -f `
...
if ($failed.Count -gt 0) {
    Write-Host ("[wt-history-patch-6-11-2-host-matrix] FAILED -- {0}" -f `
        ($failed -join ', ')) -ForegroundColor Red
    exit 2
}
if (-not $Quiet) {
    Write-Host '[wt-history-patch-6-11-2-host-matrix] OK -- Hotfix 6.11.2 provenance reproduces under both PowerShell hosts.' -ForegroundColor Green
}
exit 0
