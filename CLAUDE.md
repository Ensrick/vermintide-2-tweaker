...
| 10 | Never claim a runtime issue "fixed" / "feature-complete" until the USER confirms in-game. Compile success and structural review are not runtime verification. Repository-only tooling/docs may be verified autonomously with the issue's named deterministic check. |
| 11 | Loc: raw setting keys in widget fields, never pre-localize before registration; escape a literal `%` as `%%`. No em dashes in menu-facing strings. Player-facing localization in every stream omits issue/lifecycle metadata (`[working]`, `[untested]`, `[Issue N]`, `[verify-fix]`, `[diag]`, `[WIP]`, `(Experimental)`, etc.); keep that state in GitHub/changelogs/logs. Functional qualifiers such as `(CWV)`, `[Host Only]`, and `[WARNING]` remain valid. See `docs/LOCALIZATION_STANDARD.md` § 13. |
...
| 13 | Deferred/blocked work goes to GitHub Issues (search first), not floating TODO comments. Every OPEN issue carries exactly one lifecycle: `not-started`, `diagnostics-armed`, or `verify-fix`. The latter two mean runnable live in-game testing NOW and require the newest exact `## CURRENT LIVE TEST` comment to be the one and only pinned exact card (semantic version + `[mod:LOAD]`, or a clearly labeled exact versioned banner such as `[WOC] v0.1.42-dev loaded`; topology; numbered localized/player-facing steps; expected result). Test solo first; add `coop-required` only when the card says solo passed/exhausted and the remaining topology is co-op. Blocked issues are `not-started` and carry no test qualifier. `Fixed` and `verify-fix-coop` are invalid while open. Tooling/docs never enter the live-test queue; verify them autonomously and close. Also apply exactly one type (`bug`/`enhancement`/`feature`, with `crash` a flag on `bug`) + mod tag. Never invent a status label. Issue format: title 8 words max; body = empirical data only (~150 words: bold Symptom/Evidence/Fix/Refs labels, no `##` headings). Multiline bodies MUST use `gh ... --body-file`; never embed escaped `\n`. Full scheme: `PROJECT_STANDARDS.md` §11 "Labels" + "Issue format". |
...
| 16 | Worktrees are leased capacity, not permanent folders. Read-only work gets no worktree. Create/close isolated write trees only through `tools/worktrees/worktree.ps1`; maximum 8 secondary trees / 12 GiB total, and close each finished agent tree in the same session. |
...
| character_dialogue | `character_dialogue` | 3765055148 (friends-only) | single | Source-generated dialogue catalogue, sparse per-line eligibility overrides, and local preview transport. Tweaker: GUI dev owns the Dialogue tab renderer; Character Dialogue owns persistence and natural-selection policy. See Issue #605. |
...
| buff_tweaker | `bt` | 3730358590 | **RETIRED** | Retired; archived to `_archive/buff_tweaker_v0.1.12-alpha/`. Was the shared Big Rebalance registry + `net_replay` ring buffer. The former consumer guard (`is_br_active`) no longer exists outside `_archive/` - BR widgets are hidden and modules unloaded under the #321 retirement contract, enforced by `qa/check_retired_big_rebalance.ps1` (blocking gate in `qa/run_all.ps1`). |
...
**Cross-mod refs always target stable.** External mods (CWV, cosmetics_tweaker, et, etc.) that consume sibling mods via `get_mod("cim")` / `get_mod("gt")` / `get_mod("mp")` MUST resolve against the stable mod_id. Dev clones are isolated test surfaces; nothing external consumes them. If you need a cross-mod hook to fire against dev for testing, edit the consumer's `get_mod(...)` call to point at the dev id locally - never ship that change to a stable directory.
...
- **Worktree lifecycle**: read-only subagents share the current checkout. When isolated writes genuinely require a linked worktree, create it with `tools/worktrees/worktree.ps1 -Action Create`, which defaults outside `source\repos` and enforces the 8-tree / 12-GiB budget. After the work is committed or merged, run `-Action Close` from the primary checkout in the same session (never execute the close script from inside its target on Windows). The close path refuses dirty source and ambiguous ignored content. `qa/check_worktree_budget.ps1` is a blocking Quick/full-QA backstop.
