...
| weapons_of_chaos | `WOC` | 3753880932 | **VMB** | single | Lets player characters wield ENEMY weapons + named keep-trophy artifacts, built the duplicate-item way (clone a player base weapon template, swap the held mesh to an enemy/prop `.unit`) — same approach as `character_weapon_variants`, but the mesh source is an enemy/trophy unit. First item: **Blightreaper** (`woc_blightreaper`) — Kruber's 1H sword (`es_1h_sword`/`one_handed_swords_template_1`) equippable by all 20 careers. The intended Bögenhafen keep-trophy diorama prop (`units/props/inn/hub_trophy/hub_trophy_bogenhafen`) proved **NOT runtime-loadable** (no standalone `.package`, absent from the boot-loaded bogenhafen DLC + base keep `inn` bundles; force-loading the unit path hard-crashed the game on keep entry — v0.1.1-dev, 2026-06-29 log), so the **interim held mesh is the base Empire 1H sword** (`HELD_UNIT`, crash-free); swap it to an extracted Blightreaper `.unit` once a real model is authored. friends-only. Scaffolded 2026-06-28; **v0.1.3-dev live (crash fixed 2026-06-29)**. Research + crash post-mortem in `weapons_of_chaos/DEVELOPMENT.md`. |
...
**Why the split exists.** The five public-Workshop mods (`ct`, `cim`, `gt`, `verminious_dreams_lighting`, `gut` — the last joining the split 2026-06-24, with its public item still a friends-only public beta in progress) have a mixed audience: public subscribers expect a stable bundle that doesn't break weekly, and the friends cohort wants visibility into in-flight work as it's being iterated on. Shipping every dev iteration to the public Workshop item caused subscriber loss (~80 cim subs in a few days in May 2026 from reflex uploads pushing unstable mid-fix builds). The fix is two parallel Workshop items per split mod: dev = friends-only, stable = public.

**What's split.** Only the five public mods (`ct`, `cim`, `gt`, `verminious_dreams_lighting`, `gut`). Everything else is single-stream (already friends-only or unpublished — `wt`, `cosmetics_tweaker`, `cwv`, `et`, `crt`, `bt`, `dcp`, `mp`).
...
**Canonical one-shot:** `tools\ship\ship.ps1 -Mod <name>` does the whole release in one call — `VMBLauncher.exe all` (build+deploy+upload), then `publish-release.ps1` (GitHub release), then it verifies the deploy hashes and the `workshop_log.txt` upload status and prints a loud "restart Steam" reminder. Prefer it over the two-step manual flow below. Flags: `-AllowPublic`, `-NoRemote`, `-SkipGitHub`.

```powershell
.\tools\ship\ship.ps1 -Mod <mod>                  # CANONICAL: build+deploy+upload+github+verify
# ...or the manual two-step it wraps:
