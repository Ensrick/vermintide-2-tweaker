...
| 13 | Deferred/blocked work goes to GitHub Issues (search first, label for triage), not floating TODO comments. |
...
The `upload_*.ps1` wrappers at repo root carry a visibility-regression guard on top of `VMBLauncher.exe upload` (verify `itemV2.cfg` visibility before a `--allow-public` push); `ship.ps1` is the preferred path. Do not author new `.ps1` wrappers; extend the launcher instead. The legacy `deploy_*.ps1` wrappers were archived 2026-05-21 to `_archive/legacy_deploy_scripts/`; use `VMBLauncher.exe deploy <mod>` directly.
