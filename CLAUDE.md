...
| 13 | Deferred/blocked work goes to GitHub Issues (search first), not floating TODO comments. Every OPEN issue carries exactly one universal lifecycle: `diagnostics-armed` while a bounded evidence path is ready, `verify-fix` when a complete fix is ready for solo/autonomous verification, or `verify-fix-coop` when runtime verification needs 2+ people. Tooling/docs use the same lifecycle but their modifier routes them to autonomous verification and keeps them out of in-game playtest docs. `not-started` and `Fixed` are retired for open issues. Co-op diagnostics keep `diagnostics-armed` and add orthogonal `coop-required`; `verify-fix-coop` is self-describing. Also apply exactly one type (`bug`/`enhancement`/`feature`, with `crash` a flag on `bug`) + mod tag. Never invent a status label. Issue format: title 8 words max; body = empirical data only (~150 words: bold Symptom/Evidence/Fix/Refs labels, no `##` headings). Multiline bodies MUST use `gh ... --body-file`; never embed escaped `\n`. Full scheme: `PROJECT_STANDARDS.md` §11 "Labels" + "Issue format". |
