...
- **Worktree lifecycle**: read-only subagents share the current checkout. When isolated writes genuinely require a linked worktree, create it with `tools/worktrees/worktree.ps1 -Action Create`, which defaults outside `source\repos` and enforces the 8-tree / 12-GiB budget. After the work is committed or merged, close it in the same session with `-Action Close`. The close path refuses dirty source and ambiguous ignored content. `qa/check_worktree_budget.ps1` is a blocking Quick/full-QA backstop.
