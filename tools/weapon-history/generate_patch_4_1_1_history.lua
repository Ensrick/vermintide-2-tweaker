-- Generate the bounded Patch 4.1.1 history catalog for Tweaker: Weapons (#1436).
--
-- Usage:
--   lua5.1 generate_patch_4_1_1_history.lua <source-repo> <evidence-dir> <output.lua>
--
-- The operation set comes from the adjacent 4.0.1 -> post-4.1.1 source diff.
...
local function validate_snapshot(snapshot, rehydrated, catalog)
    ...
    assert(operation.unset == false and operation.value == true,
        "historical ammo-pickup reload value drift")
    if rehydrated then
        assert(operation.expected_current_unset == false
                and rawget(operation, "expected_current") == false,
            "current ammo-pickup reload guard drift")
    end
    return operation
end
...
assert(source_catalog.schema == 1, "unsupported Patch 4.1.1 source catalog")
...
file:write("-- AUTO-GENERATED by tools/weapon-history/generate_patch_4_1_1_history.lua; DO NOT HAND-EDIT.\n")
