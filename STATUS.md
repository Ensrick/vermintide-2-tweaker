...
**🏷 Label taxonomy (user rules 2026-07-11, revised 2026-07-20; codified §11/runbook/CLAUDE.md):** four lifecycle states (`not-started` / `verify-fix` / `diagnostics-armed` / `Fixed`), exactly one per open issue. When 2+ testers are needed, KEEP the lifecycle label and add the orthogonal `coop-required` qualifier (`verify-fix` + `coop-required`); the old `verify-fix-coop` label is retired and must never be applied. `Fixed` = user-verified, post-fix pass (hardening/docs/regression tests) owed before close.
