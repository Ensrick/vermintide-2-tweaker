...
-- check (ct 0.7.162-dev). The extra-chip node_key resolution further down is
...
-- a second chip per EXTRA duplicate peer. UNHOOKED elsewhere in ct (verified
	...
	for _profile_index, list in pairs(by_profile) do
		if #list > 1 then
			...
			for i = 1, #list - 1 do
				...
				-- /ct_regression_test `dup_chip_no_current_node_fallback` (ct
...
-- UNHOOKED elsewhere in ct (verified 2026-06-21). Sole hook on this pair.
