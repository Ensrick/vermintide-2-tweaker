<!--
REVIEW (2026-05-01): This entire file is severely stale. It predates:
- The 2026-05-01 VMB migration (every "Build" / "Workshop Upload" / "Useful Paths" instruction
  here describes the OLD SDK pipeline that survives only in /tweaker. References to wt.mod at
  repo root, settings.ini per mod, .build/OUT/, upload/content/, _run_build.bat, build.bat,
  install_local.bat, deploy.ps1, upload.ps1, upload_all.ps1 are all out of date — those scripts
  are now in old-backup/.)
- The split of cosmetics_tweaker, general_tweaker, enemy_tweaker, character_weapon_variants
  (only chaos_wastes/weapon/career are listed; status column says "Not started" for career and
  cosmetics_tweaker is missing entirely).
- The visibility = "public" default (line ~83) directly contradicts feedback_workshop_metadata_user_dictates.md
  in memory which mandates private as default; also contradicts the actual itemV2.cfg for
  weapon_tweaker (currently "private"). DO NOT use the visibility = "public" example here — it
  is dangerous (Steam removal-from-community is irreversible).
- The Workshop upload procedure (Sections "Step-by-step (proven working 2026-04-23)") describes
  staging into SDK/ugc_uploader/sample_item/, which is no longer how active mods upload. The
  current pipeline is: ugc_tool reads <mod>/itemV2.cfg directly with content="bundleV2".

RECOMMENDATION: This file overlaps almost entirely with DEVELOPMENT.md (which is current).
Either (a) delete ANTIGRAVITY.md and let DEVELOPMENT.md be the single source of truth, or
(b) prepend a "STALE - SEE DEVELOPMENT.md" banner. Until then, do NOT follow any build/upload
instructions in this file. The "Chaos Wastes Internals" subsection at the bottom is still
mostly accurate, but is also redundant with DEVELOPMENT.md "Chaos Wastes Internals".
-->
