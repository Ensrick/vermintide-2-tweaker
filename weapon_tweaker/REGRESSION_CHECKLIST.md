...
| Default | **Kruber's Longbow** history defaults to **Current (Game Version 6.12.1)** and performs zero gameplay writes. |
...
| Log evidence | Public beta and the development mirror rely on the two named runtime checks. The consumed `[wt:316]` base-zoom observer was retired under #499 because it did not exercise weapon-special cycling or Bounty Hunter; its historical rows are not current acceptance evidence. |
