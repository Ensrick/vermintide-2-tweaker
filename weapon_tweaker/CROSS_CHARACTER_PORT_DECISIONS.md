...
| B-dualaxes | Dwarf Dual Axes (`dr_dual_wield_axes`) | Empire Hammer & Sword (`es_dual_wield_hammer_sword` → `to_dual_hammer_sword`) | |
| B-dualhammers | Dwarf Dual Hammers (`dr_dual_wield_hammers`) | Empire Hammer & Sword (`es_dual_wield_hammer_sword` → `to_dual_hammer_sword`) | |
...
| # | Source weapon (key) | Display name | Target (3P) | Class |
|---|---|---|---|---|
| BS-A1 | `dr_1h_axe` | Bardin: 1H Axe | Saltzpyre 1H Axe (`wh_1h_axe`) | melee |
| BS-A2 | `dr_1h_hammer` | Bardin: 1H Hammer | Saltzpyre 1H Hammer (`wh_1h_hammer`) | melee |
...
| BS-B2 | `dr_dual_wield_axes` | Bardin: Dual Axes | Saltzpyre Dual Skull-Splitters (`wh_dual_hammer`) | melee | Per user: WP dual hammer animations are shared with non-WP Saltzpyre because they use the same model; Zealot can use several WP weapons, so the dual-hammer skeleton vocab is reusable here. |
| BS-B3 | `dr_dual_wield_hammers` | Bardin: Dual Hammers | Saltzpyre Dual Skull-Splitters (`wh_dual_hammer`) | melee | Same `wh_dual_hammer` 3P target as BS-B2 — dual-blunt vocab fits. |
