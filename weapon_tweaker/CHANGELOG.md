...
- Adds **Bardin's Grudge-Raker: Game Version 6.10.0 (Ammunition Only)** to
  the source catalog. Both normal and Versus maximum-ammunition leaves
  restore 16 from current 20 as one guarded atomic family state.
...
- Reissues the never-published 0.12.331-beta Swiftbow ammunition content
  under this version; in-game verification of both slices remains outstanding.
...
- Never published to the Workshop: its version claim went stale, and this
  content is superseded by 0.12.332-beta.
...
- Extends **One-handed Hammer/Mace (Kruber, Bardin, and Saltzpyre)** with
  ...
  **Current (Game Version 6.12.1)** remains the zero-write default at `120`
  and `4`.
...
- Advances the canonical current-source anchor to exact Game Version 6.12.1
  commit `25fd7b8433e839b678d1c98a7a9af80918cbc252` after proving that all 36
  weapon-history input blobs are unchanged and none of the seven upstream
  changed paths intersects them. Schema 2 records the same-commit anchor;
  strict, case-sensitive schema-1 compatibility remains covered for old
  release evidence.
...
- The entry is now 1,328 nonblank lines. The machine decomposition contract
  requires ten owners and marks the public Weapon Tweaker phase complete.
...
- Entry drops from 3,989 to 3,071 non-blank lines; both decomposition
...
- **Verify (single player for most of it).** Load line `[wt:LOAD] v0.12.300-beta`;
  ...
  each behaves exactly as on 0.12.299-beta. Files: `weapon_tweaker.lua` (MOD_VERSION; both
  blocks replaced by dofiles), `_wt_cross_char_template_patches.lua` (new),
  `_wt_weapon_balance_patches.lua` (new).
## 0.12.299-beta (2026-08-10) -- Moonfire Bow AOE revert becomes its own owner module (#1159) [untested]
...
- Entry drops from 4,159 to 3,989 non-blank lines; both
...
- New offline suite `test_wt_moonfire_aoe_owner` (6 checks per stream, 13 cases
  ...
  `rt_textual_invariants` rows follow the code.
- **Verify (single player, no coop needed).** Load line `[wt:LOAD] v0.12.299-beta`;
  ...
  cluster must behave exactly as on 0.12.298-beta. Files:
  `weapon_tweaker.lua` (MOD_VERSION; block replaced by the dofile),
