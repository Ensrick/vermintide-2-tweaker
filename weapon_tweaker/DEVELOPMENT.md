...
|---|---|
| `weapon_tweaker.lua` (entry) | Thin orchestration manifest: MOD_VERSION (launcher parses it here — never move it), load banner/echo, dependency publication, ordered owner installation, mod lifecycle callbacks, and the small remaining force-load/feature coordination seams. Cross-character transforms, presentation, template patches, wire-sensitive balance registration, and engine-fatal guards have explicit owners. `feature_enabled` + `_local_career_name` remain here because the animation funnel captures them as generic hot-path helpers. |
| `_wt_grip_offset_policy.lua` | Pure receiver/hand routing for baked scale, grip-offset, and rotation descriptors, plus bounded retained transform readbacks (#701 position, #735 rotation). It owns no transform values and sends no RPC; `_wt_transform_runtime.lua` owns the catalogs. |
| `_wt_transform_runtime.lua` | Canonical source-baked scale, 3P grip-offset, and 3P rotation catalogs plus owner/bot/remote-husk registration and durable reapplication. Exposes the exact helper surface consumed by the preview owner, sends no transform RPC, and leaves first-person grip/rotation unchanged. |
| `_wt_cross_character_safety.lua` | Deepwood Staff missing-finger-node refusal, `AnimationSystem.anim_event_with_variable_float` unknown-variable guard with `skip_sync` preservation, attachment-source validation, and the sole universal `GearUtils.link_units` sanitizer. |
...
| `_wt_rework_master_policy.lua` + `_runtime.lua` | Issue #445's engine-free single source for the 15 active Weapon Tweaks, the `wt_rework_master_ensrick` identity, exact-state derivation, minimal batch plan, and `[Ensrick]` label decoration. The small runtime adapter owns non-notifying writes and one injected post-batch live-owner dispatch. Retired `br_*` settings are intentionally excluded. Both files are byte-identical in the dev mirror. |
...
| `_wt_axe_balance.lua` | Engine-free reversible Weapon Tweaks transactions for #601/#621/#622/#623. Owns exact action/template capability boundaries, private cleave-profile generation, deterministic registration, authored speed snapshots, and hot-toggle restoration. `M.install(mod)` adds the one engine-facing seam: it holds the policy instance, publishes the `mod._wt_apply_axe_balance` adapter (per-setting arms plus the unconditional #431 NetworkLookup wire floor), and applies once. The entry calls install at the manifest position that precedes the #431 parity beacon dofile, and keeps only the lifecycle call sites. |
| `_wt_fire_sword.lua` | Issue #943's engine-free Fire Sword heavy policy. Captures one immutable per-template-identity baseline (the two idle `action_one.default` heavy edges plus `heavy_attack_spell` clock/window scalars), then projects the default-off `wt_fire_sword_sweep_opener` (idle heavies open with the sweep) and `wt_fire_sword_nova_slowdown` (nova 10%% slower with aligned hit/window/chain timing) settings from it, composing independently and restoring exact captured values off. `M.install(mod)` owns the `mod._wt_apply_fire_sword` adapter (setting-id filter, force_off revert, one load-time apply); the entry keeps the manifest dofile, the install call, and the lifecycle call sites, and `_wt_settings_runtime.lua` owns single-setting dispatch. Wire-safe, no NetworkLookup surface. Byte-identical in the dev mirror. |
...
| `_wt_cross_char_template_patches.lua` | #1159 owner for the cross-character base-template 3P patches: the five source-template patchers (brace of pistols, Empire longbow, elf longbow, Moonfire Bow, repeating pistol), the shared `wield_anim_career_3p` applier fed by `wt_wield_patches.lua`, and the not-loaded / no-ammo wield fallback applier (#536). Writes 3P template fields only and keeps the longbow crossbow remaps career-scoped in `_3p_template_remaps` rather than mutating the shared template (#210). Publishes `mod._wt.wield_patches_module` and `mod._wt.wield_anim_career_patches` for the runtime-check dependency table. Extracted byte-identically; the entry keeps only a bare dofile at the former execution position. |
| `_wt_weapon_balance_patches.lua` | #1159 owner for the three toggle-gated in-place `Weapons.*` rebalance rewrites: Authentic Brace of Pistols (damage/spread/ammo/speed plus the `wt_authentic_pistol` clone and `mod._wt431_brace_repoint`), the issue #348 Kruber Empire 1h sword push-combo revert, and the Warrior Priest punch buff (`wt_priest_punch_buffed` clone plus `mod.wt_apply_priest_punch_buff`). Both clones register into `NetworkLookup.damage_profiles` unconditionally and record their vanilla fallback for the #431 send floor, so the bare dofile MUST stay at the former execution position - the load order is the wire index. |
| `_wt_moonfire_aoe.lua` | #1159 owner for the Moonfire Bow pre-nerf AOE revert. Owns the `moonfire_aoe_revert` toggle read, the `wt_moonfire_aoe_revert` ExplosionTemplates + NetworkLookup registration and its recorded wire-safe `machinegun_poison_arrow` fallback (#535), the `wt_535_moonfire_explosion_registered` regression check, and the PlayerProjectile Unit/Husk impact-hook loop. Extracted byte-identically; the entry keeps only a bare dofile at the former execution position. |
| `_wt_ingame_3p_swap_owner.lua` | #1159 owner for the in-mission cross-character 3P mesh swap. Holds the mod's single `GearUtils.spawn_inventory_unit` registration (`traced_hook`) with the Kruber brace-of-pistols -> repeating-handgun body inline, plus the three helpers it dispatches to as plain functions: `_wt_longbow_3p_swap_apply` (bow -> empire crossbow AND arrow -> bolt, sourcing the crossbow template's linking), `_wt_repeating_pistol_3p_swap_apply` (substitutes the handgun template's `wielded` table to dodge the weapon-mesh node fatal), and `_wt_hammer_book_3p_swap_apply` (#181, delegates to `_wt_skullsplitter_hand.apply_runtime`). It also owns the paired SimpleInventory/SimpleHusk visibility re-hide callbacks for the extra brace pistol and book. Reads `_dbg`, `_dbg_alert`, `_unit_career_name`, the crossbow item predicate, the three 3P unit constants, and the Skullsplitter policy back off `mod._wt`. 3P-only; vanilla's 1P pair is captured once and forwarded untouched. |
| `_wt_menu_preview_owner.lua` | #1159 owner for the keep/hero inventory preview surface. Holds both `MenuWorldPreviewer` registrations: the single `equip_item` `hook_safe` (brace -> repeater inline, dispatching the longbow, repeating-pistol and Skullsplitter & Tome preview helpers plus the weak-keyed item-key capture) and the full `_spawn_item_unit` wrapper (pre-spawn `_wt_validate_attachment_sources`, post-spawn scale/offset/#569 rotation, the v0.12.146-dev preview wield-pose correction, and the debug-gated missing-node probe). Also installs the #735 `_wt_paired_preview_transform` adapter and owns the #603 Ranger dual-wield stance selector, which it publishes as `mod._wt.post_spawn_preview_event` for the entry's runtime-check dependency table - the slice's one outbound crossing. Hook the DERIVED class, never `HeroPreviewer`. Extracted byte-identically apart from that publication; the entry keeps only a bare dofile at the former execution position. |
...
| `wt_dev_hold_pose.lua` | #616 local-owner transform authoring. Owns separate first-person and third-person right/left offset, Euler rotation, and absolute scale channels; channel-local weak baseline caches; non-destructive enable/bypass/restore; per-frame apply; reset/dump commands; and its widget/localization subtree. #1023 adds the mod's single `IngameHud.update` post-hook: one read-only HUD row rendered from the same `_component_plan` the apply path consumes, following the last edited hand/channel and fail-closed unless master, channel, and a live unit all resolve. The entry dispatches `wt_dev_hp_*` setting changes and disable cleanup. It must never target previewers, bots, husks, score presentation, or committed appearance definitions. |
...
- **New `wield_anim_career_3p` / not-loaded / no-ammo template patch, or a new cross-character source-template patcher** → `_wt_cross_char_template_patches.lua`; the per-career wield rows themselves stay in `wt_wield_patches.lua`, and a per-action remap belongs in `_3p_template_remaps`, never as a global mutation of the shared template (#210).
- **New toggle-gated in-place rebalance of a vanilla `Weapons.*` template** → `_wt_weapon_balance_patches.lua`; register any cloned damage profile UNCONDITIONALLY (§9.3) and record its clone source in `mod._wt431_custom_profile_fallback`. Do not move that module's dofile: the append order IS the `NetworkLookup.damage_profiles` index every peer must agree on.
- **New in-mission per-weapon 3P mesh swap** → `_wt_ingame_3p_swap_owner.lua`, as a plain helper function dispatched from the existing `spawn_inventory_unit` body. Never add a second registration on that pair; source the TARGET template's attachment linking, not the source weapon's, or `Unit.node` engine-fatals past `pcall`. Every path must return vanilla's `v_w1p, v_a1p` untouched.
- **New inventory-preview mesh swap, preview transform, or preview pose correction** → `_wt_menu_preview_owner.lua`. Preview swaps mutate `_item_info_by_slot[...].spawn_data` from the ONE `equip_item` `hook_safe` (it does not chain); hand-scoped transforms belong in the `_spawn_item` adapter, not `_spawn_item_unit`, which has no hand indicator. Hook `MenuWorldPreviewer`, never `HeroPreviewer`.
- **Anything touching the port pipeline runtime, the grip offsets, or a P0 guard** → stays in `weapon_tweaker.lua` until a later phase; grep ALL files for an existing hook on the `(Class, method)` before adding one (VMF drops the second — NON-NEGOTIABLE 8).
