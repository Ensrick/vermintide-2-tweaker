...
| `_wt_fire_sword.lua` | Issue #943's engine-free Fire Sword heavy policy. Captures one immutable per-template-identity baseline (the two idle `action_one.default` heavy edges plus `heavy_attack_spell` clock/window scalars), then projects the default-off `wt_fire_sword_sweep_opener` (idle heavies open with the sweep) and `wt_fire_sword_nova_slowdown` (nova 10%% slower with aligned hit/window/chain timing) settings from it, composing independently and restoring exact captured values off. The entry owns `mod._wt_apply_fire_sword` lifecycle/setting dispatch; wire-safe, no NetworkLookup surface. Byte-identical in the dev mirror. |
