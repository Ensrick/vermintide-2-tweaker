...
| `_wt_ingame_3p_swap_owner.lua` | #1159 owner for the in-mission cross-character 3P mesh swap. Holds the mod's single `GearUtils.spawn_inventory_unit` registration (`traced_hook`) with the Kruber brace-of-pistols -> repeating-handgun body inline, plus the three helpers it dispatches to as plain functions: `_wt_longbow_3p_swap_apply` (bow -> empire crossbow AND arrow -> bolt, sourcing the crossbow template's linking), `_wt_repeating_pistol_3p_swap_apply` (substitutes the handgun template's `wielded` table to dodge the weapon-mesh node fatal), and `_wt_hammer_book_3p_swap_apply` (#181, delegates to `_wt_skullsplitter_hand.apply_runtime`). Reads `_dbg`, `_dbg_alert`, `_unit_career_name`, the crossbow item predicate, the three 3P unit constants, and the Skullsplitter policy back off `mod._wt`. 3P-only; vanilla's 1P pair is captured once and forwarded untouched. Extracted byte-identically; the entry keeps only a bare dofile at the former execution position, which must stay after the two NetworkLookup-appending owners and before the preview owner. |
