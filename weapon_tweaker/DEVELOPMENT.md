...
| `_wt_weapon_overrides.lua` | The three toggleable template-mutating weapon reworks (issue 2 decomposition): Authentic Brace of Pistols (`wt_authentic_pistol` damage-profile clone, widened primary/secondary spread clones, halved ammo, no aim mode / rapid fire / manual reload), the issue 348 Kruber Empire 1h sword push-attack combo revert (wire-safe: re-routes `sub_action` between vanilla action names only), and the Warrior Priest punch buff (`wt_priest_punch_buffed` clone repointed onto the priest greathammer punch action only, never mutating the shared `light_blunt_smiter_stab`). Reads `mod._wt.dbg`; publishes `mod.wt_apply_priest_punch_buff` and `mod._wt431_brace_repoint`. Both damage profiles register UNCONDITIONALLY at load so peer `NetworkLookup` indices align; only the repoint is toggle-gated. Manifest position is load-bearing: it must sit immediately before `_wt431_damage_profile_parity.lua`, which needs all three registration sites populated. No hook, no command. |
...
- **A new toggleable rework that mutates a vanilla `Weapons.*` template at load** → `_wt_weapon_overrides.lua`; register any cloned damage profile UNCONDITIONALLY and gate only the repoint, and keep the module ahead of `_wt431_damage_profile_parity.lua`.
