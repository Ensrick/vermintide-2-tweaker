...
-- Observe ordinary MODDED-realm openings passively so no test command can
-- accidentally enqueue an extra request. Official-realm observation is
-- structurally impossible: unapproved Workshop mods never load there
-- (mod_manager.lua:275 scans approved-only when eac-untrusted is unset, and
-- the realm is fixed at launch, application_parameter.lua:150), so the gate
-- requires eac_untrusted == true - the realm where this mod actually runs and
-- where loot delivery needs observing. The bounded ledger contains no
-- backend/player ids.
...
_rt_register("mp607_modded_loot_capture_is_bounded_and_realm_gated", function()
...
mod:command("mp_loot_diag", "Show or reset bounded issue #607 loot diagnostics", function(action)
    if action == "reset" then
        ...
        mod:echo("#607 loot diagnostic ledger reset. Passive modded-realm capture remains active.")
        return
    end
