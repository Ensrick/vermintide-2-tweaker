...
- [ ] #1637: with Tweaker: Weapons Dev and Tweaker: Cosmetics enabled (they hook the items interface INSTANCE), the keep loads and the hero spawns holding a melee weapon both with the default `Level 35, default inventory` and after switching to `Fresh (level 1)`; the log shows one `[mp:fresh] instance_join methods=...` line under Fresh and never `Tried to wield default slot`; `/mp_regression_test` passes `mp840_fresh_route_spawn_lookup_closed` and `mp840_fresh_route_instance_shadows_joined`.
- [ ] #1637: a fresh install without a saved `starting_state` uses the official inventory (no `route state=active` line until Fresh is chosen).
