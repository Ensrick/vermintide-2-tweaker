...
## 0.2.40-dev (2026-09-20) - Fresh route resolves its own ids at spawn, official inventory by default (#840, #1637) [verify-fix]

- **Crash (RainReligion, #1637):** entering the keep as Sienna Unchained stopped the game with `Tried to wield default slot slot_melee for bw_unchained that contained no weapon`. Both console logs carry `[mp:fresh] route state=active generation=1 slices=items` before the crash: the Fresh (level 1) profile route was active without the player choosing it, because `starting_state` defaulted to Fresh when unset (`modded_progression.lua:181`, `modded_progression_data.lua`). The bisect (Pusfume off, then Tweaker: Cosmetics off) crashed identically, which the sequence below predicts.
- **Exact spawn sequence, read from source:**
  1. `SimpleInventoryExtension.extensions_ready` runs `add_equipment_by_category("weapon_slots")`, which asks `BackendUtils.get_loadout_item(career, slot, self.is_bot)` for each weapon slot (`simple_inventory_extension.lua:112,375-391`).
  2. Vanilla `BackendUtils.get_loadout_item` (`backend_utils.lua:30-46`) first calls `BackendUtils.get_loadout_item_id` (`:14-20`), which resolves the loadout interface for the slot: the items interface INSTANCE (`backend_manager_playfab.lua:329-341`).
  3. That instance carries instance-table VMF hooks on `get_loadout` and `get_loadout_item_id` from Tweaker: Cosmetics (`_cos_la_loadout_safety.lua:160,190`) and Tweaker: Weapons Dev (`weapon_tweaker_backend.lua:738,894`; the log lines `Hooking 'get_loadout_item_id' from [table: 0x03476c60] (Origin: function: 0x0047a580)` at 17:16:54.598). VMF maps an instance hook's origin back to the vanilla function (`hooks.lua:305-308`, the `_registry.uids` remap) and builds a SEPARATE chain on the instance whose tail calls that vanilla function directly (`hooks.lua:106-120,176-193`). The class chain (Pusfume, then Modded Progression's route) is never entered from the instance.
  4. Weapons Dev's hook misses its cache for a non-Weapons-Dev weapon and Cosmetics' hook has no cosmetic row, so both fall through to vanilla `get_loadout_item_id` (`backend_interface_item_playfab.lua:512-538`), which reads `self:get_loadout()`: the instance chain again, ending in vanilla `get_loadout` (`:447-459`) and the native `self._loadouts` that `_refresh_loadouts` fills from `backend_mirror:get_character_data` (`:109-124`), which GUI Dev's mirror hook serves from its store (`[gut_dev:NATIVE_LOADOUTS] #387 gear-read ... value=C5AFFE018915B352 source=store-yes` at 17:17:04.504). The id is the NATIVE Crafting in Modded id `C5AFFE018915B352`.
  5. Back in `get_loadout_item`, `backend_items:get_item_from_id(C5AFFE018915B352)` has no instance shadow, so it runs the class chain: WOC, Crafting in Modded (`standard_forge.lua:1446`, template-cache miss), vanilla `get_item_from_id` (`:384-389`), which reads `self:get_all_backend_items()` through the class chain, Pusfume, then Modded Progression's active route: the Fresh view's items, which hold only `mp840_g1_*` ids. Result nil.
  6. GUI Dev's `BackendUtils.get_loadout_item` hook (`_gut_bot_pose.lua:63-73`) receives nil (0.2.350 had no recovery; 0.2.352's candidates were the same native ids), both weapon slots are skipped, and `extensions_ready` ferrors (`simple_inventory_extension.lua:162-174`).
  7. The dump's "native ids a millisecond later" is the same path: its `BackendUtils.get_loadout_item_id` (`:398`) walks the same instance chain and prints the same native id, and its `item_string` is `item and item.name or "Item exists"` (`:407`), which prints `Item exists` for a nil item exactly as for a record without a `name` field. The dump never proved the item resolved; `get_item_from_id(native id)` was nil on both calls. Nothing behaved differently: ids came from the native mirror on both calls, the item table came from the Fresh view on both calls, and a native id looked up in the Fresh table is nil.
- **Fix 1, instance join:** every tick, the route now joins the live items interface instance for every routed method a sibling shadowed on the instance table (`R.join_instance`, once per instance and method, one bounded `[mp:fresh] instance_join methods=... backend=none` receipt per batch). The joined wrapper is the same closure the class hook uses, so the per-call decision is identical on both paths: Fresh answers while the route is active, the sibling chain and vanilla otherwise. This also closes the write side: Weapons Dev shadows `set_loadout_item` / `add_loadout` / `delete_loadout` on the instance (`weapon_tweaker_backend.lua:682,853,873`), which under Fresh would have reached the native mirror.
- **Fix 2, routed lookup:** `get_item_from_id` is now routed (`Runtime.LOOKUP_METHODS`) and answers from the Fresh view directly instead of depending on the hook topology of `get_all_backend_items`. A miss continues down the chain so a sibling's synthetic ids (Crafting in Modded template rows) still resolve; the chain ends at vanilla, which reads the routed root, so no native record can answer while the route is active. Not routed, because vanilla derives them purely from `get_item_from_id`: `get_item_rarity`, `get_item_power_level`, `get_traits`, `get_skin`, `get_key`, `get_item_masterlist_data`, `get_item_amount` (`backend_interface_item_playfab.lua:320-382`); and from routed roots: `get_loadout_by_career_name`, `get_cosmetic_loadout`, `equipped_by`, `get_item_from_key`, `get_item_name`, `get_filtered_items`, `has_item`.
- **Fix 3, slot fallback:** `get_loadout_item_id` under the route never answers an equipment id the same view cannot resolve. An unresolvable weapon or jewellery id (a vanished Emporium grant, a corrupt record) falls back to the seeded career default for that slot, else nil exactly where vanilla answers nil for an empty slot; one bounded `[mp:fresh] slot_fallback career=... slot=... unresolved:<id> to=<id>` receipt per (career, slot, id). Cosmetic and pose slots are unchanged.
- **Default changed:** `Starting State` now defaults to `Level 35, default inventory` (the official inventory) in both the options widget and `get_starting_state()`. The Fresh route is still incomplete (pending slices `xp_talents,currencies,crafting_loot`), and a default that replaced every player's inventory without opting in caused this crash. `Fresh (level 1)` stays selectable and behaves as before once chosen.
- **Checks:** `/mp_regression_test` gains `mp840_fresh_route_spawn_lookup_closed` (a throwaway profile seeded from live client data, never persisted: every career/slot id resolves in the same view to a record with `data`, `backend_id`, `key`, `rarity`, and for equipment `power_level`, `properties`, `traits`; every Chaos-Wastes-playable career answers both weapon slots; under an active route the live handlers are audited the same way; when inactive the wrappers must hand the native lookup back untouched) and `mp840_fresh_route_instance_shadows_joined` (every routed method shadowed on the live instance is joined). Offline `test_mp_fresh_profile_routing.lua` grows from 10 to 15 tests: Fresh-first lookup with chain continuation, fault latch and inactive delegation; seeded-default fallback and nil when no default remains; instance join (idempotent, per instance, replaced interface, unavailable backend, unresolved route never hooks); both checks driven with planted failures; the new default.
- **Known boundary:** unchanged from 0.2.38-dev; XP/talents, currencies, crafting and loot stay official-backed pending their slices.

**Test (solo, modded realm, Crafting in Modded + Tweaker: Weapons Dev + Tweaker: Cosmetics + Tweaker: GUI Dev enabled):** launch, confirm `[mp:LOAD] v0.2.40-dev`, and enter the keep as Sienna Unchained with the loadout that crashed, leaving `Starting State` untouched. Expected: the keep loads, the hero holds the saved melee weapon, and the newest log has no `route state=active` line (the default is now the official inventory) and no `Tried to wield default slot`. Then set `Starting State` to `Fresh (level 1)`, switch to another Sienna career and back to Unchained: expected the hero respawns holding the Chaos Wastes starter melee weapon, the log has one `[mp:fresh] route state=active` line and one `[mp:fresh] instance_join methods=...` line naming at least `get_loadout,get_loadout_item_id`, and no `route state=official:fault`. Run `/mp_regression_test`: `mp840_fresh_route_spawn_lookup_closed` and `mp840_fresh_route_instance_shadows_joined` must PASS alongside the existing four `mp840_fresh_*` checks. Set the state back to `Level 35, default inventory`, reopen the inventory, and confirm the official loadout is back with one `route state=official:setting` line.
...
- New `_mp_fresh_profile_runtime.lua` routes 16 root reads and 5 writes of `BackendInterfaceItemPlayfab` to that state ONLY when the modded realm is active, `Fresh (level 1)` is selected, every routed method resolved on the live class at boot, and no runtime fault has latched; every other case delegates to the original method (official reads). Derived methods (`get_item_from_id`, `get_filtered_items`, `has_item`, `equipped_by`, ...) follow through `self:`. Writes commit copy-on-write to the profile and never call the native mirror.
- Bounded engine `printf` receipts under `[mp:fresh]`: one `route state=...` line per transition (`active`, `official:realm|setting|disabled|unresolved|fault`), one `seed generation=N ...` line per generation, one fault line with stage and sanitized error, `write_rejected`/`equip_rejected` lines on failed writes.
- While the Fresh route is active the un-routed slices keep vanilla's modded-realm "unavailable" presentation instead of being silently official-backed: level-end reward popups stay skipped, Okri's achievement rows/claim-all stay greyed (MP-owned daily quest rows keep their local bracket), and the keep bench craft button stays disabled. The #577 mirror overlay is skipped (`fresh_routed`); durable Emporium grants are served by the Fresh view instead.
...
- Added always-on `[mp:607] event=... first_missing=...` engine `printf` rows. The diagnostic never originates a request, grants a container, rolls an item, mutates the backend mirror, or retains backend/player ids; the native call chain is delegated unchanged.
- Made the unbuilt boundary explicit: `award_capable=false`, `open_capable=false`, and after one mission reaches the queue the derived first missing layer is `local_container_award`. Successful callback capture remains optional R&D evidence, not a test prerequisite.
- Extracted the adapters into `_mp_loot_diag_runtime.lua`, keeping the main entry below the 1,500-line target. Replaced the runtime check with `issue607_local_loot_layer_diagnostic_contract`; expanded `test_mp_loot_diag.lua` over request attribution, EAC rejection attribution, missing-layer diagnosis without `FunctionResult`, local container/use census, sanitization/bounds, one-hook-per-seam source contracts, and behavioral delegation through the real wrapper signatures (including trailing arguments and nil-hole returns).
- Contained every realm, context, local-store, persistence, summary, and `printf` observer action behind a no-throw boundary. Planted failures across all six wrappers now prove native delegation occurs exactly once and preserves first/middle/trailing nil returns; `pre_request` is emitted only after vanilla returns a finite positive numeric enqueue id. Observer-failure rows use a fixed reason and never retain raw exception text or identifiers.

**Diagnostic test:** In the modded realm, complete one ordinary Adventure mission, return through the end-of-level screens, run `/mp_loot_diag`, and attach the newest log. The log should contain one bounded `event=end_level`, `event=pre_request` for `generateEndOfLevelLoot`, a `local_ledger` snapshot, and (when vanilla reports its expected EAC failure) `event=rejection`; the command must report `first_missing=local_container_award`. No local chest or equipment reward is claimed by this diagnostic. If Spoils of War already shows an ordinary mission chest, opening one should additionally emit the bounded `open` flow without the diagnostic adding a request.
