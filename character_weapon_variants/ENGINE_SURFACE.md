...
class; `#N` = a GitHub issue. Grep-verified 2026-08-13 against the decompile.
...
94 hook registrations, grouped below into 8 logical rows-of-concern. `[hook]` =
...
| `SimpleHuskInventoryExtension._wield_slot` [hook] `_cwv_husk_path.lua` | Husk-side wield: resolves the template, gets the unit table, then conditionally calls `spawn_inventory_unit` once per non-nil right/left field [src: `simple_husk_inventory_extension.lua:641,661-670`; `init` stores `extension_init_data.player` at `:5-36`] | Establish one synchronous peer+slot context, select the exact Old Musket rifle/polearm parent before spawn, apply appearance/stance state, then enroll the complete custom 3P snapshot after all CWV adapters finish (#620/#760/#914/#922/#1155) | CONSOLIDATED executable owner. Context is always cleared after vanilla returns/errors. Old Musket records the exact spawned template, mode, profile, path, owner, slot, and hand; stable edges fail closed rather than paint a polearm pose onto a rifle-linked unit. The outer vanilla `wield` subsequently replaces the list from the same current inventory fields; Cosmetics separately owns post-replacement attachment composition. At spawn, exact identity first validates the extension's human `_player` and owner unit because `PlayerManager:owner` may not be populated yet; later paths fall back to the shared resolver. Readiness failures retry locally at 0.25 s for at most eight attempts; no network or per-frame unbounded replay (#786/#914). |
...
| `HeroPreviewer._spawn_item` / `MenuWorldPreviewer._spawn_item` [hooks] | Vanilla resolves preview template from inherited base item name and drops mutable stance data | Before vanilla, install only a validated stance attachment recipe or terminal resident-Handgun fallback; after vanilla, reconcile the exact spawned row and arm its stable preview edge (#474/#760/#1155) | These must remain full pre+post wrappers: a post-only `hook_safe` cannot change the recipe before vanilla links the weapon. Consume cached state only for positively identified Old Musket or the exact Outrider item. Outrider uses one receiver-career-scoped `to_repeater_pistol` replay because mutating its shared `dr_deus_01` base would alter the genuine Trollhammer. Event availability and unit liveness are validated before dispatch; failure retains vanilla presentation. |
...
| CIM Dev `cim_preview_context_v1` [public provider contract] `_cim_forge_preview.lua` | CIM wraps the Weapons, Properties, and Overview constructors and marks only each exact returned `LootItemUnitPreviewer` | CWV consumes `_cim_preview_context_for(previewer)` so all Athanor constructors classify as `cim_preview` without a second constructor hook (#1155) | Validate contract, provider, surface, constructor, exact backend identity, and a finite positive-integer generation. An absent/inactive marker uses `illusion_browser`; a present-but-throwing, malformed, foreign, or mismatched marker remains CIM-classified but is terminally rejected without reconciliation. Never infer CIM from item identity or class-wide state. |
| `LootItemUnitPreviewer.spawn_units` [hook] + `_enable_item_units_visibility` [safe] `_cwv_menu_preview_owner.lua` | Illusion/skin browser and CIM's Athanor selector spawn hidden display units, then expose them only after package and mip readiness [src: `scripts/ui/views/hero_view/loot_item_unit_previewer.lua:504-602`] | The spawn wrapper performs fallback/mesh-swap prepasses and records one Old Musket `instance_load`; the later `visible=true` edge performs the distinct `preview_open` authored-material/pose reconciliation (#419/#617/#1155) | `spawn_units` MUST remain a full hook because `_spawned_units` is assigned after return. `visible=false` is never a retry. The ordinary browser has no stance control: bind one exact previewer+selected-item token and monotonic generation from construction through visibility, independent of the equipped backend identity; a delayed lower generation may not overwrite the latest visible success (#1156). Resolve exact unit paths from returned `units` plus `spawn_data`; never fabricate the expected custom path or bind textures into a null donor material. `_load_item_units` rebinds item_data to the BASE IML entry (:254-255), so identity resolves against `self._item`. |
...
| `EquipmentUI._sync_player_equipment` / `GamePadEquipmentUI._sync_player_equipment` [safe] `_cwv_musket_ammo_hud.lua` | Both HUDs derive ammunition from the selected slot's concrete left/right 1P unit, but their sync loops call `_update_ammo_count` only for `slot_ranged`; GamePad may return early from its cached equipment-change gate [src: `scripts/ui/hud_ui/equipment_ui.lua:327-426`, `:612-683`; `scripts/ui/hud_ui/gamepad_equipment_ui.lua:516-721`, `:759-811`] | After native sync, select the exact owner+`slot_melee` extension already registered by the Old Musket pool and refresh the native counter (#1108). On selection loss/error after engagement, restore native ranged count/focus or explicitly hide the owned presentation before release. | Presentation owns no ammo state. Require the primary slot to be wielded, active pool membership, effective `ammo_data`, and exact ammo-hand unit equality. Never mutate a never-engaged HUD; never emit release before restore/hide; no update hook, shared-template write, or RPC is added. |
...
| Class.method (kind) | Vanilla behavior | Why cwv hooks it | Trap / invariant |
|---|---|---|---|
| `PackageManager.load` / `unload` / `has_loaded` [hook,tbl] `:5338`/`:5343`/`:5348`; direct lease `:4443` | Refcounted package load/unload/state query [src: `foundation/scripts/managers/package/package_manager.lua:20`/`:196`/`:286`] | Forward each Old Musket 1P/3P request to its exact vanilla Handgun lifetime alias while preserving reference, callback, async, and priority semantics; separately hold the source-derived vanilla Dual Axes FP state-machine package across CWV equip/resync (`:4443`, #586) | The aliases balance package-manager lifetime only; they do not own the custom unit or material, and they are never unconditional-resident/no-op answers. A wield-time state-machine miss C-fatals at `PlayerUnitFirstPerson.set_state_machine` [src: `scripts/unit_extensions/default_player_unit/player_unit_first_person.lua:165`]; the #586 lease is synchronous, unique-ref, idempotent, and symmetrically released. Do not queue arbitrary mod unit paths (issue #403). |
| `Unit.set_all_materials` [C-API] `_cwv_old_musket_preview.lua` | Binds one compiled material resource to every material slot on a spawned unit | Bind the Old Musket's CWV-owned five-map material after exact texture+material residency proof, then census every live mesh handle (#617/#742/#1155) | The `.unit` itself maps FBX slot `rifle_mat` to the same bundled material. Runtime per-texture painting is forbidden. Before the single native bind, prove all five exact textures and the authored material; afterward pcall-wrap `Unit.num_meshes` / `Unit.mesh` / `Mesh.num_materials` / `Mesh.material` and reject absent/empty/null `#ID[00000000]` handles. `pcall` cannot make an invalid native resource safe, and `Material.num_parameters` remains forbidden. |
...
Old Musket preview/owner/husk presentation delegates exact texture/material
residency and post-bind spawned-unit closure to `_lib_resource_residency.lua`
V2. Its `.unit` files bind a CWV-owned material whose five samplers point at
CWV-owned textures; runtime performs one `Unit.set_all_materials` only after the
complete preflight and never calls `Unit.set_texture_for_materials`. The generic
`_lib_weapon_appearance` texture seam remains censused as legacy rather than
being falsely claimed covered.
...
inventory/illusion previews, and the exactly marked CIM Athanor preview submit
the resulting live unit to `_cwv_old_musket_appearance.lua`. Lobby and score/team
remain declared vanilla-safe fallbacks because their producers do not carry an
exact selected Old Musket slot identity. The adapter owns the only
model/material/texture/pose recipe; each exact lifecycle token permits one
attempt, while a later source-backed edge (`instance_load` to `equip` or
`preview_open`) supplies the only retry. It is accepted only after an independent
local position, scale, quaternion, and material-handle readback. Unsupported
surface/edge cells retain the vanilla handgun explicitly. State exit, mod
disable, and unload disconnect the weak target ledger; dev tuning advances
descriptor generation and replays tracked units once rather than polling per
frame.

Pose identity includes an explicit attachment profile because the same mesh is
parented in different coordinate frames. Owner 1P uses `j_rightweaponattach`
for both rifle and polearm recipes but retains distinct stance profiles;
character 3P distinguishes rifle (`j_leftweaponattach`) from melee/polearm
(`j_rightweaponattach`) parents. The camera-world display carrier used by
