...
| `EquipmentUI._sync_player_equipment` / `GamePadEquipmentUI._sync_player_equipment` [safe] `_cwv_musket_ammo_hud.lua` | Both HUDs derive ammunition from the selected slot's concrete left/right 1P unit, but their sync loops call `_update_ammo_count` only for `slot_ranged` [src: `scripts/ui/hud_ui/equipment_ui.lua:327-426`, `:612-683`; `scripts/ui/hud_ui/gamepad_equipment_ui.lua:516-721`, `:759-811`] | After native sync, select the exact owner+`slot_melee` extension already registered by the Old Musket pool and refresh the native counter (#1108) | Presentation owns no ammo state. Require the primary slot to be wielded, active pool membership, effective `ammo_data`, and exact ammo-hand unit equality. Any mismatch preserves the native ranged-slot result; no update hook, shared-template write, or RPC is added. |
