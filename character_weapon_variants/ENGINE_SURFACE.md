...
| `GearUtils.hot_join_sync` [hook] `:10689` + peer-parity `on_enable` | Re-broadcasts a peer's equipment to a joining peer [src: `gear_utils.lua:462`]; parity enables after every peer acknowledges the same mod schema | Always-null the cwv skin id during the join race, then replay the exact vanilla base item + generated skin through `rpc_add_equipment` once decoding is safe (#425, #579) | Never send a local-append skin before parity. The bounded post-ACK replay must re-send `rpc_wield_equipment` for the current slot so `SimpleHuskInventoryExtension` respawns both hands immediately; do not poll per frame. |
