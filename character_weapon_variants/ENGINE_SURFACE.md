...
| `SimpleHuskInventoryExtension._wield_slot` [hook] `:5189` | Husk-side wield: resolves the template, gets the unit table, then conditionally calls `spawn_inventory_unit` once per non-nil right/left field [src: `simple_husk_inventory_extension.lua:641,661-670`] | Establish one synchronous peer+slot Combat Style context around template resolution (#620), re-key husk mesh+transform (#392), replay cached exact Sword+Mace state (#567), and replay the receiver-native Outrider stance from positively proven semantic identity (#760) | CONSOLIDATED wrapper. Context is always cleared after vanilla returns/errors. Style/identity edges re-wield only the already-active slot through vanilla `SimpleHuskInventoryExtension.wield` [src: `:314`], never per frame. Outrider replay validates the bidirectional vanilla `NetworkLookup.anims` entry, fails closed, emits at most 16 deduplicated records, and sends no custom animation id or extra RPC. |
