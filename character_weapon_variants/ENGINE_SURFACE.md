...
55 registration sites, grouped below into 8 logical rows-of-concern. `[hook]` =
...
| `SimpleInventoryExtension.game_object_initialized` [hook] | First network broadcast of the loadout once the unit's GO id is ready, via `rpc_add_equipment` [src: `simple_inventory_extension.lua:249-282`] | Net-safe skin nulling plus same-mod schema-2 `cwv_item_identity`; schedule bounded transition replay and peer-targeted hot-join retry until the receiver acknowledges the exact slot fingerprint; publish exact Sword+Mace state after the vanilla call (#396/#416/#483/#567/#660) | Never let a modded skin ride while parity is false. Identity retry is capped at eight attempts per peer/slot/fingerprint at 0.5-second cadence and stops only on a matching receiver acknowledgement. VMF-only identity and exact-pair channels never enter `NetworkLookup`. |
| `SimpleInventoryExtension._spawn_resynced_loadout` [hook] | Respawns and re-broadcasts one queued equipment slot [src: `simple_inventory_extension.lua:1429-1468`] | Same net-safe skin path plus changed-slot owner marker and exact Sword+Mace state on live resync (#396/#567) | Marker sends are edge-deduplicated; native items clear stale ownership; exact-pair publication is transition-driven. |
| `SimpleHuskInventoryExtension._wield_slot` [hook] `:5189` | Husk-side wield: resolves the template, gets the unit table, then conditionally calls `spawn_inventory_unit` once per non-nil right/left field [src: `simple_husk_inventory_extension.lua:641,661-670`] | Establish one synchronous peer+slot Combat Style context around template resolution (#620), re-key husk mesh+transform (#392), and replay cached exact Sword+Mace state (#567) | CONSOLIDATED wrapper. Context is always cleared after vanilla returns/errors. Style edges re-wield only the already-active slot through vanilla `SimpleHuskInventoryExtension.wield` [src: `:314`], never per frame. |
...
| `HeroPreviewer._spawn_item` / `MenuWorldPreviewer._spawn_item` [post hooks] | Vanilla resolves preview template from inherited base item name and drops mutable stance data | Resolve exact backend-instance stance, compose the complete saved 3P transform, and replay the selected template's career-aware wield animation (#474) | Consume cached state only for positively identified Old Musket; leave native handgun previews unchanged. |
...
| VMF `custom_gui_textures` renderer injections [data contract] | Loads a private atlas only into the explicitly named GUI renderers | Package nine paired Dual Axe inventory icons and publish their exact renderer/fallback capabilities through `_cwv_inventory_icons.lua` (#620/Athanor hardening) | A packaged material is not globally resident. `hero_view` is safe; an unlisted renderer must use the declared vanilla fallback or `ui_passes.lua` can receive a nil texture. |
...
| `GearUtils.hot_join_sync` [hook] + peer-parity `on_enable` | Re-broadcasts a peer's equipment to a joining peer [src: `gear_utils.lua:462-495`]; parity enables after every peer acknowledges the same mod schema | Always-null the cwv skin during the join race, replay the owner marker before the exact vanilla base+skin+wield sequence, and independently publish exact Sword+Mace state to CWV peers (#396/#425/#474/#567/#579) | Mixed peers never decode a local-append skin. The exact-pair channel is transition-driven and caches state until the remote unit exists. |

### Same-mod network channels

| Channel | Payload | Purpose / safety |
|---|---|---|
| `cwv_item_identity` schema 2 | Descriptor `{ slot, state, provider, item_key, base_item, skin, right_model, left_model, fingerprint, ... }`; acknowledgement sentinel `{ slot = "cwv_identity_ack", owner_peer, acknowledged_slot, fingerprint }` | Carries provider-qualified exact identity for a vanilla-base equipment wire shape (#396/#660). Receivers reconstruct models from local registries, validate schema/base/fingerprint, and acknowledge only after acceptance. Owner retry is peer+slot+fingerprint keyed, 0.5-second paced, and capped at eight attempts; wrong/stale acknowledgements do not clear it. Native/unavailable state fails closed, unit paths never cross the wire, and no field enters vanilla `NetworkLookup`. |
