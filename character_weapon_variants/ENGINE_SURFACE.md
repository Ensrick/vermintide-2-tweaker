...
| `SimpleInventoryExtension.game_object_initialized` [hook] `:10667` | First network broadcast of the loadout once the unit's GO id is ready, via `rpc_sync_loadout_slot` [src: `simple_inventory_extension.lua:249`] | Net-safe substitution/null of cwv keys before the encode; if transition-time parity is transiently unconfirmed, schedule a bounded post-confirmation identity replay (#416/#483) | Never let a modded skin ride while parity is false. The replay polls at most twice per second for 60 seconds and sends only after positive parity, covering the case where the shared gate misses a full state edge. |
...
| `GearUtils.hot_join_sync` [hook] `:10689` + peer-parity `on_enable` | Re-broadcasts a peer's equipment to a joining peer [src: `gear_utils.lua:462`]; parity enables after every peer acknowledges the same mod schema | Always-null the cwv skin id during the join race, then replay the exact vanilla base item + generated skin through `rpc_add_equipment` once decoding is safe (#425, #474, #579) | Never send a local-append skin before parity. The bounded post-ACK replay must re-send `rpc_wield_equipment` for the current slot so `SimpleHuskInventoryExtension` respawns both hands immediately; this includes cross-slot Old Musket `es_handgun` + `cwv_es_musket_old_skin`. Do not poll per frame. |
