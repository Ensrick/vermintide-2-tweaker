...
| `SimpleInventoryExtension.game_object_initialized` [hook] | First network broadcast of the loadout once the unit's GO id is ready, via `rpc_add_equipment` [src: `simple_inventory_extension.lua:249-282`] | Net-safe skin nulling plus same-mod `cwv_item_identity` owner marker; if transition-time parity is transiently unconfirmed, schedule bounded replay; publish exact Sword+Mace state after the vanilla call (#396/#416/#483/#567) | Never let a modded skin ride while parity is false. VMF-only identity and exact-pair channels are validated against the received vanilla base and never enter `NetworkLookup`. |
