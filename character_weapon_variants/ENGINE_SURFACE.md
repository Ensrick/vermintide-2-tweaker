...
|---|---|---|---|
| `SimpleInventoryExtension.wield` [safe] `:1519` | Sets the wielded slot, flips 1p/3p unit visibility, updates career/weapon state [src: `simple_inventory_extension.lua:627`] | Track local player's cross-access melee weapon+career, feeding the network-bound `_play_3p_anim` remap; debug wield dump; publish exact generated Sword+Mace cosmetic state after swap (#567) | CONSOLIDATED - `hook_safe` does not chain; a v0.1.336 duplicate silently shadowed this and broke 3P remap (`docs/VMF_RECIPES.md` §1; `_cwv_wield_hook_registration_count` guards it). Exact-pair publication is event-driven, not polled. |
...
| `SimpleHuskInventoryExtension._wield_slot` [safe] `:4634` | Husk-side wield: gets the unit table, then conditionally calls `spawn_inventory_unit` once per non-nil right/left field [src: `simple_husk_inventory_extension.lua:641,662-670`] | Re-key husk mesh+transform off base+career for cwv items (`:4634`, #392); upstream `get_item_units` preselection handles variants whose authored hand differs from the base (#478); replay cached exact Sword+Mace state after husk reconstruction (#567) | A correction inside `spawn_inventory_unit` is too late to create a hand call the earlier branch omitted. The #567 cache applies the exact skin through `add_equipment` and re-wields only the active slot, with recursion suppression. |
