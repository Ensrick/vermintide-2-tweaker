...
`character_weapon_variants.lua` unless a private module is named. `§N` = a `docs/BUG_CLASSES.md`
...
57 registration sites, grouped below into 8 logical rows-of-concern. `[hook]` =
...
| `SimpleHuskInventoryExtension._wield_slot` [hook] `:5189` | Husk-side wield: resolves the template, gets the unit table, then conditionally calls `spawn_inventory_unit` once per non-nil right/left field [src: `simple_husk_inventory_extension.lua:641,661-670`] | Establish one synchronous peer+slot Combat Style context, apply exact appearance/stance state, then enroll the complete custom 3P snapshot after all CWV adapters finish (#620/#760/#922) | CONSOLIDATED wrapper. Context is always cleared after vanilla returns/errors. The outer vanilla `wield` subsequently replaces the list from the same current inventory fields; Cosmetics separately owns post-replacement attachment composition. Style/identity edges resolve the human with `_cwv_peer_resolver`, then re-wield only the active slot. Readiness failures retry locally at 0.25 s for at most eight attempts; no network or per-frame unbounded replay (#786). |
...
| `ActionThrownProjectile._fire` / `_use_ammo` [hook] | The owner action emits the projectile, then consumes one ammunition [src: `scripts/unit_extensions/weapons/actions/action_thrown_projectile.lua:117-137,165-190`] | With #424 peer capability unproven, suppress an exact CWV Tuskgor Javelin before projectile creation and preserve its ammo; vanilla `we_javelin` passes | Identify through the live wielded slot's CWV backend/skin identity, never inherited `item_data.name == we_javelin`; emit one notice per blocked action object, not per frame |
...
| `LootItemUnitPreviewer.spawn_units` [hook] `:13031` | Illusion/skin browser and CIM's Athanor selector spawn display units, writing `self._spawned_units` AFTER return via `_spawn_items` [src: `scripts/ui/views/hero_view/loot_item_unit_previewer.lua:538`, `:504`] | Pre-pass `_om._cwv_browser_meshswap_apply` rewrites base-mesh spawn_data to variant units (#419), then applies shared appearance and Old Musket's per-unit texture binding (#617) | MUST be `[hook]` not `[safe]` - `_spawned_units` is nil at safe-callback time. Old Musket painting first proves all texture resources and binds its resident vanilla 3P parent material; `pcall` alone cannot catch a Stingray texture/material access violation. `_load_item_units` rebinds item_data to the BASE IML entry (:254-255), so resolve against `self._item`; dual-rig `j_leftweaponattach` (see dead-ends). |
...
| `EquipmentUI._sync_player_equipment` / `GamePadEquipmentUI._sync_player_equipment` [safe] `_cwv_musket_ammo_hud.lua` | Rebuilds HUD equipment state but refreshes ammunition only from `slot_ranged`, even when an ammo-bearing Old Musket is wielded in `slot_melee` [src: `equipment_ui.lua:327-426`; `gamepad_equipment_ui.lua:516-721`] | After vanilla sync, select the exact owner+`slot_melee` extension already registered by the shared Old Musket pool and refresh the native counter (#1108) | Presentation owns no ammo state. Require primary slot wielded, active pool membership, effective `ammo_data`, and exact ammo-hand unit equality before overriding the ranged-slot result. Cover desktop and gamepad without an update hook or RPC. |
...
| `TransientPackageLoader.hot_join_sync` [hook] / `GameNetworkManager.set_peer_synchronizing` [hook] / `NetworkServer.is_network_state_fully_synced_for_peer` [hook] / `remove_peer` [safe] | During `PeerStates.Loading`, transient sync encodes tracked projectile/husk names as numeric ids; immediately before `GameSession.add_peer`, the network manager begins GameObject synchronization and checks the sync predicate; real leave removes the peer [src: `scripts/game_state/components/transient_package_loader.lua:175-209`; `scripts/network/peer_states.lua:232,383-393`; `scripts/network/network_server.lua:242`; `scripts/managers/network/game_network_manager.lua:814,830-836`] | #424 shadows CWV transient refs to registered vanilla ids, calls `require_peer` at the first sender, removes live custom recovery pickups before object replay, holds a cleanup-failed peer outside `GameSession`, and forgets proof on real leave | Numeric lookup equality is never inferred from same-mod presence. Never wait in either join hook; saved inventory/settings remain untouched |
...
|---|---|---|
| `cwv_item_identity` schema 2 | Semantic descriptor `{ slot, state, provider, item_key, base_item_key, skin_key, fingerprint }`; acknowledgement sentinel `{ slot = "cwv_identity_ack", owner_peer, ack_slot, fingerprint }` | Carries provider-qualified exact identity for a vanilla-base equipment wire shape (#396/#660/#741). Receivers reconstruct unit paths from local registries, validate schema/base/fingerprint, and acknowledge only after acceptance. Owner retry is peer+slot+fingerprint keyed, 0.5-second paced, and capped at eight attempts; wrong/stale acknowledgements do not clear it. Native/unavailable state fails closed, unit paths and numeric lookup indexes never cross this wire. |
