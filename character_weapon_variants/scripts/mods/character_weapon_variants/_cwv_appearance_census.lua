...
-- cwv_es_musket_old: self-contained custom mesh + authored PBR material,
-- cross-slot stance toggle, NetworkLookup.inventory_packages lifetime alias.
families.old_musket = {
	matrix = matrix(
		...
		{
			owner_1p = row(UNSUP,
				"Stable create_equipment construction, post-wield equip, and explicit tuner generation are empirically routed. Mission transition and respawn may recreate equipment, but are not claimed until their own edge is observed.",
				{ instance_load=IMPL, equip=IMPL, customize=IMPL }),
			owner_3p = row(UNSUP,
				"Stable create_equipment construction, post-wield equip, and explicit tuner generation are empirically routed. Mission transition and respawn may recreate equipment, but are not claimed until their own edge is observed.",
				{ instance_load=IMPL, equip=IMPL, customize=IMPL }),
			bot = row(UNSUP,
				"Bot create_equipment construction and post-wield equip are explicitly classified; transition and respawn are not separately observed and therefore remain unsupported.",
				{ instance_load=IMPL, equip=IMPL }),
			husk = row(UNSUP,
				"Exact husk spawn, stable wield, and peer-ready consumers admit the locally resident custom unit only when semantic identity, stance-selected parent template, attachment profile, and authored material closure all agree. Any ambiguity preserves the resident es_handgun.",
				{ instance_load=IMPL, equip=IMPL, peer_ready=IMPL }),
			inventory_preview = row(UNSUP,
				"Inventory preview binds the authored material after exact slot construction and replays once at loading_done; other edges preserve vanilla.",
				{ instance_load=IMPL, preview_open=IMPL }),
			illusion_browser = row(UNSUP,
				"LootItemUnitPreviewer binds after exact construction and retries once at its source-backed visible/mip-stable edge; other edges preserve vanilla.",
				{ instance_load=IMPL, preview_open=IMPL }),
			cim_preview = row(UNSUP,
				"Only a LootItemUnitPreviewer carrying CIM's exact cim_preview_context_v1 marker is admitted; construction and visible/mip-stable edges are bounded.",
				{ instance_load=IMPL, preview_open=IMPL }),
			lobby = row(UNSUP,
				"No TeamPreviewer producer supplies an exact selected Old Musket identity in lobby/parading views; preserve the resident vanilla presentation rather than infer from another loadout slot."),
			score_team = row(UNSUP,
				"Score synchronization carries the vanilla es_handgun name and local outgoing identity is not present in the remote descriptor map; preserve the resident vanilla presentation until an exact selected-slot bridge exists."),
