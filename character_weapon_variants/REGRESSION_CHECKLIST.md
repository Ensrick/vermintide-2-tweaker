...
| Field | Check |
|---|---|
| Fix version(s) | CWV v0.1.422-dev; v0.1.430-dev (#644 input/donor parity); v0.1.433-dev (#648 Bretonnian balance/indicator); v0.1.434-dev (#644/#648 Greatsword dedup/receiver transform) |
| Repro | Select supported Greatsword, Bretonnian Longsword, Greathammer, and Tuskgor Spear instances, cycle with the contextual `Switch to:` button, then use the hotkey. Native Greatsword must follow `Kerillian -> Bretonnian -> Greatsword`; Bretonnian must follow `Greatsword -> Kerillian -> Bretonnian`. Confirm the small unboxed inventory ordinal advances once with each transition. Compare Greatsword's Bretonnian style on owner 3P, remote husk, and preview surfaces against first person and a native Bretonnian control. Restart and hot join with a second CWV peer; include pre-#620 Infantry Spear, Imperial Longsword, and Black Guard UUIDs. Open CIM's Athanor on Dual Axes. |
