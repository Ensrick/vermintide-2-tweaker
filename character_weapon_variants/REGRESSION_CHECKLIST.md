...
| Fix | `_cwv_musket_ammo_hud.lua` runs after native equipment synchronization and replaces the displayed count only when the wielded primary slot, pool owner/slot, effective template ammo hand, and exact live 1P ammo unit all agree. After engagement, it restores the current native ranged item and correct wield focus before releasing, or explicitly defocuses/hides its owned presentation when native state is unreadable. Never-engaged HUD instances remain untouched. |
...
| Detection | `/cwv_regression_test` passes `issue1108_primary_slot_musket_ammo_hud_contract` by executing engage -> native-ranged restoration; bounded `[cwv:1108]` engage/release receipts identify the live presentation edge. Offline tests execute both installed native callbacks and cover right/left hands, owner/spectator resolution, dead/unregistered/replaced/mismatched primary state, stance exit, throwing readers, controller cached-sync cadence, native focus restoration, hide fallback, and never-engaged non-mutation. |
...
| Field | Check |
|---|---|
| Fix version(s) | CWV v0.1.438-dev (owner/bot/remote-husk world slice); v0.1.452-dev (acknowledged bounded hot-join replay; umbrella remains open) |
...
## #660 Greatsword Combat Style descriptor adoption

| Field | Check |
|---|---|
| Scope | First W4 family migration after the verified Old Musket pilot. Greatsword/Imperial Longsword Combat Styles use one immutable semantic descriptor across local planning, exact transport, receiver reconstruction, effective-template selection, husk re-wield postconditions, and 1P/3P transform selection. Other Combat Style families remain on the bounded compatibility catalogue until migrated separately. |
| Repro | Equip Kruber's Greatsword and switch among Greatsword, Kerillian, and Bretonnian Combat Styles. Repeat with Bretonnian Longsword and one migrated Imperial Longsword UUID. Verify owner 1P/3P, inventory, Athanor, swap away/back, Keep-to-mission transition, same-mod remote husk, hot join, and disconnect/rejoin. Use a vanilla Greatsword with no style change as the control. |
| Expected post-fix | Every consumer sees the same style, template, model, and authored transform from one descriptor fingerprint. A style switch produces one new fingerprint and one bounded rebuild. Identity-first and style-first delivery converge; a duplicate style message cannot erase the descriptor. Missing/foreign/tampered state preserves the local vanilla fallback. Peer teardown removes only that peer's descriptor and retry state. No backend UUID, custom numeric lookup id, per-frame RPC, or global dedupe reset is used. |
| Detection | Offline `test_cwv_combat_style_appearance.lua` executes builder-to-payload-to-receiver reconstruction, fingerprint parity, style/template/transform changes, dedupe, foreign/missing/tampered inputs, and legacy hand compatibility. `test_cwv_appearance_lifecycle.lua`, `test_cwv_husk_adapter.lua`, `test_cwv_combat_styles.lua`, and `test_cwv_identity_peer_pull.lua` cover style-rider equality, both channel arrival orders, descriptor-owned postconditions, same-style/new-fingerprint rebuild, targeted disconnect cleanup, and exact single-hook teardown. `/cwv_regression_test` must pass `issue660_greatsword_style_appearance_transaction`. |
...
| Field | Check |
|---|---|
| Fix version(s) | CWV v0.1.380-dev; hardened v0.1.435-dev; committed-state read v0.1.499-dev; EXACT-CATALOG conversion (this slice) |
...
| Repro (exact axis) | Both peers run CWV, but one also runs another mod that appends to `NetworkLookup.damage_profiles` (or a different CWV build), so the two peers' `cwv_*` integers disagree while the presence beacon reports a full lobby. |
| Expected post-fix | Mixed, unknown, or catalog-DISAGREEING parity sends only the recorded vanilla donor id, then vanilla `default` as a fallback; if neither is provable the unsafe hit is suppressed. A `cwv_*` damage-profile index never reaches a host that did not prove the identical numeric catalog. Only a proven exact catalog (channel `cwv_damage_profiles_exact_v1`) plus the COMMITTED gate state (`applied_state`) retains tuned CWV damage; the authoritative server path is unaffected. Same-mod presence alone no longer authorizes a custom id. |
| Detection | Offline `test_cwv_damage_profile_wire.lua` passes all policy cases, the generation pin (a producer recording a mapping after finalization invalidates the catalog), and the divergent-beacon fixture (classifier says all-acked, committed state still disabled -> must still substitute). `/cwv_regression_test` passes `cwv_wire_safe_damage_profile_gate`, whose fixture now stubs `mod._cwv_damage_peer_parity`. Boot log carries one `[cwv:423] exact damage catalog=... rows=N` line; mixed-lobby substitutions emit one bounded `[cwv:423]` row per profile and the host remains connected. |
...
| Field | Check |
|---|---|
| Fix version(s) | CWV v0.1.352-dev onward (senders, hot-join fence); world-resident bomb sweep v0.1.499-dev; EXACT-CATALOG conversion + in-flight cleanup (this slice) |
...
| Repro (in-flight axis) | All-CWV lobby. Host throws a Tuskgor Javelin and a player WITHOUT CWV joins while the spear is still travelling, before it links or lands. |
| Repro (exact axis) | Two CWV peers whose appended `pickup_names` / `husks` / `projectile_units` integers disagree (a second lookup-appending mod, or different CWV builds). The presence beacon reports a full lobby. |
| Expected post-fix | No `cwv_*` `pickup_names`, `husks`, or `projectile_units` index reaches a peer that has not proven the identical numeric catalog, by any route. Throws are refused before the encode while the gate is closed - and the gate is closed whenever the exact thrown catalog (channel `cwv_thrown_resources_exact_v1`) is unproven, not merely when a peer lacks CWV. Impact pickups degrade to the vanilla throwing-axe keys ONLY when that donor is proven end to end (`pickup_names` + `AllPickups` + `husks` + `go_types`); an unprovable or undeclared donor DROPS the pickup instead of riding the custom id. The in-flight mesh degrades to the vanilla javelin unconditionally, and a malformed resolution is refused at `ProjectileSystem.spawn_player_projectile` before vanilla's `.projectile_unit_name` dereference. On the world and in-flight axes the joining player loads in without a crash: both live pickups and tracked in-flight projectiles are removed before the join completes, and a deletion that did not take holds the peer outside the GameSession rather than reporting success. |
| Detection | `/cwv_regression_test` passes `cwv_wire_safe_thrown_variant_installed`, which now also fails if an undeclared `cwv_` pickup key resolves to anything but DROP. Offline `test_cwv_thrown_wire_policy.lua` drives the gate's real hook bodies: three-valued disposition totality, donor revalidation per broken table, the ProjectileSystem preflight, the in-flight sweep, and the failed-deletion path. Boot log carries one `[cwv:424] exact thrown catalog=... rows=5` line; `[cwv:424] hot-join preflight ... removed_pickups=N removed_projectiles=N fenced=N` must report the full fenced count (`fenced=2` means the bomb never enrolled), and each gate close emits one `[cwv:424] gate closed; world sweep ok=... projectiles=... pickups=... fenced=...`. |
| Repro (rejoin axis) | A peer that has proven the exact catalog disconnects and immediately rejoins under the same peer id, inside the parity library's bounded absence-expiry window (`_lib_peer_parity.lua:576-583`). |
| Expected post-fix (rejoin axis) | The rejoining peer is unproven on EVERY channel until it handshakes again: `GameNetworkManager.remove_peer` retires the presence beacon and both exact channels in one consolidated `hook_safe` body, per-peer and deduplicated by instance identity. A pre-disconnect ack can never authorize custom ids after the rejoin. |
| Residual | Sweep and fence are host-side by construction (a client cannot retract a server-owned pickup unit). A bomb spawning between the sweep and the joiner's game-object extraction is theoretically reachable. |
...
| Field | Check |
|---|---|
| Fix version(s) | CWV v0.1.422-dev; v0.1.430-dev (#644 input/donor parity); v0.1.433-dev (#648 Bretonnian balance/indicator); v0.1.434-dev (#644/#648 Greatsword dedup/receiver transform); v0.1.436-dev (#645 Saltzpyre membership/receiver remaps); staged #692 reciprocal Bretonnian transform; staged #657 reciprocal native Sword-and-Shield family |
| Repro | Select supported Kruber and Saltzpyre Greatswords, Bretonnian Longsword, Greathammer, Tuskgor Spear, Empire Sword and Shield, and Bretonnian Sword and Shield instances; cycle with the contextual `Switch to:` button, then use the hotkey. During light, charged-heavy, block, push, and dual-hand actions, press the hotkey and confirm the action ends before one style change; activate a career skill as an unaffected control. Each Sword-and-Shield item must cycle to the other item's complete native moveset while retaining its own sword, shield, illusion, glow, and icon. Both native Greatswords must follow `Kerillian -> Bretonnian -> Greatsword`; Bretonnian must follow `Greatsword -> Kerillian -> Bretonnian`. Confirm the small unboxed inventory ordinal advances once with each transition. On Witch Hunter Captain, Bounty Hunter, and Zealot, exercise light/heavy/charge/block in both foreign Greatsword styles and compare owner 3P with a remote husk. Compare Kruber Greatsword's Bretonnian style on owner 3P, remote husk, and preview surfaces against first person and a native Bretonnian control. Then equip the native Bretonnian Longsword, switch to Greatsword and Kerillian styles, and compare its grip and proportions across owner 1P, owner/peer 3P, inventory, and Athanor; swap away/back and enter a mission. Restart and hot join with a second CWV peer; include pre-#620 Infantry Spear, Imperial Longsword, and Black Guard UUIDs. Open CIM's Athanor on Dual Axes. |
| Expected post-fix | Each exact instance retains its style. One physical equipment-button click consumes exactly one release edge and commits one transition; button, hotkey, and ordinal orders are identical. A live weapon action finishes once with the engine's `interrupted` reason before the style rebuild; failure leaves style/persistence unchanged, and career abilities are untouched. Native Greatsword exposes three distinct movesets; its Bretonnian receiver uses the Imperial scale/grip on 3P/presentation surfaces only. Native Bretonnian Longsword uses the exact reciprocal scale `{1, 1.25, 1.111111...}` and Z offset `+0.065` under Greatsword/Kerillian styles on 1P and every 3P/preview consumer; its native style remains untransformed. Retired Imperial/Black Guard rows do not receive that inverse because their physical meshes are Empire Greatswords. Bretonnian exposes three and its receiver-specific Kruber package has matched native reach, 125% stagger, and 75% cleave. Kerillian style has 85% attack timing, 107.5% damage, 125% stagger, and 125% cleave. Legacy Imperial UUIDs retain their hidden Longsword style until explicitly cycled away. Greathammers expose two and Tuskgor Hunter/Infantry. Foot Knight has Tuskgor by default. Legacy UUIDs become their native item+style without illusion/forged-field loss, and retired rows are absent from new crafting/WT. All render surfaces agree without stale transforms, interrupted-action residue, per-frame RPCs, query loops, or missing-icon crashes. |
| Detection | Offline `test_cwv_combat_styles.lua` passes no-action, one/two-hand interrupt, hand deduplication, career-action protection, stop failure/no-persistence, both reciprocal Greatsword presentation descriptors, native Bretonnian untransformed state, retired Imperial transform isolation, descriptor preparation before live rebuild, and peer-scoped descriptor/retry teardown; it also extracts and executes the shipped `issue774_mission_combat_style_interruption` check and proves a disconnected helper turns that check red. `test_cwv_combat_style_appearance.lua` owns the migrated Greatsword descriptor transaction and adversarial receiver cases. The suite additionally passes the Saltzpyre member cycle, receiver template clone, source-backed Kerillian/Bretonnian event remaps, descriptor catalogue, DLC, reciprocal Spear and Shield donor clone (including its first-person `parry_pose` block contract), exact-instance/remote effective-template contract, one owner rebuild, one deduplicated husk rebuild, and empty-slot fail-closed guard. `test_wt_cwv_effective_template.lua` covers both Sword and Shield donor directions plus native fallback; `/cwv_regression_test` passes `issue620_per_instance_combat_styles`, `issue660_greatsword_style_appearance_transaction`, `issue774_mission_combat_style_interruption`, and `issue645_reciprocal_style_descriptors`; in-game Saltzpyre must show the style control and match foreign-style 3P animations on owner and peer, while Kruber with Elven Spear and Shield style must visibly enter/hold/leave first-person block before #645 is verification-ready; transition-only `[cwv:620]`/`[cwv:944]` state/interrupt/refresh and capped `[cwv:645]` candidate evidence remain bounded. |
...
- [ ] `/cwv_regression_test` passes `issue774_mission_combat_style_interruption`: ordinary action stops once and commits once, career action stops/commits nothing, and a failed stop performs no persist or slot rebuild.
...
## #604 Dawi Crowbill perspective transform

| Field | Check |
|---|---|
| Fix version(s) | CWV v0.1.432-dev, v0.1.442-dev, v0.1.443-dev |
