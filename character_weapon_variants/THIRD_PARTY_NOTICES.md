...
## Outrider Grenade Launcher 3D model

Issue #627 applies a user-supplied custom launcher mesh to the
`cwv_es_outrider_grenade_launcher` variant. The maintainer supplied the archive
directly; it carries **no author, title, source URL, license, or attribution
metadata** (the FBX metadata is sanitized to a placeholder path, epoch time, and
"Blender 4.2.8"). The only stable identity is the archive SHA-256. Derived
resources are shipped beneath:

- `units/cwv_launcher/launcher_01/*` and `textures/cwv_launcher/launcher_01/*`
...
| Provenance | User-supplied by the maintainer |
...
The original archive is preserved unmodified outside the repository. No licensing
terms accompanied the asset; this row records that gap honestly rather than
asserting a license the asset does not carry. If redistribution terms surface
later, update this row.

### Changes made to the original

- Source FBX (~500,000 triangles) collapse-decimated to 11,999 triangles with
  Blender 4.4, joined/applied transforms, normalized so the longest dimension is
  two Blender units with the barrel along positive X.
- Material slots collapsed to the short Stingray-safe name `launcher_mat`.
- Identical 1P and `_3p` FBX/unit siblings emitted with perspective-appropriate
  shadow settings.
- The packed glTF metallicRoughness map was unpacked (G = roughness, B =
  metallic); albedo and normal converted to PNG; all maps capped at 2048 pixels;
  a neutral white AO map supplied (the source carries none). No new artistic
  texture content was added.

The pinned conversion commands and integration requirements are recorded in
`tools/LAUNCHER_ASSET_PIPELINE.md`.
