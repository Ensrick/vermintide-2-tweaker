...
## 0.1.514-dev (2026-08-13) -- pin the exact clone base identity (#1125) [untested]
...
- The entry falls from 3,849 to 1,490 nonblank lines and every new owner remains below 1,500. The decomposition contract advances from partial to complete and requires 19 owner modules. The five loads remain at the positions their inline blocks occupied; the bootstrap's explicit phases preserve the two intervening registrar/registration boundaries.
- Runtime behavior is preserved: strict lint reports the same 91 hook registrations with no duplicate pair, Lua 5.1 parses the entry and all five owners, and 2,685 engine-free tests pin hook cardinality, cross-peer identity payloads, acknowledgement retry bounds, transform ownership, Javelin resource leases, Rapier construction, skin registrar order, and all affected diagnostics.
- The three moved Javelin package loads retain lifetime-lease behavior and carry one file-level `resource-safety: cwv1159-javelin-runtime-force-load` marker linked to exact offline coverage. No package unload, per-frame network traffic, or new engine boundary was introduced.

## 0.1.509-dev (2026-08-10) -- the musket equip surface moves to its own owner file (#1159) [untested]

- **Internal (#1159): everything vanilla has to do differently for a CWV musket, from the inventory grid to the destroyed weapon unit, now lives in `_cwv_musket_equip_surface.lua` instead of the entry script.** Every surface in the owner exists for one root cause: a CWV variant deliberately inherits its base weapon's `item_data.name` (clobbering it crashes equip), so each name-keyed vanilla lookup on the equip path resolves the BASE item, not the variant. The owner holds all of the corrections: the cross-slot recognizer `_is_cwv_musket_item` with its prefix list and the v0.1.327 all-candidate-id ladder (a single short-circuit on the inherited base name is what once made the musket vanish from the melee grid); the v0.1.338 scoped career `slot_melee` override, which appends "ranged" only to careers that own a `cross_slot` variant, with the ItemGridUI category/filter pair and the single `get_filtered_items` post-filter that narrows the result back down; the defensive `WeaponSpreadExtension` init/update pair, which is the same inherited-name failure one layer down (vanilla reads the BASE spear template, whose missing spread template leaves `spread_settings` nil and faults the next update frame); and the bayonet child-unit lifecycle end to end - the 1P/3P sword unit paths, the tuned translation, rotation and scale constants, the unit-data slot, the weak-keyed pair ledger, the package force-loads, and the spawn, attach, detach, orphan-prune and visibility-sync helpers with the five hooks that drive them.
- Nothing else changed. The 914 moved lines are two unbroken byte-identical ranges (409 and 505 nonblank), mechanically proven by reconstructing the pristine entry byte-for-byte from the new entry plus the two ranges taken back out of the new owner - SHA-256 identical, and the version bump is the only non-move edit to surviving entry text. Hook cardinality is provably identical against a pristine `git archive` of the mod: 91 registrations across the same 91 class+method pairs before and after, zero duplicate pairs on either side, so each of the ten moved pairs is still the mod's sole registration.
- The block was INTERLEAVED, so install is two-phase. The recognizer, the career override, the spread guard and the bayonet constants ran BEFORE the dual-weapon first-person residency, the husk-residency owner load and the husk wield diagnostic; the spawn and wield hooks ran AFTER all three. `install` therefore runs the first half at the first former position and hands the second half back as a closure, which the entry calls at the exact second former position. Both halves share the file's upvalues, so the bayonet ledger and the tuning constants declared in phase one are the same objects phase two mutates. Ordering is offline-locked: phase one, then the residency owner, then the husk wield diagnostic, then phase two.
- The career re-extension handler travels with its mutation, and its position still matters. `mod.on_all_mods_loaded` re-runs the `slot_melee` extension because `CareerSettings` can still be partial at load time; `_cwv_husk_residency_owner` CHAINS whatever handler is installed when it loads. Phase one runs above that load, so the assignment still happens first and the chain is intact - two handlers mod-wide, offline-asserted.
- Boundary: this owner covers the EQUIP PATH, not the weapon and not the husk. The Old Musket's item, template, stance-swap and ammo behavior stay in `_cwv_musket_runtime` and the entry's consolidated `BackendUtils.get_item_template` hook; the husk render helpers stay in `_cwv_husk_path`; the mesh gaps stay in `_cwv_custom_mesh_runtime`; the Tuskgor Javelin template, its thrown-resource wire and its availability gate stay where they are. Three sibling owners dispatch through the spawn and wield hooks that moved - the husk adapter's pre- and post-spawn halves with the `not owner_unit_1p` discriminator, the Old Musket appearance, FX-proxy and ammo-pool surfaces, and the issue 922 fade and issue 604 Crowbill edges - and every one of those is a late-bound `_om` lookup resolved at fire time, so the calls travelled unchanged and their owners keep their jobs. The v0.1.322 javelin 3P spare-offhand hide rides the wield and camera hooks for the same reason.
- Three `Managers.package:load` leases moved with the block (both bayonet sword units and the polearm first-person state machine, across two call sites, since the bayonet units share one loader). They are covered by one file-level `resource-safety: cwv1159-musket-equip-surface-force-load` marker rather than inline annotations, so the moved lines stay byte-identical; the gate collects markers per file. There is no matching `unload`: all three are held for the mod's lifetime, which is why the bayonet spawn helper fails CLOSED on `has_loaded` instead of force-loading from inside the spawn path.
- All nineteen file-scope locals in the moved ranges had ZERO references outside them before the move, so nothing left in the entry can reach into the owner; shedding them for one buys back eighteen slots against the entry chunk's Lua 5.1 200-local ceiling. Everything other files read stays published on `_om` from the same statements (`_om._collect_cross_slot_careers`, `_om._slot_extension_log_only`, `_om._cwv_filter_slot_name`), so the relocated issue 1148 slot-extension regression check reads the same namespace it did before. All three context bindings arrive by value, declared once at entry file scope above the load point and never rebound, so the capture cannot go stale; `_om` travels as a shared table reference.
- Entry file drops from 4,737 to 3,849 nonblank lines; the offline decomposition contract ratchets to that new ceiling and now requires `_cwv_musket_equip_surface.lua` as its fourteenth owner. New offline coverage pins the owner's fifteen producers, ten hook registrations, three `_om` publications and seven log markers, asserts the entry keeps none of them, locks the two-phase install order against the residency owner and the husk wield diagnostic, locks the resource-safety marker and the residency-gated bayonet spawn, and checks the owner absorbs no sibling owner's job. Four gates that were path-pinned to the entry follow the code: the issue 392 husk-discriminator invariant (now with an `absent` companion row forbidding a shadowing spawn-hook registration in the entry), the husk-adapter and husk-path hook-consolidation tests, the issue 935 cross-slot production test, and the issue 1148 marker-contract test.

## 0.1.508-dev (2026-08-10) -- custom-mesh runtime moves to its own owner file (#1159) [untested]

- **Internal (#1159): everything that makes a CWV mod-bundled custom weapon mesh behave like a unit the engine shipped now lives in `_cwv_custom_mesh_runtime.lua` instead of the entry script.** A custom mesh has no sibling `.package` file, no entry in the strict `NetworkLookup.inventory_packages` table, no skeleton for the vanilla template's attachment links and no flow graph for the weapon FX its actions fire; each gap used to be patched by a separate inline block. The owner now holds all four: the Loremaster's-Armoury-pattern package shims (`PackageManager.load` / `unload` / `has_loaded` no-op or report success for a path the master bundle already carries) with the issue 474 husk bundle-unit predicate that lets the husk mesh re-key accept those same two paths; the forward-only `inventory_packages` aliases (the Old Musket 1P/3P pair onto the vanilla empire-handgun indices, plus the issue 597 Greataxe and issue 604 Crowbill alias installs); the eight authored Old Musket transform constants with the single `_om._old_musket_transform_components` reader every render surface resolves through, the issue 617/742 texture-policy re-exports and the issue 1155 Phase 3 appearance pilot with its preview descriptor and texture-target seams; the issue 474 stance-channel load; the v0.1.293 FX-proxy lifecycle with the four `Unit` redirect hooks (`node`, `has_node`, `flow_event`, `set_flow_variable`) that route a lookup on our skeleton-free mesh to the hidden vanilla rifle linked beside it; and the v0.1.290 `GearUtils.link_units` attachment-node filter that drops named rig targets our FBX has no node for.
- Nothing else changed. The 384 moved lines are ONE contiguous byte-identical block, mechanically proven by reconstructing the pristine entry byte-for-byte from the new entry plus the region taken back out of the new module - the version bump is the only non-move edit in the file. No statement changed neighbours, so nothing reordered. The owner loads at the exact point the moved block ran, and hook cardinality is provably identical against a pristine `git archive` of the mod: 91 registrations across the same 91 class+method pairs before and after, with zero duplicate pairs on either side, so each of the eight moved pairs is still the mod's sole registration.
- The issue 474 stance channel is dofiled from INSIDE the moved block and stays there, at its original position in the load sequence: its `_om` exports must exist before the fire dispatch above it first runs and before the identity register far below routes into them. Moving the surrounding block wholesale preserves that exactly; a module loading a sibling module is the same shape `_cwv_weapon_transform_owner` and `_cwv_appearance_fade` already use.
- Boundary: this owner covers the MESH, not the weapon. The Old Musket's item, template, stance-swap and ammo behavior stay in `_cwv_musket_runtime` and the entry's consolidated `BackendUtils.get_item_template` hook; the texture preflight and the native paint calls themselves stay in `_cwv_old_musket_preview` (this file only re-exports its four entry points onto `_om`, exactly as the entry did); and the bayonet child-unit lifecycle with its visibility hooks stays in the entry directly above the new load point, because a bayonet is a second VANILLA unit linked to the rifle, not a custom-mesh gap. No native-resource boundary moved: the block contains no particle, GUI, texture-write or package-manager call of the kind the resource-safety gate tracks.
- Both file-scope locals in the moved range (`_LA_PATTERN_CUSTOM_PACKAGES` and the pre-hook `Unit.has_node` capture) had ZERO references outside it before the move, so nothing in the entry can reach into the owner; shedding them also buys back two slots against the entry chunk's Lua 5.1 200-local ceiling. All three context bindings arrive by value and are declared exactly once at entry file scope above the load point and never rebound, so the capture cannot go stale; `_om` travels as a shared table reference, so every slot the owner publishes and every slot it reads later from a hook body stays the entry's own.
- Entry file drops from 5,072 to 4,737 nonblank lines; the offline decomposition contract ratchets to that new ceiling and now requires `_cwv_custom_mesh_runtime.lua` as its thirteenth owner. New offline coverage pins the owner's nine producers, eight hook registrations and four log markers, asserts the entry keeps none of them, locks the boundary against the item-template, wield and bayonet surfaces the entry retains, checks the owner absorbs no sibling owner's job, and re-pins three invariants the moved code exists to protect: the forward-only direction of the `inventory_packages` alias, the v0.1.298 rule that all four authored rotations stay boxed (a raw Stingray Quaternion is a per-frame temporary), and the v0.1.290/291 rule that attachment targets are existence-tested with `Unit.has_node` because `Unit.node` throws an engine-level fatal `pcall` cannot catch.

## 0.1.507-dev (2026-08-10) -- weapon transforms move to their own owner file (#1159) [untested]

- **Internal (#1159): everything that decides and applies a CWV weapon transform now lives in `_cwv_weapon_transform_owner.lua` instead of the entry script.** The owner answers one question end to end - which transform record applies to an item, skin or spawned unit, and how that record is written onto the unit. It holds the authored `_type_transforms` type table and the per-variant-over-type precedence resolver `_resolve_field`; the four registries built from it at load (`_transform_map`, `_skin_transform_map`, the issue 597 Greataxe per-model defs, the issue 604 Crowbill per-model defs with their by-unit index, the `_custom_illusions` synthetic defs and the longest-match inherit pass that gives a dynamically-registered cross-character illusion its variant's scale); the shared WeaponAppearance instance from issue 420 with its `_apply_scale` / `_apply_offset` / `_transform_unit` wrappers and both `/cwv` compatibility handles; the issue 604 Crowbill presentation owner, its rotation ops and the durable / relative-scale / evidence / transform-runtime library wiring behind `mod._cwv_crowbill_set_mode` and friends; the single per-hand applier `_apply_cwv_hand_transform` with its bounded 64-shot scheduling evidence; and the three resolvers the surfaces call - `_resolve_cwv_def`, `_om._cwv_resolve_crowbill_transform` and the issue 604 husk transform-policy bind.
- Nothing else changed. The 573 moved lines are three unbroken byte-identical ranges, mechanically proven by reconstructing the pristine entry byte-for-byte from the new entry plus the three ranges taken back out of the owner - the version bump is the only non-move edit to surviving entry text. The owner loads at the exact point the first moved block ran, so registry construction still completes before the Combat Style install reads its one authored record out of the type table and before any surface can resolve a def. Hook cardinality is provably identical: 94 registrations across the same 92 class+method pairs before and after, and the owner registers none of them.
- Boundary: this owner is a PRODUCER, not a surface. It installs no hook, no network channel and no command. All four consumers stay exactly where they were and keep calling in - world and bot equipment through the entry's `GearUtils.create_equipment` hook (which keeps its own transform-miss evidence counters), keep and menu previews through `_cwv_menu_preview_owner`, remote husk display through `_cwv_husk_path` and the husk transform policy, and the issue 1158 exact-appearance descriptors through the entry's world-descriptor resolver and `BackendUtils.get_item_units` hook. The Combat Style install also stays put: it authors weapon TEMPLATES and only reads the Imperial Longsword record out of the published type table.
- Thirteen of the twenty-three moved file-scope locals had zero references outside the moved ranges before the move (proved with a word-boundary scope probe over the pristine entry) and are now unreachable from the entry. The other ten are published under one `_om.weapon_transform` namespace and re-bound in the entry under their original names, which is why every surviving statement in the entry is byte-identical rather than rewritten. All five context bindings are declared once at entry file scope above the load point and never rebound, so the by-value capture cannot go stale; the registries travel as shared table references.
- Entry file drops from 5,606 to 5,072 nonblank lines and sheds a net 13 top-level locals (measured against the Lua 5.1 200-local ceiling: headroom rises from 95 to 108). The offline decomposition contract ratchets to that new ceiling and now requires `_cwv_weapon_transform_owner.lua` as its twelfth owner. New offline coverage pins the owner's sixteen producers and eleven published handles, asserts the entry keeps none of them, proves the owner registers nothing and crosses no native-resource boundary, locks the four consumer surfaces on the entry side, checks it absorbs no sibling owner's job, and pins the issue 409 and issue 417 registration gate signals plus every hand-tuned scale and grip triple. The issue 420 shared-library gate and the issue 692 appearance contract now follow the code into the owner.

## 0.1.506-dev (2026-08-10) -- keep/menu preview surfaces move to their own owner file (#1159) [untested]

- **Internal (#1159): every menu-side reconstruction of a CWV variant now lives in `_cwv_menu_preview_owner.lua` instead of the entry script.** The move covers the whole keep/menu render path: the previewer definition resolver (`_find_preview_slot_info`, `_crowbill_def_from_spawn_data`, `_resolve_preview_def`) and the shared post-spawn applier behind BOTH `HeroPreviewer._spawn_item` and `MenuWorldPreviewer._spawn_item` (the issue 482 backend-id ladder rung the previewer needs because its info table carries no item_data, the string-slot_type to numeric-slot_index key bridge, the issue 760 Outrider stance replay, the per-hand transforms, the issue 604 Crowbill presentation with its TeamPreviewer identity diagnostic, and the issue 474/617/792 Old Musket stance, texture and armed-pose branch); the issue 604 TeamPreviewer identity bridge and its `_spawn_hero` hook; the cosmetic picker's cwv-only illusion filter (`HeroWindowItemCustomization._setup_illusions`) with its layout recompute; the two preview teardown edges (`HeroPreviewer._destroy_item_units_by_slot`, `LootItemUnitPreviewer._destroy_units`); and the illusion browser / Athanor craft pane (`LootItemUnitPreviewer.spawn_units`) with its issue 597 mod-unit fallback, issue 419 browser mesh-swap pre-pass and issue 617 Old Musket texture rebind.
- Nothing else changed. The 484 moved lines are ONE contiguous byte-identical block, mechanically proven by reconstructing the pristine entry byte-for-byte from the new entry plus the region taken back out of the new module - the version bump is the only non-move edit in the file. No statement changed neighbours, so nothing reordered. The owner loads at the exact point the moved block ran, and hook cardinality is provably identical: 94 registrations across the same 92 class+method pairs before and after, with each of the seven moved pairs still the mod's sole registration on that pair.
- Boundary: this owner is the MENU third of CWV's three presentation surfaces. World and bot equipment stays in the entry's `GearUtils.create_equipment` hook directly above the new load point, keeping its own transform-miss evidence counters; remote husk display stays in `_cwv_husk_path`; and the shared producers both surfaces call (`_resolve_cwv_def`, `_find_def`, the transform maps) stay in the entry. The five moved file-scope helpers each had ZERO references outside the moved range before the move, so nothing left behind can reach into the owner.
- All ten entry dependencies arrive through an explicit context table by value. Each is declared once at entry file scope above the load point and never rebound - verified with a block-depth scope probe over the pristine entry - so no late-binding accessor is needed; the three transform maps travel as shared table references, so their in-place population after this point still reaches the owner.
- Entry file drops from 6,058 to 5,606 nonblank lines; the offline decomposition contract ratchets to that new ceiling and now requires `_cwv_menu_preview_owner.lua` as its eleventh owner. New offline coverage pins the owner's seven producers, seven hook registrations and five log markers, asserts the entry keeps none of them, locks the boundary against the world-equipment surface and the sibling husk / registration / skin owners, and re-pins the key-bridge warning that has been refactored away twice. The issue 474 preview-lease and issue 760 stance gates now read the owner for the surfaces that moved.

## 0.1.505-dev (2026-08-10) -- item registration moves to its own owner file (#1159) [untested]

- **Internal (#1159): the definition-to-backend-item registration path now lives in `_cwv_item_registration_owner.lua` instead of the entry script.** The move covers everything between a variant definition row and a live, network-safe backend item: the issue 482 identity ladder (`_registered_keys`, the positively-validated backend-id to variant-key cache, its 16-shot legacy diagnostic and the shared `_om._cwv_key_for_item` resolver every bid-keyed consumer calls), the `_build_entry` ItemMasterList clone constructor (the `cwv_variant` cross-mod marker, the issue 482 `cwv_key` self-stamp, per-hand unit / icon / can_wield / template overrides, the issue 279 ammo-unit clear, the item_type to skin_combination_table map and the issue 592 definition-vs-instance `mod_data` split), the deferred once-per-session `_auto_register_all` pass on `StateInGameRunning.on_enter` (ItemMasterList mirror, issue 661 career-action integration, the issue 567 WeaponSkins reverse-index invalidate and rebuild, the NetworkLookup.item_names append, issue 592 Blacksmith seed registration and the legacy auto-grant purge), and the issue 273 Deus identity installer with its `DeusMechanism._setup_run` last-chance boundary.
- Nothing else changed: the 695 moved lines are byte-identical to what the entry ran, mechanically proven by reconstructing the pristine entry from the new entry plus the two moved regions. The owner loads at the point the first moved block executed, which is what the entry's `mod._cwv_resolve_item_key = _om._cwv_key_for_item` handle requires. Hook cardinality is provably identical - 94 registrations across the same 92 class+method pairs before and after - and both moved hooks are the mod's sole registration on their pair.
- The give command, the shared preview / illusion-browser spawn descriptors and `_find_def` deliberately stay in the entry: the first is command surface owned by `_cwv_commands_lifecycle`, the second is the issue 1158 exact-appearance channel, and the third is reached by both plus `_cwv_husk_path`. The inline bidirectional `NetworkLookup.item_names` register moved verbatim rather than folding onto the shared lookup helper; that fold is a separate behavior-adjacent change (#428).
- Entry file drops from 6,688 to 6,058 nonblank lines; the offline decomposition contract ratchets to that new ceiling and now requires `_cwv_item_registration_owner.lua`. New offline coverage pins the owner's producers and log markers, asserts the entry keeps none of them, locks the boundary against the give command / exact-appearance descriptors and the sibling husk and skin owners, and asserts the #428 fold did not ride along.

## 0.1.504-dev (2026-08-09) -- husk residency moves to its own owner file (#1159) [untested]

- **Internal (#1159): the cross-character husk residency machinery now lives in `_cwv_husk_residency_owner.lua` instead of the entry script.** The move covers the three blocks that make a remote-player weapon spawn survivable on a client that is not playing the source career: the `dr_shield_axe` base-unit force-load (the issue 280 crash floor for the no-skin base-path spawn), the data-driven override-unit residency pass that walks every variant definition and force-loads any per-hand mesh differing from its base weapon (issues 396/401, including the shared `_om._husk_override_unit_needs_residency` predicate the regression test derives from and the attempt-capped `on_all_mods_loaded` retry), and the `SimpleHuskInventoryExtension.start_weapon_fx` nil-slot guard that backs both up (issue 280). Nothing else changed: the 248 moved lines are byte-identical to what the entry ran, the module loads at the exact point the inline blocks used to execute, so the boot force-load order, the `on_all_mods_loaded` chain position and the hook order are unchanged. Hook cardinality is provably identical - 101 registrations and the same 82 class+method pairs before and after.
- The husk wield diagnostic (`SimpleHuskInventoryExtension._wield_slot`) deliberately stays in the entry: it dispatches the exact-identity, combat-style, Crowbill and fade channels rather than residency, and VMF silently drops a second hook on the same class+method pair. The first-person dual-weapon residency block (issue 586) also stays put - that is local-player residency, a different owner.
- Entry file drops from 6,914 to 6,688 nonblank lines; the offline decomposition contract ratchets to that new ceiling and now requires `_cwv_husk_residency_owner.lua`. New offline boundary coverage pins the two husk owners apart and asserts the entry keeps no shadowing `start_weapon_fx` registration.

## 0.1.503-dev (2026-08-09) -- musket runtime moves to its own owner file (#1159) [untested]

- **Internal (#1159): both musket families now live in `_cwv_musket_runtime.lua` instead of the entry script.** The move covers the four weapon templates (`musket_template`, `musket_template_melee`, `old_musket_template`, `old_musket_template_melee`), their cloned damage profiles and NetworkLookup registrations, the special-key stance toggle with its destroy/add/wield cycle, the issue 474 ActionHandgun remote fire-report observation, and the issue 932 shared reserve-ammo pool. Nothing else changed: the 989 moved lines are byte-identical to what the entry ran, the module loads at the exact point the inline block used to execute, and every `_om` export keeps its old name, so template construction order, the single ActionHandgun hook and the ammo-pool install all fire in the same sequence as before. The consolidated `BackendUtils.get_item_template` hook deliberately stays in the entry, where it also routes combat-style and Crowbill templates - VMF silently drops a second hook on the same class+method pair.
- Entry file drops from 7,813 to 6,914 nonblank lines; the offline decomposition contract ratchets to that new ceiling and now requires `_cwv_musket_runtime.lua`.
...
- **Fix (#424): a Tuskgor Javelin bomb lying in the level crashed a joining non-cwv player.** Already live before this build: the throw block, the grenade-slot `ActionUtils` sender guard, the two thrown-pickup name substitutions, the unconditional in-flight `projectile_units` swap, the `TransientPackageLoader.hot_join_sync` ref shadow, and the pre-roster `set_peer_synchronizing` fence with its live-pickup sweep. The gap was that the sweep only knew the two thrown-javelin recovery pickups, and the bomb's parity gate handled the spawn pool by ejecting the entry - which stops future rolls but cannot retract a bomb the pool already rolled. That pickup needs no cwv RPC to be dangerous: `PickupSystem._spawn_pickup` routes through `spawn_network_unit` (`pickup_system.lua:1278`), the pickup game object carries the name as a `NetworkLookup.pickup_names` INDEX (`game_object_initializers_extractors.lua:880`), and the joining client decodes that index with a strict read at `game_object_initializers_extractors.lua:3411`. The sweep now iterates a registry (`_cwv_javelin_gate.fence_pickup`) that the grenade-slot block enrols itself into, and it is additionally wired to the javelin gate's own `on_disable`, so the world is cleaned on every gate close rather than only on the `set_peer_synchronizing` seam that happens to detect the missing peer first.
- **Hardening (#423): the damage-profile sender now follows the committed parity gate.** The `WeaponSystem.send_rpc_attack_hit` floor, the vanilla-source substitution map, and the fail-closed drop were already live and are unchanged. `_wire_parity_live()` was reading the raw roster classifier `all_peers_have()`, which reports "everyone acked" the instant the last acknowledgement lands - up to `SETTLE_ENABLE` seconds before the gate actually commits, and without following a `_force_disable` that does not change an already-acked peer's ack. It now reads `applied_state() == "enabled"`, the same accessor cwv's own thrown-pickup sender uses and the one wt reads in `_wt431_profiles_allowed` (`_wt431_damage_profile_parity.lua:112-117`), so all three senders degrade on identical edges. This is a consistency and settle-window fix, not a newly found crash: every shape that closed the classifier already closed the committed gate too.
- **Audited and left alone (#423):** the engine has one real bypass of the `send_rpc_attack_hit` choke point - `rpc_request_spawn_drones` (`projectile_system.lua:1287`) re-broadcast as `rpc_spawn_drones` (`:1335`), where the server forwards a client-supplied `damage_profile_id` unvalidated and every client strict-decodes it at `:1367`. cwv cannot reach it: the drone damage profile comes from the Deus power-up buff settings (`morris_buff_settings.lua:128`) and cwv has zero references to drones, Deus power-ups, or that settings table. No gate added rather than ship dead code. Likewise `PlayerProjectileUnitExtension._get_projectile_units_names` (`:1129`) is a separate unhooked copy of the resolver cwv patches on `ProjectileSystem`, but its result reaches only `spawn_local_unit` (`:1292`, `:1309`) and never the wire.
- **Diagnostics:** the hot-join preflight row now reports the fenced-key count (`[cwv:424] hot-join preflight peer=... gate=disabled removed_pickups=N fenced=N`) so a log proves which keys the sweep covered - `fenced=2` means the bomb never enrolled. Pool ejection emits one bounded `[cwv:424] bomb pool eject world sweep: ok=... removed=...` per gate close. The two `[cwv:423]` rows are unchanged.
- **Regression:** `cwv_wire_safe_thrown_variant_installed` now fails if the bomb key or any mapped thrown pickup is missing from the fence registry, or if the sweep is not exposed for the pool-eject path. `cwv_wire_safe_damage_profile_gate` gained a divergent-beacon fixture (classifier says all-acked, committed state still disabled) that fails if the sender follows the classifier; its existing stubs now supply both accessors, since a fixture with only the classifier would have passed for the wrong reason. Offline: fence-registry idempotence and input rejection, plus source-pattern locks on the enrolment, the eject-then-sweep order, and the committed-state read.

**Residual windows (not closed by this build):** the fence and the sweep are host-side, so they fire on the host's `set_peer_synchronizing` and on gate transitions; a bomb pickup that spawns in the interval between the sweep and the joiner's game-object extraction is still theoretically reachable, and the pool is already ejected by then only if the gate had closed first. A javelin in flight at the moment of a hot join is covered on the husk axis by the unconditional `projectile_units` swap but is not itself despawned. Both are unverified in game.

**DoD:** G-THROWN wire safety - offline gates green; live mixed-lobby verification owed on the pinned card (host with cwv rolls the bomb into the world, a non-cwv player then joins).
...
- Closed the #474 base-class hook trap on the Old Musket preview pose: `_cwv_old_musket_preview_pose.lua` hooked only `HeroPreviewer._update_units_visibility`, but the keep inventory previewer is `MenuWorldPreviewer`, whose methods are copies taken at class-definition time (`foundation/scripts/util/class.lua:51-57`) and whose own `_update_units_visibility` override (`menu_world_previewer.lua:315`) can early-return past the base entry. The same consume-once callback is now registered on BOTH classes; double delivery on one update is safe because `take_when_stable` consumes the pending record on the first stable edge. (#474)
