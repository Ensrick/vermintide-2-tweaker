...
MOD_VERSION `0.8.132-dev`, friends-only Workshop 3733366851), the ACTIVE working
...
| `HeroWindowItemCustomization._update_state_craft_button` [hook] `standard_forge.lua:144` | Computes `disable_button = force_disable or not has_all_requirements or script_data["eac-untrusted"]` [src: `hero_window_item_customization.lua:1928`] | CONSOLIDATED: for the exact modded-realm `apply_weapon_skin` surface, evaluate vanilla state inside the shared realm authority's bounded EAC-off bracket; the ordinary standard-forge override remains separate | `_cim_is_modded_realm` + `_cim_with_eac_off` are the authority. The raw EAC marker is never sufficient by itself, official realm passes through untouched, and `_cim_illusion_apply_presentation.lua` owns the final button/input postcondition (#1465). |
...
| `HeroWindowItemCustomization._enable_craft_button` [hook] `illusion_swap.lua` -> `_cim_illusion_apply_presentation.lua` | Enables/disables the Apply button and updates native generic input actions, while vanilla bakes in the `eac-untrusted` gate [src: `hero_window_item_customization.lua:1877-1905`] | On the exact modded-realm Apply surface, traverse the real method inside the shared EAC-off bracket, then reassert one complete presentation state: button/hotspot, requirement flag, progress widgets, and released input flags | Official realm and other recipes pass through. Missing widgets or resolver failures deny and clear; a never-active adapter does not mutate downstream presentation. Public/Dev owners are byte-identical (#1465). |
| `HeroWindowItemCustomization._on_illusion_index_pressed` [hook] `illusion_swap.lua` -> `_cim_illusion_apply_presentation.lua` | Handles an illusion-grid click; vanilla independently writes lock state, material identity, dirty state, and the Apply presentation [src: `hero_window_item_customization.lua:1437-1500`] | For a DLC-owned, registered, provably different skin, call vanilla and then atomically enforce the backend skin material, `_skin_dirty`, requirements, Apply visibility/hotspot, and window-local #563 commit intent | `ignore_item_spawn` remains presentation-only. Current/default skin, missing backend id, unowned DLC, missing current/default identity, malformed widgets, or throwing readers fail closed. Preview/cancel never persists. |
...
| `HeroWindowItemCustomization._apply_weapon_skin_craft_complete` [safe] `illusion_swap.lua` | Semantic completion for every successful Apply Skin craft: resolves the now-current item, equips/presents it, marks its illusion selected, and disables the native Apply presentation [src: `hero_window_item_customization.lua:2502-2547`] | Preserve the exact explicit skin intent, release every #1465 adapter-owned presentation/input field, then atomically replace the exact-ID saved override (fallback: resolved final skin), including when Cosmetics Tweaker owned the local craft bypass (#563 reopened) | Persist at completion, not grid click. Release precedes persistence and is owner-gated, so hidden desktop/controller progress cannot remain active and an adapter-never-active window is untouched. |
