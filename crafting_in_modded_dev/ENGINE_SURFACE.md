...
| `LootItemUnitPreviewer.spawn_units` [hook] `crafting_in_modded_dev.lua` | Spawns + links each weapon hand unit to the spin-pivot link unit at `item_template.<hand>_hand_attachment_node_linking.third_person.display` [src: `loot_item_unit_previewer.lua:292/310,557`] | #404 DIAGNOSTIC: `printf [cim:404]` each spawned unit's world position + delta from the pivot, once per item key, to MEASURE the far-left ranged offset (per-weapon hand-node translation, not derivable statically) instead of shipping a guessed `preview_position` offset | Full `mod:hook` (reads the returned units; singleton, no prior hook on this method). Gated on `_custom_forge_active` (Athanor only); also surfaces `n_units=0` if a ranged preview is being stripped. The far-left root: uniform `preview_position {-0.85,3,0}` (`hero_window_weave_properties.lua:2954`) puts the pivot center-ish, but a long 2H ranged mesh orbits WIDE of it |
