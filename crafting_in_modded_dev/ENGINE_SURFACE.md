...
MOD_VERSION `0.8.105-dev`, friends-only Workshop 3733366851), the ACTIVE working
...
salvage-eligibility, and deletion-partition policy
...
arithmetic) and `_cim_accessory_property_runtime.lua` (the single owner of the
four property-picker hooks). This keeps the frozen entry size from growing and
prevents same-class/method hook duplication.
...
| `BackendInterfaceCommon.filter_items` [hook] `_cim_inventory_filter.lua` | Applies the recipe infix; vanilla `can_salvage` admits weapon/accessory slots only when rarity is not default/promo/magic, the item is unequipped, and it is not favorited; the recipe also excludes every saved loadout [src: `backend_interface_common.lua:404-423`; `crafting_recipes.lua:13`] | Re-admit only an exact persisted CIM instance through `_cim_synthetic_item_contract.is_salvage_eligible` when every vanilla safety guard passes | Fail closed if equipped/favorite/loadout queries fail. Normalization, selection, and salvage consume the contract's same canonical identity ladder, including exact CustomData, nested provider metadata, and the legacy backend-id band. Definition/Blacksmith rows, rarity-only lookalikes and unpersisted `cwv_`/`woc_` ids are never ownership. Runtime checks: `issue628_saved_instance_contract`, `issue628_identity_resolvers_unified`. |
| `PlayFabMirrorBase.remove_item` [call] `crafting_in_modded_dev.lua` + `_cim_bulk_cleanup_command.lua` #277/#628 cleanup | Unmarks the exact backend id as new, then deletes only `_inventory_items[backend_id]` [src: `scripts/managers/backend_playfab/playfab_mirror_base.lua:2547-2555`] | One exact-owner transaction serves `/forge_delete`, bulk cleanup, and standard Salvage: remove the mirror row, forged persistence, every CIM saved-loadout reference and the exact illusion override | Selected ids must exist in `_forged_weapons` and pass the shared synthetic owner/schema/provider contract. Bulk scope is the contract's complete craftable set (weapons plus necklace/Charm/trinket); ordinary and unresolved rows fail closed. Preview fingerprints canonical identity and the mirror-vs-MIL route, and execution revalidates before mutation. The command adapter receives live store/backend accessors from the entry and owns no copy of mutable state. No official backend request is made. |
...
| `HeroWindowWeaveProperties.{_create_unit_previewer, _set_essence_upgrade_cost, _upgrade_magic_level, _draw}` [hook/safe] `crafting_in_modded_dev.lua` | Preview unit spawn, essence-cost display, magic-level upgrade, per-frame draw [src: `hero_window_weave_properties.lua`] | Route preview/cost/upgrade through cim's local store; drive the amulet accessory panel | `_draw` gated on `_AMULET_PANEL_ENABLED and _custom_forge_active`; VMF numeric widget has no `step` field (memory `reference_vmf_numeric_widget_no_step`) |
| `HeroWindowWeaveProperties.{_populate_menu_option_widget, _find_slot_by_key, _can_clear_slots, _clear_slots}` [hook/safe] `_cim_accessory_property_runtime.lua` | Vanilla property-row usage counts the full key array; `_find_slot_by_key` ignores the category argument; clear membership is key-based [src: `hero_window_weave_properties.lua:718-740,2483-2501,2540-2632`] | Preserve #239's hidden fake-cost presentation and use `_cim_accessory_property_policy.lua` to make usage, right-click removal, and Clear category-aware after CIM widens one key into multiple accessory categories (#959) | Accessory layers are offence 1-10 (Charm), defence 11-20 (Necklace), utility 21-30 (Trinket) [src: `hero_window_weave_properties.lua:24-65`]. Weapon/single-item editing and vanilla Weaves fall through untouched. One extracted adapter owns the complete hook set, preventing duplicate registration. |
...
| `HeroWindowWeaveProperties._create_item_previewer` [hook] `_cim_forge_preview.lua` + `LootItemUnitPreviewer.spawn_units` [hook] + `.update` [safe] `crafting_in_modded_dev.lua` | Properties authors every preview at `{-0.85,3,0}` while the sibling native weapon browser uses centered x=0 [src: `hero_window_weave_properties.lua:2954`; `hero_window_weave_forge_weapons.lua:276`]. Spawn then links each hand unit through its `third_person.display` attachment [src: `loot_item_unit_previewer.lua:292/310,557`] | #882 (split from #404) owns the dedicated runtime seam that corrects only CIM ranged properties previews to native x=0, preserving this surface's y/z and melee placement; writes the live link position and `_unit_start_position_boxed` together so zoom cannot revert it. The decision stays engine-free in `_cim_forge_preview_policy.lua`. #481 retains the per-spawn and one-shot post-Cosmetics diagnostics for LA first-open/scale/identity evidence | Construction hook is CIM-active + ranged-only, installed once from the entry, and runs once per preview; no transport/per-frame writes. `spawn_units` remains a singleton full hook and `update` a read-only, one-shot `hook_safe` (`_cim481_snapped`), both CIM-forge gated. The July 18 #404 root-position samples were non-discriminating because GearUtils links a child attachment node while `Unit.world_position(unit,0)` remains the root; the correction therefore follows the exact sibling-native preview coordinate rather than those zero-root deltas |
...
by its adapter and `cw_conversion` deliberately unrouted (named by the capped
`provider gate unrouted walks=` self-report). `gate_record` guarantees a
