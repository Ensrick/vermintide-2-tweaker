...
bubble-cap resolver through call-time accessors; mutable loadout writes remain
entry-owned. Offline coverage locks registration order, idempotence, all active
facade values, native fallthrough arity, and the five existing protected
fallbacks.
...
keeps their five-hook order, resolves the mutable Athanor-active flag at callback
...
| `HeroWindowWeaveProperties._create_item_previewer` [hook] `_cim_forge_preview.lua`, installed by `_cim_forge_preview_owner.lua`; `LootItemUnitPreviewer.spawn_units` [hook] + `.update` [safe] in the same owner | Properties authors every preview at `{-0.85,3,0}` while the sibling native weapon browser uses centered x=0 [src: `hero_window_weave_properties.lua:2954`; `hero_window_weave_forge_weapons.lua:276`]. Spawn links each hand unit and update retains `_spawned_units` [src: `loot_item_unit_previewer.lua:538-566,95-159`] | #882 corrects only CIM ranged properties previews to native x=0. #481 retains per-spawn and one-shot post-Cosmetics diagnostics | Exact owner order is display-link spawn guard, held-unit load guard, properties construction, attached-unit spawn, retained update. `spawn_units` stays a full hook; `update` stays read-only one-shot `hook_safe`. |
