...
| `[rpc] cim_modded_slot` (`mod:network_register`, `crafting_in_modded_dev.lua`) | - | cim<->cim boolean-only presentation side-channel: fire alongside each `sync_loadout_slot` so a CIM receiver can restore "modded" chrome after vanilla's decode; mirror the same exact boolean into sender-local state because VMF's `others` target excludes the sender (#598) | VMF delivers only to peers with the same mod-id + handler; a vanilla client has no handler and drops it silently. Schema-gated on `CIM_RPC_SCHEMA` (drop on mismatch, § rpc schema / VMF_RECIPES §10). This is presentation metadata and remains independent of the force-disabled persistence feature. Explicit `false` clears stale local chrome. No icon/model/material identity is sent. |
