...
| `_cim_ranalds_import.lua` | Hook-free #1360 transaction coordinator. Preflights career/DLC/provider/item/property/trait/talent legality and the exact five-slot snapshot before mutation. Weapon slot admission consumes the current career's `item_slot_types_by_slot_name`, preserving explicitly approved dual-melee secondary slots; accessories require their exact slot kind. It then creates and persists five fresh CIM items, writes all five selected-index slots plus six talents, performs one live refresh, or compensates every boundary on failure. |
...
| `_cim_direct_craft_owner.lua` | Shared #1141 adapter from Standard Forge and SaveWeapon into Temper's inject/persist/rollback transaction. Standard Forge's post-commit diagnostics are isolated here: a failed read/echo/probe cannot revoke the durable success or prevent its completion request from being published. Failure telemetry is raw-printf, protected, and capped at eight records per owner. |
...
| `_cim_athanor_seed_owner.lua` | #1141 Weapon Select-to-Properties handoff. Consumes Temper's provider-authenticated CWV seed resolver for list synchronization, preview, locking, and Equip/Craft navigation; refuses unavailable identity instead of fabricating an owned-looking item. Native non-CWV paths remain independent. |
...
| `_cim_tab_preview.lua` + `_cim_tab_preview_core.lua` | One existing Hold-Tab post-hook owns #246 exact equipment-skin reconciliation and #598/#921 boolean rarity convergence. The #598 owner-only Cursed candidate additionally requires the current live local human, the exact equipped backend instance, current slot owner equal to the items interface, enabled WOC and a registered local texture. This Adventure/items-only override touches the widget, not shared loadout/backend/equipment or RPC state; a weak-key ledger restores its prior value before fallible reads. Remote/bot/foreign-owner behavior remains unchanged. |
...
- **Post-commit Standard Forge observations** -> `_cim_direct_craft_owner.lua`; keep them inside `finish_standard_craft`, never between successful persistence and request publication in the synth. The observer cannot compensate or reclassify success. Installed-hook fault injection is in `test_cim_direct_craft.lua`; the live suite checks `issue1141_postcommit_observation_boundary`.
- **Craft registration return contract** -> `_cim_forge_state_owner.lua` returns `(true, normalized_entry)` only after saving; Temper forwards that entry as its second success result, never the boolean flag. `test_cim_direct_craft.lua` composes the actual persistence owner, installed craft hook, and `cim_debug.lua` to prove immediate diagnostics and the two-frame visibility check consume the committed record without duplicating completion.
