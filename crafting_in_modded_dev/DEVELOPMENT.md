...
| `_cim_illusion_apply_presentation.lua` + `illusion_swap.lua` | Issue #1465 exact owner for the equipped-weapon illusion Apply transaction. The engine-free owner validates semantic selection/DLC/backend identity, then atomically owns material id, requirements, generic-input refresh, button/hotspot/progress presentation, completion release, and a named runtime check. Public and Dev copies are byte-identical. `illusion_swap.lua` retains the synthetic skin ids, local craft/persistence boundary, and unlocked-skin marking, and publishes schema-1 `_cim_illusion_swap_provider` so Cosmetics can yield at callback time. |
