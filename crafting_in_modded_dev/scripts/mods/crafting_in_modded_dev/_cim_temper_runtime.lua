...
-- Player-facing button text for the Temper Item editor and the Weapon Select
-- Craft handoff. Keys live in crafting_in_modded_dev_localization.lua; the
-- literal fallback keeps a missing, throwing, or unresolved localizer on the
-- same exact text as the engine-free tests.
local BUTTON_LABELS = {
...
local function button_label(mod, key)
    local fallback = BUTTON_LABELS[key] or key
    if mod and type(mod.localize) == "function" then
        local ok, value = pcall(mod.localize, mod, key)
        if ok and type(value) == "string" and value ~= "" and value ~= key
                and value ~= "<" .. key .. ">" then
            return value
        end
    end
...
local function install(ctx)
    ...
    state.button_label = function(key) return button_label(mod, key) end
    ...
    state.commit_craft = function(weapon_data, new_backend_id, evidence)
        ...
        local registered_call, registered, record_or_error = pcall(
        ...
        if not registered_call or not registered then
            local rolled_back, rollback_error = rollback_item(
                new_backend_id, ownership_token, rollback_capability)
            local reason = registered_call and record_or_error or registered
        ...
        -- The canonical forge owner returns (true, normalized_entry), not the
        -- entry alone. Preserve that exact record for all downstream readers;
        -- passing the success flag silently aborts the real diagnostic probe.
        return true, record_or_error
    end
    ...
    mod:hook_safe("HeroWindowWeaveProperties", "_set_essence_upgrade_cost",
        function(self)
            ...
            local label = not item and state.button_label("temper_button_craft_accessories")
                or action == "craft" and state.button_label("temper_button_craft")
                or action == "apply" and state.button_label("temper_button_apply")
                or state.button_label("temper_button_unavailable")
