...
local function install(ctx)
    ...
    -- The Weapon Select Craft label shares Temper's localized button text so
    -- both surfaces read the same exact string (#1141).
    local function craft_label()
        local temper = get_temper_state()
        local resolve = type(temper) == "table" and temper.button_label or nil
        if type(resolve) == "function" then
            local ok, value = pcall(resolve, "temper_button_craft")
    ...
    mod:hook("HeroWindowWeaveForgeWeapons", "_update_equip_button_status",
        function(func, self, equipable_item, is_item_equipped)
            ...
            button.content.title_text = craft_label()
        end)
