...
local _restore_modded_loadout -- forward declaration; bound at the _cim_modded_loadout_owner seam below
-- Forward-declared because `_cim_weave_economy` installs ABOVE the
-- `_cim_weave_loadout_owner` seam that defines this function: its
-- get_property_mastery_costs hook resolves the upvalue at CALLBACK time, which
-- is after mod load, so the binding is in place by the time it fires. Without
-- the forward declaration that closure resolves `_bubble_cap` as a nil global
-- and the Athanor crashes the first time a property button asks for its cost
-- array. Burned by `feedback_lua_forward_reference`. The owner's other exports
-- have NO consumer above the seam, so they are plain locals assigned there.
local _bubble_cap
...
-- `_forge_load` rebinds `_forged_weapons`.
