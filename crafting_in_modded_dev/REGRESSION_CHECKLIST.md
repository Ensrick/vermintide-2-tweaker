...
| Detection | Offline `test_cim_weave_loadout_owner.lua` (14 tests) covers seam position and ordering, hook order/cardinality, accessor liveness across a rebind, idempotence, reload rebinding, context validation, the exported surface, the forward-declaration split, the stamina/movespeed caps and their key normalization, the layer-aware #86 trim, and #959 policy delegation. `test_cim_entry_decomposition.lua` and `qa/check_decomposition_contracts.ps1` enforce the current 1,427-line ceiling and owner retention; five `qa/rt_textual_invariants.psd1` rows pin the hooks and the commit block to their files. |
...
| Detection | Offline `test_cim_weave_economy.lua` covers owner wiring, order/cardinality, idempotence, active values, native argument forwarding, and protected fallback values. `test_cim_entry_decomposition.lua` and `qa/check_decomposition_contracts.ps1` enforce the current 1,427-line ceiling and owner retention. |
...
| Fix version(s) | Unreleased source candidate |
| Category | STATIC |
| Expected post-fix | `_cim_forge_preview_owner.lua` installs once at the original boundary in exact `_spawn_link_unit`, `_load_item_units`, Weapons constructor, Properties constructor, `spawn_units`, `update` order. Overview composes the same context dispatcher into `_cim_mission_forge_safety.lua`'s sole existing #882 constructor hook, so VMF never sees a duplicate pair. While the custom forge is active, every real constructor exposes one exact stack-scoped context during synchronous unit resolution and attaches the same generation to only its returned previewer. Inactive calls stay native; nesting, nil holes, and errors cannot leak scope. The guard remains fail-closed, accepts either a real package or resident unit, diagnostics remain bounded, and the mutable active flag is read at callback time. |
| Detection | Offline `test_cim_forge_preview_owner.lua` executes guard, six-hook owner order/cardinality/idempotence, sole-Overview-hook composition, all three decompiled constructors, exact context visibility, marker identity, nested restoration, error cleanup, nil-hole arguments/returns, and the late-bound gate. `test_cwv_old_musket_preview.lua` retains the #882 runtime contract. The strict mod linter rejects any restored duplicate Overview registration. `/cim_regression_test` keeps the behavioral resident-unit proof and existing bounded #481 diagnostics. |
...
| Gate attribution (#481) | Every substituted or omitted row's `[cim:617] icon ...` line names the refusing gate (`gate=` from the residency contract, `material=` the refused Gui material). This surface deliberately has NO lease recovery: a Stingray Gui resolves materials only from the fixed list baked at `World.create_screen_gui()` time (ui_renderer.lua:246-251), so `Managers.package:load` cannot retro-add one; the fallback chain plus per-list-build re-proof IS the recovery path. |
| Detection | Offline `test_cim_athanor_icon_policy.lua` proves all nine current CWV paired icons close to vanilla, rejects missing material variants, and asserts gate attribution on refusals. `/cim_regression_test` passes `issue617_athanor_icon_resource_closure`; `[cim:617] Athanor icon closure` reports `omitted=0` in a healthy live catalog. |
...
### issue598/921-hold-tab-rarity-convergence - Custom frames follow the current slot item

| Field | Value |
|-------|-------|
| Scope | Existing Modded/ordinary rarity convergence for owner and CIM observers, plus the source-only #598 candidate for a current local human's exact items-owned Cursed instance in Adventure. Remote Cursed and Deus/Weaves Cursed are not implemented by this candidate. No live acceptance or release readiness is implied. |
...
| Solo prerequisites | Use separately confirmed released versions; this source-only tree supplies no release identity. CIM Dev on, public CIM off. For the Modded CWV control, use an **already owned CIM-crafted instance whose actual rarity is Modded**, not a Blacksmith seed and not crafting through blocked #1141. If none is owned, mark that control untestable; vanilla and WOC controls can still be collected. Enable CWV only for its control and WOC for an owned Blightreaper whose current instance rarity is Cursed. |
| Solo repro first | In Adventure, inspect a vanilla weapon, the owned Modded CWV control if available, and the owned Cursed Blightreaper in separate equip/Tab cases. Inspect the first Tab refresh after each swap, replace custom items with vanilla again, then repeat after career replacement, death/respawn and mission transition. Do not treat a skipped/unavailable control as a pass. |
| Expected post-fix | The owned Modded instance uses Modded chrome; the exact local Cursed Blightreaper uses **Cursed**, not Modded or promo. Vanilla replacements immediately use their actual ordinary rarity, with no previous occupant's frame. The new Cursed path is widget-only: shared loadout rarity remains its safe wire value, and backend/equipment identity is unchanged. Provider/instance/resource loss clears the candidate's old widget override on the next existing callback. |
| Compatibility boundary | Remote and bot rows retain existing wire/boolean policy. Non-CIM peers retain vanilla-safe `unique` for Modded; WOC's wire-safe `promo` remains the remote/no-WOC fallback for Cursed. That fallback is **not** accepted as the local owner's Cursed frame. No negotiated Cursed enum or new RPC is introduced. |
| Co-op follow-up | Only after solo passes: host and client inspect their own and each other's rows for Modded/ordinary convergence, enter the Chaos Wastes so the slot becomes a common-rarity weapon, and repeat in the opposite direction with the Imperial Crowbill. Repeat with one peer lacking CIM. Owner Cursed in Adventure needs separate host-local/client-local observation; remote Cursed presentation remains an outstanding scope, not a candidate pass. |
| Detection | Offline tests invoke the real installed `IngamePlayerListUI._update_dynamic_widget_information` callback, including first refresh, immutable backend/wire/equipment/cache snapshots, swaps, respawn, career, foreign owner and missing/throwing dependencies (`qa/lua/tests/test_cim_tab_preview.lua`). `/cim_regression_test` includes `issue921_tab_rarity_state_is_tristate` and `issue598_owner_cursed_frame_is_widget_only`; the latter checks synthetic policy/restoration only. `[cim:598] owner_frame` shows bounded retained widget state, not rendering proof. A live screenshot/observation is still required for each frame and topology; `[cim:921] current=false` alone cannot prove Cursed. |
| Deferred | Deus/Weaves exact-instance Cursed ownership, remote Cursed semantics, and immediate restoration when CIM itself is disabled before native refresh remain unverified/out of this candidate's bounded implementation. Keep the issue held until scoped review/publication and actual required observations. |
