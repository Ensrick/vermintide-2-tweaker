...
| `on_player_killed_enemy` event + `mod.update` [event/update] | Death reactions publish the killing blow; Shyish's `mutator_death` spawns `vfx_animation_death_spirit_02`, waits/chases, and converts health through damage/heal [src: `scripts/unit_extensions/generic/death_reactions.lua:677`; `scripts/settings/mutators/mutator_death.lua:7-115,190-224`] | Attribute direct and poison kills to the exact wielder, spawn the native network unit on host, reproduce rank-one movement/audio/FX, and convert green health with the native health extension | Host only; 32 active cap; O(active) update; 16 log lines; leave one green health; delete all units at state exit. The log-proven nonresident unit is acquired through the real `DLCSettings.mutators_batch_04.package_name` package (installed hash `64E79277358D543D`), never by loading the unit path. |
