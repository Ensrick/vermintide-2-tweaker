...
| `TeamPreviewer._spawn_hero` + `HeroPreviewer.equip_item` [hook] | Creates asynchronous live-lobby and end-score hero rows from profile/career data; the score conversion keeps peer/local-player fields but bot rows share their owner's peer, while `preview_items` remain vanilla/net-safe item keys | Stamp only an exact non-bot live profile+career or exact player-controlled score row, retain its original slot rows, and substitute the immutable local WOC key only when the accepted host lease snapshot says that exact peer+slot is active | A vanilla preview item is never proof of WOC identity. Bots and incomplete rows fail closed. A late authenticated snapshot queues one generation/key/session-bound replay for an already-created consumer; identical replay dedupes, and a stale generation, replaced session, or destroyed world is a no-op. No polling and no client-authored preview identity. |
| `HeroPreviewer._spawn_item` / `MenuWorldPreviewer._spawn_item`; animation methods + `post_update`; item destroy / `on_exit` [hooks] | Spawns and links inventory, lobby, and score/end-screen equipment; later pose/character animations may rewrite child transforms before the next preview update | Apply the canonical transform, retain its absolute target in a weak record, and after a concrete animation call queue one coalesced next-`post_update` reapply with immediate numeric readback | Preview records never enter the gameplay per-frame repair loop. Repeated animation calls before the drain coalesce; destroyed units and retired sessions are forgotten. This is structural retention evidence only and does not claim an in-game visual pass. |
