...
| Class.method (kind) | Vanilla behavior at the seam | Why WOC hooks it | Trap / invariant |
|---|---|---|---|
| Private combat-template clone [data contract] | Sienna's Crowbill supplies the whole graph, baked geometry, dodge buffs, 1P state machine, and Wwise bank; Bardin's Greataxe supplies boot-stable armor-capable profiles in the native split (lights `medium_slashing_smiter_2h`, heavies `heavy_slashing_axe_linesman`) plus `axe_2h_hit` / `melee_hit_axes_2h` / `blunt_hit_armour`; Executioner Sword supplies the whoosh bank [src: `scripts/settings/equipment/weapon_templates/1h_crowbills.lua`; `2h_axes.lua:189/328/469/614/760`; `2h_swords_executioner.lua:1311-1313`] | Keep the Crowbill graph at 83% speed, bake +15% crit, apply the native Greataxe light/heavy profile split, use Greataxe impact presentation, install `wield_anim_career_3p`, and make the Executioner audio bank resident | Clone complete authored sweeps; never pair an animation with another weapon's baked geometry. Never swap the light/heavy profile rows - the decompiled source disproves the "reversed" claim. Use only vanilla damage-profile ids because they cross the attack RPC. `+50% Power vs. Order` remains an inert display row. |
| `ActionSweep.client_owner_start_action` [hook] | Derives attack/impact/combined cleave budgets from the selected damage profile and current power/buffs [src: `scripts/unit_extensions/weapons/actions/action_sweep.lua:187-204`] | Multiply the three already-derived budgets by exactly `2` for the private Blightreaper template, then retain the existing local Executioner whoosh call | One bounded write per attack start; foreign actions pass through. Never add a private damage-profile id or per-frame combat hook. |
...
| `on_player_killed_enemy` event + `mod.update` [event/update] | Death reactions publish the killing blow; Shyish's `mutator_death` spawns `vfx_animation_death_spirit_02`, waits/chases, and converts health through damage/heal [src: `scripts/unit_extensions/generic/death_reactions.lua:677`; `scripts/settings/mutators/mutator_death.lua:7-115,190-224`] | Attribute direct and poison kills to the exact wielder, spawn the native network unit on host, reproduce rank-one movement/audio/FX, then apply `death_explosion` damage and a same-amount `mutator` heal | Host only; 32 active cap; O(active) update; 24 relevant and 8 rejected-kill automatic evidence lines covering live-backend/relic identity through contact; leave one green health; delete all units at state exit/disable/unload. Spawn gates on the held reference for the real `DLCSettings.mutators_batch_04.package_name` package (installed hash `64E79277358D543D`), never `Application.can_get` or a unit-path load. |
...
| `TeamPreviewer._spawn_hero` + `HeroPreviewer.equip_item` [hook] | Creates asynchronous live-lobby and end-score hero rows from profile/career data; the converted hero row drops peer identity, while the source score snapshot retains peer/local-player fields and bot rows share their owner's peer; `preview_items` remain vanilla/net-safe item keys | Recover only an exact non-bot profile+career+local-player-1 wearer from the live roster or player-controlled score snapshot, retain its original slot rows, and substitute the immutable local WOC key only when the accepted host lease snapshot says that exact peer+slot is active | A vanilla preview item is never proof of WOC identity. The lease is peer-keyed and accepts only local player 1, so every other, missing, or throwing local-player identity fails closed. Bots and incomplete or ambiguous rows also fail closed. A late authenticated snapshot queues one generation/key/session-bound replay for an already-created consumer; identical replay dedupes, and a stale generation, replaced session, or destroyed world is a no-op. No polling and no client-authored preview identity. |
| `HeroPreviewer._spawn_item` / `MenuWorldPreviewer._spawn_item`; animation methods + `post_update`; `_destroy_item_units_by_slot` / `clear_units` / `on_exit` [hooks] | Spawns and links inventory, lobby, and score/end-screen equipment; later pose/character animations may rewrite child transforms before the next preview update; respawn and TeamPreview teardown call `clear_units` directly | Apply the canonical transform, retain its absolute target in a weak record, and after a concrete animation call queue one coalesced next-`post_update` reapply with immediate numeric readback | Preview records never enter the gameplay per-frame repair loop. Repeated animation calls before the drain coalesce. Both slot destruction and `clear_units` forget unit records before vanilla destruction; `on_exit` retires only the remaining preview identity/event maps. This is structural retention evidence only and does not claim an in-game visual pass. |
| `HeroWindowWeaveProperties._create_item_previewer` [hook,varargs] | Constructs the Athanor `LootItemUnitPreviewer` and returns it to the weave-properties window | Mark only the exact returned instance as the CIM preview surface while preserving every factory argument | Do not infer Athanor from the shared previewer class, item shape, or caller stack. Generic Loot previewers remain generic. |
| `LootItemUnitPreviewer.load_package` / `spawn_units` / `_destroy_units` / `_unload_packages` [hook] | Owns generic item, illusion/customization, store, and exactly marked Athanor preview package leases and units | Borrow/unload one vanilla lease per custom key, transform returned WOC units, and forget their weak records on destruction | Never load the WOC unit path through PackageManager; the master package owns residency. The ordinary forge layout does not instantiate this previewer, so this hook proves no vanilla crafting-bench surface. |
...
The ordinary loadout RPC remains `es_1h_sword` for non-WOC safety. WOC's host
lease publishes one authenticated, authority-epoch/generation-bound complete
holder-and-slot snapshot. WOC-capable receivers derive their peer+slot cache
from that snapshot, re-key `GearUtils` husk spawn, and expose the same immutable
view to exact TeamPreviewer consumers. Peer-ready clients query the current
snapshot, so identity is not limited to an equip-time sideband. Absent-WOC peers
receive no custom resource identity.
...
- **Enemy weapon meshes have no `_3p` sibling and use a different attach rig.**
  ...
  Blightreaper is an explicit exception: its reviewed canonical offset and
  rotation are shared by its separate 1P and 3P units, while scale remains
  perspective-specific (`0.8` 1P, `0.9` 3P).
