...
function M.resolve_score_peer(profile_index, career_index, scores)
	...
	-- The shared-relic lease is intentionally peer-keyed and accepts intents
	-- only from local player 1.  Do not collapse a split-screen row onto that
	-- peer: until the lease itself grows a (peer, local-player) axis, every
	-- other local id must fail closed.
	if type(match.peer_id) == "string" and match.peer_id ~= ""
			and match.local_player_id == 1 then
		return match.peer_id, "score_snapshot"
	end
	return nil, "score_snapshot_untrusted"
end

-- `profile_for(player)` is the engine-facing seam. It must return the exact
-- profile, career, and local-player indexes or nil; this pure owner never falls
-- back to career-only or peer-only matching.
function M.resolve_live_peer(profile_index, career_index, players, profile_for)
	...
	for _, player in pairs(players) do
		if type(player) == "table" and player.bot_player ~= true
			...
			local ok, resolved_profile, resolved_career,
				resolved_local_player_id = pcall(profile_for, player)
			if ok and resolved_profile == profile_index
					and resolved_career == career_index
					and resolved_local_player_id == 1 then
