...
--      now supplies exact authenticated consumers for lobby/score identity and
--      husk peer-ready. Husk customize remains unsupported because the
--      Blightreaper is immutable. This conversion still
...
-- appearance for that pair, with a source file/function citation below. "implemented"
...
local HUSK_HOLE_NOTES = {
	mission_transition = FB_TRANSITION,
	mod_disable_restore = FB_DISABLE,
	customize = "Blightreaper is immutable and has no customization edge; a cosmetic-driven husk rebuild is therefore not an implemented WOC appearance path. The resident cloned vanilla sword remains the safe fallback.",
}
local TEAM_PREVIEW_EDGES = {
	equip = IMPLEMENTED,
	preview_open = IMPLEMENTED,
}
local TEAM_PREVIEW_FALLBACK = "#613 proves exact TeamPreviewer wearer resolution, initial item selection, spawned-unit transform dispatch, and deterministic teardown. This edge has no composed retained-state fixture yet, so it remains explicit vanilla-safe fallback rather than inheriting coverage from a sibling preview edge."
	...
	families = {
		enemy_weapon_relic = {
			matrix = {
				...
				husk = row(IMPLEMENTED, nil, HUSK_HOLES, HUSK_HOLE_NOTES),
				...
				lobby = row(UNSUPPORTED, TEAM_PREVIEW_FALLBACK, TEAM_PREVIEW_EDGES),
				score_team = row(UNSUPPORTED, TEAM_PREVIEW_FALLBACK, TEAM_PREVIEW_EDGES),
