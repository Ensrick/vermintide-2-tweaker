...
| `BackendInterfaceItemPlayfab.refresh_bot_loadouts` [safe] `_gut_bot_loadout_snapshot.lua` | Rebuilds `_bot_loadouts` from `_loadouts` | Overlay a detached persisted bot snapshot after vanilla builds the table; migrate old index-only designations once | The backend cache receives a second copy and cannot alias the persisted snapshot; official/readonly modes remain native |
