...
| Class.method (kind) | Vanilla behavior at the seam | Why gut hooks it | Trap / invariant |
|---|---|---|---|
| `PlayFabMirrorAdventure.get_character_data` [hook] `_gut_native_loadouts.lua:522` | Resolves one slot's value from `_career_data[career][index or selected]`, nil if absent [src: `playfab_mirror_base.lua:1909-1919`] | STORE: serve the slot from the modded store; per-read official fallback for a gear id unresolvable RIGHT NOW | NEVER destructively sanitize - synthetic ids (cim craft, LA/cosmetics UUID) register LATE (2026-07-02 spawn-fatals); resolve via RAW field reads only, NEVER `iface:get_item_from_id` (recursion -> 1 GiB heap, v0.2.173 burn); weapon-slot last resort is `get_default_loadouts` [src: `:1955-1966`] |
...
| `IngameHud.update` [safe] `_gut_respawn_timer.lua:83` | Per-frame HUD composite | DRAW the respawn timer (the proven HUD-composite draw path, #285) | Distinct from `IngameHud.post_update` (surface 3a) - different method, no dup; the `_rt_register` gate asserts the draw still rides this seam |
...
### Surface 5a - Mod Tweaker view injection + ESC entry (owner: `docs/engine/09`; main, `_mod_tweaker_view.lua`, `_gut_ckc_bridge.lua`)
...
| `OptionsView.cb_crosshair_kill_confirm_setup` [hook] `_gut_ckc_bridge.lua` / `cb_crosshair_kill_confirm` [hook] / `cb_crosshair_kill_confirm_saved_value` [hook] / `build_checkbox_widget` [hook] / `update` [safe] | The vanilla synthesized `cb_<setting>` callbacks + native checkbox builder + per-frame tick for the crosshair-kill-confirm option row [src: `options_view.lua:308-313,1029-1147,1483-1504`; `options_view_definitions.lua:1299-1421`] | Take over the native option row as a checkbox that LIVE-toggles the standalone CKC mod (append a cog that opens Mod Tweaker), never staging the native user_setting | The `cb_` takeover class (memory `reference_vt2_optionsview_synthesized_cb_takeover`); registered only when CKC is present + togglable; its temporary row-type rewrite rides the existing `build_settings_list` singleton below |
| `OptionsView.build_settings_list` [hook] `_gut_video_profiles.lua` | Builds each native options-tab widget list from its definition | Prepend five-slot graphics profile controls only to `video_settings_list`; when the CKC bridge is active, temporarily classify only `crosshair_kill_confirm` as `checkbox`, then restore the shared definition after the build | Consolidated singleton for #292/#528; source `options_view.lua:1029-1147`; unsupported Video values are skipped, and CKC-absent lists remain untouched |
...
| In-mission keep menus in `_gut_mission_inventory.lua` [~10 hooks] (`HeroWindowLoadoutConsole._customize_item` `:164` etc., `HeroWindowItemCustomization._create_item_preview_widget_definition` `:240`/`_register_object_sets` `:273`, `HeroWindowPanelConsole.on_enter` [safe] `:325`, `HeroWindowCosmeticsLoadoutConsole._equip_item_presentation` `:411`, `HeroWindowCosmeticsLoadoutPoseInventoryConsole.draw` `:432`, `HeroWindowLoadoutSelectionConsole._show_context_menu` `:448`) | The keep inventory/loadout/cosmetic console windows | Enable the keep inventory + loadout menus mid-mission | Each `(Class, method)` singleton; `_show_context_menu` is distinct from the native-loadouts hooks on the same class (surface 2) |
| Mission map/start-game in `_gut_mission_map.lua` [~7 hooks] (`StartGameWindowBackgroundConsole._create_viewport_definition` `:253`/`_update_object_sets` `:355`/`_setup_object_sets` `:370`/`post_update` `:391`, `StartGameWindowAreaSelectionConsoleV2._assign_video_player` `:430`, `StartGameWindowAreaSelection._setup_video_player` `:443`, `MatchmakingStateWaitForCountdown.on_enter` [safe] `:462`, `GameModeManager.complete_level` `:495`) | Start-game map viewport/video/countdown; level completion | Enable the in-mission map + auto-start arm | `_create_viewport_definition` is a full wrapper because the vanilla body derefs a keep-only field; auto-start arm is observe-only |
