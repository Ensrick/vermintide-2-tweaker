...
The equip-time capture (mirror write hooks + the outer `BackendUtils.set_loadout_item` capture) only fires on equip EVENTS, so state that reaches the live interface through a path that fires none of them (LA-cloned cosmetic dispatch #353, a WT cross-character weapon whose enable/apply lands outside a captured equip #354) is never stored and is lost on quit. The exit-time snapshot backstop closes that gap: `M.exit_snapshot(edge)` (in `_gut_native_loadouts.lua`, pure diff in `_gut_exit_snapshot_core.lua`) is driven from three bounded exit edges chained off gut's VMF lifecycle callbacks in `gui_tweaker_dev.lua` - `on_game_state_changed` (`exit StateIngame`, `enter StateTitleScreen`) and `on_unload`. Before reading a slot it compares `Managers.backend:get_loadout_interface_by_slot(slot)` with `Managers.backend:get_interface("items")`; only identity with the durable items interface is eligible. This is required because Deus overrides melee/ranged to its own interface while leaving cosmetics on items [src: `backend_interface_deus_base.lua:7-14`], and `_setup_run` resets/grants generated Deus inventory identities [src: `deus_mechanism.lua:1120-1175`]. Persisting those run-local ids poisons the Adventure row after override teardown (#273). Eligible slots are re-read via `BackendUtils.get_loadout_item_id`/`get_loadout_item` (the per-slot, LA-aware readers - NOT a mirror-read hook, so the `get_item_from_id` recursion trap does not apply here, same as `/gut_loadout_status`), canonicalized to the store's format (gear = backend id; cosmetics = `override_id or ItemId`), and reconciled into the SELECTED row through the SAME single writer. It adds NO hook. It is non-destructive (foreign owner, unknown owner, or unresolved id is a skip, never a clear), idempotent, inert outside the modded Adventure realm, and never touches official cloud data. Diagnostic: `[gut:persist] edge=<name> diverged=<n> written=<bool> foreign_slot_reads=<n>`.

Issue #1033's presentation half: `reset_modded_loadouts` is a persistence transaction
(store rewrite + `_dirtify()`), and vanilla republishes LIVE equipment only on equip
EVENTS - the hero-view equip request drains into
`SimpleInventoryExtension.create_equipment_in_slot`, whose peer publish is
`send_rpc_clients("rpc_add_equipment")` [src: `hero_view_state_overview.lua:697-716`;
`simple_inventory_extension.lua:1372,:1455`], and skin changes take one profile respawn
[src: `hero_view_state_overview.lua:1158-1162`; `ingame_ui.lua:1290-1303`] - so a DEFAULT
reset used to leave the keep character (and its husk on every peer) rendering pre-reset
equipment. `_gut_default_reset_refresh.lua` (NO hooks; a bounded capture-prev `mod.update`
watch only) now mirrors that respawn for the ACTIVE career after the reset: one
host-mediated `ProfileRequester:request_profile(peer, lpid, profile.display_name,
career.name, force_respawn=true)` [src: `profile_requester.lua:46-58`;
`sp_profiles.lua:470-489`], KEEP ONLY (`game_mode_key() == "inn"`) because a mission
force-respawn teleports to level start with fresh health/ammo (the `_gut_career_swap.lua`
caveat); in a mission the reset defers to the next spawn boundary, which consumes the new
rows naturally. Classification is the pure `_gut_default_refresh_core.lua`
(offline-locked by `test_gut_default_refresh.lua` + the issue1033 rt check). Diagnostics:
`[gut:1033] refresh=<respawn|defer|skip>` pre-receipt (desired vs live melee/ranged/hat)
and `refresh=complete ... match=<bool>` / `refresh=timeout` after-receipt.
