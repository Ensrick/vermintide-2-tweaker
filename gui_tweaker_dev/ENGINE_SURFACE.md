...
| `CutsceneSystem.flow_cb_cutscene_effect` [hook,tbl] `_gut_cutscenes.lua` | Fires a named cutscene flow effect incl. `fx_fade` | Order-independent fade classification (#140/#257/#274 ROUND 3): every `fx_fade` classifies against the system's live cutscene EPISODE in the pure `_gut_cutscene_skipwindow.lua` - one-shot in-skip-call fade, deferred-skip PENDING window, post-skip straggler window (rolling grace + hard cap), and the bounded pre-identity INTRO WATCH (#257, Well of Dreams = `dlc_termite_3`) | CONSOLIDATED: all fade concerns live in this SAME body (VMF drops a 2nd hook); the swallow site carries the `_gut_cutscene_fade_swallow_site` marker pinned by rt-check + `qa/rt_textual_invariants.psd1`. `_gut_diag_cutscene257.lua` stays behavior-free (32-record cap) and records the REAL per-callback verdict. Trade-off: a standalone `fx_fade` during a live window/watch is also swallowed (cosmetic pop, documented) |
...
| `BackendUtils.set_loadout_item` [hook,tbl] `_gut_native_loadouts.lua`, transaction owner `_gut_loadout_lifecycle_owner.lua` | The stable OUTER equip entry point the hero view calls [src: `hero_view_state_overview.lua:1108`] | Capture gear **and cosmetics** that route through an LA-CLONED interface and bypass the concrete mirror hooks (#353); #354 also records enabled WT cross-character captures here; #925's saved-loadout writer publishes a bounded local presentation generation after each successful call | TABLE-form against the post-LA `BackendUtils`, installed deferred once the backend answers; cosmetics resolve through the selected loadout interface and persist vanilla's `override_id or ItemId` identity [src: `backend_interface_item_playfab.lua:635-665`], never the transient backend id; unresolved cosmetics skip with bounded evidence; seeds before first STORE write (#375). The executable capture owner is dependency-injected and is called by the live hook plus the named runtime/offline checks, so production wiring cannot disappear behind a passing serializer-only test. The #925 wrapper samples the generation before the synchronous call and publishes only if an inner Cosmetics provider did not already do so; no second hook or persistent cache is added. The #354 trace reads raw tables, filters to enabled WT selected weapon rows, deduplicates, and caps at 24 records. |
| `BackendInterfaceItemPlayfab.refresh_bot_loadouts` [safe] / `get_bot_loadout` [hook] `_gut_bot_loadout_snapshot.lua` | `refresh_bot_loadouts` replaces `_bot_loadouts` from `_loadouts` [src: `backend_interface_item_playfab.lua:128-160`]; both `get_loadout_item_id` and `get_loadout_by_career_name` consume it through `get_bot_loadout` [src: `:461-466`,`:493-523`] | Overlay a detached persisted bot snapshot after vanilla builds the table, verify it again at the shared bot-read boundary, and import a pre-owner native `PlayerData` designation once | A startup refresh can precede mod hook installation or run before the Adventure gate is externally resolvable, so refresh-only ownership is insufficient. A read-boundary PASS is also meaningless when zero snapshots are owned: the live check must reconcile native designation identity with the GUT store. Native indexes are copied only after their saved row exists; existing GUT ownership wins. The read check repairs a replaced cache or same-table drift, emits bounded changed-state records, and leaves official/readonly modes native. Comparison and detached copying share hard depth/key/career ceilings; a wide, cyclic, corrupt, or throwing value returns the native getter result and remains retryable rather than mutating either owner. This closes the runtime-identity gap without a frame loop; it is separate from #375's selected saved-row resolution and #402's complete-slot integrity repair. |
| `HeroWindowLoadoutSelectionConsole._save_bot_equipment` [hook] `_gut_bot_loadout_snapshot.lua` / `_populate_context_menu_loadout` [hook] `_gut_native_loadouts.lua` | Bot checkbox writes realm-shared `PlayerData.loadout_selection` [src: `hero_window_loadout_selection_console.lua:671-683`]; context-menu preview reads each gear id with NO nil-guard [src: `:913-933`] | Persist `bot_index` for UI identity plus a detached exact-slot snapshot and skip the PlayerData write (#954); the first eligible read imports any earlier native assignment and marks that migration durable; a native assignment for a career the store never seeded (a modded career; Pusfume has no `playfab_name`, so vanilla's refresh at `backend_interface_item_playfab.lua:137-138` skips it) imports on the next bounded read from official rows or the backend's current bot row (0.2.355-dev); substitute any unresolvable equipment id in a SHALLOW COPY so hovering a saved loadout does not CTD at `ui_utils.lua:248` (#372) | Two DIFFERENT methods on this class, plus `_show_context_menu` in `_gut_mission_inventory.lua` (the #231 consolidated site) and the #231 paging owner's set below - all distinct pairs; player row edits and bot/backend cache mutations cannot cross the snapshot boundary |
| `HeroWindowLoadoutSelectionConsole.on_enter` / `_populate_loadout_buttons` / `_handle_mouse_input` / `_handle_gamepad_input` / `_handle_add_loadout_gamepad_input` / `_on_enter_add_loadout_gamepad` / `_handle_context_menu_input` / `_change_loadout` / `_add_loadout` [hook], `on_exit` / `_update_animations` / `_draw` [safe] `_gut_loadout_paging.lua` | Six physical buttons are built once at module load [src: `hero_window_loadout_selection_console_definitions.lua:1101-1109`] and indexed directly by LOGICAL loadout index in the fill, hover, context-menu, add, delete, bot and draw paths [src: `hero_window_loadout_selection_console.lua:186-202`, `:368-386`, `:420-459`, `:500`, `:529`, `:709`, `:778-780`, `:972`]; `_add_loadout` gates on the PHYSICAL widget count [src: `:985-987`]; the cap is the custom-row count of `InventorySettings.loadouts` [src: `:162-168`, `inventory_settings.lua:210-218`] | Modded STORE Adventure only (#231): reuse the six buttons as one page of six, thirty slots over five pages through `_gut_loadout_page_mapper.lua`, text Roman numerals, `<` / `>` / page-counter widgets, Left/Right arrow paging (`move_left_raw` / `move_right_raw` [src: `controller_settings.lua:5748-5757`]), auto-reveal of the selected slot; window-scoped rows 7-30 + `MAX_NUM_CUSTOM_LOADOUTS` 30 via `_gut_loadout_capacity_policy.lua` `expand` / `contract` | Never hook `_save_bot_equipment` / `_populate_context_menu_loadout` / `_show_context_menu` here (owned elsewhere; the consolidated `_show_context_menu` calls `Paging.after_show_context_menu`); `qa/rt_textual_invariants.psd1` forbids any `_loadout_button_widgets[<logical>]` in the owner; the expansion is refcounted per live window and restored on `on_exit`, so official / READONLY / Versus keep the six-row table; vanilla's `on_exit` row persistence runs only for Versus game modes [src: `inventory_settings.lua:222-225`] |
...
| `DamageUtils.add_damage_network` [hook] `_gut_damage_numbers.lua` / `add_damage_network_player` [hook] | Apply already-final damage on the authoritative machine (DoT/bomb path; player-weapon path) | Observe damage to render floating numbers | Static fns, no `self`; distinct methods from gt's godmode hooks (gut has no godmode) - no cross-mod pair collision. The helper's font override preserves vanilla scaling through `NetworkConstants.damage.max` and caps only larger visual scale; raw text remains unchanged (#938). |
...
| `UnitFramesHandler._sync_player_stats` [hook] `_numericui_cooldown_realtime.lua` / `CareerExtension.current_ability_cooldown` [hook] | Sync a unit-frame's stat block / report an ability's cooldown | Real-time cooldown and authoritative teammate ammo (NumericUI compatibility; #249) | One consolidated sync wrapper. After NumericUI derives absolute ammo from static template capacity, GUT replaces only its retained text with `InventoryExtension:ammo_status()`; owner and husk implementations expose the engine's exact current/max pair [src: `simple_inventory_extension.lua:1231-1234`; `simple_husk_inventory_extension.lua:66-72`]. No RPC, buff replay, or template mutation. Cooperates with the hb NumericUI-compat disable in `hb_on_all_mods_loaded` (disables NumericUI's own `_create_unit_frame_by_type` hook). |
...
| Mission map/start-game in `_gut_mission_map.lua` [11 hooks] (`StartGameWindowBackgroundConsole._create_viewport_definition`/`_update_object_sets`/`_setup_object_sets`/`post_update`, `StartGameWindowAreaSelectionConsoleV2._assign_video_player`, `StartGameWindowAreaSelection._setup_video_player`, `VoteManager._server_start_vote`/`_start_vote_base`, `IngameVotingUI.start_vote` [safe], `MatchmakingStateWaitForCountdown.on_enter` [safe], `GameModeManager.complete_level`) | Start-game map viewport/video/countdown; mission-vote creation and title presentation; level completion | Enable the in-mission map, surface the vanilla game-settings vote to clients in the mission HUD (#700), and arm auto-start | `_create_viewport_definition` is a full wrapper because the vanilla body derefs a keep-only field. `game_settings_vote` is already in `NetworkLookup.voting_types`, but vanilla marks it `ingame_vote=false` [src: `vote_templates.lua:306-319`]. Its active per-vote template is shallow-copied and promoted only in a live Adventure mission, satisfying both the HUD draw gate [src: `ingame_voting_ui.lua:267-301`; `vote_manager.lua:282-283`] and the separate input gate [src: `vote_manager.lua:331-373`] while leaving the shared template, keep votes, unrelated votes, RPCs, and localized option keys unchanged. The copy installs a bounded `modify_title_text` that contains an existing authored modifier, retains its usable output, and otherwise returns the separately resolved native or dedicated GUT title. `IngameVotingUI.start_vote` is the exact one-shot edge that writes `background.content.info_text` [src: `ingame_voting_ui.lua:113-123`]; its post-hook records source/input/modifier/final lengths once per active vote without mutating presentation. |
...
The equip-time capture (mirror write hooks + the outer `BackendUtils.set_loadout_item` capture) only fires on equip EVENTS, so state that reaches the live interface through a path that fires none of them (LA-cloned cosmetic dispatch #353, a WT cross-character weapon whose enable/apply lands outside a captured equip #354) is never stored and is lost on quit. The exit-time snapshot backstop closes that gap: `M.exit_snapshot(edge)` (in `_gut_native_loadouts.lua`, pure diff in `_gut_exit_snapshot_core.lua`) is driven from three bounded exit edges chained off gut's VMF lifecycle callbacks in `gui_tweaker_dev.lua` - `on_game_state_changed` (`exit StateIngame`, `enter StateTitleScreen`) and `on_unload`. `_gut_loadout_lifecycle_owner.lua` constructs those exact callback adapters and is consumed unchanged by the entry point, named runtime checks, and offline harness; snapshot or prior-callback failures are contained and reported. Before reading a slot it compares `Managers.backend:get_loadout_interface_by_slot(slot)` with `Managers.backend:get_interface("items")`; only identity with the durable items interface is eligible. This is required because Deus overrides melee/ranged to its own interface while leaving cosmetics on items [src: `backend_interface_deus_base.lua:7-14`], and `_setup_run` resets/grants generated Deus inventory identities [src: `deus_mechanism.lua:1120-1175`]. Persisting those run-local ids poisons the Adventure row after override teardown (#273). Eligible slots are re-read via `BackendUtils.get_loadout_item_id`/`get_loadout_item` (the per-slot, LA-aware readers - NOT a mirror-read hook, so the `get_item_from_id` recursion trap does not apply here, same as `/gut_loadout_status`), canonicalized to the store's format (gear = backend id; cosmetics = `override_id or ItemId`), and reconciled into the SELECTED row through the SAME single writer. It adds NO hook. It is non-destructive (foreign owner, unknown owner, or unresolved id is a skip, never a clear), idempotent, inert outside the modded Adventure realm, and never touches official cloud data. Diagnostic: `[gut:persist] edge=<name> diverged=<n> written=<bool> foreign_slot_reads=<n>`; failures use `[gut:persist] edge=<name> error=<error>`.
...
#314's Simple UI compatibility is a separate generic-window surface. `_gut_simple_ui_compat.lua` installs no engine or external-mod hook: its chained `mod.update` tick reads the installed mod's public `SimpleUI.windows.list`, applies the pure viewport policy, and mutates each existing `window.position` table in place. A fitted window is clamped wholly inside `UIResolution()`; an oversized window is pinned so its left edge and top title/drag edge remain reachable. It is inert when Simple UI is absent and does not overlap the scenegraph-node HUD Customizer. Direct source/resource absorption remains barred until the upstream repository supplies a redistribution license; see `SIMPLE_UI_INTEGRATION_PLAN.md`. Phase 2 (`_gut_simple_ui_dropdowns.lua`, loaded from the compat tail) stays on the same public object model: Simple UI copies its widget prototype functions into every instance and calls those instance fields each frame (upstream `simple_ui.lua:862-884`, `:1407-1432`), so per live dropdown the owner wraps that instance's own `update` (fitted layout, scroll offset, mouse wheel read through `Mouse.axis(Mouse.axis_index("wheel"))` [src: `scripts/managers/debug/debug_manager.lua:289-296`] while the cursor from `Mouse.axis(Mouse.axis_id("cursor"))` [src: `scripts/ui/ui_scenegraph.lua:399`] is over the open list), replaces only that instance's `extended_bounds` with the fitted hit box (upstream extends downward by every option, `:2168-2176`, and draws the background and routes clicks from the same box, `:1527-1531`, `:2100-2126`), and chains each option's public `before_update` callback (`:1433-1436`) to place the row from the pure layout or park a scrolled-out row off-screen. No prototype is touched and no engine or external-mod hook is installed; the **Fit Simple UI Dropdowns** option (default on) installs nothing when off.
