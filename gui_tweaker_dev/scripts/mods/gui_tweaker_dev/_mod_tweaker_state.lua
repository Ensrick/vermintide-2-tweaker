...
local function _vmf_categories()
    ...
    for _, list in ipairs(widget_data) do
        ...
        if type(mod_name) == "string" and _MY_MODS[mod_name] then
            ...
            -- #318: a VMF-DISABLED mod must NOT get a Mod Tweaker tab at all (parity
            -- with _mod_tweaker_view.lua). Skip it entirely so only enabled
            -- whitelisted mods become tabs. `enabled` stays true when is_enabled is
            -- absent/errors, so an indeterminate mod still shows.
            if enabled then
                out[#out + 1] = {
