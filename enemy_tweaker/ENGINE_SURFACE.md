...
|---|---|---|---|
| `ConflictDirector.update` [tick] `_et_pacing.lua:60` | The master AI tick: runs horde pacing, mini-patrol, specials; its spawn-queue bookkeeping (`update_spawn_queue`, decrements) runs AFTER the spawn call [src: `conflict_director.lua:1430`; queue bookkeeping `:1835-1891`] | Temporarily override `RecycleSettings.max_grunts` (concurrent trash cap) so pacing decisions read it, restoring on the way out; reset the #213 freeze counter (`:60`) | ROW OF CONCERN. Registered via `_hook_wrap_tick`, NOT `_hook_wrap`: vanilla is NOT re-runnable - a spawn that throws stays queued, so the plain wrapper's vanilla-fallback re-popped the same entry and doubled partial state (issue 479, guard-that-delegates class § issue 270). Vanilla runs under `_call_with_override` so the override restores on BOTH paths; on error the tick is skipped with one `[et:479]` printf, never re-run |
