...
| `_crt_balance_catalog_early.lua`, `_crt_balance_catalog_focused_spirit.lua`, `_crt_balance_catalog_late.lua` | Disjoint balance-definition owners. Focused Spirit is isolated because its `buffer="both"` authority and two-bit consensus form a distinct boundary; all three receive closure dependencies explicitly and register no hooks or lifecycle callbacks. | each returns `build(ctx) -> definitions` | loaded only by `_crt_balance_catalog.lua` |
...
2. **A new rework / balance tweak** → `_crt_balance_catalog.lua` (`BALANCE_MODS`).
