...
| `_crt_rework_master_policy.lua` | Engine-free #445/#936 family-master planner plus the canonical authorship-prefix metadata shared by runtime, localization, and QA. The third `Enable All Reworks` preset enables both families in one bounded batch. Its restore-first coordinator prevents an old Tourney snapshot from overwriting a newly selected Ensrick value at exact conflicts. | returns `{ new, apply_bounded_master, reconcile_engines, family_for_setting, is_leaf_localization_key, decorate_label, FAMILIES, MASTER_ALL }`; runtime publishes the module and policy as `mod._crt.rework_master_module` / `.rework_master_policy` | after balance and tourney; required independently by localization before script load |
