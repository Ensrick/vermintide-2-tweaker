...
The #221 Armor Master (0.4.30-beta) retains its first pre-toggle snapshot on
repeated ON and uses the optional settings-owner protocol to carry its
validated preimage through Tweaker: GUI Dev 0.2.346-dev profiles; both builds
landed together with GUT #998 safeguards retained. In-game verification waits
for the remaining #221 subgroups. The concrete protocol and legacy-profile
semantics are in `MENU_UMBRELLA_AUDIT_221.md`; the burned .29 draft number
...
| `_crt_focused_spirit.lua` | Pure #472 description, owner/server/bot transition policy, and bounded edge-driven two-setting consensus. It carries no engine hook or update loop. | `mod._crt.focused_spirit`; returns description/transition decisions plus the installed `crt_focused_spirit_config` consensus owner | after damage classification, before balance |
...
| `career_tweaker_balance.lua` | The sub-1,000-line balance orchestrator: apply/restore engine, crt_* buff pre-registration, issue-425/#776 parity/wire-safe proc wrappers, and ordered hook/catalog owner installation. | returns `{ apply, restore, active_count, parity_gate_ok, wire_parity_live, network_unsafe_ids, BALANCE_MODS }`; sets `mod._crt_registered_buff_names`, `mod._crt_mod_registered_buff_names` | after the pure policies (entry captures `balance`) |
| `_crt_balance_catalog.lua` | Hook-neutral composition owner for the bounded declarative catalogs; rejects duplicate setting ownership. | returns `build(ctx)` | loaded once by `career_tweaker_balance.lua` |
| `_crt_balance_catalog_early.lua`, `_crt_balance_catalog_engineer.lua`, `_crt_balance_catalog_focused_spirit.lua`, `_crt_balance_catalog_late.lua` | Disjoint balance-definition owners. The Engineer owner contains only the three Outcast Engineer definitions and receives its restore-stub dependency explicitly; Focused Spirit remains isolated because its `buffer="both"` authority and two-bit consensus form a distinct boundary. All four register no hooks or lifecycle callbacks. | each returns `build(ctx) -> definitions` | loaded only by `_crt_balance_catalog.lua` |
...
| `_crt_tourney_catalog.lua` | Engine-free #936 owner for the 17 stable per-career preset IDs and their 46 independent `[TB]` mutation leaves. Plans changed-only preset writes, derives exact preset state, and expands legacy saved ON presets once. | returns `{ CAREERS, MASTER_IDS, LEAF_IDS, plan_master, derive_master, plan_legacy_migration }` through `mod._crt.tourney.CATALOG` | loaded by the Tourney engine |
| `career_tweaker_tourney.lua` | Tourney Balance Testing port. Only the 46 catalog leaves own gameplay mutations; the old per-career IDs are UI presets and migrate existing saved ON values once. Same `{apply,restore,active_count}` contract, plus catalog metadata. | `mod._crt.tourney`; returns the contract table | after balance |
| `_crt_rework_master_policy.lua` | Engine-free #445/#936 family-master planner plus the canonical authorship-prefix metadata shared by runtime, localization, and QA. The third `Enable All Reworks` preset enables both families in one bounded batch. Its restore-first coordinator prevents an old Tourney snapshot from overwriting a newly selected Ensrick value at exact conflicts. Also owns the #221 cluster families (`FAMILIES.armor`: explicit `ARMOR_IDS` under `rework_master_armor`) whose master is a snapshot/restore transaction over private `rework_master_armor_saved_*` rows; every `rework_master_` id is a control or storage row and never a family leaf. | returns `{ new, apply_bounded_master, reconcile_engines, family_for_setting, cluster_for_setting, cluster_for_master, is_leaf_localization_key, decorate_label, FAMILIES, ARMOR_IDS, CONTROL_PREFIX, MASTER_ALL, MASTER_ARMOR }`; the policy adds `plan_cluster_custom`, `cluster_for_leaf`, `cluster_ids`; runtime publishes the module and policy as `mod._crt.rework_master_module` / `.rework_master_policy` | after balance and tourney; required independently by localization before script load |
| `_crt_armor_settings_owner.lua` | #221 armor-only pre-write participant and validated profile preimage. Owns the three public setting IDs and six durable armor rows; consumes the existing policy for master-only edits. | Factory published as `mod._crt.armor_settings_owner_factory`; its instance is composed by `_crt_settings_owner.lua` | entry installs after armor policy/dispatch helpers; no hooks |
| `_crt_settings_owner.lua` | #1575 composite Mod Tweaker settings-owner v1 provider: delegates armor capture/prepare and owns the derived family masters and Tourney career presets. `profile`/`reconcile` consume them without writes; `edit` classifies changed flags before writes and runs family presets, then career presets (OFF before ON), before GUT commits the staged leaves. | Factory `mod._crt.settings_owner_factory`; `mod._crt.profile_family_ids` / `.profile_career_ids`; instance `mod.mod_tweaker_settings_owner` (`version`, `capture`, `prepare`) | installed after the preset appliers; no hooks |
...
| `career_tweaker_armor_overcharge.lua` | Seven armor/overcharge/Focused-Spirit controls using one `DamageUtils.apply_buffs_to_damage` hook and one consolidated `PlayerUnitHealthExtension.add_damage` hook. Owns Focused Spirit's owner-writer/server-mirror proc wrapper, post-success cooldown request, and one-frame cooldown re-arm; the stacking template fields remain in balance's reversible lifecycle. | installs its own hooks; exports `mod._crt_focused_spirit_tick(dt)`, `_crt_focused_spirit_reset()`, and `/crt_verify_focused_spirit` | after tourney |
...
| `_crt_umbrella_audit_policy.lua` | Read-only #221 ownership census for the subgroup-master boundary. It counts the live native/Tourney catalogs and the Unchained, Engineer, and armor clusters without writing settings or installing hooks; `cluster_gates=N/4` derives from the cluster families registered in the rework-master policy (`gated_clusters`, `cluster_ids`) rather than a literal. | `mod._crt.umbrella_audit_policy`; `mod._crt.umbrella_audit()`; `/crt_umbrella_audit` | after the #445 policy, before master callbacks |
...
2. **A new rework / balance tweak** → the bounded hook-neutral catalogue owner
   whose existing scope matches it (`_crt_balance_catalog_engineer.lua` for
   Outcast Engineer rows). `_crt_balance_catalog.lua` is only the ordered
   composer and duplicate-owner guard; it never owns `BALANCE_MODS` rows.
   If no existing owner has the right responsibility, add one bounded owner and
   wire it exactly once through the composer.
...
- Secondary melee reconciles both canonical live carriers: backend validation reads `CareerSettings.es_knight.item_slot_types_by_slot_name.slot_ranged`, while inventory category creation reads the Foot Knight career object under `SPProfiles`. Stock aliases them, but CRT handles independent replacement, preserves each array identity, keeps both `melee` and `ranged` accepted while enabled, and never deletes an inventory item. Both concrete category builders (`HeroWindowLoadoutInventory` and the live controller path `HeroWindowLoadoutInventoryConsole`) reconcile the exact career object before it caches the OR-filter; discovery uses `pairs`, not hole-sensitive `ipairs`, for modded profile arrays (#935).
...
The entry combines the exact eleven network-sensitive setting IDs: nine balance
rows, the Tourney Warrior Priest aura, and the hook-owned Focused Spirit chip
exemption. It
