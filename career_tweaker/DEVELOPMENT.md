...
| `career_tweaker_armor_overcharge.lua` | 5 default-OFF armor/overcharge runtime hooks (one on `DamageUtils.apply_buffs_to_damage`, one on `PlayerUnitHealthExtension.add_damage`). No apply/restore contract - VMF deactivates the hooks on disable. | installs its own hooks | after tourney |
...
| `_crt_talent_swap.lua` | Talent-tree + ability/passive swap engine: `HeroWindowTalents` on_enter/on_exit hooks, the DLC gate, apply/restore. | `apply_talent_swaps`, `restore_talent_swaps`, `refresh_talent_ui`, `ALL_CAREERS`, `get_talent_swap_originals` / `set_talent_swap_originals` | after mutex, before the entry captures its talent lifecycle locals |
...
| `_crt_regression.lua` | The `/crt_regression_test` harness + all 19 check bodies, in frozen registration order. | `mod._crt.rt_register` (for future phases) | LAST |
