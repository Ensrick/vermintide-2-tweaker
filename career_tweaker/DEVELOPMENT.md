...
| `_crt_balance_catalog_early.lua`, `_crt_balance_catalog_late.lua` | Disjoint, sub-2,500-line balance-definition owners. They receive closure dependencies explicitly and register no hooks or lifecycle callbacks. | each returns `build(ctx) -> definitions` | loaded only by `_crt_balance_catalog.lua` |
| `_crt_balance_catalog.lua` | Dependency-injected declarative owner for every `BALANCE_MODS` rework definition. It receives the existing mod, wire policy, stub builder, and wire-safe helper installer; it installs no hooks and owns no lifecycle callback. | returns `build(ctx)`, whose result is the unchanged `BALANCE_MODS` table | dofile'd exactly once by `career_tweaker_balance.lua`, before the hook owner and apply engine |
