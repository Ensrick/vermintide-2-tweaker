...
24 registration sites (`tools/mod-lint/lint-mod.ps1 -Mod career_tweaker`
...
| `HeroWindowTalents.on_enter` / `on_exit` [safe] `_crt_talent_swap.lua:53`/`:61` | The talent-tree window; on enter it builds the talent grid from `CareerSettings` [src: `scripts/ui/views/hero_view/windows/hero_window_talents.lua`, `[unverified]` exact line] | Apply the mod's talent/ability/passive swaps into the open window and restore on exit (`refresh_talent_ui`) | `hook_safe` does NOT chain - the two registrations are on DISTINCT methods so no collision; a second hook on either method would silently shadow (CLAUDE.md NON-NEGOTIABLE 8) |
...
| Class.method (kind) | Vanilla behavior | Why crt hooks it | Trap / invariant |
|---|---|---|---|
| `DamageUtils.apply_buffs_to_damage` [hook,tbl] `career_tweaker_armor_overcharge.lua:229` | The single chokepoint for gromril consumption and Unchained overcharge-on-damage conversion [src: `scripts/helpers/damage_utils.lua:2134`; gromril `:2335-2345`, overcharge `:2196-2224`] | Temporarily shim the victim's `be.has_buff_type` (hide the gromril marker) or `be.apply_buffs_to_value` (no-op `damage_taken_to_overcharge`) for exempt hits, then restore (`:229`) | Table-form (`DamageUtils` plain table). Host-authoritative. `pcall` + UNCONDITIONAL restore of both shims even if `func` raises; capture all 3 returns (`docs/VMF_RECIPES.md` §2). Fast early-out when all 5 toggles are off |
| `PlayerUnitHealthExtension.add_damage` [hook,tbl] `career_tweaker_armor_overcharge.lua:347` | Applies damage to a player; fires the `on_damage_taken` procs that consume a Necromancer Cursed Armor counter [src: `scripts/unit_extensions/default_player_unit/player_unit_health_extension.lua`, proc fire `:702-703`] | Flag the victim's tick exempt (`mod._crt_cursed_armor_exempt_unit`) so the crt-owned `ProcFunctions` counter-remover skips ONLY the counter consume, not every `on_damage_taken` proc (`:347`, fix v0.3.56-dev) | Table-form. Full 18-param signature verbatim (nothing dropped). `pcall` + save/restore the exempt flag on both paths; VT2 single-threaded so the flag lives only across the synchronous call |
