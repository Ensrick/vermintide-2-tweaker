...
## 0.7.295-dev (2026-07-18) - issue 52 skull census rearmed, premise corrected [diagnostics-armed]

- Diagnostics only, zero gameplay change. The `[ct:skull52]` object-set census (armed 0.7.218-dev) was pointed at the wrong collectible and could not have produced a usable answer. Correction from the decompile: `gargoyle_head` (`pickups.lua:440-453`) is a **dlc_portals** pickup backing the collect-4 `portals_survive` mission `mission_restore_the_gargoyle_heads` (`penny_level_settings_part_1.lua:61-65`); Tower of Treachery is the `wizards` DLC, and its skulls are not a Pickups entry at all - they are level-flow interactables in the dlc_wizards_tower binary that call `flow_callback_on_tower_skull_found` -> `Managers.state.achievement:trigger_event("on_tower_skull_found")` (`flow_callbacks.lua:5627-5629`), 10 of which complete `achievements.tower_skulls` (`achievement_templates_wizards_part_2.lua:48-69`).
- The census is no longer deus-only. A single deus run cannot name the skull-bearing set; it is identified by DIFFING a vanilla-adventure run of dlc_wizards_tower against an injected deus run. The line now carries `mode=` so both runs are distinguishable in one log, plus per-set `units=` (unit-index count) and `kind=` so empty and always-spawning `flow_`/`team_` sets can be dismissed (`game_mode_helper.lua:58-111`). It still logs before the #156 mutation, so it records the raw engine decision. #156 behavior is unchanged.
- The census body moved out of the entry into a new `_ct_diag_skull52.lua` side module (the established `_ct_diag_*` pattern), so the near-limit main chunk shrinks 11338 -> 11334 non-empty lines rather than growing. New `/ct_regression_test` check `skull52_object_set_census_armed` asserts BOTH halves - the module's `census` function and the `mod._ct_skull52_probe_armed` hook beacon - so a refactor that drops either fails the suite instead of silently blinding the probe. A source self-grep was not usable (`io` is nil in the retail sandbox).

**Verify (this is a data-capture run, not a fix):** play **dlc_wizards_tower (Tower of Treachery) as a normal Adventure map** and keep the log, then play it again **injected as a Chaos Wastes adventure map** and keep that log. Grep both for `[ct:skull52]`. Any set that is `spawned=true` under `mode=adventure` and `spawned=false` under `mode=deus`, with `units=` greater than 0, is the fix target.
