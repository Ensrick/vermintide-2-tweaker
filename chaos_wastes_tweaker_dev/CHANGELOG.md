...
- Keeps the host-authoritative reserve/commit/refund boundary and updates the prompt/localization to describe the effective absolute price without charging reward collection.
- Ports vanilla's fixed widget-local collectible offset loop for injected Chaos Wastes rows and adds a hook-order recovery constructor for GUI Tweaker Dev, preventing banner animation coordinates from feeding back into the child layout.
- Adds bounded runtime evidence plus Lua 5.1 regression coverage for the cost policy, localization contract, native collectible geometry, and cross-mod recovery path.
...
- **Cluster A (issue 322, linked-pickup network id) [untested]:** the two-value `PickupSystem._spawn_pickup` capture/return (landed v0.7.245-dev; vanilla pair at `pickup_system.lua:1283`, consumed by `rpc_link_pickup` at `:1441-1447`) is now pinned by an offline regression suite (`qa/lua/tests/test_ct_spawn_pickup_two_return.lua`): singleton hook registration, the exact two-value capture and `return spawned, go_id`, the issue 294 guard staying a BARE return (nil, nil - matching vanilla's own early returns at `:1212-1228`), and a statement-level census that flags any new value-returning branch. No mod-side behavior change.
- **Cluster B (issues 249, 256, 289 evidence) [untested]:** the Quiver Cascade (`ct_meta_ammo`) stack grant is now SERVER-AUTHORITATIVE. Evidence `[ct:289]`: a client tracked 5 effective boons with 0 active stacks while the host tracked correctly - the old proc added stacks through the LOCAL `BuffExtension.add_buff` path, which never replicates, so a client's `total_ammo` capacity never applied and its HUD lagged the real count (issue 249, 36 vs 62). Vanilla fires `on_boon_granted` on the HOST for every gainer (locally at `deus_run_controller.lua:1152-1160`, and for client gainers in the host's `rpc_deus_add_power_ups` receiver at `:1397-1404`), so the host proc now grants via `BuffSystem.add_buff(unit, name, unit, true)` - added locally AND broadcast to every client via `rpc_add_buff`, re-sent on hot-join (`buff_system.lua:277-311`, `:66-97`); client peers defer to replication. The networked path is gated on the issue 426 peer-parity beacon (`mod._ct_wire_safe`; unconfirmed parity degrades to the old host-local add - zero wire exposure), and the ct_-prefix strip already covers the stacks on parity loss. The issue 256 [0, max] clamp arithmetic moved verbatim into the pure `_ct_ammo_guard_core.lua` (offline-tested; `_max_ammo` never written). New rt check `ct_meta_ammo_server_auth_grant_249` + offline suite `test_ct_ammo_guard_policy.lua`.
- **Cluster C (issues 288, 464, Anath Raema permanent reload) [untested]:** mechanism now PROVEN from the decompile - wield applies trait buffs via `SimpleInventoryExtension.apply_buffs` -> `buff_extension:add_buff` (`simple_inventory_extension.lua:824`), and `BuffExtension.add_buff` resolves its template AT CALL TIME through `BuffUtils.get_buff_template` (`buff_extension.lua:165,173`) reading the live `BuffTemplates` global (`buff_utils.lua:256-262`). The v0.7.268 enforcement inside CT's add_buff hook therefore lands the replacement at the exact native lookup boundary regardless of load order (issue 288 hypothesis 3, template availability, was the real one; the upvalue and NetworkLookup-snapshot hypotheses are disproven at this seam - the wire carries the template NAME index, content resolves per-peer). The issue 464 root (+0.5 on the INVERSE `reload_speed` stat = 50 percent SLOWER) stays fixed at -0.5. No new code; the sign, dual-registry swap, add-boundary enforcement, and marker are now pinned offline (`test_ct_anath_raema_contract.lua`). The capped `[ct:288]` wield-time receipts remain armed for in-game proof.
- **Cluster D (issues 60, 132, 349, 251, trial-chest cap cross-path ledger) [untested]:** two-sided fix.
  - Over-spawn (issues 60, 132, 349): ONE settled post-populate reconciliation pass now runs from the existing delayed census finalize (about 8s after the host's populate, after the `[ct:349]` audit line). It censuses every live `deus_cursed_chest` in appearance order (`DeusCursedChestExtension.extensions_ready`, every spawn path) against the host cap and PRUNES the excess from the END - but only units that provably came through `PickupSystem._spawn_pickup` (deleted via the engine's own pickup path, `unit_spawner mark_for_deletion`, exactly what vanilla uses at `pickup_system.lua:1345`, `:1451-1455`) and only while their replicated state is still WAITING (`deus_cursed_chest_extension.lua:13`, `:367-373`). Raw baked level units and activated chests are never deleted - they are reported `unprunable` in the one-line `[ct:132] reconcile` receipt. Gates: host only, deus run only, EXPLICIT cap only (`cursed_chest_count` at Default -1 leaves vanilla per-level counts untouched). The Belakor locus (`deus_02`) is never pruned. The under-spawn direction deliberately does NOT spawn post-hoc chests (placement belongs to the conversion path); missing-altar reports stay visible via the existing pedestal probes.
...
- Closed the separate Loot Die bypass. Chest bonus dice do not call `PickupSystem`: vanilla `InteractionDefinitions.chest.server.stop` constructs `pickup_system.pickup_name = "loot_die"` and calls `UnitSpawner.spawn_network_unit` directly. CT now rewrites this exact owner-spawn record to the native `deus_soft_currency` unit/template/identity before game-object creation.
- The existing `PickupSystem._spawn_pickup` boundary now converts `loot_die`, `lorebook_page`, and `painting_scrap` through one pure policy, host-only. A new exact `UnitSpawner.spawn_network_unit` boundary catches direct collectible spawns. The host serializes the final coin identity into the network game object; clients create the replicated coin and never make a second conversion decision. Stock Adventure, non-collectibles, clients, and missing coin settings pass through unchanged.
- Added bounded `[ct:351] collectible_conversion` receipts, engine-free policy/immutability/authority coverage, and extended `/ct_regression_test` check `cw_collectible_and_big_casket` to require all three identities plus the direct-spawn rewriter.

**Source audit:** `scripts/unit_extensions/generic/interactions.lua:2112-2138` rolls a chest Loot Die and calls `Managers.state.unit_spawner:spawn_network_unit` directly. `scripts/entity_system/systems/pickups/pickup_system.lua:1208-1274` rejects client-owned `_spawn_pickup` calls and sends successful host spawns through `UnitSpawner`. `scripts/network/unit_spawner.lua:336-352` creates the authoritative game object, while `:470-490` constructs client husks from that replicated object.

**Verify after deployment (host + one client):** both peers confirm `ct_dev 0.7.272-dev`, then enter a CT-injected Adventure mission with Ravaged Art and open ordinary chests capable of bonus Loot Dice. Every such position/drop must show a Pilgrim's Coin, never art or dice, on both peers. The host log should contain bounded `[ct:351]` rows with `source=pickup_system` for art and `source=unit_spawner` for a chest die; the client must see the same coin without a client-authority conversion row. Run `/ct_regression_test` and require `cw_collectible_and_big_casket` to pass. Start the same map as stock Adventure and confirm Ravaged Art/Loot Dice remain vanilla.
...
- Fix: the existing two DEUS counters and replicated value readers are unchanged. Their widgets now reflow after vanilla's draw pass updates the live scenegraph: two columns when both measured localized cells fit, one-column wrapping otherwise. Horizontal bounds intersect the native right-banner edge with the engine safe rectangle; vertical placement shifts above its lower inset and scales only when wrapped rows would meet the Collectibles divider. Text uses the renderer's native dynamic-width sizing inside the computed cell. No fixed screen-resolution offset and no new hook.
- Diagnostics: one `[ct:571] collectible layout ...` line whenever resolution, UI scale, safe rectangle, or relevant scenegraph position changes. It reports resolution, scale, safe inset, available width, columns/rows, icon scale, and shift.
- Regression: `issue571_cw_tab_collectibles_safe_reflow` exercises 16:9, ultrawide, clamped high-DPI/UI-scale, safe-rectangle, and oversized-localization wrapping contracts. The existing #533 test continues to lock Chests-of-Trials/Pilgrim's-Coin values and the Adventure passthrough boundary.
- Verification: in an injected Adventure mission, hold Tab at 16:9 and ultrawide (and with non-default HUD/UI scale and safe rectangle). Both counts must remain readable inside the right pane; a narrow/localized case may wrap to two rows. Confirm `[ct:571]`, complete a Chest of Trials, collect coins, and verify the values still update. A vanilla CW mission keeps the same DEUS content; a normal Adventure mission remains byte-for-byte vanilla. Run `/ct_regression_test` and require the #533 and #571 checks to pass. Not shipped.
...
- **Bug 1 root cause (rows invisible, header fine).** The 0.7.251 rows were anchored on the `reward_item` scenegraph node, which defines NO size and therefore inherits banner_right's full 660x1080 rect (`ui_scenegraph.lua:109` - `node_def.size or parent.size`). That node's origin computes to world y = -750, BELOW the screen; vanilla only renders content there by pass-level `vertical_alignment = "top"` inside the inherited rect (`create_reward_item` icon style, `ingame_player_list_ui_v2_definitions.lua:1750`, applied by `UIUtils.align_box_inplace`, `helpers/ui_utils.lua:542-558`). Our rows used center/default alignment, so every icon and name drew at negative screen y - invisible, silently eaten by the per-widget pcall - while the header, anchored on the SIZED `reward_divider` node ({264,32}, world y about 348 at the 1080p reference), rendered. Exactly the user's screenshot: header with count, zero rows.
- **Bug 1 fix.** All row widgets now anchor on `reward_divider` (the node the header provably renders on) and stack below the header via per-row style offsets: 2 columns x 9 rows, 28px row pitch, 330px column pitch, 24px icons, font 16, boon names UTF-8-safe-truncated at 30 bytes. 18 boons visible, the rest collapse into "+N more" (`/ct_preview_boons` always lists everything). The band used (screen y about 340 down to about 90) holds vanilla's deed-reward strip and keep collectibles, both empty in the gated context below. Icon-and-name-as-separate-widgets degradation and the guarded shared `_draw` pass (also carries the issue 533 counters) are unchanged.
- **Bug 2 fix (user: "should only show up in the Chaos Wastes part of the keep as the team prepares for a mission").** New gate `mod._ct_preparing_cw_expedition()`: a direct port of vanilla `MatchmakingManager.is_matchmaking_versus` (`matchmaking_manager.lua:1256-1266`) retargeted at mechanism "deus". Peer-correct: the host writes `lobby_data.mechanism = "deus"` for public AND private expeditions (`matchmaking_manager.lua:985`); party clients are always sent `rpc_set_matchmaking(true, ...)` (`:1083`) so their state leaves idle and they read the host lobby's replicated mechanism field. Resets cleanly: loading into any hub rewrites `matchmaking = "false"` and refreshes the mechanism field (`state_loading.lua:2584/:2597`), and cancel returns every peer to `MatchmakingStateIdle` - so the block is gone after a cancel or a finished run. Gate re-evaluates on EVERY Tab press (the build point `_setup_deed_reward_data` runs on each `set_active(true)`, `ingame_player_list_ui_v2.lua:1224`). Interpretation note: "the Chaos Wastes part of the keep" is implemented as "a CW expedition is queued" - the only cited, peer-correct seam; there is no clean signal for merely standing in the CW alcove pre-queue.
- **Evidence lines.** `[ct:461] boon preview built: N boons (CW expedition queued)` on a rendering Tab press; `[ct:461] boon preview suppressed: no Chaos Wastes expedition queued` when the toggle is on but nothing is queued.
- **Regression test.** `issue461_boon_preview_wired` extended: marker bumped to v0.7.258, the gate helper must exist and return a boolean anywhere, and a build probe asserts no preview widget ever anchors on the sizeless `reward_item` node again (the off-screen bug class).
- **Files.** `chaos_wastes_tweaker_dev.lua` (`MOD_VERSION` `0.7.257-dev` -> `0.7.258-dev`, gate helper, builder rewrite, build-hook gate + printfs, `/ct_preview_boons` gate-state line, regression check), `chaos_wastes_tweaker_dev_localization.lua` (tooltip mentions the queue gate). No new hooks (existing `_setup_deed_reward_data`/`_draw` bodies edited). Tag `[untested]`.
- **Verify in-game (solo host, full Steam restart, confirm `[ct:LOAD] v0.7.258-dev`).** (1) Enable "Preview Starting Boons on Tab" + the same starting boons as before; hold Tab anywhere in the keep BEFORE queuing: no block, console shows the suppressed line. (2) Walk to the Chaos Wastes expedition map, start (host) a journey, and while still in the keep hold Tab: "Starting Boons (N)" header WITH the boon rows under it, two columns, icon left + name right; console shows the built line. Please eyeball: do the ICONS render (or names only), and is icon/name alignment right - both still the untested specifics from 0.7.251. (3) Cancel the expedition, hold Tab: block gone. (4) `/ct_preview_boons`: full numbered list plus a line saying whether an expedition is queued. (5) `/ct_regression_test`: `PASS: issue461_boon_preview_wired`. Coop (2nd tester on ct_dev, optional): client in the party holds Tab after the host queues - same block from the host's settings.

## 0.7.257-dev (2026-07-13) - #533 FIX: Tab-hold collectibles show deus counters (Chests of Trials + Pilgrim's Coins) in CW runs [untested]

### #533 (bug) - "The Collectibles for Chaos Wastes Mission on tab"
- **Root cause.** The Tab-hold right pane builds its collectible counters purely from the CURRENT level's `LevelSettings.loot_objectives` (`ingame_player_list_ui_v2.lua:436-441`), and the LevelSettings post-processor defaults `loot_objectives = { grimoire=2, tome=3, loot_die=0, ... }` onto every `mechanism == "adventure"` level (`level_settings.lua:1889-1896`). Vanilla CW (deus) levels have no `loot_objectives`, so vanilla builds no counters there - but an Adventure level injected into a CW run inherits the adventure default, so the pane showed tome/grim/dice counters for pickups that do not even exist on it (ct converts those spawners to Chests of Trials and Pilgrim's Coins).
- **Fix.** Full wrapper on `IngamePlayerListUI._setup_mission_data` (sole ct hook on that method). Outside a deus run (stock Adventure, keep, hubs): passthrough, byte-identical vanilla. Inside a deus run: vanilla's build is skipped (no tome/grim/dice widgets, vanilla's own Collectibles header stays off via `_mission_count == 0`) and the pane gets the DEUS counters instead - **Chests of Trials** and **Pilgrim's Coins** - under the vanilla Collectibles header/divider, on the vanilla `loot_objective` anchor with vanilla's own row layout (widget is a copy of vanilla `create_loot_widget` minus its glow pass; icons `deus_icons_boon` / `deus_icons_coin` from the standard icon atlas). Shown on ALL missions of a CW run (injected adventure AND vanilla CW maps) for consistency; gate is "the mechanism exposes a live deus run controller" - the every-peer-correct idiom, deliberately NOT `IS_INJECTED_ADVENTURE_LEVEL`, which is empty on clients until the graph snapshot lands (issue 68 lesson).
- **Counter data (peer-correct on host and client - both read the replicated deus-run SharedState).** Chests of Trials = `get_cursed_chests_purified(own_peer)` (`deus_run_controller.lua:2070`; server-incremented for every peer on chest completion `:767-777`). Pilgrim's Coins = `get_player_soft_currency(own_peer)` (`:925-927` - the exact getter the vanilla HUD coin indicator polls on every peer, `deus_soft_currency_indicator_ui.lua:51-68`), floored like that HUD. Values refresh while the pane is open with vanilla's `_sync_missions` write-on-change cadence.
- **Hook discipline.** The draw rides the EXISTING (IngamePlayerListUI, `_draw`) hook from the issue 461 boon preview - one body now draws both overlays (VMF silently drops a second hook on the same pair). Build + draw are fully pcall-guarded with a vanilla fallback, so a failed build or non-resident texture can never take out the pane.
- **Regression test.** `issue533_cw_tab_collectibles_wired` (runtime, no io): version marker, all three `mod._ct_*` helpers wired, and the value reader's contract (nil outside a deus run, `{chests=n, coins=n}` inside).
- **Files.** `chaos_wastes_tweaker_dev.lua` (`MOD_VERSION` `0.7.256-dev` -> `0.7.257-dev`, #533 block + `_setup_mission_data` hook, issue 461 `_draw` hook body extended, regression check), `chaos_wastes_tweaker_dev_localization.lua` (2 chrome keys). Tag `[untested]`.
- **Verify in-game (solo host, full Steam restart, confirm `[ct:LOAD] v0.7.257-dev`).** (1) Enable Inject Adventure Missions, host a CW expedition, enter (or `/ct_load_mission` into) an injected Adventure mission; hold Tab: the right pane's Collectibles section must show "Chests of Trials x0" and "Pilgrim's Coins x<current coins>" - NO tome/grim/dice counters; console shows `[ct:533] Tab-hold collectibles -> deus counters`. (2) Complete a Chest of Trials, pick up coins, hold Tab again: chest counter reads x1 and the coin counter matches the HUD coin indicator top-right. (3) A vanilla CW mission in the same run shows the same two counters. (4) REGRESSION - leave CW, start a NORMAL Adventure mission (Helmgart quickplay): tome/grim/dice counters unchanged, no `[ct:533]` line. (5) `/ct_regression_test` shows `PASS: issue533_cw_tab_collectibles_wired`. Coop (2nd tester on ct_dev, optional): client holds Tab mid-run - counters show and track the client's own coins/chests.

## 0.7.256-dev (2026-07-13) - #464 follow-up: Anath Raema's Swiftness selectable as a boon (Offensive > Ranged) [untested]

### #464 (bug follow-up) - "I can't find the boon to set it as a starting boon... is it missing from our options menus?"
- **Root cause of the omission.** Anath Raema's Swiftness is a weapon TRAIT (`deus_ammo_pickup_reload_speed`, `weapon_traits_morris.lua:528`, rolled in the RANGED trait pools `deus_ranged_ammo` :853 and `deus_trollhammer_torpedo` :975), NOT a deus power-up - `deus_power_up_settings.lua` has no boon form of it. The Disabled/Starting Boons menus enumerate `BOON_TREE` (power-ups plus ct's mod boons only), so there was never an entry to show. Nothing was removed by the 0.7.247-dev sign fix; the boon never existed. The trait's BAN checkbox (`ban_trait_deus_ammo_pickup_reload_speed`) was already present under trait bans - only the boon surfaces lacked it.
- **Fix: 5th trait-as-boon.** `ct_boon_anath_raema_swiftness` added to `CT_TRAIT_BOONS` (Unique, same registration path as the other four; pre-registered unconditionally for NetworkLookup index parity, pool insert gated by the new `enable_boon_anath_raema_swiftness` toggle in Reworks > Reworks: Boons and by the issue 426 peer-parity gate). The boon is ALWAYS the permanent variant: new `literal_buffs` spec field carries a fixed `reload_speed` stat_buff, multiplier -0.5 (inverse stat: composed as hold time x (multiplier + 1), `weapon_unit_extension.lua:966` / `buff_extension.lua:1431-1432`, so -0.5 = reload time halved - the corrected #464 sign). literal, not a `source_buff` clone, because the trait template is mutated live by the `tweak_anath_raema_permanent` rework and a load-time snapshot would make the boon's behavior depend on that toggle's state at load.
- **Menu category.** Listed under **Offensive > Ranged** in BOTH the Disabled Boons and Starting Boons trees (BOON_TREE `offensive_boons > ranged`), beside vanilla `boon_range_01` "Anath Raema's Cruel Volley" - matching the vanilla ranged trait pool and where the user looked. The four older `ct_boon_*` trait-boons stay in Mod Boons (shipped positions preserved).
- **Starting boon works without the Rework/enable toggles** (grant enumerates the pre-registered `DeusPowerUpsArray`); the enable toggle only controls whether it ROLLS in chests. Setting ids auto-join the host-settings sync registry, so clients inherit the host's choices as usual.
- **Regression test.** `anath_raema_trait_boon_464` (runtime, no io): boon in the modded registry, template is a NEGATIVE `reload_speed` stat_buff (the #464 sign-error class), and all three widgets (`disable_boon_*`, `start_boon_*`, `enable_boon_*`) exist in the realized widget tree.
- **Files.** `chaos_wastes_tweaker_dev.lua` (CT_TRAIT_BOONS spec + `literal_buffs` support in `pre_register_trait_boon_lookups`, MOD_BOON_LOC display name/description, regression check; `MOD_VERSION` `0.7.255-dev` -> `0.7.256-dev`), `chaos_wastes_tweaker_dev_data.lua` (BOON_TREE ranged entry, `enable_boon_anath_raema_swiftness` checkbox), `chaos_wastes_tweaker_dev_localization.lua` (6 new keys). No new hooks. Tag `[untested]`.
- **Verify in-game (solo host, full Steam restart, confirm `[ct:LOAD] v0.7.256-dev`).** (1) ct_dev options: Disabled Boons > Offensive > Ranged AND Starting Boons > Offensive > Ranged both list "(Mod Boon) Anath Raema's Swiftness". (2) Check its Starting Boon box, `/ct_load_mission` into any CW mission with a reserve-ammo ranged weapon (crossbow/handgun): the boon shows in your boon list and reload is visibly FASTER (about half the hold time) with NO ammo pickup and NO trait on the weapon; toggle the starting boon off for baseline comparison - ON must never be slower. (3) `/ct_regression_test` shows `PASS: anath_raema_trait_boon_464`.
...
- **Spec correction (user).** "To avoid underflow, we allow duplicates of the same mission, when there are fewer than 4 (or however many non-arena, non-shrine actual mission nodes there are on a given expedition)." The 0.7.250-dev pool floor filled an emptied TRAVEL/SIGNATURE pool by backfilling a vanilla level from the pristine snapshot - which re-added missions the user had DISABLED, so his underflowed runs surfaced arenas / random maps. The fill must instead DUPLICATE the missions he left ENABLED, so an underflowed run repeats HIS choices.
- **Rework.** `_adventure_pool.lua`: removed the snapshot single-level backfill (`enforce_pool_floor` + its `first_snapshot_key` helper are gone). Underflow is now routed entirely through the existing duplicate-alias machinery (`inject_duplicate_aliases`), which clones the pool's current keys - and after filter+inject those keys are exactly the user's ENABLED missions (enabled adventure missions + non-disabled CW scenarios). So a pool with 1..threshold-1 enabled missions gets `<key>_dupN` clones of THOSE, and the run repeats them. New pure classifier `AdventurePool.classify_pool_floor(enabled_count)` returns `ok` / `duplicate` / `fallback`; both the duplication guard and the fallback pass consume it.
- **Needed-count derivation.** The only wired TRAVEL/SIGNATURE validator is `prevent_same_level_choice` (branch-sibling width; `deus_map_populate_settings.lua:222` - `prevent_same_level_on_same_path` is defined but never wired). Provable worst case: a node has <= `MAX_INCOMING_CONNECTIONS_PER_NODE` (3) parents each with <= `MAX_CONNECTIONS_PER_NODE` (2) children, so <= 3 same-type siblings + itself = **4 distinct keys** (`deus_map_base_gen_settings.lua:6-8`). So `POOL_SAFETY_THRESHOLD = 4` is provably sufficient AND matches the user's "fewer than 4" trigger. His "however many mission nodes the expedition has" framing over-counts: levels MAY repeat across non-sibling nodes down a path (the path-wide validator is unwired), so the need is branch width, not node count. Threshold kept at 4.
- **Zero-enabled edge case (documented, not silent).** A pool the user empties of ALL enabled missions has nothing to clone, and an empty TRAVEL/SIGNATURE pool freezes the graph solver. New `fall_back_zero_enabled_pools()` restores THAT pool's vanilla contents only (never silently re-adding disabled levels into an otherwise-honored pool), and a chat notice names it: "no TRAVEL missions enabled - using the default Chaos Wastes TRAVEL maps so the run can load. Enable at least one TRAVEL mission to change this."
- **Notice wording.** The underflow notice now says missions will REPEAT ("fewer TRAVEL missions enabled than the run needs - your enabled TRAVEL missions will repeat to fill it."), not that anything was backfilled. Kept the `[ct:487]` printf pair (one for the fallback, one for the repeat). Notice is deduped per pool by STATE, so a pool crossing from repeat -> fallback (user disables the last enabled mission) re-fires the correct message.
- **Regression test.** `pool_floor_underflow_duplicates_487` (runtime, no io): asserts `classify_pool_floor(0)=="fallback"`, `(1)=="duplicate"`, `(threshold-1)=="duplicate"`, `(threshold)=="ok"`, and `POOL_SAFETY_THRESHOLD>=4`.
- **Files.** `_adventure_pool.lua` (classifier + fallback + dup-pool reporting + notice rework; removed snapshot backfill), `chaos_wastes_tweaker_dev.lua` (`MOD_VERSION` `0.7.253-dev` -> `0.7.254-dev`, new regression check), `_ct_diag_freeze487.lua` (header comment). No new hooks. Tag `[untested]`.
- **Verify in-game (solo host, full Steam restart, confirm `[ct:LOAD] v0.7.254-dev`).** (1) Enable Inject Adventure Missions, then enable EXACTLY ONE mission (one DLC/campaign master, everything else off, Chaos Wastes Missions master OFF). Host a Chaos Wastes run. EXPECTED: the run builds and REPEATS that one mission across its TRAVEL/SIGNATURE nodes - NO arenas or random maps beyond the graph's own single end-arena/shrine nodes; a chat notice says your enabled missions will repeat; console shows `[ct:487] POOL-FLOOR ... underflow; duplicated enabled missions` and a `GRAPH-SOLVE end ... graph=N nodes` (not `graph=NIL`, no freeze). (2) ZERO-ENABLED: turn OFF every master (nothing enabled), host. EXPECTED: chat notice "no TRAVEL/SIGNATURE missions enabled - using the default Chaos Wastes maps...", the run loads on vanilla CW maps, no freeze, `[ct:487] POOL-FLOOR ... fell back to vanilla`. (3) Recovery: re-enable 4+ missions, host - normal varied pool, no notice. (4) `/ct_regression_test` shows `PASS: pool_floor_underflow_duplicates_487`.

## 0.7.253-dev (2026-07-12) - #256 FIX: meta-ammo boon no longer leaves negative reserve ammo [untested]

### #256 (bug, 1-major) - "ct: 'ammo per boon' boon can result in NEGATIVE ammo"
- **Root cause (consumption-side, LOWER bound).** The `ct_meta_ammo` boon ("Quiver Cascade", +5% total ammo per active boon) refreshes the wielded ranged weapon on every boon grant via the `ct_meta_ammo_refresh_capacity` apply_buff_func, which calls vanilla `GenericAmmoUserExtension:refresh_buffs()`. Vanilla `refresh_buffs` (generic_ammo_user_extension.lua:105-108) recomputes reserve as `_available_ammo = math.min(_start_ammo - _current_ammo, _available_ammo)` with NO floor at 0. `_start_ammo = round(_original_ammo_percent * _max_ammo)` is scaled by the ammo fraction the weapon SPAWNED with; when a weapon that entered a map at a partial fraction (`_original_ammo_percent` < 1) later has its loaded clip refilled above that scaled `_start_ammo` (ammo pickup + reload), `_start_ammo - _current_ammo` is negative and the reserve goes below zero. Vanilla only self-heals in the `ammo_percent == 1` branch (:110-112 `reset()`); at any other fraction the negative persists and the HUD shows negative ammo. There is NO fassert on `_available_ammo` (unlike `_current_ammo`, guarded at :234/:483), so it is silent - matching the report of a visible negative rather than a crash.
- **Fix.** New `mod._ct_clamp_current_ammo_256(ax, seam)` clamps CURRENT ammo back into range immediately after each `refresh_buffs()` at the ct seam: reserve (`_available_ammo`) floored at 0, loaded clip (`_current_ammo`) clamped into [0, `_max_ammo`]. It does NOT touch `_max_ammo` (max-resource doctrine; the issue 34 UPPER-bound clamp at `_apply_buffs` is a different, unrelated bound and is untouched). `refresh_buffs` is the ONLY ct seam that derives current/reserve ammo, so both the left- and right-hand ranged units get clamped after their refresh.
- **Coop.** `AmmoSystem` runs `GenericAmmoUserExtension` locally on each peer for that peer's own weapon; the only network path (`rpc_give_ammo_fraction_to_owner`, ammo_system.lua:105) carries a [0,1] fraction that flows through the already-clamping `add_ammo` path, never a raw ammo value. The meta-ammo boon buff applies locally, so `ct_meta_ammo_refresh_capacity` runs on whichever peer owns the weapon - the seam clamp covers host and client alike. No receive-side clamp is needed.
- **Relationship to #249 (client HUD ammo desync 36-vs-62).** Same boon, same `refresh_buffs` seam, but a DIFFERENT root: #249 is a divergence between two POSITIVE values (host vs client `_max_ammo`/HUD count), whereas #256 is a single value crossing below zero. This clamp floors the negative case (which may have contributed to some desync reports) but does NOT synchronize host and client, so it does NOT by itself fix the 36-vs-62 desync. #249 stays open.
- **Evidence line.** `[ct:256]` printf fires ONLY when a clamp actually engages (before/after value, max, seam, item). Doubles as the in-game repro proof.
- **Files.** `chaos_wastes_tweaker_dev.lua` (helper `_ct_clamp_current_ammo_256` in the `ct_meta_ammo_refresh_capacity` do-block + call after each `refresh_buffs()`; new regression check `ct_meta_ammo_current_floor_256`). `MOD_VERSION` `0.7.252-dev` -> `0.7.253-dev`. Tag `[untested]`.
- **Verify in-game (solo host, full Steam restart, confirm `[ct:LOAD] v0.7.253-dev`).** Repro pre-fix / expected post-fix: (1) Host a Chaos Wastes run on a ranged career whose weapon has a reserve+clip (crossbow, handgun, blunderbuss - NOT an all-loaded bow). (2) Fire the ranged weapon down to a partial clip, then top the clip back up from an ammo crate so the loaded clip exceeds the fraction the weapon carried into the map. (3) Pick up the `ct_meta_ammo` boon ("Quiver Cascade") - or take several boons so it stacks. EXPECTED post-fix: reserve floors at 0 (never negative) and the console prints `[ct:256] clamped reserve ammo <neg> -> 0 ...` the moment the clamp engages. Pre-fix the HUD reserve showed a negative number. (4) Sanity: `/ct_regression_test` shows `PASS: ct_meta_ammo_current_floor_256`; `/verify_meta_ammo` still prints the hyperbolic curve and the `[ct:ammo249]` live-ammo dump. Coop (2nd tester on ct_dev): same repro on a client peer - the client's own reserve floors at 0 too (the boon runs locally on the client's weapon).

## 0.7.252-dev (2026-07-12) - #458 FEATURE: Buy Starting Boons (start-node shrine shop), load-bearing core [untested]

### #458 (enhancement, 1-major) - "Toggle to make starting node a unique Shrine/Shop to buy/pick starting boons"
- **Spec (from the issue).** A top-level toggle "Buy Starting Boons" (like Progressive Difficulty). When ON, the starting node immediately puts the player in the shrine boon shop, acting as a unique shrine tunable independently of later ones: sliders for how many boons offered, how many can be picked, a cost multiplier (0-200% in 10% steps), a miracle count (0-3), and a collapsible pool of which miracles are eligible (chosen at random when more than the count are enabled). Explicit: "does NOT change the starting node's appearance to a shrine, it simply puts the player in the shop menu."
- **Why node substitution is the WRONG lever (and what is right).** Both the "start" node and "shop" nodes run inside `GameModeMapDeus`, whose shared-state machine toggles between MAP_DECISION (the map screen) and SHOP (a shrine overlay) - both on the SAME persistent `dlc_morris_map` world (ferry-lady hub), so a shrine is an overlay + spawned idol props, not a level load (`game_mode_map_deus.lua:44-50`/`:245-269`, `deus_shop_view_v2.lua:146`/`:190`). Changing the start node's `node_type` to "shop" would recolor its map token (the appearance change the issue forbids) and ride the sticky graph/hash path. Instead we leave the graph untouched and, when the HOST enables the toggle, force the game mode into its SHOP state at run start (current node still "start"). Vanilla's own SHOP -> WAITING_FOR_PLAYERS_AFTER_SHOP -> MAP_DECISION path (`game_mode_map_deus.lua:341-349`) returns every peer to the normal first map choice after buying. `handle_shrine_entered` is deliberately NOT called (it would mark the start node traversed), so the node's map appearance is unchanged.
- **Config lever.** `DeusShopView.start` resolves its offer from `DeusShopSettings.shop_types[current_node.level]` (`deus_shop_view_v2.lua:181-184`). The start node's level is `dlc_morris_map`, never a shop in vanilla, so `shop_types["dlc_morris_map"]` is ours alone - we register it (`power_up_count` + `blessings`) from host-effective settings. The start node carries `system_seeds.power_ups` (`deus_populate_graph.lua:1054-1058`), so `generate_random_power_ups` is safe there. Boon OFFERS are per-peer-seeded exactly like a vanilla shrine (each hero sees their own); the miracle SUBSET is seeded by the start node's `system_seeds.blessings` so every peer selects the same miracles.
- **Coop.** The `ct_*` setting_ids are auto-added to `SYNCED_SETTING_NAMES` (not per-peer), so `effective_setting` gives clients the HOST's boon/miracle counts and pool. Only the host writes the shared server state; clients follow via the existing `GameModeMapDeus` shared-state sync. Config is registered on every peer at run start so the shop view always resolves it.
- **Wire safety.** The shrine reuses vanilla's buy path (`shop_buy_power_up` / `shop_buy_blessing`, vanilla RPC indices). Offered boons come from the `DeusPowerUpRarityPool`, which ct already parity-ejects for modded boons under unconfirmed peer parity (#426), so a non-ct peer can never be offered (thus never buy) a modded index; miracles are all vanilla blessing keys (`deus_blessing_settings.lua`). Net: NO new peer-sync exposure.
- **Interaction with free Starting Boons (`_add_initial_power_ups`).** They STACK and are orthogonal - the free grant runs at run setup (before the shrine), so `generate_random_power_ups` sees those boons as existing and will not re-offer them (`deus_run_controller.lua:1110`). A pure buy-your-own start = leave the free list off (stated in the toggle tooltip).
- **Menu + command.** Top-level `ct_buy_starting_boons` toggle beside Progressive Difficulty, with advanced sub-widgets `ct_start_shrine_boon_count` (0-8, default 4), `ct_start_shrine_miracle_count` (0-3, default 0), and a `ct_start_shrine_miracle_pool_group` of 8 blessing checkboxes (default ON). New `/ct_verify_start_shrine` reports the toggle, counts (local + host-effective), enabled pool, and the registered shop config. Regression check `issue458_start_shrine_config`.
- **Files.** `chaos_wastes_tweaker_dev.lua` (feature block after the #461 preview: `do`-block helpers `_ct_build_start_shrine_config`/`_ct_build_start_shrine_blessings` + the ONLY `(GameModeMapDeus, local_player_game_starts)` hook + command + regression check), `_data.lua` (toggle + sub-widgets after `progressive_difficulty`), `_localization.lua` (titles with `[untested] [Issue 458]` / `[untested]` tags + tooltips). `MOD_VERSION` `0.7.251-dev` -> `0.7.252-dev`. Tag `[untested]`.
- **#458 remaining scope (deferred, NOT in this build).** (1) Per-shrine COST multiplier (0-200%): both the shop display and buy read the shared `DeusCostSettings.shop.power_ups[rarity]` (`deus_shop_view_v2.lua:654`, `deus_run_controller.lua:1733`) and the discount field only REDUCES cost, so a >100% multiplier needs a gated interception of those reads while the start shrine is active - risky to ship untested and left out. (2) PICK LIMIT: hook `DeusRunController._try_buy_power_up` gated on start-shrine-active, count per-shrine purchases, reject beyond N (returns false to refuse). Both are clean follow-ups; the sliders are intentionally absent until wired (no dead menu options).
- **Verify in-game (solo host first, full Steam restart, confirm `[ct:LOAD] v0.7.252-dev`).** (1) Keep sanity: `/ct_regression_test` shows `PASS: issue458_start_shrine_config`; `/ct_verify_start_shrine` prints the toggle/counts/pool. (2) Enable "Buy Starting Boons", set Boons Offered = 4, Miracles Offered = 2, leave the pool defaults, host a Chaos Wastes run. EXPECTED: at run start you land directly in a shrine shop (idol backdrop, boon cards + 2 miracles) BEFORE the map; console shows `[ct:458] Buy Starting Boons applied: forced SHOP at run start (... boons=4 miracles=2)`. Buy a boon or two, press Ready -> you return to the normal first map choice, and the start node on the map looks unchanged. (3) Toggle OFF, host again: run begins straight at the map (no shrine), byte-identical to vanilla. (4) Miracles Offered = 0: shrine opens with boons only, no miracles. (5) Coop (2nd tester on ct_dev): with the HOST toggle ON, BOTH peers get the shrine at start (each their own boon offer); with only a CLIENT's toggle ON, NO shrine. Watch for any brief map->shop flicker on the client (a known cosmetic possibility, not a desync).

## 0.7.251-dev (2026-07-12) - #461 FEATURE: Preview Starting Boons on the Tab-hold panel [untested]

### #461 (feature, 1-major) - "Preview Starting Boons"
- **Spec (from the issue + comment).** "When holding TAB in the keep, show a preview list of the starting boons with their icons on the right pop-out panel," reflecting the boons "set from the host." (The comment's "active mutators" half is Event Tweaker's surface, out of scope here.)
- **Surface chosen: the vanilla `IngamePlayerListUI` right panel (`banner_right`).** That panel already draws the deed-reward icon strip (`reward_item` passes) AND the CW node-info boon icon (`terror_event_power_up_icon`), so `ui_top_renderer` is vanilla-proven to resolve Deus boon-icon textures on this exact panel (`ingame_player_list_ui_v2.lua` / `_definitions.lua`). The data (which `start_boon_*` toggles are on) is the mod's own host config, so the list is available in the keep before any run exists - no CW-run state needed.
- **Isolated, crash-proof render (untestable UI, PROJECT_STANDARDS "no-guessing" doctrine).** We do NOT append into vanilla's `_reward_widgets` (those draw in vanilla's UNguarded loop, so a boon-icon atlas not resident in the keep could assert mid-loop and kill the whole panel). Instead we build our OWN widgets and draw them in our OWN `begin_pass`/`end_pass`, each `draw_widget` individually pcall-wrapped - worst case our pass no-ops and the panel is byte-identical to vanilla. Icon and name are SEPARATE widgets per row, so a boon whose icon texture will not resolve still shows its NAME (fonts are always resident): graceful degradation, never a blank/crashing row.
- **Host-effective data.** `mod._ct_collect_start_boons` mirrors the `_add_initial_power_ups` grant enumeration (`DeusPowerUpsArray` x `start_boon_<name>`) but resolves through `effective_setting`, so a CLIENT previews the HOST's configured boons once the host-settings sync (`ct_sync_host_settings_chunk`) has landed. Display names via the existing `mod._ct_boon_display_name` (global `Localize` on `DeusPowerUpTemplates[name].display_name`); icons via `DeusPowerUpTemplates[name].icon`; modded boons flagged via `mod._ct_is_modded_power_up`.
- **Two hooks, distinct methods (singleton-clean; ct had NO hook on this class).** `IngamePlayerListUI._setup_deed_reward_data` [safe] = build point (fires once on panel activation, keep-only via `self._is_in_inn`, gated on the new `preview_starting_boons` toggle; builds onto the instance so `_draw` only draws - zero per-frame allocation). `IngamePlayerListUI._draw` [safe] = the guarded draw pass (runs after vanilla closed its pass at `:1755`).
- **Menu + command.** New `preview_starting_boons` checkbox (default ON) at the top of Starting Boons (renders below the boon categories per the groups-first rule). New `/ct_preview_boons` chat command lists the host-effective starting boons textually - the robust fallback if the panel icons do not render, and the `/verify` surface. Regression check `issue461_boon_preview_wired` (marker `CT_BOON_PREVIEW_461_MARKER` + both build helpers + toggle).
- **Files.** `chaos_wastes_tweaker_dev.lua` (feature block after the starting-boons hook + regression check), `_data.lua` (`preview_starting_boons` in `starting_boons_group`), `_localization.lua` (`preview_starting_boons` + tooltip + `ct_boon_preview_header`). `MOD_VERSION` `0.7.250-dev` -> `0.7.251-dev`. Tag `[untested]`.
- **Known limitation (v1).** Client previews use `effective_setting`, which falls back to the client's OWN `start_boon_*` toggles until the host broadcasts its config (the host broadcasts on setup_run / settings-change, not necessarily on inn entry). On the HOST the preview is always exact. Icon vertical centering + the keep-atlas residency of boon icons are the two things that need eyeballing - hence `[untested]`; the names are guaranteed regardless.
- **Verify in-game (solo host, full Steam restart, confirm `[ct:LOAD] v0.7.251-dev`).** (1) In the ct menu > Starting Boons, enable 3-4 starting boons (e.g. a Property + a couple Mod Boons). (2) In the keep, run `/ct_preview_boons` - expect a numbered chat list of exactly those boons with their rarity (modded ones flagged). (3) HOLD Tab in the keep - expect a "Starting Boons (N)" header + a vertical list of those boons (icon on the left, name to the right) on the right-hand panel. Confirm: do the icons render, and are icon+name aligned? (4) Turn `Preview Starting Boons on Tab` OFF, hold Tab again - the panel shows no boon block (byte-identical to vanilla). (5) Disable every starting boon, hold Tab - no block; `/ct_preview_boons` reports "No starting boons configured". (6) Sanity: nothing renders mid-mission (starting boons already granted), and `/ct_regression_test` shows `PASS: issue461_boon_preview_wired`.
