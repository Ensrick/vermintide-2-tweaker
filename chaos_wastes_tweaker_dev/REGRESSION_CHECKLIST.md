...
| Detection | Offline `test_peer_parity_transition.lua` covers missing, pre-roster unknown, all-acked, and leave/rejoin states plus hook order; `test_ct_boon_split.lua` proves `_ct_peer_parity_owner.lua` is the sole synchronous owner and rejects duplicate installation. `/ct_regression_test`: `peer_parity_beacon_installed`, `peer_parity_gate_classify`, `issue426_hot_join_fence`, `ct_wire_strip_name_predicate`. `/ct_426_diag` reports the live gate, catalog, synchronized-state census, and whether custom state is currently present. |
