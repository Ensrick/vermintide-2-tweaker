...
| Offline guard | `qa/rt_textual_invariants.psd1` requires the exported predicate, the single post-initialize hook, the bounded `[ct:470]` marker, and the runtime regression registration. `test_ct_sorcerer_rank_health.lua` executes the exact predicate extracted from production source and locks its unconditional hook plus targeted-loader vocabulary. The in-game `curse_sorcerer_rank8_backfill` check covers the same live table contract. |
| Reachability | The 2026-07-18 v0.7.294-dev host log proves Single Mission Loader launched `pat_forest_nurgle_path1` with Skulking Sorcerer. Closed #487 no longer blocks expedition initialization. #505's remaining upgrade-chest verification is downstream and is not required to exercise #470. |
| Live test | From the Pilgrimage Chamber, use the localized Single Mission Loader to start a normal Chaos Wastes mission at **Deepest (99.9%)**, **Cataclysm 3**, with the **Skulking Sorcerer** curse. Command equivalent: `/ct_load_mission pat_forest 0.999 curse_skulking_sorcerer cataclysm_3`. Confirm one `[ct:470]` backfill marker, active sorcerer spawns, no `math.clamp` error, and no hit-reaction crash. |
