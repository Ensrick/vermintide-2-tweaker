...
| Expected post-fix | Entry loads balance, registry, then meta/trait exactly once at the original load point; the short-lived context is cleared; entry stays at or below 10,128 physical lines (9,560 non-empty); the journey-stat guard installs once at its original boundary; the #461 helper owner installs after tooltip runtime and before its panel hooks while the singleton #461/#533/#571 `_draw` seam remains unsplit; the weapon-trait owner installs at the original generation boundary and retains the exact four-hook order, save/restore bracket, public APIs, and disable-time cache reset; the bot weapon-chest/reusable-altar owner installs after the separate grant-source hooks, retains exact `extensions_ready` -> `purchase` -> singleton `open_chest` order, a stable bot-equip surface, and late-bound resettable altar state; the Boss Grudge owner retains one stable native baseline, exact apply-hook/command order, and one settings-sync facade without taking #426 transport; the 13-command owner installs once after settings lifecycle and before the `/ct_regression_test` suite; the suite installs once via `_ct_regression.lua` at its original position (append order preserved, `mod._ct_rt_register` still owned by the entry); every module stays below 2,500 lines. |
| Detection | `qa/check_file_sizes.ps1`; `qa/check_lua_unit_tests.ps1` including `test_ct_boon_split`, `test_ct_boon_preview_helpers`, `test_ct_command_owner`, `test_ct_journey_difficulty_guard`, `test_ct_weapon_trait_generation`, `test_ct_bot_weapon_chest_owner`, `test_ct_boss_grudge_marks`, and `test_ct_entry_decomposition`; strict mod lint must report the expected hook inventory, zero duplicates, and zero forward/late-local warnings. |
...
| Field | Value |
|---|---|
| Symptom | CT dev option titles expose issue numbers or lifecycle/status tags. |
...
| Detection | Offline `test_ct_issue345_loc_status.lua`; blocking `qa/check_loc_tags.ps1`. |
...
| Field | Value |
|---|---|
| Symptom | The original #458 feature lacks its requested 0-200% boon-price control and maximum-purchases control, an implementation changes normal later shrine prices, or every starting-shop card shows `-` and cannot be bought (#1146). |
| Source boundary | The view displays/checks prices through `DeusShopView._get_power_up_costs` (`deus_shop_view_v2.lua:826-846,1265-1273`), while the authoritative controller charges inside `_try_buy_power_up` (`deus_run_controller.lua:1725-1765`). `DeusShopView._create_ui_elements` passes boolean `false` for every offer without a discount (`deus_shop_view_v2.lua:289-303`), so the CT policy must normalize exactly `false`/`nil` to zero without accepting arbitrary booleans. Remote requests are revalidated by the host at `rpc_deus_shop_power_up_bought` (`deus_run_controller.lua:1062-1082`). |
...
| Automatic detection | `test_ct_start_shrine_policy` executes all 21 price settings, malformed values, displayed/charged rounding, vanilla's exact `false` no-discount sentinel through UI decoration and a complete purchase, run/peer/local-player ledger isolation, exact-node scoping, hook consolidation, and config-before-full-sync ordering. `/ct_regression_test` validates the realized settings and module contracts. |
...
| Detection | `/ct_regression_test` in ct checks the patch is wired. In long sessions, dump `_num_states` via `/ct_flow_states` (if available). The adjacent `_ct_curse_lighting_owner.lua` owns the throttled `perf104` census; `qa/lua/tests/test_ct_curse_lighting_owner.lua` guards its hook placement, reload dispatch, and marker/window exports. |
