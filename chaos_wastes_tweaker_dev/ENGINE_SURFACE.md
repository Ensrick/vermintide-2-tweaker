...
| `DeusCursedChestExtension._set_state` [safe] `:8161` + single `mod.update` consumer `:1873` | Cursed-chest state machine (`CURSED_CHEST_STATE_OPEN = 3` server-side; hot-join clients use 4); dead players spawn linked to an assisted-respawn beacon, and `set_respawned_network(true)` begins owner-side recovery before the owner reports `health_state=alive` [src: `deus_cursed_chest_extension.lua:173-175,367-373`; `respawn_handler.lua:341-358,487-503`; `player_character_state_waiting_for_assisted_respawn.lua:30,59,93-103`] | Chest-of-Trials revive option + #299 ordered return to the living team | Hot-join clients see state 4, not 3 (AUDIT_FINDINGS #4). #299 must unlink and teleport the still-disabled player **before** sending assisted-respawn; otherwise bots can follow the temporary beacon first. In `mod.update`, select teammates from fresh `Unit.world_position` scalars, never retained `POSITION_LOOKUP` vectors (BUG_CLASSES section 21). Seed a fresh destination into `POSITION_LOOKUP` only immediately before host-local `teleport_to`, log protected-call errors, and bound recovery to 30s / four errors / one corrective move. Pure lifecycle policy: `_ct_chest_revive_policy.lua`; offline contract: `test_ct_chest_revive_teleport.lua`. |
