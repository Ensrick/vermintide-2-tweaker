...
MOD_VERSION `0.7.298-dev`, friends-only Workshop 3733366926), the ACTIVE working
...
(`:48`/`:85`), and every ct hook signature (strict mod-lint confirmed, 115 sites).
...
4,329 physical / 4,114 nonblank lines, with the boon runtime split into
`_ct_boon_balance.lua`, `_ct_boon_registry.lua`, and `_ct_meta_trait_boons.lua`,
weapon generation owned by `_ct_weapon_trait_generation.lua`, peer/version
diagnostics owned by `_ct_peer_manifest_owner.lua`, and the bulk of the
...
| `DeusShopView._get_power_up_costs` [hook] / `_update_shop_widgets` [safe] (`_ct_start_shrine_runtime.lua`) | `_get_power_up_costs` drives affordability and purchase telemetry; `_update_shop_widgets` enables/disables each offer from coins and ownership [src: `deus_shop_view_v2.lua:826-846,1265-1273`] | #458 scales only `dlc_morris_map` start-node prices and disables remaining boon offers when that local hero reaches the configured limit | Display decoration, affordability, telemetry, client prediction, and host charge all call `_ct_start_shrine_runtime.price`. Missing/malformed synthetic config disables purchase; every other shop type delegates byte-for-byte to vanilla. |
