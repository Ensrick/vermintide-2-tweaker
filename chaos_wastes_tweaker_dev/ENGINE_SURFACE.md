...
consumer resolving `get_mod("ct")` will NOT see the dev clone. The entry is a
manifest/orchestrator; engine seams may live in the explicit `_ct_*_owner.lua`
named in each row.
...
1,600 physical / 1,498 nonblank lines. Host/graph transport, run/backend
orchestration, adventure presentation, boon/grudge runtime, and settings
...
**137 hook sites** (`mod:hook`/`mod:hook_safe`, strict mod-lint) + **6 VMF RPC channels**
...
### Surface 1a - Deus run controller: run state, power-up grants, blessings (owner: `docs/engine/11`, `/03`; `chaos_wastes_tweaker_dev.lua`, `_ct_journey_difficulty_guard.lua`)

| Class.method (kind) | Vanilla behavior at the seam | Why ct hooks it | Trap / invariant |
|---|---|---|---|
| `DeusRunController.add_power_ups` [hook] `:7814` | Grants a set of power-ups to a player, appends to run-state lists, rides the run-state sync to peers [src: `deus_run_state_spec.lua:60` encode] | CONSOLIDATED grant choke point: issue-211 disable gate + #426 peer-parity filter (drop modded boon names when parity unconfirmed); covers chest/cursed-chest/shop/set-reward/end-of-level/debug + #466 bot mirror/random grants with an independent affordability gate | The single grant filter; `_ct_bot_mirror_active` grants bypass the pre-filter so the bot picker carries its OWN parity check (DEVELOPMENT "Sharp edges"). #466 charges only boon-altar grants; Chest-of-Trials/end-level rewards stay free. Never toggle-gate the wire safety (memory `reference_vt2_wire_safety_never_toggle_gated`) |
| `DeusRunController._try_buy_power_up` [hook] | Shrine-shop purchase checks ownership and currency, then writes the buyer's boon, currency, telemetry, and bought-boon state directly; it does not route through `add_power_ups` [src: `deus_run_controller.lua:1725-1765`] | Consolidated purchase choke point: #458 applies the synthetic start-node price and per-run/player boon-purchase limit; #466 mirrors or independently rolls a same-rarity boon for each bot and charges that bot's ledger | Host validates every remote purchase through `rpc_deus_shop_power_up_bought` [src: `deus_run_controller.lua:1062-1082`]. The client keeps vanilla prediction, but both peers call the same pure price/ledger policy. Normal later shrines are exact pass-throughs. Bots use their real `(peer_id, local_player_id)` ledger row and are charged before grant/refunded on failure. |
...
| `DeusRunController.setup_run` [hook] / `rpc_deus_set_initial_soft_currency` [hook] | One-time run setup writes the supplied rollover currency; a joining client sends its initial currency and vanilla adds progress catch-up coins [src: `deus_run_controller.lua:273-330,350-382`] | Starting Coins replaces the rollover baseline with the exact configured integer, including zero (#912); #466 initializes each present bot's independent ledger from the host's starting balance; #467 emits a bounded boon census; #919 snapshots the profile/effective setting before consumption | The two concerns stay in the existing consolidated hooks. Zero is authoritative, not a vanilla-pass-through sentinel; every setup replay rewrites arg 5. The host RPC replaces only the client's baseline, preserving vanilla progress catch-up. Invalid settings fail open. |
| `DeusRunController.get_run_difficulty` [hook] `chaos_wastes_tweaker_dev.lua:2845` / `StatisticsUtil._register_completed_journey_difficulty` [hook] `_ct_journey_difficulty_guard.lua:33` (installed at entry `:2870-2871`) | Reports run difficulty; records completed-journey difficulty into career stats | Progressive difficulty override; clamp the RECORDED difficulty so a ramped run never writes a ceiling the stat array lacks | Journey-stat cataclysm-ceiling CTD (#291, memory `reference_vt2_journey_stat_cataclysm_ceiling_crash`) - clamp before the stat write |
...
| `DeusWeaponGeneration.generate_weapon` / `generate_weapon_for_slot` / `generate_item_from_item_key` / `upgrade_item` [hook] `_ct_weapon_trait_generation.lua:373-391` (installed at entry `:4176-4181`) | Deterministic CW weapon rolls (properties/traits from run_progress+rarity+seed) | Temporarily expand/filter source trait pools, call vanilla, restore source tables, then apply rarity-tier and final ban policy to the detached result | One owner and exact hook order. Preserve nilable arity and restore `DeusWeapons` even when vanilla throws. The melee/ranged class cache and fallback global-union cache are reset together on disable. `_generate_upgraded_weapon` is DISTINCT from `_generate_stored_weapon`; upgrade-altars use their separate per-go_id reuse seed mix (CHANGELOG :805). |
...
|---|---|---|---|
| `DeusChestExtension.open_chest` [hook] `_ct_bot_weapon_chest_owner.lua:351` | Opens an altar/chest, grants its stored reward, then completes purchase/equip work | CONSOLIDATED wrapper: publishes the exact pre-purchase altar price for #466 bot affordability, then runs all existing altar-reuse + reward post-work | The canonical `(DeusChestExtension, open_chest)` singleton: call vanilla exactly once, restore the short-lived price context even on error, and keep all post-work in this body. Burned v0.7.129/.130 with a 2nd hook on the pair |
| `DeusChestExtension.update` / `get_purchase_cost` / `_generate_stored_power_up` / `_generate_stored_weapon` / `_generate_upgraded_weapon` / `update_upgrade_chest_color` / `can_be_unlocked` [hook/safe] `_ct_altar_reuse_owner.lua`; `extensions_ready` / `purchase` [safe/hook] `_ct_bot_weapon_chest_owner.lua:273,307` | Per-frame chest tick, cost query, stored-reward generation, upgrade-chest recolor, unlock gate, spawn init, purchase | Altar cost/mix tuning, reward-seed mixing, walk-through collision setup at the earlier entry-owned `extensions_ready` seam, diagnostic subscription at the owner seam, reuse economy | Walk-through uses `Actor.set_collision_filter(actor,"filter_trigger")` NOT `set_scene_query_enabled(false)` (breaks "press E", DEVELOPMENT "Walk-through pattern"); the extracted diagnostic subscription remains once per live EventManager and the purchase filter always restores `Unit.flow_event` |
| `DeusCursedChestExtension._set_state` [safe] `_ct_chest_revive_owner.lua:164` + single `mod.update` consumer (entry `:1299` -> `_ct_chest_revive_owner.lua:515`) | Cursed-chest state machine (`CURSED_CHEST_STATE_OPEN = 3` server-side; hot-join clients use 4); dead players spawn linked to an assisted-respawn beacon, and `set_respawned_network(true)` begins owner-side recovery before the owner reports `health_state=alive` [src: `deus_cursed_chest_extension.lua:173-175,367-373`; `respawn_handler.lua:341-358,487-503`; `player_character_state_waiting_for_assisted_respawn.lua:30,59,93-103`] | Chest-of-Trials revive option + #299 ordered return to the living team | Hot-join clients see state 4, not 3 (AUDIT_FINDINGS #4). #299 must unlink and teleport the still-disabled player **before** sending assisted-respawn; otherwise bots can follow the temporary beacon first. In `mod.update`, select teammates from fresh `Unit.world_position` scalars, never retained `POSITION_LOOKUP` vectors (BUG_CLASSES section 21). The one `POSITION_LOOKUP` write is a guarded in-place REFRESH of an entry the engine already maintains, immediately before host-local `teleport_to` (which reads `.z` via `set_falling_height`); it must never SEED a missing entry, because that flips `ALIVE[unit]` truthy for every consumer and leaves expired frame-pool userdata behind. Log protected-call errors and bound recovery to 30s / four errors / one corrective move. Pure lifecycle policy: `_ct_chest_revive_policy.lua`; offline contracts: `test_ct_chest_revive_teleport.lua`, `test_ct_chest_revive_owner.lua`. |
...
| `DeusCursedChestExtension.on_server_interact` [hook] `_ct_cot_cost.lua` | Sole authoritative WAITING -> RUNNING activation; receives the real interactor unit [src: `deus_cursed_chest_extension.lua:293-302`] | #63 optional absolute per-activator Pilgrim's Coin debit for native and injected Chests of Trials | Cost 0 is a strict vanilla pass-through. Positive values are 0-1000, snap to 25, and default to 0. Host resolves owner from interactor; no buyer RPC. Reserve/refund transaction commits only after state leaves WAITING. Rewards are not charged. Prompt key composes into the existing `get_interaction_action` and single `_G.Localize` hooks, never duplicate-hooks them. |
| `DeusShopView._create_ui_elements` / `DeusCursedChestView.create_ui_elements` [safe] + `DeusShopView.update` / `DeusCursedChestView.update` [hook] `_ct_boon_offer_view_owner.lua` | Shrine-shop and cursed-chest view build + per-frame tick: both lay their offer widgets on a fixed vertical arc at build time and never scroll | Sole owner of boon-offer LAYOUT: the degenerate-arc NaN offset repair, and the #115/#114 scroll affordance (row-snapped reflow, off-window parking at y = -20000, injected track+thumb widget, wheel/drag/track-page input) | NaN widget-offset fix only needed for the 1-widget case (AUDIT_FINDINGS #6). One registration per (Class, method) pair, so the build body also dispatches `_ct_start_shrine_runtime.decorate_shop` and `_ct_boon_pricing_runtime.enforce_shop` - both resolved off `mod` behind a nil guard because those modules install later. `update` wraps (not safe) so reflow precedes vanilla draw in the same frame; every path is pcall-guarded and degrades to no-scroll. Offline contract: `test_ct_boon_offer_view_owner.lua` |
| `DeusCursedChestView._on_button_pressed` [hook] entry `:5831` / `DeusUpgradeWeaponInteractionUI._populate_widget` [safe] `_ct_altar_reuse_owner.lua` | Cursed-chest pick button; upgrade-altar widget build | #211 grant-source tagging around the pick (provenance, not layout); reroll button on the upgrade altar | VMF numeric widget has no `step` (memory `reference_vmf_numeric_widget_no_step`), so #458's 0-200% control is a 21-value dropdown rather than a numeric pseudo-step |
| `DeusShopView._get_power_up_costs` [hook] / `_update_shop_widgets` [safe] (`_ct_start_shrine_runtime.lua`) | `_get_power_up_costs` drives affordability and purchase telemetry; `_update_shop_widgets` enables/disables each offer from coins and ownership. `_create_ui_elements` writes boolean `false` for a normal non-discounted offer and a number only when `i <= max_discounts` [src: `deus_shop_view_v2.lua:289-303,826-846,1265-1273`] | #458 scales only `dlc_morris_map` start-node prices and disables remaining boon offers when that local hero reaches the configured limit; #1146 normalizes vanilla's exact `false` sentinel to zero at the price-policy boundary | Display decoration, affordability, telemetry, client prediction, and host charge all call `_ct_start_shrine_runtime.price`. Missing/malformed synthetic config disables purchase; `true` and nonnumeric discount values remain invalid; every other shop type delegates byte-for-byte to vanilla. |
| `IngamePlayerListUI._setup_mission_data` [hook] / `_setup_deed_reward_data` + `_draw` [safe] `:8320-8790` | Builds Adventure collectible widgets from `loot_objectives` with two entries per row and widget-local measured offsets (`ingame_player_list_ui_v2.lua:436-513`); the draw pass later applies the parent scenegraph transform (`ui_renderer.lua:340-344,406-410`) | #533 replaces impossible Adventure book counters with live DEUS chest/coin counts during a run; #461 previews Starting Boons only on the exact `morris_hub` Pilgrimage Chamber level, including before queueing; #1004 adds canonical localized boon descriptions on native-cursor icon hover; #571 ports the vanilla collectible offset loop for those CT rows | One consolidated `_draw` hook. #1004 consumes `self._cursor_active` and hotspot state without owning `update`, `_set_active`, or cursor capture/release. Mouse pages use native `scroll_axis`; controller pages use source-proven `right_press` + `right_hold` (`right_shoulder`/`r1`) with an explicit latch and active-device hint. Tooltip construction is lazy on first hover. Canonical text is capped at 16 KiB, layout at 16 pages / 256 measurements, and oversized/pathological input fails neutral. It measures wrapped text through `UIUtils.get_text_height` on `_ui_top_renderer`, bounds the tooltip to the production 660x1080 `banner_right` / `{20,-700}` `reward_divider` geometry, shrinks 18-14pt, then pages supported overflow without dropping text. #461 and #505 share `_ct_pilgrimage_context.lua`; mechanism or matchmaking state is too broad/late for physical chamber context. Compute custom-row offsets from icon/text geometry exactly as vanilla does; never feed the animated `banner_right.world_position` back into widget offsets. `gui_tweaker_dev` also suppresses vanilla Deus mission rows through a full `_setup_mission_data` wrapper, so CT's draw seam owns a one-shot recovery constructor and cannot depend on cross-mod hook order. Stock Adventure calls vanilla unchanged. |
...
| `GameModeMapDeus.local_player_game_starts` [hook] / `DeusShopView.start` [hook] (`_ct_start_shrine_runtime.lua`) | Sets initial MAP_DECISION and calls `full_sync()` (`game_mode_map_deus.lua:144-159`); SHOP view immediately indexes `DeusShopSettings.shop_types[current_node.level].blessings/power_up_count` without a nil check (`deus_shop_view_v2.lua:179-184`) | #458 Buy Starting Boons: every peer registers and validates `shop_types["dlc_morris_map"]` **before** vanilla startup/full-sync; host publishes SHOP only after validation; the view-start guard rebuilds or returns to MAP_DECISION before vanilla dereferences | These are ct's only hooks on the two exact pairs. Never move peer config creation after `func`: a client can consume replicated SHOP inside vanilla `full_sync()`. Config validity includes boon count, blessing list, 10-step price percentage, and purchase limit. `states.MAP_DECISION=1`, `states.SHOP=3` are file-local constants (`:45-49`). Never call `handle_shrine_entered`; vanilla SHOP->WAITING->MAP_DECISION (`:341-349`) returns to the map. |
...
| Class.method (kind) | Vanilla behavior | Why ct hooks it | Trap / invariant |
|---|---|---|---|
| `BuffExtension.add_buff` [hook] `_ct_meta_trait_boons.lua:1886` | Adds a local buff instance from a template name [src: `buff_extension.lua`] | Meta-boon stacking + boon-application observation | Consolidated buff-apply observation; resolve template from `ProcFunctions[name]`, not `BuffFunctionTemplates.functions` for merged CW procs (combat-hooks banner :75) |
| `BuffFunctionTemplates.functions.apply_pockets_full_of_bombs_buff` [hook,tbl] `_ct_meta_trait_boons.lua:1963` / `remove_deus_potion_buff` [hook,tbl] `:1980` | Bomb-boon apply callback / deus-potion buff removal | Endless-bombs cooldown, potion-duration tuning | Table-form on `BuffFunctionTemplates.functions[key]`; the apply-callback category lives in `buff_function_templates` (unlike `chain_lightning`, a merged `proc_functions` entry) |
| `ProcFunctions.chain_lightning` / `lightning_adjecent_enemies` / `mark_of_nurgle_explosion` / `career_ability_apply_dot_to_adjecent_enemies` / `boon_dot_burning_01_spread` [hook,tbl] `_ct_combat_hooks.lua:90-1077` + `drop_item_on_ability_use` / `grenade_explode_buff_area` [hook,tbl] `chaos_wastes_tweaker_dev.lua:10886`/`:10942` | CW boon/trait proc functions: chain lightning, AoE lightning, Nurgle gas explosion, career-ability DoT spread, Wildfire spread, ability-drop, grenade-area | Manann's Tempest 8s cooldown + #358 owner timer, AoE cap crash-guard (#129), gas-cloud rate cap (#104), Wildfire colour-match by source burn, bomb-boon tweaks; #357 owner timer | `ProcFunctions` is the GLOBAL merged table (`DLCUtils.merge("proc_functions",...)` at `buff_templates.lua:9533`), NOT `BuffFunctionTemplates.functions`; #357/#358 stamp their gameplay gate first, then send at most one owner-targeted display event. Manann boon/trait buckets use distinct local templates; no display drives timing |
| `GenericAmmoUserExtension._apply_buffs` [safe] `_ct_combat_hooks.lua:1308` / `use_ammo` [hook] `_ct_meta_trait_boons.lua:486` / `PlayerUnitEnergyExtension.drain` [hook] `:507` / `PlayerUnitOverchargeExtension.add_charge` [hook] `:545` | Ammo max/consume, Moonfire energy drain, overcharge add | Larger-clip boon (#34), meta-ammo boons (per-cast reduction) | Consumption-side only - never mutate `_max_*` (memory `feedback_vt2_max_resource_consumption_side`); `max_overcharge` >60 fatals the network bound - use `reduced_overcharge` (DEVELOPMENT "max_overcharge"). v0.7.338-dev (#249/#289): the meta-boon STACK grant is server-authoritative - host adds via `BuffSystem.add_buff(unit, name, unit, true)` (replicates + hot-join re-send [src: `buff_system.lua:277-311,66-97`]), clients defer; networked path parity-gated by `mod._ct_wire_safe` (#426), fallback = host-local add. Pure kernel `_ct_ammo_guard_core.lua` (grant_plan + #256 clamp_value) |
| `ActionChargedProjectileUtility.fire_charged_projectile` [hook] `_ct_combat_hooks.lua:1210` / `PlayerUnitHealthExtension.sync_health_state` [hook] `_ct_chest_revive_owner.lua:534` / `RespawnHandler._respawn_player` [safe] `_ct_chest_revive_owner.lua:554` | Charged-projectile fire, health-state sync, player respawn | Projectile boon tuning, health-boon reapply on respawn | Respawn reapply feeds the same persistent-buff path as `DeusSpawning._apply_initial_buffs` |
...
| Class.method (kind) | Vanilla behavior | Why ct hooks it | Trap / invariant |
|---|---|---|---|
| `MutatorHandler.tweak_pack_spawning_settings` [hook] (`_ct_level_load_owner.lua`) | Applies zone + per-pack mutators' `tweak_pack_spawning_settings` to a pack; `no_roamers` iterates `pack_spawning_settings.difficulty_overrides` with `pairs()` [src: `mutator_no_roamers.lua`] | Strip `no_roamers` on adventure-injected levels where `difficulty_overrides` is nil (else `pairs(nil)` fatals, #41) | **RESOLVED v0.7.241-dev (#356) - arity trap.** Vanilla is a STATIC function (no `self`), called dot-form at [src: `main_path_spawning_generator.lua:327`], def [src: `mutator_handler.lua:748`]. The ct hook signature `function(func, self, zone_mutator_list, ...)` shifts every arg by one: `pack_spawning_settings` is ALWAYS nil (strip runs every call) and the SIGNATURE-zone `zone_mutator_list` - where `no_roamers` actually lives (DEVELOPMENT "adventure mutators") - is the ONE list NOT filtered. The fix dropped `self` and binds the four real params in vanilla order; it is pinned by `CT_NO_ROAMERS_ARITY_FIX_MARKER`, the `/ct_regression_test` check `no_roamers_strip_arity_356`, and a path-pinned needle in `qa/rt_textual_invariants.psd1`. Verify against `docs/engine/07` before touching |
...
| `MutatorHandler.init` [hook] (`_ct_weave_metal_runtime.lua`) | Constructor: the server initializes every listed mutator inside init [src: `mutator_handler.lua:27-48`], the initialized map is shared state [src: `mutator_handler.lua:95-99`], hot join replays active entries [src: `mutator_handler.lua:148-170`] | #253 Metal wind adapter: host + Deus + adapter on = hand vanilla a COPY of the list with `metal` appended before init, so vanilla transport stays authoritative | Default-off, `/ct_weave_metal` only; never mutates the composed list; every vanilla parameter named and passed back; singleton (`_ct_consolidated_mutator_handler_init_hook`) |
| `MutatorHandler.destroy` [safe] (`_ct_weave_metal_runtime.lua`) | Deactivates every active mutator with `is_destroy` [src: `mutator_handler.lua:60-83`]; the wrapped stop restores armor [src: `mutator_templates.lua:63-73,134-138`] | #253: prove the eleven breeds' `primary_armor_category` returned to the pre-init snapshot, then drop the per-level context | Read-only after vanilla; no-op unless the adapter armed this level; singleton (`_ct_consolidated_mutator_handler_destroy_hook`) |
| `metal_server.start_function` [hook] (`_ct_weave_metal_runtime.lua`) | The wrapped SERVER start of `MutatorTemplates.metal` [src: `mutator_handler.lua:678-682`]; the vanilla body reads the global Weave manager [src: `mutator_metal.lua:58-63`] | #253: while a CT level context is armed, skip the vanilla body and supply `data.wind_strength`/`data.buff_system` from the explicit `{ wind, wind_strength }` context | A different table from the Isha `server_tbl` hook; the skipped default only handles `remove_pickups` [src: `mutator_templates.lua:107-132,260-268`], asserted absent by `issue253_metal_wind_adapter`; real Weaves run vanilla untouched |
...
| `ConflictDirector.start_terror_event` [hook] `_ct_combat_hooks.lua:374` / `TerrorEventMixer.start_event` [hook] `:491` / `TerrorEventMixer.init_functions.spawn_around_origin_unit` [hook,tbl] `:297` | Start a terror event / mixer event / spawn-around-origin init step | Chest-of-Trials enemy-count multiplier (#64), seed uniqueness + force-rotation (#117), Skaven Warlord trial (#324), spawn-composition diagnostic (#471) | Per-mission GLOBALS `_ct_cursed_chest_seq`/`_ct_cot_block_last` shared with main-file setup_run reset - stay globals, never localized (combat-hooks cross-file contract). #471 (v0.7.248): the spawn hook now also emits `[ct:471] cot_spawn` (raw printf: `pre_req`/`built_req`/`placed`) per cursed-chest element; scale reads `element.difficulty_amount` (vanilla `terror_event_mixer.lua:97-131`), actual spawns = `#event.spawn_positions` after the run fn (`:1043`) - a placed<built_req gap = position-finder/budget cap, not a scale miss. `cursed_chest_elites` category is diagnosed but NOT scaled today. #471 FIX (v0.7.304): the diag proved a finder cap (`built_req=54 placed=23`; even mult=1 lost one) - the engine gives each slot ONE separation-filter attempt (`break` inside `if spawn_pos`, [src: `conflict_utils.lua:1678-1693`]) and the fixed annulus (radius 8 +/- spread/2, [src: `deus_generic_terror_events.lua:95-100`]) saturates at ~23 positions at 2 m spacing; the hook now tops `event.spawn_positions` up to `#event.spawn_table` post-call by re-driving `ConflictUtils.find_positions_around_position` over the bounded widening pass plan in `_ct_cot_placement_policy.lua` (both cursed-chest categories, always-on, host-only, pcall-degrades to vanilla; new positions keep spacing + telegraph decal). Residual shortfall is named per `[ct:471]` line |
| `TerrorEventUtils.apply_breed_enhancements` [hook,tbl] `_ct_boss_grudge_marks.lua` / `EnemyPackageLoader.setup_startup_enemies` [hook] (`_ct_level_load_owner.lua`) | Apply per-difficulty breed enhancements / preload the mission's breed packages | The Grudge owner strips host-banned marks at the universal apply choke point; the separate injected-level path force-loads breeds | Terror-event files capture `TerrorEventUtils.*` as upvalue at BOOT before mods load. The owner also restores/mutates the `_G.BossGrudgeMarks` data read at call time, retains one stable native baseline across reloads, and never owns settings transport (CLAUDE.md upvalue-capture; the v0.7.76 grudge-mark failure). |
...
| `DeusCursedChestExtension.extensions_ready` [safe] + the delayed pickup census, both `_ct_pickup_population_owner.lua` (#1159) | Extension seam fires once for every live cursed chest regardless of spawn path. `_spawn_pickup` synchronously calls `UnitSpawner.spawn_network_unit` -> `create_unit_extensions` -> `EntityManager2.add_unit_extensions` -> `extensions_ready` before returning [src: `deus_cursed_chest_extension.lua:39-51`, `pickup_system.lua:1208-1274`, `unit_spawner.lua:327-352`, `entity_manager2.lua:120-176`] | #132 per-chest ground truth; #349 settled Mission-of-Mercy count classification; v0.7.338-dev feeds the unit into the #132/#60 reconcile ledger (`_ct_diag_cursed_chest132.lua`), whose settled finalize PRUNES over-cap chests - host-only, deus-only, explicit-cap-only, pickup-path units still in WAITING only, via the engine's own delete path [src: `pickup_system.lua:1345,1451-1455`; state field `deus_cursed_chest_extension.lua:13,367-373`] | Never compare the in-hook pickup census as final: CT's `_spawn_pickup` post-hook has not resumed yet. Classify only from the existing delayed census after the spawn pass; compiled level-unit flags are outside the Lua source dump. The reconcile never deletes raw baked level units or non-WAITING chests (reported `unprunable`), and never fires under the Default (-1) cap |
| `LimitedItemTrackSystem.on_add_extension` / `register_group` / `activate_group`; `LimitedItemTrackSpawner.spawn_item` / `socket_item`; `MissionSystem.flow_callback_update_mission` [hook] `_ct_diag_gargoyle1124.lua` | Old Haunts registers authored limited-item spawners, resolves `gargoyle_head_spawner_vs` to configured pickup or default `gargoyle_head`, spawns and retains each item, records sockets, then advances the manual `portals_survive` collect mission to 4/4 [src: `limited_item_track_system.lua:191-233`; `limited_item_track_spawner_templates.lua:472-522`; `limited_item_track_spawner.lua:42-67`; `objective_socket_system.lua:42-54`; `mission_system.lua:279-303,369-380`; `penny_level_settings_part_1.lua:62-66`] | #1124 development-only evidence ledger for the exact injected `dlc_portals` objective path: authored identity, group state, spawn attempts vs confirmed/valid network pickups, sockets, and mission completion | Observation only. Exact injected-base gate before object reads; vanilla once outside `pcall`; all arguments/returns preserved; pre/post read faults isolated; raw `[ct:1124]` output capped at 96/run and 512/process without suppressing vanilla. Never add these heads to `pickup_settings`, mutate the spawner ledger, or turn this diagnostic into a gameplay fix without live evidence |
...
| **Table contact (NOT hooks):** `_adventure_pool.lua` `inject_pool()` writes `LevelSettings[<key>]` + `LEVEL_AVAILABILITY.*` | `num_levels` = count of `LevelSettings` at join, folds into `combined_hash`; populate resolves `pickup_settings[difficulty]` and falls back to `.default` with a warning on a missing key [src: `pickup_system.lua:409-419`] | Inject Adventure missions x 6 themes into TRAVEL/SIGNATURE pools with `_dupN` safety aliases; v0.7.338-dev (#251): `make_cw_pickup_settings` emits FRESH per-key entries for every reachable difficulty (normal..cataclysm_3 [src: `difficulty_settings.lua:402-411`]) so no run rides the fallback | STICKY - can't un-register; toggling OFF mid-session does NOT reduce `num_levels` (restart only); ARENA/SHOP/finale/Belakor node types NEVER touched (`_adventure_pool.lua` header); per-difficulty entries must stay distinct tables (the populate hook mutates them in place) |

### Surface 5 - Economy / traits / potions / lifecycle + peer-parity beacon (owner: `docs/engine/11`, `/03`; `chaos_wastes_tweaker_dev.lua`, `_ct_peer_parity_owner.lua`, `_lib_peer_parity.lua`)

| Class.method (kind) | Vanilla behavior | Why ct hooks it | Trap / invariant |
|---|---|---|---|
| `DeusRunController.on_soft_currency_picked_up` [hook] `:653` | Awards soft currency (coins) on pickup; server-branch skips counters when 2nd arg is nil | Coin multiplier / economy tuning; #466 credits every active bot ledger with the final multiplier-adjusted host pickup | Boon-count detection by value-range scan `[1,10]` not fixed arg index (defends signature drift, AUDIT_FINDINGS #3); bot credit happens after vanilla so the ledger sees the effective amount |
| `_G Localize` [hook] `chaos_wastes_tweaker_dev.lua:5929` | Global loc-key -> string lookup; returns `<key>` for unknown keys | Supply display names/descriptions for `ct_*`/`power_up_ct_*` boon + miracle keys | VMF `_localization.lua` is NOT registered into global `Localize` (memory `reference_vmf_localize_before_registration`); resolve `DeusPowerUpTemplates[name].display_name` through the wrapped call |
| `VMFOptionsView.callback_setting_changed` [hook] `chaos_wastes_tweaker_dev.lua:11487` | VMF settings-menu value-change callback | Mutex-cluster (Miracle of Isha A/B) + re-apply per-boon tweaks + re-sync host-dependent state on edit | `cb_<setting>` takeover pattern (memory `reference_vt2_optionsview_synthesized_cb_takeover`); the sync fan-out (`sync_host_dependent_state`) re-runs pool registration, so it MUST route through the parity-guarded `_add_dormant_to_pool` |
...
#253 has two parts. `_ct_weave_curse_audit.lua` stays observation-only: it reads
...
`_ct_weave_metal_runtime.lua` is the first slice of that boundary ("Metal first"):
a host-owned, default-off, `/ct_weave_metal`-gated adapter that appends `metal` to
a host Deus level's mission list in the `MutatorHandler.init` pre-hook, replaces
the template's only Weave-manager read at the wrapped server start with an
explicit `{ wind, wind_strength }` context, and proves armor restoration in the
`MutatorHandler.destroy` safe hook (rows in Surface 3). It never touches
`Managers.weave`, `node.curse`, the graph or any reward contract, and it has no
widget until the two-player verification matrix in
`REGRESSION_CHECKLIST.md` ("Metal wind curse adapter") passes.
