...
| `DeusRunController._try_buy_power_up` [hook] | Shrine-shop purchase writes the buyer's boon and coin state directly; it does not route through `add_power_ups` | #466 mirrors or independently rolls a same-rarity boon for each bot and charges that bot's ledger at the vanilla discounted shop price | Host-authoritative and owner-purchase-only; each bot is charged before grant and refunded on grant failure. Selection keeps the same disabled-boon and peer-parity gates as the consolidated grant path |
