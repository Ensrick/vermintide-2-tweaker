...
--
-- Owns the core ct_dev runtime regression checks (issue-locking + structural
...
-- The final ten resource-safety checks live in a bounded child installer that
-- is loaded before registration and invoked at their original tail position.
--
...
-- against; the child receives the same immutable context. One check that reads mutable per-run state
...
local _install_resource_safety_checks = mod:dofile(
    "scripts/mods/chaos_wastes_tweaker_dev/_ct_regression_resource_safety")
assert(type(_install_resource_safety_checks) == "function",
    "CT resource-safety regression installer is required")
...
_install_resource_safety_checks(mod, ctx)
end
