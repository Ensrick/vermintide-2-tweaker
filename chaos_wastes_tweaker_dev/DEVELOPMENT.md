...
**The gate (v0.7.240-dev):** peer-parity beacon - a verbatim copy of `tools/shared_lib/_lib_peer_parity.lua` (master; DO NOT edit the copy - edit the master and re-copy) instantiated on channel `ct_peer_parity_present` with `CT_RPC_SCHEMA`. Fail-safe: modded content inert until every other human peer positively acks; solo enables immediately; beacon failure keeps content off. Five surfaces, all in `chaos_wastes_tweaker_dev.lua` (search `[ct:426]`):
...
- Honest residual: a non-ct peer hot-joining a run that already granted modded boons can win the race against beacon detection (~0.5-1s). Not closable from Lua; the gate prevents the class for pre-run lobbies and shrinks the mid-run window.
