...
3. `_ct_meta_trait_boons.lua` owns CT-authored trait boons, resource hooks, buff observation, and Endless Bombs lifecycle hooks. New CT boon behavior belongs here.
4. `_ct_peer_parity_owner.lua` owns the complete synchronous #426/#1191 wire-safety control plane: exact-catalog beacon, pool gating, full-state strip/census, diagnostic, hot-join/remove-peer hooks, both update branches, Mod Tweaker bridge, public `mod._ct_*` parity surfaces, and their eight runtime checks. New CT wire-safety behavior belongs here.

`chaos_wastes_tweaker_dev.lua` constructs a short-lived `mod._ct_boon_runtime_context`, then `_ct_settings_lifecycle_owner.lua` loads balance, registry, and the meta/trait wrapper exactly once at the original contiguous block's load point before clearing that context. The wrapper synchronously invokes `_ct_meta_boon_owner.lua` and `_ct_peer_parity_owner.lua` at their former inline positions; those two explicit nested owners never self-load elsewhere. Do not move hooks to a later lifecycle callback or take ownership of settings/RPC state already owned by the entry. Preserve this order because registry consumers require both earlier contracts, runtime-check append order is observable, and `_ct_stack_rebroadcast_owner.lua` immediately consumes `mod._ct_peer_parity`.

The structural guard is `qa/lua/tests/test_ct_boon_split.lua`; the repository size ratchet is `qa/check_file_sizes.ps1`. The entry remains at its completed 1,498 canonical-line ceiling in `qa/decomposition_contracts.psd1`; the trait wrapper and both nested owners must each remain below the 1,500-line target.

### Peer-parity owner (`_ct_peer_parity_owner.lua`) — issues #2 / #426 / #1191 / #249

The owner is a side-effect-free factory until `_ct_meta_trait_boons.lua` invokes its returned installer. The wrapper passes the existing runtime-check registrar, setting-id collector, RPC schema, dormant registry functions/state, trait catalog, and trait pool callback explicitly. Before touching parity state, the installer validates those dependencies plus `mod.dofile`, `mod.hook`, `mod.command`, and `mod.echo`, snapshots the exact raw public/update surface (including false or missing values), and reserves a terminal `false` marker. A second invocation therefore rejects before another receiver, hook, update wrapper, command, check, or public export can be registered, whether the first invocation committed or failed partway.

The moved implementation remains one transaction-shaped lifecycle owner. It finalizes the exact catalog and channel, ejects modded pools first, then performs every throwable command/gate/feature/hook/check registration while their retained callbacks are inert behind the false marker. The beacon's receiver/update install is the final irreversible call. A false/throwing install restores the exact raw public/update snapshot but leaves the false marker terminal; success performs only local/raw publication, restores the two-layer update order, and flips the marker true. It owns one `hot_join_sync` and one `remove_peer` hook and preserves the public `mod._ct_*` fields plus all eight runtime-check names/order. `_ct_stack_rebroadcast_owner.lua` remains the immediate next consumer and must never load before this installer returns. `qa/lua/tests/test_ct_boon_split.lua`, `test_peer_parity_transition.lua`, `test_peer_parity_install_transaction.lua`, and `test_ct_426_exact_catalog.lua` guard the extraction and the retained-callback failure matrix.

### `/ct_regression_test` suite owners (`_ct_regression*.lua`) — issue #2 / OOP W5

The core `/ct_regression_test` suite remains a pure `return function(mod, ctx)` installer in `_ct_regression.lua`, dofiled once at the suite's original entry position. It preloads `_ct_regression_resource_safety.lua` before registering any check, then invokes that child only after `localization_format_safe`; the child owns the exact final ten registrations from `mission_catalog_localization_format_safe_564` through `cursed_chest_reconcile_132`. This preserves append/print order while keeping both owners below the 1,500-line target. Both use the entry-owned `mod._ct_rt_register`; neither creates a second `_RT_CHECKS`.

The shared context retains the existing immutable captures. The child consumes only the two `_dump_pickup_*` forward declarations and `CT_VARIADIC_ARITY_MARKER` from it; `mod._ct_*` surfaces and marker globals such as `_CT_CHUNK_*` stay late-bound at check-call time. ONE mutable-state check stays inline in the entry: `starting_coins_value_matches_setting` reads `_starting_coins_applied_for_run`, which the `setup_run` hook updates per run. Put a new cross-owner safety check in the bounded child only when it belongs after the existing ten; otherwise register it with the module that owns the guarded behavior. The structural guards are `qa/lua/tests/test_ct_entry_decomposition.lua` and `test_ct_regression_resource_safety.lua`.
...
### Metal wind curse adapter owner (`_ct_weave_metal_runtime.lua`) - issue #253

Pure installer `return function(ctx)` with `ctx.mod` required; every engine seam (`templates`, `breeds`, `lookup`, `mechanism_name`, `is_server`, `buff_system`, `get_setting`, `set_setting`, `log`) defaults to the live game and is injectable for tests. The entry installs it exactly once, directly after `_ct_weave_curse_audit`, as `({ mod = mod })` (the entry sits at the 200-local ceiling and the frozen 1498-line completion ceiling). The engine-free contract lives in `_ct_weave_metal_policy.lua`: the `{ wind, wind_strength }` context, the `/ct_weave_metal` grammar, the pure `compose_mutators` list copy, and the armor snapshot/mismatch helpers. The owner registers three singleton seams (`MutatorHandler.init` pre-hook, `MutatorHandler.destroy` safe hook, and a table-form hook on the wrapped `MutatorTemplates.metal.server.start_function`, a different table from the Isha `server_tbl` hook), one command (`ct_weave_metal`, whose callback is a straight line: helper call, one literal `[ct:253:diag]` receipt, one echo), and one regression check (`issue253_metal_wind_adapter`). State is exposed as `mod._ct_weave_metal_state`; `enabled` is session-only and default-off, `strength` persists in `weave_metal_strength`. Add further winds as sibling owners with their own policy module; never widen this owner into a multi-wind switch. Guards: `qa/lua/tests/test_ct_weave_metal_policy.lua`, `test_ct_weave_metal_runtime_owner.lua`, and the #253 needles in `qa/rt_textual_invariants.psd1`.

### Journey-stat difficulty guard (`_ct_journey_difficulty_guard.lua`) — issue #291

The verified journey-completion crash guard is a direct `return function(mod)` installer loaded once after replacement-progression setup and before the consolidated `DeusRunController.setup_run` hook. Since 0.7.336-dev all three of those neighbours live inside `_ct_run_creation_owner.lua` (below), which is what loads this guard; the relative order is unchanged, and that owner's install site is where the entry used to hold it. It owns the sole CT hook on `StatisticsUtil._register_completed_journey_difficulty` and assigns `CT_JOURNEY_DIFFICULTY_GUARD_MARKER` synchronously before `_ct_regression.lua` installs. The 39-line implementation is byte-identical to its former entry block: only an unsupported RECORDED journey difficulty is clamped to the final value returned by `get_default_difficulties`; gameplay difficulty is untouched. It captures only `mod`; engine globals remain late-bound. Do not fold progressive-difficulty state, settings synchronization, RPCs, or logging migrations into this owner. `qa/lua/tests/test_ct_journey_difficulty_guard.lua` guards hook cardinality/order, pass-through arity and multiple returns, clamp behavior, marker availability, test-global cleanup, and the optional decompiled vanilla source contract.
...
`_ct_peer_parity_owner.lua`, synchronously installed by `_ct_meta_trait_boons.lua`,
which loads LATER. Every call site here is written
...
**The gate (v0.7.240-dev, hot-join fence v0.7.291-dev, exact catalog v0.7.322-dev):** peer-parity beacon - a verbatim copy of `tools/shared_lib/_lib_peer_parity.lua` (master; DO NOT edit the copy - edit the master and re-copy) instantiated by `_ct_peer_parity_owner.lua` on channel `ct_boon_catalog_exact_v1` with `CT_RPC_SCHEMA` and `opts.wire_identity`. Fail-safe: modded content inert until every other human peer positively acks; solo enables immediately; beacon failure keeps content off. Six surfaces, all in that owner (search `[ct:426]`):
