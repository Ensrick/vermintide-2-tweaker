<#
.SYNOPSIS
  Write gut_mod_settings.toml from a Tweaker: GUI /gut_export_settings log dump.
...
.DESCRIPTION
  ...
  Edit that file by hand; the game reads + applies it on load (or /gut_reload_config).

  Round-trip:
    in-game  ->  /gut_export_settings        (dumps TOML to the log)
    desktop  ->  .\gut-settings.ps1           (writes the .toml from the log)
    you      ->  edit gut_mod_settings.toml
    in-game  ->  restart, or /gut_reload_config
...
if (-not $tomlLines) { throw "No [gut:toml] lines in the log. Run /gut_export_settings in-game first." }
...
Write-Host "Edit it, then restart the game or run /gut_reload_config."
