...
local function _to_toml(by_mod)
    local out = {
        ...
        "# Edit the values below. On game load (or /gut_reload_config) these OVERRIDE",
...
local function apply()
    if not mod:get("gut_config_override") then return 0 end
...
mod:command("gut_export_settings", "Dump your tweaker mods' settings as TOML to the log (companion writes the file)", function()
    _export_to_log(false)
end)

mod:command("gut_reload_config", "Re-read the .toml and override settings now (no restart needed)", function()
    local path = _config_path()
    if not path then mod:echo("Config path unavailable on this build."); return end
    local f = io.open(path, "r")
    if not f then mod:echo("No config file found at " .. path .. " - run /gut_export_settings + the companion script first."); return end
    f:close()
    local n = apply()
    mod:echo(string.format("Applied %d setting(s) from %s.", n, CONFIG_NAME))
end)

mod:info("[gut] Config-file feature installed (/gut_export_settings, /gut_reload_config; overrides on load)")
