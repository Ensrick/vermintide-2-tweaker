...
--
-- Owned by: gui_tweaker.lua entry point. Consumed via: mod:dofile (single call).

local mod = get_mod("gut")
local Policy = mod:dofile("scripts/mods/gui_tweaker/_gut_native_loadout_policy")
...
local function _ensure_seeded(mirror, career_name)
    ...
    if not entry then
        ...
        _persist()
        printf("[gut:NATIVE_LOADOUTS] seeded career=%s loadouts=%d selected=%d from official (fresh)",
    ...
    entry._seeded = true
    _persist()
    printf("[gut:NATIVE_LOADOUTS] repaired career=%s partial store (issue #375): rows_added=%d rows_repaired=%d loadouts=%d selected=%d",
...
local function _trace_gear_read(career_name, key, idx, value, source)
    if _trace_should_emit(career_name, key, idx, value, source) then
        printf("[gut:NATIVE_LOADOUTS] #387 gear-read career=%s slot=%s idx=%s value=%s source=%s",
...
local function _official_gear_fallback(read_official, mirror, career_name, key, store_idx)
    ...
    if WEAPON_SLOT_SET[key] then
        ...
        pcall(printf, "[gut:NATIVE_LOADOUTS] WEAPON SLOT UNRESOLVED career=%s slot=%s store_idx=%s: official selected row and default loadout both nil, serving nil",
...
local function _capture_bu_equip(mode, mirror, career_name, slot_name, value, source)
    if mode == MODE_READONLY then
        ...
        _dirtify()
        printf("[gut:NATIVE_LOADOUTS] BU equip capture career=%s idx=%s slot=%s source=%s -> readonly overlay",
    ...
    _persist()
    printf("[gut:NATIVE_LOADOUTS] BU equip capture career=%s idx=%s slot=%s source=%s -> store",
        tostring(career_name), tostring(idx), tostring(slot_name), tostring(source))
end
...
local function _install_bu_capture()
        ...
        if mode ~= MODE_OFF and career_name and backend_id and is_loadout_slot then
            ...
            elseif COSMETIC_SLOT_SET[slot_name] then
                ...
                if not _bu_unresolved_seen[token] then
                    ...
                    printf("[gut:NATIVE_LOADOUTS] BU cosmetic capture SKIP career=%s slot=%s bid=%s reason=%s mirror=%s",
    ...
    printf("[gut:NATIVE_LOADOUTS] BackendUtils.set_loadout_item capture installed (post-LA)")
end
...
mod:hook("PlayFabMirrorAdventure", "set_character_data", function(func, self, career_name, key, value, set_mirror, optional_loadout_index)
    ...
    if m == MODE_READONLY then
        ...
        if action == "preserve" then
            ...
            printf("[gut:NATIVE_LOADOUTS] set_character_data career=%s idx=%s key=%s value=%s -> readonly overlay (official untouched)",
        ...
        elseif action == "clear" then
            ...
            printf("[gut:NATIVE_LOADOUTS] set_character_data career=%s key=%s value=%s -> official fallback (cleared_mod_overlay=%s)",
                tostring(career_name), tostring(key), tostring(value), tostring(cleared))
            return
        end
        printf("[gut:NATIVE_LOADOUTS] set_character_data career=%s key=%s BLOCKED (read-only non-modded loadouts)",
    ...
    _persist()
    printf("[gut:NATIVE_LOADOUTS] set_character_data career=%s idx=%s key=%s -> store (blocked official write)",
...
mod:hook("PlayFabMirrorAdventure", "set_loadout_index", function(func, self, career_name, loadout_index)
    ...
    if m == MODE_READONLY then
        printf("[gut:NATIVE_LOADOUTS] set_loadout_index career=%s BLOCKED (read-only non-modded loadouts)", tostring(career_name))
        return
    end
    _prepare(self, career_name)
    local store = _store()
    ...
    if entry.loadouts[loadout_index] then   -- mirror vanilla's `if career_data[loadout_index]` guard (:1975)
        ...
        _persist()
        printf("[gut:NATIVE_LOADOUTS] set_loadout_index career=%s -> %d (store)", tostring(career_name), loadout_index)
...
mod:hook("PlayFabMirrorAdventure", "add_loadout", function(func, self, career_name)
    ...
    if m == MODE_READONLY then
        printf("[gut:NATIVE_LOADOUTS] add_loadout career=%s BLOCKED (read-only non-modded loadouts)", tostring(career_name))
        return
    end
    _prepare(self, career_name)
    local store = _store()
    ...
    if n < M.max_loadouts() then   -- cap from InventorySettings.MAX_NUM_CUSTOM_LOADOUTS, never hardcoded
        ...
        _persist()
        printf("[gut:NATIVE_LOADOUTS] add_loadout career=%s now=%d selected=%d (store)",
...
mod:hook("PlayFabMirrorAdventure", "delete_loadout", function(func, self, career_name, loadout_index)
    ...
    if m == MODE_READONLY then
        printf("[gut:NATIVE_LOADOUTS] delete_loadout career=%s BLOCKED (read-only non-modded loadouts)", tostring(career_name))
        return
    end
    _prepare(self, career_name)
    local entry = _store()[career_name]
    ...
    _persist()
    printf("[gut:NATIVE_LOADOUTS] delete_loadout career=%s removed=%d now=%d selected=%d (store)",
...
mod:hook("PlayFabMirrorAdventure", "set_career_read_only_data", function(func, self, character, key, value, career, set_mirror, loadout_index)
    ...
    if m == MODE_READONLY then
        ...
        if action == "preserve" then
            ...
            _dirtify()
            printf("[gut:NATIVE_LOADOUTS] set_career_read_only_data career=%s idx=%s key=%s value=%s -> readonly overlay (official untouched)",
                tostring(career), tostring(idx), tostring(key), tostring(value))
            return
        elseif action == "clear" then
            local cleared = _overlay_clear(self, career, key, loadout_index)
            if cleared then _dirtify() end
            printf("[gut:NATIVE_LOADOUTS] set_career_read_only_data career=%s key=%s value=%s -> official fallback (cleared_mod_overlay=%s)",
                tostring(career), tostring(key), tostring(value), tostring(cleared))
            return
        end
        printf("[gut:NATIVE_LOADOUTS] set_career_read_only_data career=%s key=%s BLOCKED (read-only non-modded loadouts)",
    ...
    entry.loadouts[idx][key] = value
    _persist()
    printf("[gut:NATIVE_LOADOUTS] set_career_read_only_data career=%s idx=%s key=%s -> store (blocked official write)",
...
-- hook_safe (post) -- no other gut hook targets refresh_bot_loadouts.
...
mod:hook("HeroWindowLoadoutSelectionConsole", "_save_bot_equipment", function(func, self)
    ...
    if m == MODE_READONLY then
        printf("[gut:NATIVE_LOADOUTS] bot_equipment BLOCKED (read-only non-modded loadouts)")
        return
    end
    ...
    _persist()
    printf("[gut:NATIVE_LOADOUTS] bot_equipment career=%s bot_index=%s -> store (skipped PlayerData write)",
...
--
-- Pre-flight (2026-07-06): grepped gui_tweaker for hooks on
...
mod:hook("HeroWindowLoadoutSelectionConsole", "_populate_context_menu_loadout", function(func, self, loadout, loadout_index)
    ...
    if not ok then
        printf("[gut:NATIVE_LOADOUTS] issue #372: suppressed loadout-preview crash (unresolvable equipment item; career=%s idx=%s)",
...
mod:command("reset_modded_loadouts", "Reset modded loadouts to re-seed from official (optional: career name)", function(career_arg)
    ...
    if career_arg and career_arg ~= "" then
        if not store[career_arg] and not overlay[career_arg] then
            ...
            printf("[gut:NATIVE_LOADOUTS] reset requested for unknown career=%s", tostring(career_arg))
            return
        end
        ...
        printf("[gut:NATIVE_LOADOUTS] store reset career=%s (will re-seed from official)", career_arg)
    else
        local seen = {}
        ...
        printf("[gut:NATIVE_LOADOUTS] store reset ALL (%d careers, will re-seed from official)", n)
    end
end)
    ...
    for career_name, entry in pairs(store) do
        if not filter or career_name == filter then
            ...
            for i = 1, #rows do
                ...
                printf("[gut:NATIVE_LOADOUTS] status career=%s row=%d selected=%s slots=[%s] talents=%s",
                ...
                if row then
                    for _, s in ipairs(GEAR_SLOT_NAMES) do
                        ...
                        if id ~= nil then
                            ...
                            printf("[gut:NATIVE_LOADOUTS] status-resolve career=%s row=%d %s id=%s %s get_item_from_id=%s",
                ...
                if type(row) == "table" then
                    for _, slot in ipairs(SCRUB_SLOTS) do
                        ...
                        if id ~= nil and _probe_resolve(id) == nil then
                            broken = broken + 1
                            printf("[gut:NATIVE_LOADOUTS] #402 official BROKEN career=%s row=%d slot=%s id=%s",
                            ...
                            if do_apply then
                                ...
                                if repl ~= nil then
                                    ...
                                    if ok_w then
                                        fixed = fixed + 1
                                        printf("[gut:NATIVE_LOADOUTS] #402 official REPAIRED career=%s row=%d slot=%s -> %s",
                                    ...
                                    skipped = skipped + 1
                                    printf("[gut:NATIVE_LOADOUTS] #402 official NO-OWNED-REPLACEMENT career=%s row=%d slot=%s (re-equip this slot manually)",
...
-- ==================================================================
-- Regression markers (issue #175 requirement 11). Registered by gui_tweaker.lua.
-- ==================================================================
M.HOOK_TARGETS = {
...
printf("[gut:NATIVE_LOADOUTS] module loaded (%s) modded_realm=%s hooks=%d",
