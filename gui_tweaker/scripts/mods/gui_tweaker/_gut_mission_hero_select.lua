...
--     and the /gut_hero_select command (registered below) calls it directly.
...
mod.gut_open_mission_hero_select = function()
    ...
    -- (#173) IN-MISSION ONLY. In the KEEP the native hero select (full character/career change)
    -- is already on the menu, and this shortcut only opens the talents-only layout there — which
    -- reads as "hero select opens talents". DamageUtils.is_in_inn is a boolean field (set by
    -- state_ingame.lua:137, true in the keep), so bail there and let the native keep menu handle
    -- it. The keybind + /gut_hero_select both route here.
    ...
    -- system, so opening the hero/loadout flow crashes. Covers the /gut_hero_select
...
mod:command("gut_hero_select", "Open the hero/talents screen mid-mission (HeroView talents layout; live-safe -- talents apply immediately, no respawn). Career PICK stays keep-only by design.", function()
