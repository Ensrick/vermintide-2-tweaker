...
-- cim presence, DEV-CLONE-AWARE. gut is tested WITH cim_dev (stable cim disabled),
...
-- behind ~9 "no effect" iterations. Matches the rest of gut (gui_tweaker.lua:695 /
...
-- other dev-first cim checks already in gut.
...
-- single user-facing feature. All gt_dev namespacing re-pointed to gut; chat
...
--     gui_tweaker.lua, plus once at this module's own load.
--
-- Module dofile's from gui_tweaker.lua AFTER the main chunk, so the
...
-- isolation rule — never cim_dev, even from gut.
...
-- 2026-07-13 across cim_dev / cosmetics_tweaker / gut / gt_dev -- they hook other
...
-- gui_tweaker.lua (they call this at the points the gt file-local was called).
