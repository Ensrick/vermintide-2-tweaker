...
_rt_register("uitweaks_bridged_to_stock_settings", function()
    ...
    local call_form = "uitweaks_live_group" .. ".apply(out"
    for _, fn in ipairs({ "_mod_tweaker_view.lua", "_mod_tweaker_state.lua" }) do
        local txt = _gut_read_all(dir .. fn)
        if txt then
            if not txt:find(call_form, 1, true) then
                return fn .. " no longer calls the shared UI Tweaks live-group owner (#312)"
            end
    ...
    local owner = _gut_read_all(dir .. "_gut_uitweaks_live_group.lua")
    if owner and (not owner:find(marker, 1, true)
            or not owner:find(live_marker, 1, true)
            or not owner:find("external_group.find_mod_list", 1, true)
            or not owner:find("external_group.replace_group_children", 1, true)
            or not owner:find("exclude_owner_from_profiles = true", 1, true)) then
        return "shared UI Tweaks live-group owner lost bridge/profile contracts (#312)"
    end
