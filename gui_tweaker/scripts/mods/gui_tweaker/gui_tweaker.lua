...
_rt_register("gut_cutscene_no_global_latch", function()
    ...
    --    so the on_setting_changed re-latch (which lives in gui_tweaker.lua) escaped it
    --    and shipped for many versions. gui_tweaker.lua is the sibling of the toggle's
    ...
    local main_path = src_path:gsub("_gut_cutscenes%.lua$", "gui_tweaker.lua")
...
local _gut_loadout_slot_policy = mod:dofile(
    "scripts/mods/gui_tweaker/_gut_loadout_slot_policy"
)
...
do
    local labels = mod:dofile("scripts/mods/gui_tweaker/_mod_tweaker_tab_labels")
...
_rt_register("issue559_search_expansion_transaction", function()
    local ok_s, Search = pcall(mod.dofile, mod, "scripts/mods/gui_tweaker/_mod_tweaker_search")
    ...
    local ok_v, View = pcall(mod.dofile, mod, "scripts/mods/gui_tweaker/_mod_tweaker_view")
...
_rt_register("mod_tweaker_dropdown_filter_api", function()
    ...
    local ok_d, D = pcall(mod.dofile, mod, "scripts/mods/gui_tweaker/_mod_tweaker_definitions")
...
_rt_register("mod_tweaker_numeric_caret_geometry", function()
    ...
    local N = mod:dofile("scripts/mods/gui_tweaker/_mod_tweaker_numeric_editor")
...
_rt_register("cim_bench_write_through_present", function()
    ...
    if dir then
        local data_txt = read_all(dir .. "gui_tweaker_data.lua")
...
pcall(mod.dofile, mod, "scripts/mods/gui_tweaker/_gut_gui_material_guard")
...
mod.on_all_mods_loaded = function(...)
    ...
    if cim then
        if mod:get("gut_cim_bench_adopted") ~= true then
            ...
            printf("[gut:CIM_BENCH] adopted cim allow_in_mission=%s into gut toggle (one-time)",
...
local _gut_native_loadouts = mod:dofile("scripts/mods/gui_tweaker/_gut_native_loadouts")
...
do
    local ok, api = pcall(mod.dofile, mod,
        "scripts/mods/gui_tweaker/_gut_bot_pose")
...
do
    local ok, api = pcall(mod.dofile, mod,
        "scripts/mods/gui_tweaker/_gut_tab_property_refresh")
    if ok and type(api) == "table" then
        for _, check in ipairs(api.rt_checks or {}) do
            _rt_register(check.name, check.fn)
        end
...
pcall(mod.dofile, mod, "scripts/mods/gui_tweaker/_gut_damage_numbers")
...
pcall(mod.dofile, mod, "scripts/mods/gui_tweaker/_gut_mainmenu")
...
pcall(mod.dofile, mod, "scripts/mods/gui_tweaker/_gut_camera")
...
pcall(mod.dofile, mod, "scripts/mods/gui_tweaker/_gut_freecam")
...
do
    local ok, cutscenes = pcall(mod.dofile, mod, "scripts/mods/gui_tweaker/_gut_cutscenes")
...
pcall(mod.dofile, mod, "scripts/mods/gui_tweaker/_gut_monologue")
...
do  -- do-block: locals release back to the chunk (Lua 5.1 200-local ceiling)
    local ok_invbd, invbd = pcall(mod.dofile, mod, "scripts/mods/gui_tweaker/_gut_inventory_backdrop")
...
do
    local ok, api = pcall(mod.dofile, mod, "scripts/mods/gui_tweaker/_gut_original_thp_names")
    if ok and type(api) == "table" then
...
do
    local ok, api = pcall(mod.dofile, mod,
        "scripts/mods/gui_tweaker/_gut_hidden_passives")
    if ok and type(api) == "table" then
        for _, check in ipairs(api.rt_checks or {}) do
            _rt_register(check.name, check.fn)
        end
...
pcall(mod.dofile, mod, "scripts/mods/gui_tweaker/_gut_simple_ui_compat")
...
pcall(mod.dofile, mod, "scripts/mods/gui_tweaker/hb/hb_data")
pcall(mod.dofile, mod, "scripts/mods/gui_tweaker/hb/hide_elements")
pcall(mod.dofile, mod, "scripts/mods/gui_tweaker/hb/level_loading_screen")
...
do
    local ok, live_page = pcall(mod.dofile, mod,
        "scripts/mods/gui_tweaker/_gut_scoreboard_live")
...
do
    local ok, retention = pcall(mod.dofile, mod,
        "scripts/mods/gui_tweaker/_gut_scoreboard_retention")
