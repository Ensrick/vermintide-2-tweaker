...
local function _restore_world(world, reason)
    ...
    if current == state.callback then
        ...
        _pf("[gut:522] inventory preview lighting RESTORE (%s)", tostring(reason or "unspecified"))
    else
        _pf("[gut:522] inventory preview callback ownership changed; restore skipped")
    end
...
local function _bind_world(world, choice)
    ...
    if state then
        if state.choice ~= choice then
            ...
            _pf("[gut:522] inventory preview lighting UPDATE: %s (exposure x%.2f)", choice, multiplier)
        end
        return true
    end
    ...
    then
        if not _warned_missing_api then
            ...
            _pf("[gut:522] inventory preview lighting unavailable: shading callback API missing")
        end
        return false
    end
    ...
    _pf("[gut:522] inventory preview lighting APPLY: %s (exposure x%.2f)", choice, multiplier)
    return true
end
