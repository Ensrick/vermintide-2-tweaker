...
-- "HeroWindowCharacterPreview" -- gut's only hit is a doc line in
-- HERO_SELECT_RESEARCH_173.md; no gut hook exists on this class anywhere. The
-- single mod:hook below is a singleton.
--
-- Module dofile's from gui_tweaker.lua after the main chunk; returns
...
mod:hook("HeroWindowCharacterPreview", "create_ui_elements", function(func, self, ...)
    ...
    if not vp then
        if bd then
            _pf("[gut:invbd] viewport definitions unresolved through package.loaded and local_require (path=%s) -> vanilla backdrop this open", DEFS_PATH)
        end
        return func(self, ...)
    end
    if bd and not _package_gettable(bd.package) then
        _pf("[gut:invbd] backdrop '%s' package %s not gettable -> vanilla backdrop this open (issue 522 degrade path)",
            tostring(choice), bd.package)
        bd = nil
    end
    _apply(vp, bd)
    if bd then
        _pf("[gut:invbd] inventory preview backdrop SWAP: %s (level=%s pkg=%s env=%s sets=%d) -- vanilla load/has_loaded gate handles residency",
...
mod.on_disabled = function(...)
    ...
    if _orig then
        ...
        if vp then
            _apply(vp, nil)
            _pf("[gut:invbd] mod disabled -> inventory preview def restored to vanilla")
        end
    end
end
