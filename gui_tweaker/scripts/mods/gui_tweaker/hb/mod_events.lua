...
-- ============================================================================
-- mod_events.lua -- fork of HideBuffs/mod_events.lua (UI Tweaks port, Phase 1)
-- ============================================================================
-- Renamespaced: get_mod("HideBuffs") -> get_mod("gut").
--
-- CRITICAL: gut already defines mod.update / mod.on_setting_changed /
-- mod.on_all_mods_loaded / mod.on_game_state_changed / mod.on_enabled /
-- mod.on_disabled / mod.on_unload. We MUST NOT overwrite them. So every
-- HideBuffs lifecycle callback is renamed to mod.hb_<name>; the gut orchestrator
-- (gui_tweaker.lua) chains each hb_<name> from the matching gut callback.
--
-- mod.update_funcs (+ its iteration in mod.hb_update) and mod.fix_invalid_alignments
-- are kept as-is so Phase-2 update-driven features can register into update_funcs.
--
...
-- YET wired into the boot chain (gui_tweaker.lua dofiles only hb_data /
...
mod.hb_on_unload = function()
	mod.persistent.was_ingame_entered = mod.was_ingame_entered
end

mod.hb_on_game_state_changed = function(status, state)
...
mod.hb_on_all_mods_loaded = function()
	...
	if not get_mod("SimpleUI") then
		pcall(printf, "[gut:startup] UI Tweaks requires Simple UI to manage presets; dependency missing")
	end
end

-- update_funcs is the registration list Phase-2 features push per-frame work into.
-- mod.hb_update iterates it; the gut orchestrator calls mod.hb_update from gut's
-- own mod.update.
mod.update_funcs = mod.update_funcs or {}
mod.hb_update = function()
	for _, update_func in ipairs( mod.update_funcs ) do
		update_func()
	end
end

mod.hb_on_setting_changed = function(setting_name)
	if not mod.SETTING_NAMES then
		return
	end
