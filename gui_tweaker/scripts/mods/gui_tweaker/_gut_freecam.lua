...
-- (gut 0.2.189-dev log 17:14:49-17:15:12, forced close via Steam).
    ...
    if enabled then
        ...
        if _menu_open() then
            if not _pending_menu_close then
                ...
                printf("[gut:FC] activation deferred: menu/view open")
        ...
        printf("[gut:FC] activated (input devices unblocked; character held by is_input_blocked)")
        ...
        do
            ...
            printf("[gut:FC] render-state: world=%s vp=%s base_viewport=%s ff_viewport=%s active=%s",
        ...
        if player and data and data.active then
            if _live_world(data.viewport_world_name) then
                ...
                printf("[gut:FC] skipped engine exit - free-flight world already destroyed (cleared flags instead)")
            end
        end
        _set_third_person(_prev_third_person)
...
mod.update = function(dt)
    ...
    if _pending_menu_close and not _freecam_active then
        ...
        elseif not _menu_open() then
            _pending_menu_close = false
            printf("[gut:FC] menu closed, completing deferred activation")
    ...
    if not ok and not _drive_err_logged then
        ...
        printf("[gut:FC] drive error (camera not moving): %s", tostring(err))
    end
end
...
do
    ...
    mod.on_game_state_changed = function(status, state_name)
        ...
        if _freecam_active then
            ...
            if mod:get("gut_freecam_enabled") then mod:set("gut_freecam_enabled", false) end
            printf("[gut:FC] force-exited on game state change (%s %s)", tostring(status), tostring(state_name))
