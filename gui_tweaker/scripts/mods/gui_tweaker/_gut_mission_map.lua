...
--
-- Module dofile's from gui_tweaker.lua after the main chunk. Registers FOUR
...
-- it). HOOK PRE-FLIGHT (grep 2026-07-05, re-run 2026-07-06 for the area windows): gut
...
local function _kick_preview_pkg_load(ctx)
    ...
    _pf("[gut:MM] preview backdrop package load START: %s (ref=%s ctx=%s)", PREVIEW_PKG, MM_PKG_REF, tostring(ctx))
    local ok, err = pcall(pm.load, pm, PREVIEW_PKG, MM_PKG_REF, function()
        _mm_pkg_load_in_flight = false
        _pf("[gut:MM] preview backdrop package load COMPLETE: %s (ref=%s) -> next map open uses the preview stage", PREVIEW_PKG, MM_PKG_REF)
    end, true)
    if not ok then
        _mm_pkg_load_in_flight = false
        _pf("[gut:MM] preview backdrop package load FAILED: %s (map falls back to the level-less backdrop)", tostring(err))
    end
end
...
mod:hook("StartGameWindowBackgroundConsole", "_create_viewport_definition", function(func, self, ...)
    ...
    if pm and pm.has_loaded and pm:has_loaded(PREVIEW_PKG, MM_PKG_REF) then
        ...
        _pf("[gut:MM] backdrop def-swap PREVIEW: %s not resident -> %s (env=%s object_sets=%d)",
    ...
    _pf("[gut:MM] backdrop def-swap LEVELLESS-FALLBACK: %s not resident, %s not loaded yet -> no world this open (preview stage next open)",
...
mod:hook("StartGameWindowBackgroundConsole", "_update_object_sets", function(func, self, ...)
    ...
    _pf("[gut:MM] _update_object_sets diverted (swapped backdrop: keep-only object sets/flow events)")
end)
...
mod:hook("StartGameWindowBackgroundConsole", "_setup_object_sets", function(func, self, ...)
    ...
    if self._gut_mm_none_backdrop or level_name == nil then
        self._object_sets = {}
        _pf("[gut:MM] _setup_object_sets diverted (level-less fallback backdrop: nil level_name -> empty object sets)")
        return
    end
    return func(self, ...)
end)
...
mod:hook("StartGameWindowAreaSelectionConsoleV2", "_assign_video_player", function(func, self, material_name, video_player)
    ...
    _pf("[gut:MM] area video '%s' is not in the ingame Gui -> video widget skipped (static area image kept; prevented ui_renderer draw fatal)",
...
mod:hook("StartGameWindowAreaSelection", "_setup_video_player", function(func, self, material_name, resource)
    ...
    _pf("[gut:MM] area video '%s' (PC layout) is not in the ingame Gui -> video player skipped (static area image kept; prevented ui_renderer draw fatal)",
        tostring(material_name))
end)
...
mod:hook_safe("MatchmakingStateWaitForCountdown", "on_enter", function(self, state_context)
    ...
    _pf("[gut:MM] auto-start: WaitForCountdown entered mid-mission -> countdown flag set (wait_to_start_game=%s)",
...
mod:hook(GameModeManager, "complete_level", function(func, self, ...)
    ...
    if armed_t then
        ...
        if now and now - armed_t < 15 then
            ...
            _pf("[gut:MM] auto-start: diverting complete_level -> promote_next_level_data (no fake mission win)")
...
mod.gut_open_mission_map = function()
    ...
    if not mod:get("gut_mission_map") then
        _pf("[gut:MM] gate=disabled (gut_mission_map off) -> no-op")
        return
    end
    ...
    if in_keep then
        _pf("[gut:MM] gate=keep (level_key=%s is_in_inn=%s) -> no-op; vanilla M handles the keep",
    ...
    if mech ~= "adventure" then
        _pf("[gut:MM] gate=mechanism blocked (mechanism=%s; adventure-only) -> no-op", tostring(mech))
    ...
    if host_only and not is_host then
        _pf("[gut:MM] gate=host_only blocked (host_only=true is_host=false) -> no-op")
    ...
    _pf("[gut:MM] backdrop tier: keep=%s preview_loaded=%s -> %s", tostring(keep_ok), tostring(preview_loaded), tier)
    _pf("[gut:MM] gate=opened (mechanism=adventure host_only=%s is_host=%s tier=%s) -> start_game_view_force menu_state_name=play",
    ...
    if not ok then
        _pf("[gut:MM] handle_transition FAILED: %s", tostring(err))
