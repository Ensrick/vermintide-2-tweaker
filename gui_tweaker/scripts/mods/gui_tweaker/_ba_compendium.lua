...
M.weapons = mod:dofile("scripts/mods/gui_tweaker/_ba_weapon_provider")
M.enemies = mod:dofile("scripts/mods/gui_tweaker/_ba_enemy_provider")

-- Debug probes: verify the live enumeration in-game without the UI.
mod:command("gut_ba_dump_weapons", "Bestiary&Armory: dump live weapon enumeration to console", function()
	M.weapons.dump()
end)

mod:command("gut_ba_dump_breeds", "Bestiary&Armory: dump live enemy-breed enumeration to console", function()
	M.enemies.dump()
end)
...
-- + 3D preview land in later phases; the data layer (/gut_ba_dump_*) is already live.
mod:command("gut_armory", "Open the Armory (weapon compendium)", function()
...
mod:command("gut_bestiary", "Open the Bestiary (enemy compendium)", function()
