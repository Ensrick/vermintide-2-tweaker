...
-- ============================================================================
-- Modded Manifest Broadcast (host-side) — Phase 2 producer.
-- ============================================================================
-- See PHASE2_PROTOCOL.md. Enumerates loaded mods via Managers.mod._mods
-- (mod_manager.lua:326-336), serializes one line per mod ("<id>\t<ver>\t<mode>
-- \t<workshop_id>\t<display_name>"), chunks into <=CHUNK_SIZE pieces, and
-- publishes via LobbyHost:set_lobby_data (lobby_host.lua:139) under keys
-- ltw_p / ltw_n / ltw_m0..N-1. Throttled to <=1 republish per 500 ms; heals
-- every 60 s; re-fires on VMF setting change. Host-only, hash-neutral.
...
KNOWN_MODS = mod:dofile("scripts/mods/lobby_tweaker/_known_mods") or {}
...
_resolve_mode = function(mod_id, mod_obj)
    if mod_obj and type(mod_obj._lt_mode) == "string" then
        local m = mod_obj._lt_mode:upper()
...
_publish = function(force)
    ...
    local enabled = mod:get("manifest_broadcast_enabled") and true or false
...
local _prev_on_setting_changed = mod.on_setting_changed
mod.on_setting_changed = function(setting_id)
    if _prev_on_setting_changed then
        local ok, err = pcall(_prev_on_setting_changed, setting_id)
...
-- Diagnostics ----------------------------------------------------------------
mod:command("lt_manifest_dump", "Print the locally-built modded manifest.", function()
