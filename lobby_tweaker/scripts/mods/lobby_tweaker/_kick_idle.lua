...
--
-- Persistence:
--   "ki_whitelist" = { [steam_id_string] = true, ... }
-- Toggles / numerics (live in _data.lua):
--   "kick_idle_enabled", "kick_idle_threshold_minutes"

local TAG          = "[Lobby Tweaker]"
local STORAGE_KEY  = "ki_whitelist"
local TOGGLE_KEY   = "kick_idle_enabled"
local THRESH_KEY   = "kick_idle_threshold_minutes"
local TICK_SEC     = 30          -- position-sample cadence
local WARN_SEC     = 60          -- warning lead time before the kick
...
--       warned      = boolean, true once we've chatted the 60s warning,
...
-- ---------------------------------------------------------------------------
-- VMF update callback — fires every frame; we sample every TICK_SEC.
-- A previous `mod.update` (e.g. from lobby_tweaker.lua) is chained so we
-- don't clobber whatever else may be installed.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Game-state cleanup: handled implicitly by _tick's _in_keep() check, which
-- resets all timers on the first tick after the host leaves the keep. No
-- explicit event hook is needed — and a per-module `mod:hook_safe(_G, "Boot")`
-- would collide with sibling modules' Boot hooks (vmf-hook-safe-no-chain).
-- ---------------------------------------------------------------------------
...
mod:command("lt_idle_whitelist", "Whitelist a Steam ID from idle-kicks. Usage: /lt_idle_whitelist <steam_id>",
    function(steam_id_arg)
        local sid = _normalize_id(steam_id_arg)
        if not sid then
            mod:echo(TAG .. " /lt_idle_whitelist: missing or invalid steam_id.")
            return
        end
...
mod:command("lt_idle_unwhitelist", "Remove a Steam ID from idle-kick whitelist. Usage: /lt_idle_unwhitelist <steam_id>",
    function(steam_id_arg)
        local sid = _normalize_id(steam_id_arg)
        if not sid then
            mod:echo(TAG .. " /lt_idle_unwhitelist: missing or invalid steam_id.")
            return
        end
        local wl = _load_whitelist()
        if wl[sid] == nil then
            mod:echo(TAG .. " /lt_idle_unwhitelist: " .. sid .. " not on whitelist.")
            return
        end
        wl[sid] = nil
        _save_whitelist(wl)
...
mod:command("lt_idle_status", "Show idle-kick tracker (deltas + timers).", function()
    if not _is_host() then
        mod:echo(TAG .. " /lt_idle_status: host only.")
        return
    end
    local threshold = _threshold_seconds()
