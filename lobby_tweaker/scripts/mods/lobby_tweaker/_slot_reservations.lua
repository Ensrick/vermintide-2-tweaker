...
--
-- Persistence key: "sr_reservations" = { [steam_id_string] = slot_index, ... }
-- Master toggle:   "slot_reservations_enabled"

local STORAGE_KEY = "sr_reservations"
local TOGGLE_KEY  = "slot_reservations_enabled"
...
-- ---------------------------------------------------------------------------
-- Chat commands (host-only). Registered globally; prefixed `lt_` per spec.
-- ---------------------------------------------------------------------------

mod:command("lt_reserve", "Reserve a lobby slot for a Steam ID. Usage: /lt_reserve <steam_id> <slot 1-4>",
    function(steam_id_arg, slot_arg)
        if not _is_host() then
            mod:echo(TAG .. " /lt_reserve: host only.")
            return
        end
        local steam_id = _normalize_id(steam_id_arg)
        local slot     = _parse_slot(slot_arg)
        if not steam_id then
            mod:echo(TAG .. " /lt_reserve: missing or invalid steam_id.")
            return
        end
        if not slot then
            mod:echo(TAG .. " /lt_reserve: slot must be 1, 2, 3, or 4.")
            return
        end
        local reservations = _load()
...
mod:command("lt_unreserve", "Remove a slot reservation. Usage: /lt_unreserve <steam_id>",
    function(steam_id_arg)
        if not _is_host() then
            mod:echo(TAG .. " /lt_unreserve: host only.")
            return
        end
        local steam_id = _normalize_id(steam_id_arg)
        if not steam_id then
            mod:echo(TAG .. " /lt_unreserve: missing or invalid steam_id.")
            return
        end
        local reservations = _load()
        if reservations[steam_id] == nil then
            mod:echo(TAG .. " /lt_unreserve: no reservation for " .. steam_id .. ".")
            return
        end
...
mod:command("lt_reservations", "List current slot reservations.", function()
    local reservations = _load()
...
-- the current event manager on every mod.update tick and re-register when it
-- changes. mod.update is chained so we don't clobber sibling modules.
-- ---------------------------------------------------------------------------
