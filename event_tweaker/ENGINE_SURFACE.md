...
| Class.method (kind) | Vanilla behavior | Why evt hooks it | Trap / invariant |
|---|---|---|---|
| `StartGameWindowAreaSelection._setup_area_widgets` [hook] and `StartGameWindowAreaSelectionConsoleV2._setup_area_widgets` [hook] `_evt_missions.lua` | Desktop/controller area menus iterate `AreaSettings` and omit entries whose `exclude_from_area_selection` is true [src: desktop `:91-95`; console V2 `:100-105`]. The dormant `celebrate` container still carries Bogenhafen's label/icon and `sort_order=0` [src: `level_unlock_settings_celebrate.lua:3-21`], while the stock `dwarf_fest` area carries the resident Feast presentation [src: `level_unlock_settings_dwarf_fest.lua:3-21`] | If either allowlisted mission is enabled and the menu-read contract validates, temporarily expose `AreaSettings.celebrate`, borrow the stock Feast name/description/icon, and sort it after the other visible areas while vanilla builds widgets | Restore every touched field, including formerly absent fields, even when vanilla raises. Never permanently unhide/relabel an area or rewrite its video/acts. The signature-deduplicated `area applied` proof records whether vanilla assigned the target widget (console V2 caps its pool before sorting [src: console V2 `:100-112`]). VMF disables/re-enables hooks with the mod |
