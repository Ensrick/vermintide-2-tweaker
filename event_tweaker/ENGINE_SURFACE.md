...
19 registration sites (`tools/mod-lint/lint-mod.ps1` enforces one hook per
(Class, method) mod-wide), grouped by concern. `[hook]` = full
wrapper (`mod:hook`, can rewrite args/returns); `[safe]` = `mod:hook_safe`
(post-callback, no override). All 19 are string-form (each target class has a single
...
| Class.method (kind) | Vanilla behavior | Why evt hooks it | Trap / invariant |
|---|---|---|---|
| `Pacing.update` [safe] `_evt_diagnostics.lua:154` | Runs after conflict-director init wrappers have returned and the live `Pacing` object owns the settled intensity/pacing values [src: `scripts/managers/conflict_director/pacing.lua`, method `update`] | Issue 393 one-shot-per-instance snapshot: printf the settled globals and cached director thresholds to classify the result as `intact` or `settings_stomp` (`:154`) | Read-only diagnostic, no behavior change; printf only (users run mod-logs OFF), pcall-wrapped and weak-key deduped. Diagnostics are always-on in dev, never a menu toggle |
| `MutatorHandler.ai_killed` [hook] `_evt_diagnostics.lua:333` | Notifies every active mutator of an AI death; only the server branch executes each template's `server_ai_killed_function` [src: `scripts/managers/game_mode/mutator_handler.lua:254-271`] | Issue 1309 records the Tzeentch Twins seed and spawn-queue delta around the vanilla call on the host, and the corresponding death/expected lower-tier breed on a client | Strictly read-only and inert unless `curse_change_of_tzeentch` armed this mission. Preserve all six arguments and every return. Per-kill output shares `_evt_diag_tzeentch_twins.lua`'s hard ten-row cap; counters continue after the output cap |
| `UnitSpawner.spawn_unit_from_game_object` [hook] `_evt_diagnostics.lua:388` | Materializes a host-created network unit on a client [src: `scripts/network/unit_spawner.lua:470-490`] | Issue 1309 matches a replicated lower-tier husk against a recent client-observed Tzeentch Twins death | Calls vanilla first and returns its exact unit. Client-only observation is bounded by a three-second pending window and the shared ten-row receipt cap; it never spawns, destroys, or rewields a unit |
| `MutatorHandler._activate_mutator` / `StateIngame.on_exit` [existing-owner callbacks] `_evt_cursed_adventure.lua:109,180` | Activate one mutator on each peer and tear down the current in-game state | The existing Cursed Adventure hook owner calls `issue1309_on_mutator_activated` after activation and `issue1309_flush_summary` on exit, producing one activation and one summary per peer per mission | Do not add competing hooks for these methods. The exported callbacks are the only cross-module surface, are `pcall`-contained, and remain no-ops for every other curse |

### Dormant event mission menu (issues 626/802; host menu, vanilla level transition)

| Class.method (kind) | Vanilla behavior | Why evt hooks it | Trap / invariant |
|---|---|---|---|
| `StartGameWindowAreaSelection._setup_area_widgets` [hook] and `StartGameWindowAreaSelectionConsoleV2._setup_area_widgets` [hook] `_evt_missions.lua` | Desktop/controller area menus iterate and sort visible `AreaSettings`; controller assigns the first sorted entry to its special Campaign widget [src: desktop `:91-102`; console V2 `:100-119,132-137`] | If either allowlisted mission is enabled and the menu-read contract validates, temporarily replace `AreaSettings.celebrate` with a shallow presentation copy that is visible, sorts after every visible area, and uses the resident 168x168 `level_image_dlc_dwarf_fest` atlas entry [src: `gui_level_images_atlas.lua:914-925`] | Stock `celebrate` has `sort_order=0` plus Bogenhafen presentation [src: `level_unlock_settings_celebrate.lua:3-16`]; an unhide-only change steals the controller Campaign widget. Restore the exact stock table identity even when vanilla raises. Never read or write `AreaSettings.helmgart` |
| `StartGameWindowAreaSelection._set_area_presentation_info` [hook] and `StartGameWindowAreaSelectionConsoleV2._set_area_presentation_info` [hook] `_evt_missions.lua` | Resolve the selected area's stock title/description into the two view instances [src: desktop `:186-206`; console V2 `:252-278`] | After vanilla completes for selected `celebrate`, replace only the view-local title/description with Event Tweaker's localized **Events** copy; restore the console description pass's vanilla localization mode on every non-event selection | Do not register mod strings through global `Localize` or persistently rewrite `AreaSettings`. Campaign and every DLC use the original method and widget content unchanged |
| `StartGameWindowMissionSelection._setup_level_acts` [hook] and `StartGameWindowMissionSelectionConsole._setup_level_acts` [hook] `_evt_missions.lua` | Build instance-local `_levels_by_act` from global `UnlockableLevels` [src: desktop `:108-129`; console `:98-125`]. Desktop identifies the selected area through `self.parent:get_selected_area_name()` [src: `:46`]; console uses `self._parent:get_selected_area_name()` [src: `:45`] | After vanilla builds the map, replace only `act_celebrate` with enabled entries from the exact two-level allowlist, and only when `celebrate` is the selected top-level area | Preserve the entire map by identity for every non-celebrate area and preserve unrelated act tables by identity inside celebrate. Fail closed unless the tables the menus READ are complete (`AreaSettings.celebrate` + `ActSettings.act_celebrate` + allowlisted `LevelSettings`); gating on `NetworkLookup` was the issue 626 "toggle on, nothing shows" defect. Load-time idempotent fallback may append a vanilla-missed allowlisted level to local `UnlockableLevels` / `GameActs` / `MapPresentationActs` in vanilla shape [src: `level_unlock_settings.lua:100-135`]; NEVER write `UnlockableLevelsByGameMode` or `NetworkLookup` (wire tables boot-built from `LevelSettings` [src: `network_lookup.lua:1239-1259`]; modded keys CTD non-mod peers, issue 278 / issue 371). Vanilla `LevelTransitionHandler` remains package owner [src: `level_transition_handler.lua:518-572`] |
