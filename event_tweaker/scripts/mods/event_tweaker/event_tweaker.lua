...
-- ============================================================
-- Weave-only mutator gate (issue 413)
-- ============================================================
-- Seven of the eight cat_winds weave mutators are UNSAFE outside a real
-- Weave: Managers.weave:get_active_wind_settings() returns nil unless a
-- weave template is active (weave_manager.lua:423-432), and:
...
_rt_register("issue413_weave_only_mutators_gated", function()
...
-- Boss-event mutator guard (issue 455)
...
--
-- Fix: when add() injects one of these names, wrap the template's live dispatch
-- fields (the engine folds server_*_function into template.server.* at boot;
-- update_conflict_settings dispatches off the template root) with a guard that
-- no-ops via an [et:455] printf when boss_events is absent. The check runs at
-- dispatch time, against the CURRENT level's settings, so the mutator still
-- works everywhere boss events exist. Skipping is strictly better than the
-- vanilla crash on the levels it fires (divert flow, not delegate -- issue 270
-- lesson). Host-side only: all three sites are server dispatch paths.
do
...
_rt_register("issue455_boss_event_mutators_guarded", function()
