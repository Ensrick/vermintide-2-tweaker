...
>   `docs/BUG_CLASSES.md § 17 Variant B` for the fix template. `ct_dev` has
>   since moved to the same log-only route; `qa/check_logging.ps1` now finds
>   warning-backed helpers only in the three stable streams pinned by the #427
>   floor (their Dev twins are log-only).
...
> in `general_tweaker/scripts/mods/general_tweaker/_gt_debug_probes.lua`. Its
> Dev twin already carries the VMF predicate. That swap is runtime source and
> rides the next user-authorized General Tweaker stable promotion (§6.5
> version, artifact, approval, and live-test boundaries); delete the pinned
> entry and flip the stable flag in `qa/lua/tests/test_gt_vmf_debug_gate.lua`
> in the same pass.
...
| Routine confirmations / diagnostic traces | **NEVER bare `mod:echo`** — and NEVER `mod:warning` either | Use `_dbg` (VMF-controlled debug channel) or a log-only printf helper. `mod:warning` posts to CHAT under VMF default settings (Issue #240; `BUG_CLASSES.md § 17 Variant B`). |
...
- **Publication-only mode (#1376/#1426):** canonical `ship.ps1` enters this mode
  ...
  separate #1429 local-deployment gate described in section 6.6; it is not an
  implicit deployment bypass. Publication-only mode must not create or write a
...
- Tracked deploy verification is byte-exact for compiled `.mod_bundle` files and all
  ...
  Receipt-authority local deployment instead requires the exact committed
  filename/length/SHA-256 output set, including byte-exact descriptor content.
